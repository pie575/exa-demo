...
search는 기본적으로 최대 10개의 result를 반환합니다. `numResults`를 설정하면 최대 100개까지 요청할 수 있으며, 관련성 있는 페이지가 그보다 적을 경우 더 적게 반환될 수 있습니다. search는 페이지네이션을 지원하지 않습니다.
...
질의는 자연어로 작성하세요. 주제를 포함하고, 필요하다면 원하는 소스의 종류와 기간도 함께 적어주세요.

질의는 폭넓고 탐색적이어도 괜찮습니다. `"Latest news on EU battery policy"`는 Exa가 관련 페이지를 찾아낼 수 있을 만큼 의도를 충분히 전달하지만, `"news"`는 그렇지 않습니다. 출처 유형이 중요하다면 질의에 명시하세요:

```text theme={null}
RAG 시스템에서 하이브리드 검색과 시맨틱 검색을 비교한 최신 기술 문서
```

Exa 인덱스에 어떤 데이터가 포함되어 있고 각 콘텐츠 유형을 어떻게 검색하는지는 [Exa 인덱스에 포함된 것](/ko/docs/search/data/overview)을 참고하세요.

<h2 id="search-types">
  search type 선택하기
</h2>

`type`은 search mode를 선택하며, 각 모드는 속도, 검색 깊이, synthesis 간의 균형이 서로 다르게 조정되어 있습니다. 기본값은 `auto`이며 대부분의 검색에 적합합니다.

| 유형               | 사용 시점                          |
| ---------------- | ------------------------------ |
| `auto`           | 품질과 속도의 균형이 잡힌 최적의 기본값을 원할 때   |
| `fast`           | 요청이 latency에 민감할 때             |
| `instant`        | 자동완성이나 음성처럼 실시간 경로상의 요청일 때     |
| `deep-lite`      | 가벼운 수준의 리서치와 synthesis가 필요할 때  |
| `deep`           | 다단계 검색과 더 강력한 synthesis가 필요할 때 |
| `deep-reasoning` | latency보다 완전성과 추론 깊이가 더 중요할 때  |

deep 모드는 단일 검색 과정이 아니라 리서치 프로세스를 실행합니다. 이 프로세스의 동작 방식과 추가 제어 옵션 사용법은 [Deep Search](/ko/docs/search/deep-search)를 참고하세요.

<Tip>
  장시간 실행되는 리서치, 리스트 구축, 다단계 enrichment에는 `deep-reasoning` 대신 [Exa Agent](/ko/docs/agent/quickstart)를
...
<Tabs>
  <Tab title="Highlights">
    Highlights는 질의와 가장 관련성이 높은 발췌문을 반환합니다. 각 페이지의 무관한 부분으로 컨텍스트 윈도우를
    채우지 않으면서, 모델과 agent에 필요한 evidence를 제공합니다.

    대부분의 작업에 권장되는 output 형태입니다.

    먼저 `highlights: true`만 지정해 보세요. Exa가 질의를 바탕으로 각 result에서 적절한 분량의
    콘텐츠를 선택합니다.
    ...
    Dynamic Highlights와 이를 켜야 할 시점에 대한 안내는 [Highlights](/ko/docs/search/highlights)를 참고하세요.
  </Tab>

  <Tab title="전체 텍스트">
    전체 텍스트는 정제된 페이지 본문을 반환합니다. 더 넓은 컨텍스트, 문서 구조, 또는 질의 중심 발췌문에서
    빠질 수 있는 세부 정보가 필요한 작업에 사용하세요.

    전체 페이지는 용량이 클 수 있습니다. result 개수와 페이지당 반환되는 텍스트 양을 모두 제한하세요.
...
요청당 하나의 콘텐츠 뷰만 선택하세요. highlights와 text를 함께 요청하면 같은 페이지에 대해 두 가지 뷰가 반환되고 그만큼 과금됩니다. `summary`도 세 번째 선택지이지만, result마다 언어 모델 call이 추가로 발생합니다.
...
<Warning>
  ...
  * **`/search`**는 `highlights`, `text`, `summary`를 `contents` 객체 안에 중첩합니다:
    `"contents": { "highlights": true }`
  * **`/contents`**에는 `contents` 래퍼가 없습니다. 요청 body 자체가 content options이므로,
...
순위가 매겨진 페이지는 그대로 `results`에 남아 있습니다. 생성된 값은 `output.content`로 반환되며, field 단위 소스와 confidence는 `output.grounding`에 담깁니다.

<Tabs>
  <Tab title="자유 형식 텍스트">
    자연어 문장 형태의 출력에는 `type: "text"`를 사용하세요. 형식이나 길이를 지정하려면 `description`을 추가하세요.
...
소스 선호도나 강조할 내용 같은 지시사항에는 `systemPrompt`를, response의 형태를 지정할 때는 `outputSchema`를 사용하세요. Python에서는 `system_prompt`와 `output_schema`를 사용합니다.
...
대부분의 검색에서는 이 field를 생략하는 것이 좋습니다. 가격, 재고 여부, 자주 바뀌는 페이지처럼 오래된 페이지 콘텐츠를 쓸 수 없는 경우에만 설정하세요.
...
<Columns cols={2}>
  ...
  <Card title="API reference" icon="square-terminal" href="/ko/docs/reference/search" cta="reference 열기" arrow="true">
    모든 요청 매개변수와 response field를 라이브 플레이그라운드에서 바로 확인해 보세요.
  </Card>
