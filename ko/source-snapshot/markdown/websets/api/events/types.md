...
> Webset API 내에서 발생하는 이벤트에 대해 알아보세요

Websets API는 이벤트를 통해 Webset의 변경 사항을 알려줍니다. 이러한 이벤트는 [이벤트 endpoint](/ko/docs/websets/api/events/list-all-events)로 조회하거나 [webhook](/ko/docs/websets/api/webhooks/create-a-webhook)을 설정해 모니터링할 수 있습니다.

이벤트는 60일간 보관된 후 자동으로 삭제됩니다.
...
* `webset.item.created` - 새로운 item이 Webset에 추가되면 발생합니다.
...
* `webset.export.created` - 새 내보내기가 시작될 때 발생합니다.
* `webset.export.completed` - 내보내기가 완료되면 발생합니다.
...
* `monitor.updated` - monitor의 configuration이 업데이트될 때 발생합니다.
* `monitor.deleted` - monitor가 삭제될 때 발생합니다.
* `monitor.run.created` - monitor run이 시작될 때 발생합니다.
* `monitor.run.completed` - monitor run이 종료될 때 발생합니다.
...
* search 및 enrichment의 진행 상황 추적
* 실시간 dashboard 구축
* 새로운 item이 발견되면 워크플로우 트리거
* 내보내기 상태 모니터링