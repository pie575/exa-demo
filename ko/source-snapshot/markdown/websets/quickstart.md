...
Webset은 자연어 질의와 목표 item 개수로 시작합니다. 여기에 모든 결과가 충족해야 할 criteria와 승인된 각 item에 채울 enrichment 필드를 추가하면 됩니다. 결과는 dashboard, API 또는 webhook을 통해 비동기적으로 전달됩니다.

[Dashboard](/ko/docs/websets/dashboard/get-started)에서 코드 없이 시각적으로 webset을 만들 수도 있습니다.

<Info>
  리스트 구축이나 enrichment 워크플로우를 새로 시작하시나요? [Exa Agent](/ko/docs/agent/quickstart)를 사용하세요.
  이 가이드는 기존 Websets 연동을 유지하거나 확장할 때 참고하세요.
  Websets API를 사용하려면 유료 Websets 플랜이 필요하며, Search API credits와 Websets credits는 별개입니다.
</Info>
...
1. **search 정의:** 자연어 질의와 결과 개수를 지정하고, 필요에 따라 verification criteria와 enrichment를 추가합니다.
2. **검색 및 검증:** Websets가 후보를 찾아 각각을 criteria에 맞는지 확인합니다. 조건을 충족하는 결과만 item이 됩니다.
3. **enrichment 실행:** 검증된 각 item에 대해 Websets가 CEO 이름, 투자 금액, 연락처 정보 등 요청한 추가 데이터를 검색합니다.
4. **결과 수신:** 상태를 poll하거나, webhook으로 업데이트를 받거나, item이 추가되는 대로 dashboard에서 확인하세요.
...
| 기능                        | 설명                                            |
| ------------------------- | --------------------------------------------- |
| **Criteria verification** | 직접 정의한 규칙에 따라 각 결과를 검증하므로 관련 있는 결과만 얻을 수 있습니다 |
| **Enrichments**           | 모든 결과에서 특정 데이터 포인트(텍스트, 숫자, 날짜, 불리언)를 추출합니다   |
| **Monitors**              | 반복 search를 예약해 webset을 자동으로 최신 상태로 유지합니다      |
| **Webhooks**              | item이 추가되거나 enrich될 때 실시간 HTTP 콜백을 받습니다       |
...
* [**Websets API 레퍼런스**](./api/websets/create-a-webset) - 모든 endpoint에 대한 전체 API 레퍼런스