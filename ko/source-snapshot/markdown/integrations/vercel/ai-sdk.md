...
`@exalabs/ai-sdk` 패키지를 사용하면 Vercel의 AI SDK로 구축한 애플리케이션에 Exa web search를 추가할 수 있습니다. Exa API key만 제공하면 `webSearch()` 도구가 모델의 search request를 처리해 줍니다.
...
<Info>
  예제를 실행하기 전에 key를 `EXA_API_KEY` 환경 변수로 설정하세요. 패키지가 이 환경 변수를 자동으로 읽어옵니다.
</Info>
...
아래 옵션으로 search와 콘텐츠 extraction을 원하는 대로 조정하세요:
...
| 옵션                                        | 설명                                                                                         |
| ----------------------------------------- | ------------------------------------------------------------------------------------------ |
| `type`                                    | search mode: `auto`, `fast`, `instant`, `deep-lite`, `deep`, `deep-reasoning`.             |
| `category`                                | 콘텐츠 카테고리: `company`, `publication`, `news`, `personal site`, `people`, `financial report`. |
| `numResults`                              | 반환할 결과 수.                                                                                  |
...
| `userLocation`                            | 위치를 반영한 search를 위한 두 자리 국가 코드.                                                             |
...
| `contents.summary`                                     | AI가 생성한 요약을 반환합니다. `query`를 지원합니다.                         |
...
| `contents.subpages` / `contents.subpageTarget`         | 하위 페이지를 크롤링하고, 선택적으로 특정 하위 페이지를 지정합니다.                     |
...
이 package에는 TypeScript 타입이 포함되어 있습니다:
...
<Columns cols={2}>
  <Card title="Vercel AI Gateway 사용하기" icon="cloud" href="/ko/docs/integrations/vercel/ai-gateway" cta="가이드 열기" arrow="true">
    Exa API key 없이 Vercel의 AI Gateway를 통해 Exa web search를 사용하세요.
  </Card>

  <Card title="AI SDK package 살펴보기" icon="git-branch" href="https://github.com/exa-labs/ai-sdk" cta="소스 보기" arrow="true">
    GitHub에서 소스 코드와 package 상세 정보를 확인하세요.
  </Card>
</Columns>

package는 [npm](https://www.npmjs.com/package/@exalabs/ai-sdk)에서도 확인할 수 있으며, [Vercel AI SDK web search 가이드](https://ai-sdk.dev/cookbook/node/web-search-agent#exa)도 함께 참고해 보세요.