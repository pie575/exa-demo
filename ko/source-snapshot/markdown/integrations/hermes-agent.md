...
<Steps>
  ...
  <Step title="Hermes에서 Exa 선택하기">
    ...
    **Web Search &amp; Extract**를 열고 Exa를 선택한 다음, API 키 기반 옵션을 고르세요. 입력 안내가 표시되면 Exa API key를 입력합니다. Hermes는 secret을 `~/.hermes/.env`에, 제공업체 선택 정보를 `~/.hermes/config.yaml`에 저장합니다.
  </Step>

  <Step title="웹 접근 테스트하기">
    Hermes를 실행한 뒤 search를 요청하고, 결과 중 하나를 읽도록 해보세요:

    ```text theme={null}
    Search the web for the latest Exa product updates, then read the most relevant result.
    ```

    이때 Hermes는 `web_search`를 call하고, 페이지 내용 자체가 필요하면 이어서 `web_extract`를 call합니다.
  </Step>
</Steps>
...
그런 다음 두 가지 웹 기능 모두에 Exa를 선택합니다:
...
| 도구            | Exa 동작                                             |
| ------------- | -------------------------------------------------- |
| `web_search`  | Exa로 검색해 제목, URL, 텍스트 스니펫이 포함된 순위가 매겨진 페이지를 반환합니다. |
| `web_extract` | Exa Contents를 통해 하나 이상의 URL에서 읽을 수 있는 콘텐츠를 가져옵니다.  |

Hermes는 추출한 페이지가 길 경우 설정된 character budget에 맞춰 잘라내고, 전체 텍스트는 디스크에 저장합니다. 기본값은 `web.extract_char_limit`으로 변경할 수 있으며, 개별 call마다 agent가 더 큰 `char_limit`을 요청하도록 할 수도 있습니다.
...
<AccordionGroup>
  <Accordion title="Hermes가 Exa를 선택하지 않는 경우">
    `hermes tools`를 실행한 뒤 Exa를 명시적으로 선택하세요. 파일을 직접 설정하는 경우 `web.search_backend`, `web.extract_backend` 또는 `web.backend`가 `exa`로 설정되어 있는지 확인하세요.
  </Accordion>
  ...
  <Accordion title="search는 동작하지만 extraction은 다른 제공업체를 사용하는 경우">
