...
2. Slack의 OAuth 플로우가 열립니다. Exa를 추가할 워크스페이스를 선택한 뒤 **Allow**를 클릭합니다.
...
<Note>
  빨간색 **&quot;App is not approved by Slack&quot;** 안내는 정상이며 무시해도 됩니다. Exa가 공개 Slack 마켓플레이스에
  등록되어 있지 않다는 의미일 뿐, 문제가 있는 것은 아닙니다.
</Note>
...
```text theme={null}
@Exa 샌프란시스코에 있는 시리즈 A 핀테크 스타트업을 모두 찾아줘
```
...
| `mute`            | thread에서 mention되지 않은 답글에는 Exa가 응답하지 않게 합니다. 명시적인 @Exa mention은 계속 동작합니다. |
| `unmute`          | `mute` 이후 thread follow-ups를 다시 시작합니다.                                    |
| `sleep`           | Exa가 해당 thread에서 작업을 완전히 중단합니다. @Exa를 mention하면 다시 깨어납니다.                 |
...
| ---------------------- | ---------------------------- | ----------------------------------------------------------- |
| `app_mentions:read`    | @Exa를 직접 mention하는 메시지 보기    | 채널이나 thread에서 누군가 Exa를 mention하면 요청 시작                      |
| `assistant:write`      | Slack에서 App Agent로 동작        | Slack의 agent 환경을 활용하고 DM과 채널 thread로 답변 스트리밍                |
| `channels:history`     | Exa가 추가된 공개 채널의 메시지 보기       | 공개 채널의 thread 답글을 수신해 다시 mention하지 않아도 follow-ups가 동작하도록 함  |
| `channels:read`        | 공개 채널의 기본 정보 보기              | 웹 세션 동기화 위치를 선택할 때 이미 Exa가 추가된 공개 채널 찾기                     |
| `chat:write`           | Exa app으로 메시지 보내기            | thread 최상위 메시지, 답변, 진행 상황 업데이트, 확인 메시지, 웹 동기화 메시지 게시        |
...
| `groups:history`       | Exa가 추가된 비공개 채널의 메시지 보기      | 비공개 채널의 thread 답글을 수신해 다시 mention하지 않아도 follow-ups가 동작하도록 함 |
| `groups:read`          | Exa가 추가된 비공개 채널의 기본 정보 보기    | 웹 세션 동기화 대상을 선택할 때 사용 가능한 비공개 채널을 찾고 멤버십 확인                 |
| `im:history`           | Exa와 주고받은 direct messages 보기 | DM 요청과 follow-up 답글 수신                                      |
| `im:write`             | direct messages 시작           | 인증된 사용자가 웹 세션 동기화 대상으로 선택했을 때 해당 사용자의 Exa DM 열기             |
| `users:read`           | 사용자와 기본 Slack 프로필 보기         | mention을 이름으로 변환하고 동기화된 메시지에 웹 참가자의 Slack 프로필 이미지 사용        |
| `users:read.email`     | 워크스페이스 member의 이메일 주소 보기     | team 귀속 및 맞춤형 웹 메시지 프로필 이미지를 위해 Slack 계정과 Exa 계정 매칭         |

<Note>
  `channels:read`, `groups:read`, `im:write`는 웹에서 Slack으로의 동기화를 위한 대상 discovery를 지원합니다.
  기존 설치 환경은 이 scope 없이도 현재 Slack thread를 계속 사용할 수 있지만, 해당 대상을 사용하려면
  다시 연결해야 합니다. `chat:write.customize`는 런타임에서 선택 사항입니다. 이 권한이 없으면 웹 동기화 메시지는
  기본 Exa app 아이덴티티를 유지하며, 참가자의 이름은 메시지 본문에 포함됩니다.
</Note>
