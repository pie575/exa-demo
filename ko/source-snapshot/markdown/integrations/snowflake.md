...
이 연동은 Snowflake의 [External Access](https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview) 기능을 사용해 저장 프로시저에서 Exa API를 안전하게 호출하며, 다음 두 가지 주요 워크플로우를 지원합니다:
...
| 계층                              | 세부 내용                                    |
| ------------------------------- | ---------------------------------------- |
| **Roles**                       | 프로시저와 데이터에 대한 범위 제한 접근용 `exa_agent_role` |
| **Network Rule**                | 아웃바운드 트래픽을 `api.exa.ai:443`으로만 제한        |
| **Secret**                      | API 키를 암호화해 저장하며, SQL 쿼리에 노출되지 않음        |
| **External Access Integration** | network rule과 secret을 하나로 묶음             |
