...
<div className="docs-tabs">
  <Tabs>
    <Tab title="Claude Web, Desktop 및 Cowork" icon="https://mintcdn.com/exa-52/Una64IRjof2yadw_/images/mcp-clients/claude.svg?fit=max&auto=format&n=Una64IRjof2yadw_&q=85&s=443a9b17d5b63c875f924a4aecc01e56" width="24" height="24" data-path="images/mcp-clients/claude.svg">
      <Steps>
        <Step title="커넥터 디렉터리 열기">
          새 Claude 채팅에서 더하기 버튼을 누르고 **Add connector**를 선택한 다음 **Exa**를 검색하세요.
        </Step>

        <Step title="Exa 연결">
          Exa를 열고 **Connect to Claude**를 선택한 뒤, 요청이 표시되면 접근을 승인하세요.

          <Frame caption="Claude에서 커넥터 디렉터리를 열고 Exa를 찾아 연결한 뒤 접근을 승인하는 과정">
            <img src="https://mintcdn.com/exa-52/Una64IRjof2yadw_/images/integrations/claude-web-desktop/install-claude.gif?s=259e8d897252e7f8435b94dc6ceeae5d" alt="Claude에서 커넥터 디렉터리를 열고 Exa를 찾아 연결한 뒤 접근을 승인하는 과정" style={{width: "100%", height: "auto"}} width="800" height="596" data-path="images/integrations/claude-web-desktop/install-claude.gif" />
          </Frame>
        </Step>

        <Step title="Exa 사용">
          새 채팅을 시작해 웹의 최신 정보가 필요한 질문을 던져 보세요.
        </Step>
      </Steps>
    </Tab>
    ...
    <Tab title="Claude Code CLI" icon="https://mintcdn.com/exa-52/Una64IRjof2yadw_/images/mcp-clients/claude-code.svg?fit=max&auto=format&n=Una64IRjof2yadw_&q=85&s=f7f017b187974c56e5822d7baf8272fa" width="16" height="16" data-path="images/mcp-clients/claude-code.svg">
      <Steps>
        ...
        <Step title="새 세션 시작">
          플러그인이 로드되도록 새 Claude Code 세션을 연 다음, 웹이 필요한 질문을 던져 보세요.

          <Frame caption="새 Claude Code 세션을 열고 웹이 필요한 질문을 던지는 과정">
            <img src="https://mintcdn.com/exa-52/Una64IRjof2yadw_/images/integrations/claude-web-desktop/claude-code.gif?s=1a6d69ab819e600fd2101771380a5711" alt="새 Claude Code 세션을 열고 웹이 필요한 질문을 던지는 과정" style={{width: "100%", height: "auto"}} width="800" height="502" data-path="images/integrations/claude-web-desktop/claude-code.gif" />
          </Frame>
        </Step>
      </Steps>
    </Tab>
...
```text theme={null}
우리는 Tailwind v3를 쓰고 있어. Exa로 공식 Tailwind v4 업그레이드
가이드를 찾아 읽은 다음, 이 프로젝트를 v4로 마이그레이션해줘.
```

Claude Code는 찾아낸 내용을 활용해 코드베이스를 수정할 수 있습니다. 다른 Claude client에서는 동일한 소스를 답변, 아티팩트, Cowork 작업에 활용할 수 있습니다.

답변이 최신 웹 소스나 특정 웹 소스에 의존하는 경우라면 언제든 동일한 패턴을 적용할 수 있습니다.

* &quot;이 의존성의 최신 릴리스 노트를 찾아서 호환성이 깨지는 변경 사항을 요약해 줘.&quot;
* &quot;추론 시점 스케일링에 대한 최근 1차 연구를 검색해서 방법론을 비교해 줘.&quot;
...
<Columns cols={3}>
  ...
  <Card title="리서치" icon="compass">
    여러 검색을 실행하고 유용한 페이지를 살펴본 뒤, evidence를 종합해 출처가 명시된 response를 만듭니다.
  </Card>
</Columns>
...
```text theme={null}
이 경쟁사 분석 자료를 검토하고, Exa로 현재 벤더 페이지와 대조해 모든 가격 관련
