...
<Info>
  HIPAA 준수 기능은 Exa가 해당 team에 대해 활성화한 후 Enterprise 고객이 사용할 수 있습니다. Enterprise 이용, BAA 요건, 활성화에 대한 문의는 [sales@exa.ai](mailto:sales@exa.ai)로 연락해 주세요.
</Info>

HIPAA 모드는 최상위 `compliance` field로 요청마다 제어합니다:
...
적격 team에서 이 field가 지정되면 Exa는 HIPAA 준수 통제에 따라 요청을 처리합니다. team에 이 기능이 활성화되어 있지 않으면 API는 `403 FEATURE_DISABLED`를 반환합니다.
...
호환되지 않는 요청은 `400 INVALID_REQUEST_BODY`를 반환하며, 다음과 같은 경우가 해당합니다.
