...
> 특정 API key의 usage 분석 및 billing 데이터를 조회합니다.
...
<Info>
  Team Management API는 team 단위로 활성화됩니다. 인증은 서비스 계정 API key로 이루어지며, 이 key는 해당 기능이 team에 활성화된 후 [API keys 페이지](https://dashboard.exa.ai/api-keys)의 **Service keys** 탭에서 생성할 수 있습니다. 액세스를 요청하려면 [support@exa.ai](mailto:support@exa.ai)로 문의하세요.
</Info>
...
Get API Key Usage endpoint를 사용하면 특정 API key의 지정된 기간에 대한 상세한 billing 및 usage 분석 데이터를 조회할 수 있습니다. 이 endpoint는 Exa의 billing 시스템에서 cost 데이터를 가져오므로, 해당 API key에 대해 청구되는 내역을 정확하게 파악할 수 있습니다.
...
* **group&#95;by** (선택): 결과를 그룹화할 시간 단위(`hour`, `day`, `month`)입니다. 현재는 향후 기능 확장을 위해 예약된 값으로, 응답 형태에는 영향을 주지 않습니다. 기본값은 `day`입니다.
...
* **api&#95;key&#95;name**: API key의 이름(설정된 경우)
* **team&#95;id**: 이 key가 속한 Team ID
* **period**: usage 기간의 시작일과 종료일이 담긴 객체
* **total&#95;cost&#95;usd**: 지정한 기간의 총 cost(USD)
* **cost&#95;breakdown**: 가격 유형별 cost 내역 배열이며, 각 항목은 다음을 포함합니다.
  * **price&#95;id**: 가격의 고유 식별자
...
* **Team 소유권**: 인증에 사용한 서비스 API key는 조회 대상 API key와 동일한 team에 속해야 합니다. team 간 접근은 허용되지 않습니다.
...
* API key 단위의 billing 대시보드 구축
* 특정 API key의 usage 및 cost 모니터링
* usage 임계값 기반의 자동 알림 설정
* 내부 cost 배분을 위한 usage 리포트 생성
