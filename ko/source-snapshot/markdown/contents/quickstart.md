...
Exa Contents는 URL에서 깔끔한 페이지 콘텐츠를 반환하며, JavaScript로 렌더링된 페이지, PDF, 복잡한 레이아웃도 자동으로 처리합니다.

모든 contents 기능은 [Exa Search](/ko/docs/search/quickstart)가 반환한 URL에 대해서도 사용할 수 있으며, search당 result 10개까지는 추가 비용이 없습니다(이후 1000페이지당 $1). web search 도구 활용 사례라면 Contents 대신 이런 방식으로 Search를 사용하는 것을 권장합니다.
...
<Tabs>
  <Tab title="Highlights">
    highlights는 페이지에서 그대로 발췌한 관련 구절을 반환합니다. 전체 텍스트보다 컨텍스트를 작게 유지할 수 있으므로 agent, RAG, 사실 확인 용도라면 highlights부터 시작하세요.

    highlights를 활성화하려면 `highlights: true`를 설정하세요. Contents를 사용할 때는 페이지의 content extraction 범위를 좁히기 위해 `query` 매개변수를 함께 지정하는 것을 권장합니다:
  ...
  <Tab title="전체 텍스트">
    전체 텍스트는 정제된 페이지 본문을 마크다운으로 반환합니다. 폭넓은 컨텍스트, 문서 구조, 또는 highlights에서 누락될 수 있는 세부 내용이 필요한 작업에 사용하세요.

    전체 페이지는 분량이 클 수 있으므로, limit이 필요하면 `maxCharacters`를 사용하세요:
  ...
  <Tab title="Summary">
    summary는 페이지마다 언어 모델을 call합니다. 생성된 개요가 필요하거나 JSON schema로 fields를 추출해야 할 때 사용하세요.
    ...
    서술형 문장 대신 fields를 추출하려면 `summary.schema`에 JSON schema를 전달하세요. summary는 해당 schema에 맞는 JSON 문자열로 반환되므로, fields를 읽으려면 파싱해야 합니다:
...
대부분의 요청에서는 이 field를 생략하는 것이 좋습니다. 가격, 재고 여부, 자주 갱신되는 페이지처럼 오래된 페이지 콘텐츠를 사용할 수 없는 경우에만 설정하세요. `maxAgeHours`를 낮게 설정할 때는 `livecrawlTimeout`(밀리초)과 함께 사용해 새로 가져오는 데 걸리는 시간의 상한을 정하세요.

<Accordion title="지원 중단된 livecrawl 매개변수에서 마이그레이션하기">
  `livecrawl` 문자열 매개변수(`"always"`, `"preferred"`, `"fallback"`, `"never"`)는
  `maxAgeHours`로 대체되어 지원 중단되었습니다:
...
<Columns cols={2}>
  ...
  <Card title="Highlights" icon="highlighter" href="/ko/docs/search/highlights" cta="가이드 읽기" arrow="true">
    agent 및 RAG 컨텍스트에 쓰이는 일반 highlights와 Dynamic Highlights를 비교해 보세요.
  </Card>

  <Card title="Search API" icon="search" href="/ko/docs/search/quickstart" cta="가이드 열기" arrow="true">
    content를 추출하기 전에 관련 있는 페이지를 먼저 찾아보세요.
  </Card>
