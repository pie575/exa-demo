...
> 화자 정보와 timestamp가 포함된 팟캐스트 전문을 검색하세요.

[Particle](https://particle.news)의 Podcast Intelligence는 10만 개 이상의 프로그램을 색인합니다.
방송 후 몇 분 이내에 전체 전사, 화자 분리, 화자 식별, 레이블링을 마치고 metadata로 enrich하여
...
팟캐스트 진행자들이 AI 규제에 대해 어떤 이야기를 하는지 찾아보세요.
...
## 함께 쓰면 좋은 조합 {#pairs-well-with}

* [Financial Datasets](/ko/docs/agent/connect/financialdatasets): 팟캐스트에서 오간 이야기를 공개된 뉴스와 교차 검증합니다.
* [Fiber.ai](/ko/docs/agent/connect/fiber): 언급된 인물에 회사 및 연락처 컨텍스트를 attach합니다.
