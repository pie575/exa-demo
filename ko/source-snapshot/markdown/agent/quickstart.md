...
컨텍스트 agent라고 생각하면 됩니다. 원하는 데이터와 그 데이터를 받을 형태를 설명하면, Exa Agent가 그 결과를 얻는 데 필요한 도구 call을 조율합니다. 한 번의 실행으로 여러 관점에서 search를 병렬 확장하고, 그 결과 페이지를 읽어 요약하고, 리스트 구축을 병렬 실행되는 하위 작업으로 나누고, 각 후보를 criteria에 비추어 검증하고, 연락처를 enrich하며, attach한 [Exa Connect](/ko/docs/agent/connect/overview) data partners에 질의할 수 있습니다. `/search`와 `/contents` call을 하나하나 직접 조율할 필요 없이, 취합된 컨텍스트를 하나의 근거 기반 구조화된 결과로 받게 됩니다.

각 실행은 자연어 답변, schema 검증을 거친 JSON, field 수준 grounding, metadata, 비용 내역을 반환할 수 있습니다. 완료된 실행을 나중에 조회하거나, 과거 실행 목록을 확인하거나, 이벤트를 replay하거나, 이전 실행에 이어서 진행할 수 있습니다.
...
* 열린 형태의 criteria로 목록을 만들고 각 result를 enrich
* 여러 fields에 걸쳐 citations와 함께 엔티티를 리서치
* &quot;회사를 찾은 다음 그 회사의 의사결정권자를 찾기&quot;와 같은 다단계 작업 실행
* 장시간 실행되는 웹 리서치 작업에서 구조화된 JSON 생성
* 웹 리서치와 프리미엄 data partners를 결합해 하나의 근거 기반 답변 생성
* &quot;결과를 10개 더 찾아줘&quot;와 같은 follow-up 요청으로 이전 실행에서 이어가기
...
<Note>
  ...
  엔드포인트를 `https://api.exa.ai`로 지정하고 `model: "exa-agent"`를 사용한 뒤, 동기,
  스트리밍, 백그라운드 실행 중에서 선택하세요. [OpenAI SDK
...
<Note>
  ...
  불가능할 수 있는 field는 nullable로 지정하고 `required`에서 제외하세요. 그래야
  agent가 값을 지어내지 않고 `null`을 반환할 수 있습니다. `verdict` enum은 확인
  자체에 실패한 경우(`cannot_verify`)와 실제로 부정적인 evidence가 확인된
  경우(`absent`)를 구분합니다. 접근하지 못한 사이트는 해당 페이지가 존재하지
...
| `agent_run.completed` | 완료된 Agent 실행 객체                       | `data.output.text` 또는 `data.output.structured`에서 최종 답변을, `data.output.grounding`에서 citations를 읽습니다. |
| `agent_run.failed`    | `{ id, status: "failed", error }`     | `error.code`와 `error.message`를 표시합니다. 완료된 output은 제공되지 않습니다.                                        |
| `agent_run.cancelled` | `{ id, status: "cancelled", ... }`    | 스트리밍 수신을 중단하고 해당 실행을 취소된 것으로 처리합니다.                                                                 |

동일한 리서치 단계에 속한 이벤트에는 `callId`가 포함됩니다. 이는 도구 진행 이벤트의 `item.call_id`에 해당합니다. 이 값으로 search 추적, 소스, 도구 진행 상황을 그룹화하세요. 일부 search 추적 설명은 비동기적으로 생성되므로 설명 대상인 소스나 도구 이벤트보다 늦게 도착할 수 있습니다. 따라서 도착 순서만으로 연관 짓지 마세요.
...
연락처 정보를 요청하려면 원하는 연락처 field를 `outputSchema`에 기술하세요. 이메일 주소에는 `{ "type": "string", "format": "email" }`, 전화번호에는 `{ "type": "string", "format": "phone" }`, URL에는 `{ "type": "string", "format": "uri" }`처럼 표준 JSON Schema 형태를 사용하면 됩니다. 가능하면 `maxItems`로 목록 크기를 제한해 연락처 enrichment의 최대 비용을 예측할 수 있도록 하세요.
...
`usage.agentComputeUnits`는 전체 실행에 걸친 모델 연산량을 측정합니다. 복잡한 질의, 특히 `input.data` field가 큰 질의는 더 많은 추론 단계와 도구 call이 필요하므로 ACU를 더 많이 소비합니다.
...
<Info>
  Agent Max는 latency나 비용보다 완결성과 철저함이 더 중요한 작업을 위한 최고 effort 등급으로,
  대규모 리스트 구축, 여러 소스를 활용한 deep 리서치, 검증이 까다로운 criteria 등에 적합합니다.
  현재 퍼블릭 베타 단계이며, `effort: "max"`를 사용하는 요청에는
  `Exa-Beta: agent-max-effort-2026-07-27`를 포함해야 합니다. 이 header에는
  쉼표로 구분된 베타 토큰 목록을 지정할 수 있습니다.
</Info>
...
| --------- | ------------------------------------ | --------------------------------------- | ----------------- |
| `minimal` | 최저 비용 조회, 매우 좁은 사실 확인 작업, 짧은 답변      | field 한두 개, 얕은 schema                   | 가장 저렴하지만 가장 덜 철저함 |
| `low`     | 단순 조회, 좁은 사실 확인 작업, 짧은 답변            | 소수의 field, 얕은 schema                    | 빠르고 가벼운 리서치       |
| `medium`  | 대부분의 표준 리서치 작업에 적합한 기본 출발점           | 중간 수준의 field 수, 단순한 중첩 객체               | 품질과 실행 시간의 균형     |
| `high`    | 더 어려운 리서치, 더 많은 citations, 더 엄격한 완결성 | 더 큰 schema 또는 더 섬세한 field               | 느리지만 더 철저함        |
| `xhigh`   | 비용이나 latency보다 완결성이 중요한 고가치 작업       | 복잡한 schema, 많은 field, 까다로운 verification | 고정 effort 중 가장 느림 |
| `auto`    | 가변 범위 작업, 리스트 구축, 난이도를 알 수 없는 작업     | 유연함. 엔티티 수나 필요한 작업량을 알 수 없을 때 유용        | 가변적               |
| `max`     | 최고 수준의 effort 리서치(베타)                | 복잡한 schema, 많은 field, 까다로운 verification | 가장 오래 실행됨         |

표준적인 단일 엔티티 리서치라면 `medium`으로 시작하세요. 완결성보다 비용과 latency가 더 중요하다면 `low`나 `minimal`로 낮추세요. output schema가 더 크거나, field에 verification이 필요하거나, 더 깊은 추론이 필요한 작업이라면 `high` 또는 `xhigh`로 올리세요. 리스트 구축이나 많은 엔티티를 반환할 수 있는 워크플로우처럼 범위를 미리 알 수 없을 때는 `auto`를 사용하세요.
...
* 스트리밍(`Accept: text/event-stream`)으로 실행을 생성해 output을 실시간으로 받거나, 보존 기간 내에 비동기 실행을 폴링하세요.
* 실행 데이터는 실행이 진행되는 동안, 그리고 종료 상태에 도달한 후 최대 10분까지 사용할 수 있습니다. 이 기간이 지나면 해당 실행을 조회할 수 없습니다.
