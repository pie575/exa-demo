...
> list building, KYB 인텔리전스, 채용 공고, structured output을 다루는 프로덕션 환경의 Exa Agent 예제입니다.

이 예제들은 list building, KYB 인텔리전스, 채용 공고, structured output 등 Exa Agent의 프로덕션 워크플로우 패턴을 보여줍니다.

<h2 id="find-all-code">
  Exa.ai의 모든 GTM 구성원 찾기
</h2>

Agent가 행(row)을 직접 찾아야 한다면 탐색형 prompt를 사용하세요. company disambiguation을 명확히 지정하고 현재 재직 중임을 뒷받침하는 evidence를 요구하면, 이름이 비슷한 회사가 결과에 섞여 들어가지 않습니다.
...
파트너나 벤더를 검토할 때 프로필 페이지 하나로는 부족하고 공개된 웹 인텔리전스가 필요하다면 Agent를 사용하세요. 신원 검증, 비즈니스 맥락, 공개 신호, 투자 활동, 명시적인 리스크 노트를 하나의 구조화된 객체로 요청할 수 있습니다.
...
채용 중인 직무 정보가 회사 페이지, 채용 정보 수집 사이트, 스타트업 디렉터리에 흩어져 있을 때 Agent를 활용하세요. 이 예시에서는 Source를 Exa의 Ashby 기반 채용 게시판으로 제한하지만, 그래도 company disambiguation이 필요합니다.
...
시스템에 이미 레코드가 있고 Agent가 각 레코드를 조사하거나 enrich해야 하는 경우 `input.data`를 사용하세요. 이 패턴을 사용하면 행 식별 정보를 prompt 안에 묻어두지 않고 구조화된 입력에 그대로 유지할 수 있습니다.
...
<h2 id="bounded-contact-fields-code">
  제한된 연락처 필드로 행 enrich하기
</h2>

연락처 중심 워크플로우는 별도의 contact enrich 요금을 발생시킬 수 있습니다. 여기 실린 익명화 버전은 실제 이름, 이메일, 전화번호, 프로필 URL을 노출하지 않으면서 결과 구조만 보여줍니다.
...
제품에서 이미 검토했거나 노출했거나 거절한 레코드를 Agent가 건너뛰도록 하려면 `input.exclusion`을 사용하세요.
시트 미리보기에는 가상의 회사명을 반환값으로 사용했습니다. 해당 행이 실제 운영 실행에서 나온 것처럼 보이지 않으면서도 제외 패턴을 확인할 수 있습니다.
...
시트 미리보기에서는 실제 이전 실행을 노출하지 않고 이어서 진행하는 패턴을 보여주기 위해 가상의 회사 이름을 반환값으로 사용합니다.
...
<div id="broad-list-building-with-agent-max">
  ## Agent Max로 폭넓은 list building 수행하기
</div>

지연 시간이나 cost보다 완전성과 철저함이 더 중요한 작업에는 `effort: "max"`를 사용하세요. 대규모 list building, 여러 출처를 아우르는 deep 리서치, 검증하기 어려운 criteria 등이 여기에 해당합니다. Agent Max는 퍼블릭 베타 단계입니다. 요청 시 `Exa-Beta: agent-max-effort-2026-07-27`를 함께 보내세요. 이 header에는 쉼표로 구분된 베타 토큰 목록을 지정할 수 있습니다.
...
<div id="attach-a-data-partner-with-exa-connect">
  ## Exa Connect로 data partner 연결하기
</div>

`dataSources`를 사용하면 Exa Agent가 실행 중에 프리미엄 data partner의 데이터를 가져올 수 있습니다. 아래 예시에서는 각 회사 정보에 Similarweb의 트래픽과 Harmonic의 투자 데이터를 더해 enrich하고, 이를 웹 리서치와 결합합니다. partner 전체 목록은 [Exa Connect](/ko/docs/agent/connect/overview)를 참고하세요.
...
* 작업 단위와 원하는 출처 품질을 명시한 구체적인 `query`를 Agent에 전달하세요.
* 이미 알고 있는 레코드는 프롬프트에 행으로 넣지 말고 `input.data`로 전달하세요.
* 다시 반환되지 않아야 할 레코드는 `input.exclusion`에 지정하세요.
* 다운스트림 코드가 결과를 사용한다면 `outputSchema`를 반드시 추가하세요.
* 범위와 비용을 예측 가능하게 유지하려면 배열에 `maxItems`를 지정하세요.
* 반환된 실행 `id`를 저장해 두면 나중에 폴링하거나, 이벤트를 replay하거나, 비용을 확인하거나, 해당 실행에서 이어서 작업할 수 있습니다.