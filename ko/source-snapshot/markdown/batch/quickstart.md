...
<Info>
  Batch API는 Exa가 해당 team에 대해 활성화한 이후 Enterprise 고객이 사용할 수 있습니다. Enterprise 이용 및 활성화 문의는 [sales@exa.ai](mailto:sales@exa.ai)로 연락해 주세요.
</Info>

Batch API를 사용하면 여러 Exa API 요청을 한 번에 제출하고, 그 결과를 나중에 JSONL 파일로 받아볼 수 있습니다. 수천 건의 개별 요청을 보내면서 속도 제한과 재시도를 직접 관리할 필요 없이, batch 하나를 제출하고 상태를 폴링한 뒤 모든 결과를 파일 하나로 내려받으면 됩니다.

즉시 response가 필요하지 않은 오프라인 enrichment, 백필 등의 작업에 활용하세요. 전체 요청 및 response schema는 [API reference](/ko/docs/reference/batches/create-a-batch)에서 확인할 수 있습니다.
...
| `cancelling`  | 취소가 요청되어 진행 중이던 작업을 정리하고 있습니다 |
| `cancelled`   | batch가 취소되었습니다                |
| `expired`     | 결과를 더 이상 사용할 수 없습니다           |

batch가 완료되면 `resultsUrl`에 JSONL 결과 파일의 다운로드 URL이 담기고, `expiresAt`은 결과 보관 기간의 종료 시점으로 설정됩니다.
...
response는 cursor 기반으로 페이지네이션됩니다. `data`에는 최대 `limit`개의 batch가 담기며, `hasMore`가 `true`이면 `nextCursor` 값을 `cursor` 질의 매개변수로 전달해 다음 페이지를 가져옵니다.
...
`completed`만 지원되는 값이며, 다른 값을 사용하면 오류가 반환됩니다. 완료된 목록은 만료 시점 순으로 정렬되고 별도의 cursor를 사용하므로, 모든 페이지 요청에 `status=completed`를 계속 포함해야 합니다. 완료 상태의 cursor와 필터링하지 않은 cursor는 서로 호환되지 않습니다.
...
각 JSONL 줄에는 원본 `customId`와 `response` 또는 `error` 중 하나가 포함됩니다:
