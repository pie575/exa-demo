...
title: "Batch API"
sidebarTitle: "Batch"
description: "Exa API 요청을 batch 단위로 비동기 실행합니다."
boost: 4
---

<Info>
  Batch API는 Exa가 해당 team에 기능을 활성화한 후 Enterprise 고객이 사용할 수 있습니다. Enterprise 이용 및 활성화에 대한 문의는 [sales@exa.ai](mailto:sales@exa.ai)로 보내주세요.
</Info>

Batch API를 사용하면 여러 Exa API 요청을 한 번에 제출하고 나중에 그 결과를 JSONL 파일로 받아올 수 있습니다. 수천 개의 요청을 개별적으로 보내면서 rate limits와 재시도를 직접 관리할 필요 없이, batch 하나를 제출하고 상태를 poll한 뒤 전체 결과를 파일 하나로 내려받으면 됩니다.

즉각적인 응답이 필요하지 않은 오프라인 enrichment, 백필 등의 작업에 활용하세요. 전체 요청 및 응답 schema는 [API 레퍼런스](/ko/docs/reference/batches/create-a-batch)에서 확인할 수 있습니다.
...
각 batch item은 다음 경로 중 하나로 보내는 `POST` 요청이어야 합니다:
...
각 item에는 batch 내에서 고유한 `customId`가 필요합니다. 결과 파일에도 동일한 `customId`가 반환되므로 output 행을 원래 입력 데이터와 매핑할 수 있습니다.
...
| `cancelling`  | 취소가 요청되어 진행 중인 작업을 정리하는 중입니다 |
...
응답은 cursor 기반으로 페이지가 나뉩니다. `data`에는 최대 `limit` 개수만큼의 batch가 담기며, `hasMore`가 `true`이면 `nextCursor` 값을 `cursor` 쿼리 파라미터로 전달해 다음 페이지를 가져옵니다.
...
`completed`만 지원되는 값이며, 다른 값을 지정하면 오류가 반환됩니다. 완료된 목록은 만료 시점 순으로 정렬되며 별도의 cursor를 사용하므로, 모든 페이지 요청에 `status=completed`를 계속 포함해야 합니다. 완료 필터가 적용된 cursor와 필터가 없는 cursor는 서로 호환되지 않습니다.
...
JSONL의 각 줄에는 원본 `customId`와 함께 `response` 또는 `error` 중 하나가 포함됩니다:
