---
title: "signature 검증하기"
description: "요청이 Exa에서 온 것인지 확인할 수 있도록 웹훅 signature를 안전하게 검증하는 방법을 알아보세요"
timestamp: false
---
...
Exa는 HMAC SHA256으로 웹훅 payload에 서명합니다. signature는 `Exa-Signature` header에 포함되며, 다음 값으로 구성됩니다:

* 웹훅이 전송된 시점을 나타내는 timestamp(`t=`)
* timestamp와 payload로 계산한 하나 이상의 signature(`v1=`)
...
## Verification 절차 {#verification-process}

웹훅 signature를 verification하려면:

1. `Exa-Signature` header에서 timestamp와 signature를 추출합니다
2. timestamp, 마침표, 원본 요청 본문을 순서대로 이어 붙여 서명 대상 payload를 만듭니다
3. 웹훅 secret으로 HMAC SHA256을 적용해 예상 signature를 계산합니다
...
<CodeGroup>
  ...
  function verifyWebhookSignature(payload, signatureHeader, webhookSecret) {
       ...
       * @returns {boolean} signature가 유효하면 true, 그렇지 않으면 false
       */
      try {
          // signature header 파싱
  ...
  public class WebhookTest {
      ...
      * 웹훅 payload의 signature를 검증합니다.
      *
      * @param payload 문자열 형태의 원본 요청 body
      * @param signatureHeader Exa-Signature header 값
      * @param webhookSecret 웹훅 secret
      ...
      */
      ...
      /**
      * 타이밍 공격을 방지하기 위한, 타이밍에 안전한 문자열 비교입니다.
      */
      ...
      public static void main(String[] args) {
          ...
          try {
              ...
              if (isValid) {
                  ...
                  // 여기에서 JSON을 파싱하고 웹훅 이벤트를 처리하면 됩니다
...
* **항상 signature를 검증하세요** - signature를 먼저 검증하지 않은 채로 웹훅 데이터를 처리하지 마세요. 이렇게 하면 공격자가 위조된 웹훅을 엔드포인트로 보내는 것을 막을 수 있습니다.
...
* **timestamp의 최신성을 확인하세요** - 재전송 공격을 방지하려면 timestamp가 너무 오래된(예: 5분 이상 경과) 웹훅은 거부하는 것을 고려하세요.

* **secret을 안전하게 저장하세요** - 웹훅 secret은 환경 변수나 안전한 secret 관리 시스템에 저장하세요. 애플리케이션에 하드코딩해서는 안 됩니다. **중요**: 웹훅 secret은 [웹훅을 생성할 때](/ko/docs/websets/api/webhooks/create-a-webhook)만 반환되며, 이후에는 다시 조회할 수 없으므로 반드시 안전하게 저장해 두세요.

* **HTTPS를 사용하세요** - 데이터가 전송 중에 암호화되도록 웹훅에는 항상 HTTPS 엔드포인트를 사용하세요.

* **최종 URL을 등록하세요** - 웹훅 전송은 HTTP 리디렉션(3xx response)을 따르지 않습니다. 엔드포인트가 리디렉션을 수행하면 해당 전송은 실패로 처리됩니다. 항상 payload를 직접 처리하는 URL을 등록하세요.
...
### 잘못된 Signature 오류 {#invalid-signature-errors}

서명 검증에 실패하는 경우 다음을 확인하세요:

1. **원본 payload 확인**: 파싱된 JSON 객체가 아닌 raw request body를 사용하고 있는지 확인하세요
2. **secret 확인**: 웹훅 생성 시 발급된 올바른 웹훅 secret을 사용하고 있는지 확인하세요
3. **header 파싱 확인**: header에서 timestamp와 signature를 정확히 추출하고 있는지 확인하세요
4. **인코딩 문제**: verification 과정 전반에서 UTF-8 인코딩이 일관되게 사용되는지 확인하세요
...
* [웹훅 이벤트](/ko/docs/websets/api/events/types)와 해당 payload 알아보기
