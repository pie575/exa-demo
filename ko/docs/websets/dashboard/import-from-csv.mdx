...
CSV에서 가져오기 기능을 사용하면 URL이 담긴 기존 CSV 파일을 바로 사용할 수 있는 Websets으로 변환할 수 있습니다. 이미 웹사이트, 회사, 리소스 목록을 가지고 있고, 여기에 추가 데이터를 enrich하거나 search criteria를 적용해 필터링하려는 경우에 특히 유용합니다.
...
4. URL이 enrichment와 metadata를 갖춘 Webset으로 변환됩니다
...
import할 수 있는 최대 result 수는 plan에 따라 결정됩니다.

## 다음 단계는? {#what-happens-next}

import가 완료되면 CSV는 완전한 Webset이 되며, 다음 작업을 수행할 수 있습니다:
