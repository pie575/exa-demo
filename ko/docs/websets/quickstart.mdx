...
Webset은 자연어 질의와 목표 item 개수로 시작합니다. 모든 결과가 충족해야 할 criteria와, 승인된 item마다 채울 enrichment 필드를 추가하세요. 결과는 dashboard, API 또는 webhook을 통해 비동기적으로 전달됩니다.
...
2. **검색 및 검증:** Websets가 후보를 찾아 각 후보를 criteria에 비추어 검증합니다. criteria를 충족한 결과만 item이 됩니다.
3. **enrichment 실행:** 검증된 각 item에 대해 Websets가 요청하신 추가 데이터, 예를 들어 CEO 이름, 투자 유치 금액, 연락처 정보 등을 검색합니다.
4. **결과 수신:** 상태를 poll하거나 webhook으로 업데이트를 받을 수 있고, item이 추가되는 대로 dashboard에서 확인할 수도 있습니다.
...
| 기능                        | 설명                                                  |
| ------------------------- | --------------------------------------------------- |
| **Criteria verification** | 각 결과를 직접 정의한 criteria에 따라 검증하므로 관련 있는 결과만 얻을 수 있습니다 |
| **Enrichments**           | 모든 결과에 대해 특정 데이터 항목(텍스트, 숫자, 날짜, 불리언)을 추출합니다        |
...
* [**작동 방식**](/ko/docs/websets/quickstart) - 이벤트 기반 아키텍처 심층 살펴보기
