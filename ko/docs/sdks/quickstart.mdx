...
description: "설치 and use the Exa Python and JavaScript SDKs"
---
...
<Note>
  두 client 모두 `EXA_API_KEY` 환경 변수에서 키를 읽어옵니다. 직접 지정하려면
  인라인으로 전달하세요: `Exa(api_key="your-api-key")` 또는 `new Exa("your-api-key")`.
</Note>
...
| ----------- | ----------------------------------------------------------- |
| 시작점         | `search` 사용                                                 |
| Search type | latency나 synthesis 요구사항 때문에 다른 타입이 필요하지 않다면 `auto` 유지       |
...
Search는 관련 있는 페이지를 찾아 한 번의 호출로 해당 contents까지 반환합니다.

<Tip>
  AI 답변, RAG, 검색 미리보기에는 `highlights: true`를 사용하세요. Exa는 각 result의
  발췌문 길이를 관련성에 맞춰 조정합니다. 애플리케이션에 고정된 limit이 필요한
  경우에만 `max_characters` / `maxCharacters`를 설정하세요.
</Tip>
...
<Note>
  ...
  `output.content`로 반환합니다. 소스 선호도나 강조할 내용을 지정하려면 `system_prompt` / `systemPrompt`를 사용하세요.
  grounding은 `output.grounding`에 자동으로 반환되므로 schema에 citations나
  confidence를 중복해서 정의하지 마세요.
</Note>

output을 만들기 위해 여러 차례의 searches에 걸친 리서치가 필요하다면 deep 모드를 권장합니다. 가벼운 리서치에는 `deep-lite`를, 다단계 search와 더 강력한 synthesis가 필요할 때는 `deep`을 사용하세요. 전체 요청 옵션은 [Search 가이드](/ko/docs/search/quickstart)를 참고하세요.

<div id="contents">
  ## Contents
</div>

이미 알고 있는 URL에서 highlights, 전체 텍스트, summary를 추출하세요. highlights로 시작한 다음, 질의를 추가해 필요한 정보에 초점을 맞출 수 있습니다.
...
더 넓은 컨텍스트나 문서 구조가 필요할 때는 전체 텍스트를 사용하세요. output 형태, freshness 제어, 하위 페이지 크롤링은 [Contents 가이드](/ko/docs/contents/quickstart)를 참고하세요.

<div id="answer">
  ## Answer
</div>

citations가 포함된 답변을 받아보세요.
...
Python은 비동기 작업을 위해 `AsyncExa`를 제공하며, JavaScript SDK는 모든 메서드에 대한 TypeScript 타입을 기본 포함합니다.
