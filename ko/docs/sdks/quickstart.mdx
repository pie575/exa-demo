...
| ------------ | --------------------------------------------------------- |
| 시작점          | `search` 사용                                               |
| search type  | 지연 시간이나 synthesis 요구사항으로 다른 타입이 필요한 경우가 아니면 `auto` 유지     |
| page content | `highlights: true`로 시작                                    |
| 알려진 URL      | `get_contents` / `getContents` 사용                         |
| freshness    | 오래된 콘텐츠를 사용할 수 없는 경우에만 `max_age_hours` / `maxAgeHours` 설정 |
...
Search는 관련 있는 페이지를 찾아 한 번의 call로 해당 contents까지 반환합니다.

<Tip>
  AI 답변, RAG, 검색 미리보기에는 `highlights: true`를 사용하세요. Exa는 각 결과의
  excerpt 길이를 관련성에 맞춰 조정하므로, 애플리케이션에 고정된 limit이 필요한 경우에만
...
<Note>
  ...
  `output.content`로 반환합니다. 소스 선호나 강조할 내용은 `system_prompt` / `systemPrompt`로 지정하세요.
  grounding은 `output.grounding`에 자동으로 반환되므로 schema에 citations나
  신뢰도를 중복해서 정의하지 마세요.
</Note>

출력물을 만들기 위해 여러 번의 searches에 걸친 조사가 필요하다면 deep 모드를 권장합니다. 가벼운 조사에는 `deep-lite`를, 다단계 search와 더 강력한 synthesis가 필요할 때는 `deep`을 사용하세요. 전체 요청 옵션은 [Search 가이드](/ko/docs/search/quickstart)를 참고하세요.

<div id="contents">
  ## Contents
</div>

이미 알고 있는 URL에서 highlights, 전체 텍스트 또는 summaries를 추출합니다. 먼저 highlights로 시작하고, query를 추가해 필요한 정보에 초점을 맞춰 보세요.
...
더 폭넓은 맥락이나 문서 구조가 필요하다면 전체 텍스트를 사용하세요. output 형태, freshness 제어, 하위 페이지 크롤링은 [Contents 가이드](/ko/docs/contents/quickstart)를 참고하세요.

<div id="answer">
  ## Answer
</div>

질문에 대한 답변을 citations과 함께 받아옵니다.
...
Python은 비동기 작업을 위한 `AsyncExa`를 제공하며, JavaScript SDK는 모든 메서드의 TypeScript 타입을 함께 제공합니다.
...
Python: [exa-py 소스](https://github.com/exa-labs/exa-py) 및 [PyPI 패키지](https://pypi.org/project/exa-py/). JavaScript: [exa-js 소스](https://github.com/exa-labs/exa-js) 및 [npm 패키지](https://www.npmjs.com/package/exa-js).
