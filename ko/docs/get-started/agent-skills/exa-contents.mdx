...
이 skill을 사용하면 agent가 모범 사례에 따라 cURL 또는 raw HTTP로 Exa Contents를 call하도록 학습시킬 수 있습니다.
...
다음 prompt는 skill을 설치하고 API 키를 출력하지 않은 채 검증합니다:

```text Copy this setup prompt into your agent theme={null}
이 머신에 Exa exa-contents agent skill을 설정하세요.

목표:
- exa-contents skill을 설치해서 내 coding agent가 cURL 또는 raw HTTP로 Exa Contents를 직접 call할 수 있게 한다.
- 이 대화에 키를 노출하거나, 출력하거나, 붙여넣는 일 없이 Exa API key가 동작하도록 한다.
...
1. 키 설정보다 먼저 skill을 설치하세요. repo 안에서 작업 중이라면 프로젝트 로컬 설치를 우선하고, 그렇지 않으면 위에 나열된 해당 전역 디렉터리를 사용하세요. 선택한 skills 디렉터리를 만들고 skill을 내려받으세요:
   ...
   그런 다음 <skills-dir>/exa-contents/SKILL.md 파일이 있는지 확인하세요.
2. 명령을 실행하는 당신 자신의 환경에서 Exa API key를 이미 사용할 수 있는지 확인하세요. 나에게 값을 echo해 달라고 요청하지 말고, skill을 실행할 때와 동일한 도구/셸을 사용하세요. skill은 먼저 EXA_API_KEY에서, 그다음 ~/.config/exa/key 파일에서 키를 찾으므로, 값을 절대 출력하지 말고 둘 다 확인하세요:
   printf '%s\n' "${EXA_API_KEY:+env-set}"; [ -s ~/.config/exa/key ] && printf 'file-set\n'
   당신의 셸은 비대화형일 가능성이 높고 ~/.zshrc나 ~/.bashrc 같은 대화형 프로필을 자동으로 source하지 않으므로, 내가 거기에 설정한 키가 나에게는 있는 것처럼 보여도 당신에게는 비어 있을 수 있습니다. 둘 다 나오지 않으면, 키가 당신의 셸이 건너뛰는 대화형 프로필에 있을 수 있습니다. 값을 출력하지 않고 어느 파일인지 찾으려면 `grep -l EXA_API_KEY ~/.zshrc ~/.zshenv ~/.bashrc ~/.profile ~/.config/fish/config.fish 2>/dev/null`을 사용하세요(이름만 나열됩니다. 프로필에 일반 `grep`/`cat`/`echo`를 절대 실행하지 마세요. `export EXA_API_KEY=...` 줄이 secret을 대화로 유출할 수 있습니다). 그런 다음 명령 안에서 해당 파일을 `source`하고 위 존재 여부 테스트를 다시 실행하세요. 값이 나오면 키가 필요한 이후 모든 명령 앞에 동일한 `source ...;`를 붙이세요.
3. 어디에서도 키를 찾을 수 없는 경우에만, 셸 프로필을 직접 편집하지 않고 이 대화에 키를 붙여넣지도 않는 방식으로 키를 설정하세요. https://dashboard.exa.ai/api-keys 에서 키를 만들거나 복사하라고 나에게 알려주고, 내가 직접 내 터미널에서 EXA_API_KEY를 export하거나 mode 600으로 ~/.config/exa/key에 기록하게 하세요. 키를 대화에 붙여넣으라고 절대 요청하지 마세요. 그런 다음 내가 완료했다고 확인해 줄 때까지 기다린 후 진행하세요.
...
전 과정에 적용되는 엄격한 규칙: 키는 secret입니다. 존재 여부/길이 확인(`${EXA_API_KEY:+set}`, `[ -s ~/.config/exa/key ]`)이나 HTTP 상태 코드로만 확인하세요. 키가 들어 있을 수 있는 파일이나 변수는 절대 출력하거나 `echo`, `cat`, 출력이 있는 `grep`으로 다루지 말고, 정규식으로 키 파일을 "가리려고" 시도하지도 마세요. 키가 노출되면 https://dashboard.exa.ai/api-keys 에서 교체하라고 알려주세요.
```
