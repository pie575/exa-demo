---
title: "변경 로그"
description: "Exa의 제품 업데이트 및 공지사항입니다."
rss: true
timestamp: false
---
...
<Update label="August 28, 2026" rss={{ title: "Dynamic Highlights (research preview)" }}>
  ...
  Dynamic Highlights는 각 페이지를 개별적으로 처리하는 대신 전체 결과 집합을 아울러 excerpt를 선택합니다. 유용한 출처에는 공유 컨텍스트 예산을 더 많이 배분하고, 이미 반환된 정보를 반복하기만 하는 출처에는 더 적게 배분합니다.

  * **단일 턴 RAG**: 코딩 및 일반 QA 평가 전반에서 Exa Auto 기준 토큰 효율이 약 49% 향상되고 다운스트림 품질이 2.4% 높아졌습니다.
  * **에이전트**: 에이전트 실행 경로 전체에서 토큰이 약 30% 줄었고, BrowseComp, WideSearch 및 내부 기업·인물 평가 전반에서 품질이 1% 높아졌습니다.

  `dynamic: true`를 설정하는 요청에는 `Exa-Beta: dynamic-highlights-2026-08-28` header가 필요합니다.

  [Dynamic Highlights 가이드 읽기 →](/ko/docs/contents/quickstart)
</Update>
...
<Update label="July 23, 2026" rss={{ title: "Publication research" }}>
  ...
  * **3.5억 건의 출판물**: 3억 5천만 건 규모의 출판물 인덱스를 search할 수 있습니다.
  * **더 풍부한 조직 및 인물 결과**: 이제 search 결과로 조직과 소속 인물이 함께 반환되며, 각각 출판물, 주요 공동 연구자, 연구 분야, 연구비 정보를 아우르는 상세 프로필로 제공됩니다.
  * **에이전트 기반 인물 및 조직 search**: 이제 에이전트가 인물과 조직을 대상으로 search할 수 있습니다.
  * **공개 retrieval 벤치마크**: 출판물 retrieval을 위한 공개 벤치마크를 공개했습니다.
  * **새로운 `publication` search 카테고리**: `category: "publication"`으로 학술 결과를 질의할 수 있으며, 기존 `research paper` 카테고리를 대체합니다.
  * **지원 중단 카테고리**: `pdf`, `github`, `tweet` search 카테고리는 지원 중단될 예정입니다.
  * **`startCrawlDate` / `endCrawlDate`**: 지원 중단된 파라미터로, 호환성을 위해 계속 허용되지만 이제 모든 team에서 무시됩니다.

  API에서 `publication` [search 카테고리](/ko/docs/search/quickstart)로 질의하거나, [dashboard에서 직접 사용해 보세요 →](https://dashboard.exa.ai/playground/search?type=instant).
</Update>
...
<Update label="July 1, 2026" rss={{ title: "Exa Agent and Exa Connect in MCP" }}>
  ...
  이제 Exa MCP에서 Exa Agent를 사용할 수 있습니다. 단일 search 호출만으로는 부족한 작업이라면 Claude, Cursor 등 모든 MCP 클라이언트에서 활용하세요.

  `https://mcp.exa.ai/mcp?tools=agent_run`으로 Agent 도구를 활성화한 다음, `agent_run`을 call하여 에이전트를 완료까지 실행하고 output을 반환받으세요.

  Exa Connect 데이터 소스는 Agent 플로우를 통해 사용할 수 있으므로, 실행에 web search만으로 부족할 때 프리미엄 data partners를 attach할 수 있습니다.

  [Exa MCP 가이드 읽기 →](/ko/docs/get-started/exa-mcp) · [Exa Agent 가이드 읽기 →](/ko/docs/agent/quickstart) · [공지 트윗 →](https://x.com/ExaAILabs/status/2072389192458592672)
</Update>
...
<Update label="June 24, 2026" rss={{ title: "Introducing Exa Connect" }}>
  ...
  [Exa Connect 가이드 읽기 →](/ko/docs/agent/connect/overview) · [공지 트윗 →](https://x.com/ExaAILabs/status/2069842203577651283)
</Update>
...
<Update label="June 16, 2026" rss={{ title: "Introducing Exa Agent" }}>
  ...
  Exa Agent API는 자연어 질의, `effort` 모드, structured output을 위한 `outputSchema`, 기존 데이터셋을 기반으로 확장하는 `input.data` 등의 파라미터를 지원합니다.

  [Exa Agent API 가이드 읽기 →](/ko/docs/agent/quickstart)
</Update>
...
<Update label="April 1, 2026" rss={{ title: "API 지원 중단 안내" }}>
  ...
  * **`startCrawlDate` / `endCrawlDate` (지원 중단된 요청 파라미터)**: 4월 15일부터 별도 알림 없이 무시됩니다.
...
<Update label="March 30, 2026" rss={{ title: "Exa Monitors 소개" }}>
  ...
  monitor는 Exa search를 정해진 일정에 따라 실행하고 그 결과를 webhook으로 전달합니다. 이전 실행 결과와 중복을 제거하므로 새로운 콘텐츠만 받아볼 수 있습니다.

  * **시간 흐름에 따른 주제 추적**: 경쟁사 소식, 투자 라운드, 규제 변화, 연구 papers.
...
<Update label="March 4, 2026" rss={{ title: "Exa Deep 개편" }}>
  ...
  Exa Deep이 더 빠르고 저렴해졌으며, 이제 필드 단위 grounding과 함께 structured output을 지원합니다.

  * 더 높은 effort가 필요한 작업을 위한 **새로운 `deep-reasoning` 타입**(12~~50초); `deep`은 4~~12초 만에 실행됩니다.
  * 일반 `deep` search **가격 20% 인하**.
  * `outputSchema`를 통한 **structured output** 지원, 응답에 `output.content`와 `output.grounding`(필드 단위 citations 및 신뢰도) 포함.
...
<Update label="March 3, 2026" rss={{ title: "Exa 가격 정책 업데이트" }}>
  ...
  가격 체계를 단순화하고 인하했습니다. 이제 상위 10개 search 결과의 contents가 무료로 포함되며, 새 가격은 별도 조치 없이 자동으로 적용됩니다.

  * **contents 포함 search**: 1천 건당 $7 (결과 10개, 텍스트 + highlights 포함); 추가 결과 1천 건당 $1.
  * **summaries**: search와 contents 모두 1천 건당 $1.
  * **Exa Deep**: 1천 건당 $12; **Deep (Reasoning)** 1천 건당 $15.
  * **Contents endpoint**: 콘텐츠 유형별 1천 페이지당 $1.
...
<Update label="February 5, 2026" rss={{ title: "Exa Instant Search 소개" }}>
  ...
  Exa Instant는 향상된 neural search 품질과 150ms 미만의 지연 시간을 겸비한, Exa에서 가장 빠른 search type입니다. `type="instant"`로 활성화하세요.

  * **실시간 처리를 위한 설계**: 채팅 앱, 음성 AI, coding agent, 자동 완성, 실시간 추천.
  * Exa가 제공하는 가장 낮은 지연 시간으로 구현한 **최고 수준의 품질**.
...
<Update label="February 2, 2026" rss={{ title: "highlights, content freshness, MCP 업데이트" }}>
  ...
  콘텐츠 extraction과 접근에 관한 세 가지 개선 사항:

  * **highlights용 `maxCharacters`**: 이제 highlight 길이를 제어하는 권장 방식입니다. `numSentences`와 `highlightsPerUrl`은 지원이 중단되었습니다.
  * **content freshness를 위한 `maxAgeHours`**: 불리언 `livecrawl`을 대체하는 경과 시간 기반 제어입니다(`0`은 항상 크롤링, `-1`은 캐시 전용, `24`는 24시간보다 오래된 경우 크롤링).
  * **Exa MCP 무료 티어**: 인증 없이 3 QPS, 하루 150회 call로 사용해 볼 수 있습니다. 전체 기능을 사용하려면 API key를 추가하세요.
...
<Update label="January 21, 2026" rss={{ title: "Exa Company Search 출시" }}>
  ...
  * **속성 전반의 높은 정확도**: 산업, 지역, 투자 단계, 임직원 수.
  * **구조화된 엔티티 데이터**: 타입이 지정된 기업 정보(인력, 본사, 재무, 웹 트래픽)를 반환합니다.
  * **활용 사례**: 세일즈 prospect 발굴, 시장 조사, 공급망 워크플로우.
...
<Update label="December 19, 2025" rss={{ title: "Exa People Search 출시" }}>
  ...
  이제 인물 search가 하이브리드 retrieval 시스템을 통해 10억 개 이상의 공개 프로필을 아우릅니다. `linkedin` 카테고리는 새로운 `people` 카테고리로 대체되었습니다.
...
<Update label="November 26, 2025" rss={{ title: "JS SDK: highlights 복원" }}>
  ...
  `exa-js` v2.0.11부터 JavaScript SDK에서 highlights가 다시 제공되며, 관련도 점수와 함께 핵심 문장을 반환합니다. search 및 contents 호출에서 `highlights: true` 또는 `highlights: { maxCharacters, query }`를 전달하세요.
...
<Update label="November 20, 2025" rss={{ title: "새로운 Deep search type" }}>
  ...
  Exa Deep은 여러 search를 동시에 실행하고 각 결과에 대해 고품질 컨텍스트를 반환해 더 나은 결과를 찾아냅니다. `type="deep"`으로 활성화하세요.

  * **질의 확장**: 질의를 하나만 보내면 Exa가 변형을 생성하며, `additionalQueries`로 직접 지정할 수도 있습니다.
  * **병렬 search와 스마트 랭킹**: 원본 질의와 모든 변형에 걸쳐 수행됩니다.
  * **상세한 summaries**: 각 결과마다 제공됩니다.
...
<Update label="November 5, 2025" rss={{ title: "언어 필터링 추가" }}>
  ...
  이제 Exa가 질의 언어를 감지해 해당 언어의 결과만 반환합니다. 모든 사용자에게 기본 활성화되며 별도의 설정이 필요하지 않습니다.
...
<Update label="October 28, 2025" rss={{ title: "SDK 변경: highlights 제거 및 contents 기본 반환" }}>
  ...
  * **contents 기본 반환**: 이제 search에 page contents가 포함되며, 더 빠른 search가 필요하면 해제할 수 있습니다.
...
<Update label="August 4, 2025" rss={{ title: "도메인 경로 필터 지원" }}>
  ...
  `includeDomains`와 `excludeDomains`가 더 세밀한 타기팅을 지원합니다:

  * **경로 단위 필터링**: 예) `exa.ai/blog`, `linkedin.com/company`.
...
<Update label="July 30, 2025" rss={{ title: "지리적 위치 필터 지원" }}>
  ...
  새로운 `userLocation` 파라미터는 [ISO 3166-1 alpha-2](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2) 국가 코드(예: `"us"`, `"fr"`)로 전달하며, 결과를 사용자의 지역에 맞게 보정합니다. 다지역 애플리케이션, 지역 언어 콘텐츠, 로컬 탐색에 유용합니다.
...
<Update label="July 29, 2025" rss={{ title: "New Fast Search Type" }}>
  ...
  Exa Fast는 p50 지연 시간이 425ms 미만인 간소화된 검색 모델을 사용합니다. `type="fast"`로 활성화하세요.

  * neural search와 **동일한 Exa 인덱스**의 고품질 콘텐츠를 사용합니다.
  * 다른 search type과 **파라미터가 완벽히 호환**됩니다.
...
<Update label="June 23, 2025" rss={{ title: "Markdown Contents as Default" }}>
  ...
  이제 모든 endpoint가 기본적으로 깔끔한 마크다운을 반환합니다. 마크다운은 LLM, RAG, 일반적인 텍스트 처리에 더 적합합니다. 별도의 조치는 필요하지 않습니다.

  * **`includeHtmlTags=false` (기본값)**: 콘텐츠가 깔끔한 마크다운으로 처리됩니다.
  * **`includeHtmlTags=true`**: 마크다운 처리를 거치지 않은 원본 HTML을 반환합니다.
...
<Update label="June 7, 2025" rss={{ title: "New Livecrawl Option: Preferred" }}>
  ...
  <Warning>
    과거 기록: `livecrawl` 문자열 파라미터는 현재 지원이 중단되었습니다. 새로 연동하는 경우 `maxAgeHours`와 `livecrawlTimeout`을 사용하세요. [Content Freshness](/ko/docs/contents/quickstart#content-freshness)를 참고하세요.
  </Warning>

  지원이 중단된 `livecrawl: "preferred"` 옵션은 새로 크롤링을 시도하되, 크롤링에 실패하면 캐시된 콘텐츠로 대체합니다(오류를 반환하는 `"always"`와 다릅니다). 일시적으로 접근할 수 없는 사이트 때문에 실패하는 일 없이 최신 콘텐츠를 얻고자 하는 프로덕션 앱에 적합합니다.
...
<Update label="May 22, 2025" rss={{ title: "Contents Endpoint Status Changes" }}>
  ...
  * **`error.tag`**: 예를 들어 `CRAWL_NOT_FOUND`, `CRAWL_TIMEOUT`, `SOURCE_NOT_AVAILABLE`이며, `httpStatusCode`가 함께 제공됩니다.

  [Error codes reference 보기 →](/ko/docs/admin/error-codes)
</Update>
...
<Update label="December 11, 2024" rss={{ title: "Auto search as Default" }}>
  ...
  이제 Auto search가 기본값이며, 각 질의를 최적의 검색 방식으로 자동 라우팅합니다. 별도의 조치는 필요하지 않으며, 이전 동작을 유지하려면 `type="neural"`을 설정하세요.
