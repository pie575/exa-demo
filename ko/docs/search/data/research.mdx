...
제목, 초록, 저자, 발표 학회/저널, 인용, 출판사 페이지, 프리프린트, repository 페이지 등 연구 출판물과 관련 레코드를 찾을 때 Exa Search를 활용하세요.

<Tip>
  출판물 검색 품질에 대해 자세히 알아보려면 [SOTA Search Over Academic Publications](https://exa.ai/blog/publications-search)를
  읽어보세요.
</Tip>
...
* 논문 및 프리프린트(전문 파싱이 가능한 경우 전문 청크 포함)
...
제목에 들어갈 키워드를 추측하지 말고, 방법론이나 연구 결과를 설명하세요. `publication` 카테고리를 사용하면 결과가 논문으로만 한정됩니다.
...
임상시험 registrations와 결과 페이지가 일반 기사보다 상위에 노출되도록 시험 단계, 중재, 대상 집단을 명시하세요.
...
여러 출처를 조사해야 하는 구조화된 데이터가 필요하다면 [Exa Agent 태스크 실행](/ko/docs/agent/quickstart)을 사용하세요. 필요한 출판물, 포함 기준, 출력 필드를 설명하면 Agent가 citations와 함께 schema로 검증된 결과를 반환합니다.

<Card title="Agent 태스크 시작하기" icon="bot" href="/ko/docs/agent/quickstart" cta="Agent 가이드 열기" arrow="true">
  문헌 지도를 만들거나, 포함 기준에 따라 논문을 선별하거나, 여러 출판물의 필드를 하나의 표로 모아보세요.
</Card>