...
sidebarTitle: "뉴스"
icon: "newspaper"
description: "Exa Search로 최신 보도, 업계 소식, 떠오르는 이슈를 찾아보세요."
---
...
주요 언론사, 업계 전문지, 틈새 매체의 보도를 찾으려면 Exa Search를 사용하세요. 새로 나온 기사는 발행 후 몇 분 이내에 검색할 수 있습니다. 발행 기간이 반드시 충족해야 할 조건이라면 자연어 질의와 날짜 필터를 함께 사용하세요.
...
주제, 출처 유형, 발행 기간을 명시하면 결과가 해당 이슈의 현재 국면에 집중됩니다.
...
질의에 형식과 주제를 함께 포함하세요. 이렇게 하면 웹 전반의 에피소드 페이지와 transcript까지 search 범위에 열어둘 수 있습니다.
...
찾으려는 부정적 신호와 조사 대상 엔티티 유형을 함께 설명하세요. 질의를 회사 이름에 &quot;news&quot;만 덧붙인 형태로 단순화하지 마세요.
...
여러 출처에 걸친 조사가 필요한 구조화된 데이터라면 [Exa Agent 작업 실행](/ko/docs/agent/quickstart)을 사용하세요. 필요한 기사, 필드, 기간을 설명하면 Agent가 schema로 검증된 결과와 citations를 반환합니다.
