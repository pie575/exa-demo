...
Exa Search를 사용하면 주가, 공시 자료, 전문, 그리고 이와 관련된 보도까지 한 번의 질의로 찾을 수 있습니다. 종목 티커에 대한 질문 하나로 시세, 최신 실적 발표 콜, 애널리스트 커버리지를 한꺼번에 받아볼 수 있습니다.

<div id="included">
  ## 포함 항목
</div>

* 주식, 암호화폐, 외환, 지수, 선물, 옵션, 원자재의 시세 및 최근 가격 이력
* 주요 통계와 일별 OHLCV 이력을 포함한 증권 프로필
* 발표자별로 구분된 발표 내용과 Q&amp;A가 담긴 실적 발표 콜 전문
...
티커나 회사 이름과 함께 원하는 수치를 입력하세요. `$NVDA` 같은 캐시태그도 사용할 수 있습니다.
...
관련 보도가 아닌 전문(transcript)을 가져오려면 회사명과 분기를 함께 지정하세요.
...
서식 유형만 적지 말고, 찾고자 하는 공시 내용을 구체적으로 설명하세요. `financial report` 카테고리를 쓰면 결과가 공시 자료와 보고서로 한정됩니다.
...
원하는 발표 자료와 그 안에서 확인하려는 수치를 명시하세요.
...
여러 소스에 걸친 리서치가 필요한 구조화된 데이터라면 [Exa Agent 태스크 실행](/ko/docs/agent/quickstart)을 사용하세요. 필요한 증권, 기간, criteria, 출력 필드를 설명하면 Agent가 schema로 검증된 결과를 citations와 함께 반환합니다.
