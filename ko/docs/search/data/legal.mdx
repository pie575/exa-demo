---
title: "법률 및 공공 records"
sidebarTitle: "법률 및 공공 records"
icon: "scale"
description: "Exa Search로 판례, 특허, 제재 목록, 정부 계약 등 다양한 공공 records를 찾아보세요."
---
...
Exa Search로 1차 법률 소스와 정부 records는 물론, 이에 대한 해설 자료까지 함께 찾아보세요.
...
인용 형식이 아니라 평이한 표현으로 법률 쟁점과 관할을 설명하세요.
...
여러 소스에 걸친 리서치가 필요한 구조화된 데이터라면 [Exa Agent task run](/ko/docs/agent/quickstart)을 사용하세요. 필요한 관할 구역, 레코드 유형, criteria, output field를 설명하면 Agent가 schema로 검증된 결과를 citations와 함께 반환합니다.

<Card title="Agent 작업 시작하기" icon="bot" href="/ko/docs/agent/quickstart" cta="Agent 가이드 열기" arrow="true">
  여러 레코드 유형에 걸쳐 특정 엔티티를 검토하거나, 1차 소스와 관련 보도를 통해 규제 변화를 추적하세요.
</Card>