...
icon: "users"
description: "Exa Search로 기업과 경력 프로필, 그리고 이들 사이의 관계를 찾아보세요."
---
...
Exa Search로 기업과 그와 연결된 인물을 찾아보세요. 이 두 search는 함께 사용할 때 가장 효과적입니다. 특정 인물을 적합한 후보로 만드는 기업 특성을 설명하거나, 기업의 운영 방식을 보여주는 인물과 직무를 설명해 보세요.

<Columns cols={2}>
  <Card title="기업 retrieval 벤치마크" icon="building" href="https://exa.ai/blog/company-search-benchmarks">
    Exa가 기업 retrieval과 사실 extraction을 어떻게 평가하는지 확인해 보세요.
  </Card>

  <Card title="인물 retrieval 벤치마크" icon="users" href="https://exa.ai/blog/people-search-benchmark">
    Exa가 타깃 조회와 프로필 발굴을 어떻게 평가하는지 확인해 보세요.
  </Card>
</Columns>
...
찾으려는 대상을 먼저 제시한 다음, 그 대상을 한정하는 특성과 관계를 덧붙이세요. 회사 홈페이지, 경력 프로필, 채용 공고, 개인 웹사이트처럼 출처 유형이 중요한 경우에는 그것도 함께 명시하세요.
...
<Tabs>
  <Tab title="회사" icon="building">
    ...
    해당 시장을 규정하는 고객, 제품, 역량, 단계, 지역을 설명하세요. 미리 정해둔 회사 목록에 의존하는 대신, 실제로 하는 일을 기준으로 후보를 찾아냅니다.
    ...
    찾으려는 신호와 중요하게 볼 회사 특성을 명시하세요. search는 회사 페이지뿐 아니라 채용 공고, 가격 페이지, 제품 문서, 관련 기사까지 가져올 수 있습니다.
  ...
  <Tab title="사람" icon="users">
    ...
    원하는 직무, 연차, 지역, 관련 역량, 출처 유형을 조합하세요.
    ...
    해당 인물과 회사의 관계, 그리고 그 회사를 한정하는 특성을 설명하세요. 회사 목록을 먼저 만드는 방식보다 결과가 더 좋습니다.
    ...
    직종이나 연구 분야를 명시하고, 개인 웹사이트, 강연, 인터뷰, 기고문 중 무엇을 원하는지 분명히 요청하세요.
...
필요한 관계를 한 번에 표현하는 질의를 작성하세요. Exa는 기업 페이지, 경력 프로필, 채용 페이지, 공개 자료를 하나의 결과 집합으로 반환할 수 있습니다.
...
여러 출처에 걸친 리서치가 필요한 구조화된 데이터라면 [Exa Agent 작업 실행](/ko/docs/agent/quickstart)을 활용하세요. 필요한 기업, 인물, 선별 기준, 출력 필드를 설명하면 Agent가 schema로 검증된 결과와 출처 인용을 반환합니다.

<Card title="Agent 작업 시작하기" icon="bot" href="/ko/docs/agent/quickstart" cta="Agent 가이드 열기" arrow="true">
  기업 또는 인물 목록을 구축하고 조건에 맞는지 검증한 다음, 여러 출처에서 수집한 필드로 각 레코드를 보강하세요.
</Card>