...
이를 활용해 agent를 backtest하고, 재현 가능한 평가를 실행하며, 문서, 가격 페이지, 정책, 공시 자료의 이전 버전을 비교할 수 있습니다.

<Info>
  Exa Snapshot은 Pay as you go에서 10 QPS로 제공되며, 최근 5개월 단위의 롤링 인덱스 범위를 지원합니다.
  요청 100회 이후에도 계속 사용하려면 [영업팀에 문의하세요](https://exa.ai/contact/sales).
</Info>

## 특정 datetime 시점으로 search하기 {#search-at-a-datetime}
...
Exa는 후보 URL을 찾은 뒤, `snapshotAsOf` 시점이거나 그 이전의 stored version이 있는 페이지만 남깁니다.
...
Exa는 해당 datetime 이하에서 가장 최근의 stored version을 반환합니다.
...
<Tip>
  조건에 맞는 버전이 없는 ID는 `results`에서 제외되며, `statuses`에 `"status": "error"`와
...
| Field          | 위치 | 의미                                                                 |
| -------------- | -- | ------------------------------------------------------------------ |
| `snapshotAsOf` | 요청 | datetime 컷오프. Exa는 이 시점 또는 그 이전의 stored version 중 가장 최신 버전을 반환합니다. |

두 엔드포인트 모두:

* 반환되는 페이지 콘텐츠는 해당 stored version에서 가져옵니다.
* 제목, 작성자, 발행일, 텍스트, highlights, summary는 모두 해당 버전만을 기반으로 생성됩니다.
* 5개월 기간 내에 조건에 맞는 버전이 없는 페이지는 제외됩니다.

<Note>
  search에서 컷오프는 콘텐츠의 범위만 제한할 뿐 랭킹에는 영향을 주지 않습니다. Exa는 후보 URL을
  찾기 위해 여전히 현재의 검색 신호를 사용합니다. 결과는 `snapshotAsOf`로 범위가 제한된 evidence로
  활용하시고, 당시 search가 매겼을 랭킹을 그대로 재현한 결과로 보지 마십시오.
</Note>

## Limits 및 호환성 {#limits-and-compatibility}

<AccordionGroup>
  <Accordion title="접근, 속도 제한, 조회 가능 기간">
    Pay as you go에는 10 QPS와 최근 5개월 롤링 인덱스 접근이 포함됩니다. 이 기간을 벗어난
    `snapshotAsOf` 값은 거부됩니다. 요청 100회를 초과해 계속 사용하려면 [영업팀에 문의](https://exa.ai/contact/sales)하세요.
  </Accordion>

  <Accordion title="과거 시점 요청은 저장된 content를 사용합니다">
    `snapshotAsOf`는 실시간 웹에 접근하거나 다른 페이지로 확장될 수 있는 옵션과 함께 사용하지 마세요.
    `livecrawl`, `livecrawlTimeout`, `maxAgeHours`, `subpages`는 아예 생략해야 하며, `snapshotAsOf`와
...
특정 datetime 시점에 Exa가 저장해 둔 내용에 작업 결과가 좌우되는 경우 Exa Snapshot을 사용하세요:

* 이후 페이지 업데이트의 영향을 받지 않도록 agent를 backtest합니다.
* 재현 가능한 콘텐츠 기준 시점을 놓고 평가를 실행합니다.
