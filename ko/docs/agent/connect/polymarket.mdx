---
title: "Polymarket"
description: "예측 시장의 배당률, 가격 이력, order books, trader 포지션을 가져옵니다."
...
Polymarket의 공개 시장 데이터에 대한 읽기 전용 액세스를 제공합니다.

[Exa Agent](/ko/docs/agent/quickstart) run에 `polymarket`을 attach하면
agent가 Exa web search와 함께 Polymarket을 조회합니다.

<div id="use-it-for">
  ## 활용 사례
</div>

* 특정 주제에 대한 prediction markets와 현재 market-implied odds 찾기
* 특정 결과의 implied probability가 시간에 따라 어떻게 변했는지 비교하기
* 시장 유동성, 매수/매도 호가 깊이, 상위 포지션 보유자 살펴보기
...
비용이 발생하지 않습니다. 표준
[Agent run 가격](/ko/docs/agent/quickstart#pricing)만 지불하면 됩니다.
...
| 가격 히스토리      | 특정 결과의 implied probability가 시간에 따라 변화한 추이.                             |
| Order books  | 마켓 결과에 대한 실시간 매수/매도 호가 깊이와 스프레드.                                       |
...
연준 금리 인하에 대한 market-implied odds와 지난 한 달 동안의 변화 추이를 확인합니다.
...
* [Exa web search](/ko/docs/search/quickstart): 시장 배당률에 보도 내용과 배경 정보를 더합니다.
* [Particle](/ko/docs/agent/connect/particle): 배당률 변동의 배경이 된 뉴스 보도를 가져옵니다.
* [Financial Datasets](/ko/docs/agent/connect/financialdatasets): market-implied odds을 가격, 펀더멘털, 거시 경제 데이터와 연결합니다.
