...
[Jinko](https://gojinko.com)는 실시간 가격이 반영된 항공편 및 호텔 검색을 제공하는 여행 검색 플랫폼입니다. 특정 노선과 날짜의 실시간 항공권 오퍼를 검색하고, 목적지나 특정 숙소의 호텔 객실과 객실 요금을 조회하며, 출발 공항에서 갈 수 있는 목적지를 살펴볼 수 있습니다.

[Exa Connect](/ko/docs/agent/connect/overview)를 통해 [Exa Agent](/ko/docs/agent/quickstart) 실행에 `jinko`를 attach하면, agent가 Exa web search와 함께 Jinko를 조회합니다.

## 활용 사례 {#use-it-for}

* 특정 노선과 날짜의 운임, 수하물, 변경 규정을 포함한 실시간 항공권 오퍼 검색
...
* [Similarweb](/ko/docs/agent/connect/similarweb): 특정 여행지를 다루는 여행 사이트와 예약 플랫폼을 리서치합니다.
...
<Columns cols={2}>
  ...
  <Card title="여러 제공업체 조합하기" icon="blend" href="/ko/docs/agent/connect/combining-providers" cta="가이드 읽기" arrow="true">
    하나의 실행에 최대 다섯 개의 파트너를 attach하고, 각 파트너가 모두 호출되도록 질의를 설계하세요.
  </Card>
