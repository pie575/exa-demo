...
<Info>
  예제를 실행하기 전에 키를 `EXA_API_KEY` 환경 변수로 설정하세요. 패키지가 이 환경 변수를 자동으로 읽어옵니다.
</Info>
...
| `numResults`                              | 반환할 결과 수입니다.                                                                                       |
| `includeDomains` / `excludeDomains`       | 특정 도메인을 포함하거나 제외합니다.                                                                               |
| `startPublishedDate` / `endPublishedDate` | ISO 8601 형식의 게시 날짜로 결과를 필터링합니다.                                                                    |
| `includeText` / `excludeText`             | 결과에 특정 텍스트를 반드시 포함하거나 제외합니다.                                                                       |
| `userLocation`                            | 위치를 반영한 search를 위한 두 자리 국가 코드입니다.                                                                  |
