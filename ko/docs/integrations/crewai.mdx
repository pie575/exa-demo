---
title: "CrewAI"
description: "CrewAI agent에 Exa retrieval capabilities를 추가하는 방법을 알아보세요."
...
<Card title="Coding Agent Quickstart" icon="rocket" horizontal href="https://dashboard.exa.ai/onboarding">
  Exa를 처음 사용하시나요? 1분 안에 시작할 수 있습니다.
</Card>

***

[CrewAI](https://crewai.com/)는 복잡한 작업을 함께 수행하는 AI agent들을 조율하는 프레임워크입니다.
이 가이드에서는 Exa의 search 결과를 바탕으로 뉴스레터를 작성하는 두 개의 agent로 구성된 crew를 만들어 봅니다. 다루는 내용은 다음과 같습니다:

1. Exa 기반 커스텀 CrewAI 도구 만들기
2. agent를 설정하고 Exa 기반 search 도구를 사용하는 역할을 각각 부여하기
3. agent들을 뉴스레터를 작성할 crew로 구성하기

<Note>
  CrewAI는 커스텀 래퍼를 작성하지 않고도 바로 사용할 수 있는 내장 [`ExaSearchTool`](https://docs.crewai.com/en/tools/search-research/exasearchtool)도 제공합니다. 아래의 커스텀 도구는 결과 형식을 완전히 제어하고 싶을 때 유용하며, 두 방식 모두 문제없이 사용할 수 있습니다.
</Note>
...
<Steps>
  <Step title="사전 요구 사항 및 설치">
    crewAI 코어, crewAI tools, Exa Python SDK 라이브러리를 설치합니다.
  ...
  <Step title="crewAI에서 맞춤형 Exa 기반 도구 정의하기">
    crewAI의 [@tool 데코레이터](https://docs.crewai.com/concepts/tools#utilizing-the-tool-decorator)를 사용해 [커스텀 도구](https://docs.crewai.com/concepts/tools)를 만듭니다. 이 도구 안에서 [Exa Python SDK](https://github.com/exa-labs/exa-py)의 Exa 클래스를 초기화하고 요청을 보낸 뒤 파싱된 결과를 반환할 수 있습니다.
  ...
  <Step title="crewAI agent 설정">
    ...
    * 다른 하나는 뉴스레터를 output으로 작성하는 agent(LLM 사용)

    ```Python Python theme={null}
    # 메모리와 verbose 모드를 갖춘 시니어 연구원 agent 생성
    researcher = Agent(
      role='Researcher',
  ...
  <Step title="agent의 작업 정의하기">
    다음으로, 각 agent에 대한 [태스크](https://docs.crewai.com/concepts/Tasks/)를 정의하고, 위에서 설정한 모든 구성 요소를 활용해 전체 크루를 생성합니다.
