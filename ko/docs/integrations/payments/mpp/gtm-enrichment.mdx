...
sidebarTitle: "MPP로 GTM enrichment 하기"
description: "Exa search 및 contents 요청 건별로 Tempo MPP를 통해 비용을 지불하는 GTM enrichment workflow를 구축하세요 — API key가 필요 없습니다."
icon: "/docs/images/sidebar-icons/tempo.svg"
---

이 쿡북을 활용해 Exa의 `/search` 및 `/contents` endpoint 위에 GTM enrichment agent 또는 파이프라인을 구축하고, Machine Payments Protocol(MPP)을 통해 요청 건별로 비용을 지불하세요. MPP는 여러 결제 수단을 지원하며, 여기서 다루는 예제는 [Tempo](https://tempo.xyz) 상의 스테이블코인을 사용합니다. 월 구독료도, API key도, 좌석 기반 요금제도 없습니다. wallet에 USDC.e를 충전해 두고 리드나 기업을 enrich한 만큼만 지불하면 됩니다.

<Info>
  MPP는 현재 Exa의 `/search` 및 `/contents` endpoint에서만 지원됩니다.
  Agent API(`/agent/runs`)와 `/answer`는 Exa API key가 필요하며 표준 API key billing 흐름을 따릅니다.
</Info>
...
회사명 또는 대상 설명 목록을 입력받아 동작하는 가벼운 enrichment 파이프라인입니다:

1. `type: "deep"`과 `outputSchema`를 지정한 Exa `/search`로 공식 회사 페이지를
   찾고 핵심 메타데이터를 추출합니다.
2. 반환된 결과에 `contents.highlights`를 사용해 투자 유치, 본사, 임직원 수,
   제품에 대한 출처 스니펫을 가져옵니다.
3. 입력 항목마다 CSV 또는 JSON enrichment 레코드를 생성합니다.

이 패턴은 리드 목록 enrichment, 계정 리서치, 아웃바운드 개인화에 활용할 수
있습니다. 개별적인 `/search` + `/contents` 호출로 구성되어 있으므로 모든 단계를
MPP로 결제할 수 있습니다.
...
* 런타임에 wallet private key를 안전하게 불러오는 방법(아래 참고. key를 절대 커밋하거나
  소스 코드에 노출하지 마세요).
...
<div id="load-your-private-key-safely">
  ### private key 안전하게 불러오기
</div>

private key를 절대 하드코딩하지 마세요. 아래 예제는 로컬 개발 환경에서만 사용하는 방식으로, 런타임 환경 변수에서 `WALLET_PRIVATE_KEY`를 읽어옵니다. 프로덕션에서는 1Password, AWS Secrets Manager, HashiCorp Vault 같은 secret 관리 도구에서 불러오세요.

<CodeGroup>
  ```bash TypeScript theme={null}
  # 셸 또는 CI secret 저장소에 설정하세요. 이 값은 절대 커밋하지 마세요
...
요청이 성공하면 Exa 결과와 함께 온체인 트랜잭션 해시가 담긴 `Payment-Receipt` header가 반환됩니다.
...
<Note>
  여기서는 더 풍부한 추론을 위해 `type: "deep"`을, 응답 형태를 지정하기 위해
  `outputSchema`를 사용합니다. Deep search 요금은 요청당 $0.012이며,
...
Exa는 지정한 schema에 맞는 `output` 객체를 반환합니다.
...
<Note>
  `outputSchema`는 `deep-lite` 또는 `deep` search type에서 가장 잘 작동합니다. Exa 측에서
  LLM call이 한 번 추가되므로 요금은 `deep-lite`/`deep` 기준으로 청구됩니다.
</Note>
...
MPP는 API key billing과 동일한 요청당 가격 정책을 따릅니다. MPP search 요청은
결과가 최대 10개로 제한됩니다.
...
* **결과 10개 상한에 유의하세요.** MPP search는 `numResults`를 최대 10으로 제한합니다.
* **private keys를 절대 커밋하지 마세요.** `WALLET_PRIVATE_KEY`는 소스 관리 시스템이 아닌
...
* [MPP(Tempo)로 결제하기](/ko/docs/integrations/payments/mpp/quickstart): 전체 MPP 레퍼런스
* [Exa Search API 가이드](/ko/docs/search/quickstart): search 매개변수 레퍼런스
* [Exa Contents API 가이드](/ko/docs/contents/quickstart): contents 매개변수 레퍼런스
