...
OpenAI의 [도구 호출](https://platform.openai.com/docs/guides/function-calling?lang=python)을 사용하면 모델이 코드에 정의한 함수를 호출할 수 있습니다. Exa SDK는 OpenAI용 web search 및 페이지 읽기 도구를 기본으로 제공하므로, 도구 schema를 직접 작성하거나 도구 호출을 파싱하거나 Exa 결과를 직접 포맷팅할 필요가 없습니다.
...
<Steps>
  ...
  <Step title="API key 설정">
    `EXA_API_KEY`와 `OPENAI_API_KEY` 환경 변수를 설정하세요. [OpenAI dashboard](https://platform.openai.com/api-keys)와 [Exa dashboard](https://dashboard.exa.ai/api-keys)에서 API key를 생성할 수 있습니다.
  ...
  <Step title="도구 루프에 Exa 도구 추가">
    ...
    간결하게 보여주기 위해 한 라운드만 다룬 예시입니다. 실제 agent는 모든 요청에 `tools`를 포함하고, 모델이 도구 호출 없이 응답할 때까지 handler 단계를 반복합니다. 이렇게 해야 search 결과가 후속 페이지 읽기로 이어집니다.

    factory를 인자 없이 호출하면 Exa의 권장 기본값이 적용됩니다. search의 경우 `type="auto"`와 `contents={"highlights": True}`입니다. highlights는 query와 관련된 excerpt를 반환할 뿐, 페이지 텍스트를 10,000자로 제한하지는 않습니다. contents factory는 페이지 텍스트를 반환하며, SDK의 10,000자 limit은 `text`에만, 그것도 `max_characters`를 생략한 경우에만 적용됩니다.
  </Step>
</Steps>
...
<Note>
  Chat Completions와 Responses API는 서로 다른 도구 형식을 사용하며 상대방의 형식을 허용하지 않으므로, 호출하려는 endpoint에 맞는 factory를 사용하세요.
</Note>
...
모델은 검색 `query`와 읽어올 `urls`만 선택합니다. 나머지는 모두 도구를 생성할 때 고정되므로, 모델이 크롤링하거나 추출할 대상을 바꿀 수 없습니다.

반면 `name`(기본값은 `"web_search"`와 `"get_contents"`)과 `description`은 모델에게 보이는 도구 정의를 재정의합니다. 서로 다르게 구성한 Exa 도구를 나란히 사용하거나, 해당 이름을 선점하는 다른 도구와의 충돌을 피하려면 `name`을 직접 지정하세요.

<div id="mixing-in-your-own-tools">
  ## 직접 만든 도구 함께 사용하기
</div>

handler는 메시지에 포함된 모든 도구 호출에 응답합니다. handler가 처리할 수 없는 도구를 지정한 호출도 그냥 버려지지 않고 `Error: unknown tool "<name>"` 출력을 받으므로, 후속 요청에서 필수 도구 응답이 빠지는 일이 없습니다. Exa의 도구와 함께 직접 만든 도구를 실행한다면, 다음 요청을 보내기 전에 이 오류 출력을 직접 만든 결과로 교체하세요.

<div id="writing-the-loop-by-hand">
  ## 직접 루프 작성하기
</div>

도구 schema와 실행을 직접 관리하고 싶다면, 도구를 정의하고 호출을 수동으로 처리하면 됩니다. `exa.tools.web_search()`와 `exa.tools.get_contents()`는 직접 구현한 루프에서 사용할 수 있도록 provider 중립적인 동일한 도구 사양(`run` 메서드 포함)을 제공하며, 처음부터 전부 직접 작성해도 됩니다:
