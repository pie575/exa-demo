...
title: "n8n"
sidebarTitle: "n8n"
description: "n8n 워크플로우 안에서 Exa search와 contents를 사용하세요."
icon: "/docs/images/sidebar-icons/n8n.svg"
---

공식 [Exa node for n8n](https://github.com/exa-labs/n8n-integration)을 사용하면 web search, content extraction, 근거 기반 답변, Exa Agent 실행을 시각적 워크플로우에 추가할 수 있습니다. 일반 워크플로우 단계로 사용하거나 n8n AI Agent에 도구로 연결하세요.
...
<Steps>
  <Step title="community node 추가">
    n8n node picker에서 **Exa**를 검색하세요. 사용 중인 instance에 없다면, instance 소유자가 n8n의 [community node 설치 가이드](https://docs.n8n.io/integrations/community-nodes/installation/)에 따라 `n8n-nodes-exa-official`을 설치할 수 있습니다.
...
| **Contents** | URL 목록에 대해 정제된 텍스트, highlights, summary, 링크, 이미지를 가져옵니다.                                                                        |
| **Answer**   | citations와 선택적 structured output을 포함한 근거 기반 답변을 생성합니다.                                                                          |
| **Agent**    | 여러 단계로 이루어진 Agent 실행을 생성, 조회, 목록 조회, 스트리밍, 폴링, 취소합니다.                                                                           |

## n8n AI Agent와 함께 Exa 사용하기 {#use-exa-with-an-n8n-ai-agent}

Exa node를 **AI Agent** node의 도구 입력에 연결하세요. 모델이 채워야 하는 parameters에는 n8n의 `$fromAI()` 표현식을 사용할 수 있습니다:

```javascript theme={null}
{{ $fromAI("query", "What should Exa search for?", "string") }}
```

Search와 Answer는 grounding 도구로 적합합니다. 다단계 리서치, 리스트 구축, 구조화된 enrichment, 또는 프리미엄 [Exa Connect](/ko/docs/agent/connect/overview) 데이터가 필요한 작업이라면 Agent 리소스를 사용하세요.

## Agent 실행 대기 {#wait-for-an-agent-run}

Agent 실행을 생성할 때 **Wait for Completion**은 다음을 지원합니다:

* **스트리밍**: 실행이 끝날 때까지 server-sent events 연결을 하나 열어 둡니다
* **폴링**: 일정한 주기로 실행 상태를 확인합니다

오래 걸리거나 비동기적인 워크플로우에서는 **Wait for Completion**을 끄고, 반환된 실행 `id`를 저장한 뒤 나중에 **Get Run**을 사용하세요. n8n 단계가 끝난 뒤에도 실행은 Exa에서 계속 진행됩니다.

## 문제 해결 {#troubleshooting}

<AccordionGroup>
  <Accordion title="node picker에 Exa node가 보이지 않습니다">
    instance 소유자에게 검증된 커뮤니티 패키지 `n8n-nodes-exa-official`을 설치해 달라고 요청하세요. 커뮤니티 node 사용 가능 여부는 n8n instance의 호스팅 방식에 따라 달라질 수 있습니다.
  </Accordion>

  <Accordion title="Exa credential이 거부됩니다">
    선택한 credential에 [Exa dashboard](https://dashboard.exa.ai/api-keys)에서 발급한 활성 키가 들어 있는지, 그리고 해당 키에 사용 가능한 credits이 남아 있는지 확인하세요.
  </Accordion>
...
<Columns cols={3}>
  ...
  <Card title="Search 모범 사례" icon="search" href="/ko/docs/search/best-practices" cta="가이드 읽기" arrow="true">
    더 나은 쿼리를 작성하고 알맞은 search mode를 선택하세요.
  </Card>
</Columns>