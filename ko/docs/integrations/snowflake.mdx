---
title: "Snowflake"
description: "Snowflake 저장 프로시저와 Cortex Agent에서 Exa search와 콘텐츠 retrieval을 사용하세요."
...
2. **Data enrichment** — CEO/창업자 정보, 최신 뉴스 등의 웹 데이터로 테이블을 batch 방식으로 enrich합니다
...
Snowflake 워크시트에서 다음 SQL을 실행하세요. `<<YOUR_EXA_API_KEY>>`는 실제 API 키로 바꿔야 합니다.
...
* `api.exa.ai:443`으로의 아웃바운드만 허용하는 **network rule**
* API key를 저장하는 **secret** (암호화되어 SQL에서는 보이지 않음)
* 해당 rule과 secret을 하나로 묶는 **external access integration**
...
| `include_domains`      | `STRING`  | `NULL`   | 검색 대상으로 한정할 도메인(쉼표로 구분)                                                         |
...
Exa는 플랫폼을 벗어나지 않고도 Snowflake 테이블을 CEO/창업자 정보, 최신 뉴스 등 실시간 웹 데이터로 enrich할 수 있습니다.
...
| `max_workers` | 사용 사례                                                         |
| ------------- | ------------------------------------------------------------- |
| `5`           | 보수적 — rate limits에 여유 있게 안전한 수준                               |
...
Exa 저장 프로시저를 [Snowflake Cortex Agent](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-agents)의 도구로 등록하여, 대화형으로 웹을 검색할 수 있는 기능을 부여할 수도 있습니다.
...
* &quot;최신 AI agent 프레임워크를 웹에서 검색해줘&quot;
* &quot;retrieval augmented generation에 대한 연구 논문을 찾아줘&quot;
* &quot;주목할 만한 AI 기업은 어디야?&quot;
...
| 계층                              | 세부 사항                                        |
| ------------------------------- | -------------------------------------------- |
| **역할**                          | 프로시저 및 데이터에 대한 범위 제한 접근을 위한 `exa_agent_role` |
| **Network Rule**                | 아웃바운드 트래픽을 `api.exa.ai:443`으로만 제한            |
| **Secret**                      | API key를 암호화하여 저장하며 SQL 쿼리에서는 보이지 않음         |
| **External Access Integration** | network rule과 secret을 하나로 묶어 바인딩             |
