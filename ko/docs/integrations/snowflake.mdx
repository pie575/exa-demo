---
title: "Snowflake"
description: "Snowflake 저장 프로시저와 Cortex Agent에서 Exa search와 콘텐츠 검색을 활용하세요."
icon: "/docs/images/sidebar-icons/snowflake.svg"
---

Exa의 Search API를 Snowflake에서 바로 호출해, SQL을 벗어나지 않고도 실시간 웹 정보로 데이터를 보강하세요.

이 연동은 Snowflake의 [External Access](https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview) 기능을 활용해 저장 프로시저에서 Exa API를 안전하게 호출하며, 다음 두 가지 주요 워크플로우를 지원합니다.

1. **Cortex Agent 도구** — Snowflake Cortex Agent가 웹을 search하고 page contents를 가져올 수 있도록 합니다
2. **데이터 보강** — CEO/창업자 정보, 최신 뉴스 등 웹 데이터로 표를 batch 단위로 enrich합니다

## 사전 준비 사항 {#prerequisites}

1. Exa API key
2. `ACCOUNTADMIN` role (External Access Integration 생성에 필요)
...
Snowflake 워크시트에서 다음 SQL을 실행하세요. `<<YOUR_EXA_API_KEY>>`는 실제 키로 바꿔주세요.

### Roles 및 데이터베이스 {#roles-and-database}
...
### Network rule, secret 및 외부 액세스 {#network-rule-secret-and-external-access}
...
* network rule과 secret을 하나로 묶는 **external access integration**

## 저장 프로시저 {#stored-procedures}

### EXA_SEARCH {#exa_search}

Exa의 search API를 사용해 웹을 검색합니다. 제목, URL, score를 포함한 결과를 반환하며, 선택적으로 텍스트/highlights/summary도 함께 반환합니다.
...
Exa는 CEO/창업자 정보, 최신 뉴스 등 실시간 웹 데이터로 Snowflake 표를 보강할 수 있으며, 이 모든 작업을 플랫폼을 벗어나지 않고 수행할 수 있습니다.
...
## Exa 요청 병렬 처리 {#parallelizing-exa-requests}
...
### 동시 enrichment 처리 절차 {#concurrent-enrichment-procedure}
...
이 연동은 Snowflake의 내장 보안 기능을 사용합니다:

| 계층                              | 상세 내용                                       |
| ------------------------------- | ------------------------------------------- |
| **Roles**                       | 프로시저와 데이터에 대한 범위 제한 접근을 위한 `exa_agent_role` |
| **Network Rule**                | 아웃바운드 트래픽을 `api.exa.ai:443`으로만 제한           |
| **Secret**                      | API 키를 암호화하여 저장하며 SQL 쿼리에 노출되지 않음           |
| **External Access Integration** | network rule과 secret을 함께 묶어 연결              |
