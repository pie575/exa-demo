...
keywords: ["상태", "uptime", "incident", "장애", "성능 저하", "점검"]
---
...
상태 page에서 구독하면 incident가 생성, 업데이트 또는 해결될 때 알림을 받을 수 있습니다. 알림은 이메일, Slack, Google Chat 또는 webhook으로 전달됩니다.

<div id="get-help">
  ## 도움 받기
</div>

모든 시스템이 정상으로 표시되는데도 요청이 계속 실패한다면 원인은 요청 자체에 있을 가능성이 높습니다. 먼저 [Error codes](/ko/docs/admin/error-codes)를 확인하세요.

<Columns cols={2}>
  <Card title="이메일 지원" icon="mail" href="mailto:hello@exa.ai" cta="이메일 지원" arrow="true">
    가장 빠르게 진단받으려면 응답 상태, error body, `requestId`를 함께 보내주세요.
  </Card>
