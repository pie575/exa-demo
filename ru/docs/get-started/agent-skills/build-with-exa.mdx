---
title: "Навык Build with Exa"
sidebarTitle: "Build with Exa"
description: "Навык для агента, который помогает разработчикам использовать любую часть платформы Exa API."
keywords: ["build with exa", "agent skills", "exa api", "sdk"]
---

Используйте этот навык, чтобы научить своего агента создавать приложения и агентов на API Exa с соблюдением лучших практик.

<Card title="Получите свой Exa API key" icon="key" horizontal href="https://dashboard.exa.ai/api-keys">
  Создайте ключ в панели управления. Новые аккаунты получают бесплатные кредиты.
</Card>

<Note>
  Задайте ключ в переменной `EXA_API_KEY` в окружении агента.
</Note>
...
**Вариант Б: скопируйте этот промпт в своего кодинг-агента.**

Следующий промпт устанавливает навык и проверяет ваш API key, не выводя его на экран:
...
<Card title="build-with-exa/SKILL.md" icon="file-code" href="https://raw.githubusercontent.com/exa-labs/agent-skills/main/skills/build-with-exa/SKILL.md" cta="Посмотреть исходный код" arrow="true">
