---
title: "Спорт, погода и места"
sidebarTitle: "Спорт, погода и места"
icon: "map-pin"
description: "Находите спортивные данные в реальном времени, прогнозы погоды и места поблизости с помощью Exa Search."
---
...
Используйте Exa Search для получения актуальных спортивных данных, прогнозов погоды и местной информации — без подключения отдельного API под каждую задачу. Просто задайте вопрос на естественном языке, указав интересующую команду, место и временной интервал.

<div id="write-better-queries">
  ## Пишите более точные запросы
</div>

Точно называйте место или команду и указывайте дату, если ответ меняется со временем. Добавляйте условие или признак, важный для вашей задачи, вместо того чтобы запрашивать общую информацию.
...
<Tabs>
  <Tab title="Спорт" icon="trophy">
    ...
    * **Расписания**: прошедшие результаты и предстоящие игры лиги или команды
    ...
    Покрытие включает NBA, WNBA, NFL, MLB, NHL, MLS, студенческий баскетбол и футбол, крупнейшие европейские футбольные лиги и турниры UEFA, крикет, F1, UFC, теннис и гольф.
    ...
    Запрашивайте нужные вам материалы вместе с актуальными данными.
  ...
  <Tab title="Погода" icon="cloud-sun">
    ...
    Запрос без даты возвращает прогноз на сегодня. Укажите конкретный день или диапазон, чтобы получить по одной странице на каждый день — до 16 дней вперёд или 92 дней назад.
    ...
    <div id="combine-forecasts-with-reporting">
      ### Сочетайте прогнозы с новостными материалами
    </div>
  ...
  <Tab title="Места" icon="map-pin">
    ...
    * Заведения, достопримечательности и интересные места
    * Объявления о недвижимости и записи об объектах
    * Решения о зонировании, разрешения и градостроительная документация
    ...
    <div id="name-the-record-type-and-geography">
      ### Указывайте тип записи и географию
    </div>
...
Если для получения структурированных данных нужно исследование по нескольким источникам, используйте [запуск задачи Exa Agent](/ru/docs/agent/quickstart). Опишите места, команды, даты, критерии и нужные вам поля вывода — и Agent вернёт результаты, проверенные по схеме, со ссылками на источники.

<Card title="Запустить задачу Agent" icon="bot" href="/ru/docs/agent/quickstart" cta="Открыть руководство по Agent" arrow="true">
  Сравнивайте места, составляйте сводку ко дню матча или объединяйте локальные детали и условия в структурированные результаты.
</Card>