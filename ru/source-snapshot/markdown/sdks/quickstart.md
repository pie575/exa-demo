...
<Note>
  Оба клиента считывают ключ из переменной окружения `EXA_API_KEY`. Чтобы задать его явно,
...
| ------------------- | ------------------------------------------------------------------------------------------------------ |
| С чего начать       | Используйте `search`                                                                                   |
| Тип поиска          | Оставьте `auto`, если требования к задержке или синтезу не диктуют другой тип                          |
| Содержимое страницы | Начните с `highlights: true`                                                                           |
| Известные URL       | Используйте `get_contents` / `getContents`                                                             |
| Актуальность        | Задавайте `max_age_hours` / `maxAgeHours` только если устаревшее содержимое непригодно к использованию |

<Warning>
  Оба типа запросов принимают одни и те же параметры содержимого, но указываются они в разных местах:

  | Метод                          | Где указывать параметры содержимого                                            |
  | ------------------------------ | ------------------------------------------------------------------------------ |
...
<Tip>
  Используйте `highlights: true` для ответов ИИ, RAG и предпросмотра результатов поиска. Exa подбирает
  размер фрагментов под релевантность каждого результата; указывайте `max_characters` / `maxCharacters`
  только в том случае, если вашему приложению нужно фиксированное ограничение.
</Tip>
...
<Note>
  ...
  в `output.content`. Задавайте предпочтения по источникам или расставляйте акценты через `system_prompt` / `systemPrompt`.
  Grounding возвращается автоматически в `output.grounding`, поэтому не дублируйте ссылки на источники и
  оценку уверенности в своей схеме.
</Note>
...
<Columns cols={3}>
  <Card title="Руководство по Search" icon="search" href="/ru/docs/search/quickstart" cta="Открыть руководство" arrow="true">
    Вернитесь к основному руководству по Search — там описаны шаблоны запросов, фильтры и расширенные режимы.
  </Card>
