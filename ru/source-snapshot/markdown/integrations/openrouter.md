...
При значении по умолчанию `engine: "auto"` OpenRouter использует собственный поиск провайдера, если он есть у модели, и Exa во всех остальных случаях. Укажите `engine: "exa"`, чтобы поведение поиска было одинаковым для всех моделей:
...
| ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `mode`                                | Найти баланс между задержкой и глубиной: `instant`, `fast`, `auto` (по умолчанию), `deep-lite`, `deep` или `deep-reasoning`. Режимы соответствуют [типам search](/ru/docs/search/quickstart) в Exa. |
...
| `max_total_results`                   | Ограничить суммарное число результатов по всем search в рамках одного запроса                                                                                                                    |
| `max_characters`                      | Задать точный лимит символов на результат для highlights                                                                                                                                         |
...
<Columns cols={2}>
  <Card title="Документация по серверным инструментам" icon="wrench" href="https://openrouter.ai/docs/guides/features/server-tools/web-search" cta="Открыть документацию" arrow="true">
    Полный справочник по настройкам `openrouter:web_search`.
  </Card>
