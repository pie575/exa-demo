...
> Отмена выполняющегося пакета.

Пакет прекращает постановку новых запросов и дожидается завершения уже выполняющихся, после чего переходит в состояние `cancelled`. Отмена пакета, который уже находится в состоянии `cancelling` или `cancelled`, возвращает его без изменений; пакет в состоянии `completed` или `expired` отменить нельзя.
