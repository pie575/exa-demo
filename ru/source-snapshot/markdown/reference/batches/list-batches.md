...
> Получение постраничного списка пакетов вашей команды.

Пакеты возвращаются от новых к старым. Используйте `limit` для управления размером страницы и передавайте в `cursor` значение `nextCursor` из предыдущего ответа, чтобы получить следующую страницу. Укажите `status=completed`, чтобы вывести только завершённые пакеты; для таких списков используется отдельный cursor, поэтому передавайте `status=completed` при запросе каждой страницы.
