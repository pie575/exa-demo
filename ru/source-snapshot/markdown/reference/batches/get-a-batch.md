...
Используйте эту конечную точку, чтобы опрашивать пакет, пока он не перейдёт в статус `completed`, `cancelled` или `expired`. Когда пакет завершится, поле `resultsUrl` будет содержать временную предподписанную ссылку на файл результатов в формате JSONL; запросите пакет повторно, чтобы получить новую ссылку.
