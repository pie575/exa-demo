...
> Создайте асинхронный запуск Agent. Ответ сразу возвращает объект запуска, если только вы не запросили server-sent events.

Создайте запуск с запросом на естественном языке в поле `query`. Добавьте `outputSchema` для валидированного структурированного JSON, `input.data` — для строк, которые нужно обработать, `input.exclusion` — для записей или сущностей, которые следует исключить, либо `previousRunId`, чтобы продолжить с завершённого запуска.

Укажите `Accept: text/event-stream`, чтобы получать события потоком по мере создания, старта и завершения запуска.
