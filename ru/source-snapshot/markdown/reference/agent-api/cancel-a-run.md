...
> Отмена запуска Agent, стоящего в очереди или уже выполняющегося.

Если запуск ещё активен, он немедленно переходит в статус `cancelled` и прерывается, не возвращая результатов. Запуск завершается со статусом `cancelled` и `stopReason: cancelled`. Использование, накопленное до отмены, тарифицируется. Если запуск уже достиг терминального статуса (completed, failed или cancelled), эндпоинт вернёт существующий запуск без изменений.
