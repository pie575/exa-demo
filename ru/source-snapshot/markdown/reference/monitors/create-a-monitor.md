...
> Используйте этот файл, чтобы ознакомиться со всеми доступными страницами, прежде чем переходить к дальнейшему изучению.

<div id="create-a-monitor">
  # Создание monitor
</div>

> Создаёт новый Monitor для запуска повторяющихся поисков Exa по расписанию.

Monitor автоматически выполняет ваш поисковый запрос по заданному расписанию и доставляет результаты на ваш вебхук-эндпоинт с автоматической дедупликацией:

* **Фильтрация по дате** — загружается только контент, появившийся с момента последнего запуска

* **Семантическая дедупликация** — отслеживает предыдущие результаты, чтобы показывать только новые изменения

Ответ содержит `webhookSecret`, который возвращается только один раз — при создании. Сохраните его в надёжном месте для проверки подписи вебхука.
