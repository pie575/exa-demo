...
> Обновляет существующий monitor. Все поля необязательны. Для `search` можно передать частичный объект, содержащий только те поля, которые нужно изменить. Задайте `trigger` равным `null`, чтобы удалить расписание.
