...
> Получите ответ LLM на вопрос, опирающийся на результаты Exa search. `/answer` выполняет поиск Exa и с помощью LLM генерирует один из двух вариантов:

1. Прямой ответ на конкретные запросы (например, на запрос «Какая столица Франции?» вернётся «Париж»).
2. Подробную сводку со ссылками на источники для открытых запросов (например, на запрос «Каково состояние ИИ в здравоохранении?» вернётся сводка со ссылками на релевантные источники).

Ответ включает как сгенерированный текст, так и источники, на основе которых он создан. Эндпоинт также поддерживает потоковую передачу (`stream=True`) — токены возвращаются по мере генерации.

Кроме того, можно использовать совместимый с OpenAI [интерфейс chat completions](https://exa.ai/docs/integrations/openai-sdk#answer).
