...
> Используйте этот файл, чтобы ознакомиться со всеми доступными страницами, прежде чем изучать документацию дальше.

<div id="get-a-webset">
  # Получение webset
</div>

> Возвращает Webset по `id` или `externalId`, включая его статус, searches, imports, enrichments и monitors.

Передайте `expand=items`, чтобы включить в ответ до первых 100 элементов.
