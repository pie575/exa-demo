...
> Используйте этот файл, чтобы найти все доступные страницы, прежде чем углубляться в изучение.

<div id="get-a-webhook">
  # Получение вебхука
</div>

> Возвращает вебхук по его id, включая статус, события, на которые оформлена подписка, целевой URL и метаданные. Секрет для подписи (`secret`) не возвращается.
