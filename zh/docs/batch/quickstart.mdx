...
title: "Batch API"
sidebarTitle: "Batch"
description: "以批次方式异步运行 Exa API 请求。"
boost: 4
---

<Info>
  Batch API 面向 Enterprise 客户开放，需由 Exa 为你的团队启用后方可使用。如需了解 Enterprise 访问与启用事宜，请联系 [sales@exa.ai](mailto:sales@exa.ai)。
</Info>

Batch API 让你一次性提交大量 Exa API 请求，稍后以 JSONL 文件的形式获取结果。你无需逐个发送数千个请求并自行管理速率限制和重试，只需提交一个批次、轮询其状态，即可在单个文件中下载全部结果。

它适用于离线增强、回填，以及任何无需即时响应的任务。完整的请求与响应 schema 请参见 [API 参考](/zh/docs/reference/batches/create-a-batch)。
...
| `cancelling`  | 已发起取消，正在收尾处理进行中的任务 |
| `cancelled`   | 批次已取消              |
| `expired`     | 结果已不再可用            |

批次完成后，`resultsUrl` 中会包含 JSONL 结果文件的下载 URL，`expiresAt` 则设置为结果保留期的截止时间。

<Warning>
  `resultsUrl` 是有效期很短的预签名 URL。每次需要再次下载结果时，请重新获取该批次以取得新的 URL。
</Warning>
...
响应采用 cursor 分页：`data` 最多包含 `limit` 个批次；当 `hasMore` 为 `true` 时，将 `nextCursor` 作为 `cursor` 查询参数传入即可获取下一页。
...
`completed` 是唯一受支持的值；传入其他任何值都会返回错误。已完成的条目按过期时间排序，并使用各自独立的 cursor,因此每一页请求都要携带 `status=completed`:已完成的 cursor 与未过滤的 cursor 不能互换使用。
