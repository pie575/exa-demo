...
用 Exa Search 在一次 query 中获取行情价格、申报文件、电话会议记录以及相关报道。针对某个股票代码的提问,可以同时返回报价、最新一次财报电话会议和分析师研报覆盖。
...
* 证券概况，含关键统计指标与每日 OHLCV 历史数据
* 财报电话会议记录，包含按发言人区分的预备发言和问答环节
* SEC 申报文件、已披露财务数据及国际市场申报文件
* 分析师预测、融资公告和经济数据发布
...
写明股票代码或公司名称，以及你想要的数据。使用 `$NVDA` 这样的美元标签 (cashtag) 同样有效。
...
如果结构化数据需要跨多个来源进行研究，请使用 [Exa Agent 任务运行](/zh/docs/agent/quickstart)。只需描述你需要的证券、时间区间、criteria 和输出字段，Agent 就会返回经 schema 校验的结果，并附上引用来源。

<Card title="启动 Agent 任务" icon="bot" href="/zh/docs/agent/quickstart" cta="打开 Agent 指南" arrow="true">
  筛选公司、对比申报文件，或为整个投资组合整理一份结构化简报。
</Card>