...
使用 Exa Search 检索主流媒体、行业刊物和垂直小众媒体的报道。新文章在发布后数分钟内即可被搜索到。若对发布时间范围有硬性要求，可将自然语言 query 与日期筛选条件结合使用。
...
指明主题、来源类型和发布时间范围，让结果聚焦于事件的当前进展。
...
如果你想要的是从业者的分析，而不是泛泛的新闻报道，请在查询中指明来源类型。
...
在 query 中同时写明形式和主题，这样 search 就能覆盖全网的单集页面和文字稿。
...
请同时描述负面信号和你要调查的实体类别，不要把 query 简化成公司名加“新闻”二字。
...
如果结构化数据需要跨多个来源进行调研，可以使用 [Exa Agent 任务运行](/zh/docs/agent/quickstart)。只需描述所需的报道、字段和时间范围，Agent 即可返回经过 schema 校验的结果，并附上引用来源。
