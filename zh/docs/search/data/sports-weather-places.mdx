---
title: "体育、天气与地点"
sidebarTitle: "体育、天气与地点"
icon: "map-pin"
description: "使用 Exa Search 查找实时体育数据、天气预报和本地地点信息。"
---

export const PlaygroundQuery = ({query, category, filters}) => {
  const PLAYGROUND = "https://dashboard.exa.ai/playground/search";
  const DEFAULT_FILTERS = {
    type: "auto",
    highlights: true
  };
  const params = [`q=${encodeURIComponent(query)}`];
  if (category) params.push(`c=${encodeURIComponent(category)}`);
  params.push(`filters=${encodeURIComponent(JSON.stringify({
    ...DEFAULT_FILTERS,
    ...filters
  }))}`);
  const href = `${PLAYGROUND}?${params.join("&")}`;
  return <div className="playground-query not-prose">
      <code className="playground-query-text">{query}</code>
      <a className="playground-query-run" href={href} target="_blank" rel="noreferrer" title="在 API Playground 中打开" aria-label={`在 API Playground 中打开“${query}”`}>
        {}
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          <path d="M21 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h6" />
          <path d="m21 3-9 9" />
          <path d="M15 3h6v6" />
        </svg>
      </a>
    </div>;
};

使用 Exa Search 获取实时体育数据、天气预报和本地信息，无需为每一项单独接入 API。只需用自然语言提问，说明你关注的团队、地点和时间范围即可。

## 写出更好的 query {#write-better-queries}

准确指明地点或团队，若答案会随时间变化，请加上日期。与其索取宽泛的信息，不如加上与你的任务真正相关的条件或属性。

<Tabs>
  <Tab title="体育" icon="trophy">
### 包含内容 {#included}

    可用的体育数据：

    * **比分**：某联赛某一天的比赛，包括团队、比分、状态、开赛时间和场馆
    * **积分榜**：当前联赛积分表，含分区或赛区划分
    * **赛程**：某联赛或团队的往期结果与即将进行的比赛

    覆盖范围包括 NBA、WNBA、NFL、MLB、NHL、MLS、大学篮球与橄榄球、欧洲主要足球联赛与欧足联赛事、板球、F1、UFC、网球和高尔夫。

### 指明联赛、团队和时间 {#ask-for-the-league-team-and-time}

    <PlaygroundQuery query="NBA scores last night" />

    <PlaygroundQuery query="Lakers schedule this week" />

### 补充相关报道 {#add-the-surrounding-story}

    在获取实时数据的同时，一并索取你需要的报道内容。

    <PlaygroundQuery query="NBA injury reports ahead of tonight's games" />
  </Tab>

  <Tab title="天气" icon="cloud-sun">
### 包含内容 {#included-2}

    预报包括天气状况、最高与最低气温、降水、风力、湿度、紫外线指数，以及按当地时间计算的日出与日落时间。

    未指定日期的 query 返回当天预报。指定某一天或某个时间范围，可按天各返回一页，最多支持未来 16 天或过去 92 天。

### 指明地点和日期 {#name-the-place-and-day}

    <PlaygroundQuery query="weather in San Francisco tomorrow" />

### 询问影响你计划的具体天气状况 {#ask-about-the-condition-that-affects-your-plan}

    <PlaygroundQuery query="will it rain in Austin this weekend" />

### 将预报与报道结合 {#combine-forecasts-with-reporting}

    <PlaygroundQuery query="hurricane forecast tracks for the Gulf Coast this week" />
  </Tab>

  <Tab title="地点" icon="map-pin">
### 包含内容 {#included-3}

    * 本地商户资料，包含地址、营业时间、设施和评价
    * 场馆、景点和兴趣点
    * 房地产房源和房产记录
    * 分区决定、许可证和规划记录

### 像向当地人打听那样描述地点 {#describe-the-place-like-you-would-ask-a-local}

    把类别、街区和你关心的属性结合起来。

    <PlaygroundQuery query="late-night ramen in the Sunset District with outdoor seating" />

### 指明记录类型和地理范围 {#name-the-record-type-and-geography}

    <PlaygroundQuery query="multifamily zoning variances approved in Denver" />

### 按实际条件比较地点 {#compare-places-against-practical-constraints}

    <PlaygroundQuery query="walkable neighborhoods in Austin with good public schools and under 30 minutes to downtown" />
  </Tab>
</Tabs>

## 发起请求 {#make-a-request}

三种数据类型都使用同一个 Search 端点。

<CodeGroup>
  ```python Python theme={null}
  from exa_py import Exa

  exa = Exa()

  results = exa.search(
      "weather in San Francisco tomorrow",
      type="auto",
      num_results=5,
  )
  ```

  ```javascript JavaScript theme={null}
  import Exa from "exa-js";

  const exa = new Exa();

  const results = await exa.search("weather in San Francisco tomorrow", {
    type: "auto",
    numResults: 5,
  });
  ```

  ```bash cURL theme={null}
  curl -s -X POST https://api.exa.ai/search \
    -H "Authorization: Bearer $EXA_API_KEY" \
    -H "Content-Type: application/json" \
    -d '{
      "query": "weather in San Francisco tomorrow",
      "type": "auto",
      "numResults": 5
    }'
  ```
</CodeGroup>

## 使用 Exa Agent 获取结构化数据 {#get-structured-data-with-exa-agent}

如果结构化数据需要跨多个来源进行研究，可以使用 [Exa Agent 任务运行](/zh/docs/agent/quickstart)。只需描述所需的地点、团队、日期、criteria 和输出 fields，Agent 就会返回经过 schema 校验的结果和引用来源。

<Card title="启动 Agent 任务" icon="bot" href="/zh/docs/agent/quickstart" cta="打开 Agent 指南" arrow="true">
  对比不同地点、整理比赛日简报，或把本地细节与现场状况汇总为结构化结果。
</Card>