...
sidebarTitle: "公司与人物"
icon: "users"
description: "使用 Exa Search 查找公司、职业档案及其相互关系。"
---
...
使用 Exa Search 查找组织以及与之相关的人员。这两类搜索搭配使用效果最佳：既可以用公司特征来筛选符合条件的人，也可以通过人员和职位来了解一家公司的运作方式。

<Columns cols={2}>
  <Card title="公司检索基准" icon="building" href="https://exa.ai/blog/company-search-benchmarks">
    了解 Exa 如何评估公司检索与事实提取能力。
  </Card>

  <Card title="人员检索基准" icon="users" href="https://exa.ai/blog/people-search-benchmark">
    了解 Exa 如何评估定向查找与档案发现能力。
  </Card>
</Columns>
...
先写出你想找的实体，再补充用于限定它的特征和关系。如果来源类型很重要，请直接说明，例如公司主页、职业档案、招聘信息或个人网站。
...
<Tabs>
  <Tab title="公司" icon="building">
    ...
    描述界定该市场的客户、产品、能力、阶段和地域。这样可以根据公司实际在做什么来找到候选对象，而不必依赖预先设定的公司名单。
    ...
    说明你关注的信号以及相关的公司特征。搜索不仅能返回公司页面，还能检索招聘信息、定价页面、产品文档和相关报道。
  ...
  <Tab title="人物" icon="users">
    ...
    描述此人与公司的关系，以及用于限定该公司的特征。这比先整理一份公司名单效果更好。
...
编写一个能表达你所需关联关系的 query。Exa 可以在同一结果集中返回公司主页、职业档案、招聘页面和公开提及内容。
...
若所需的结构化数据需要跨多个来源进行调研，可以使用 [Exa Agent 任务运行](/zh/docs/agent/quickstart)。描述你需要的公司、人员、筛选 criteria 以及输出字段，Agent 便会返回经过 schema 校验的结果以及引用来源。

<Card title="启动 Agent 任务" icon="bot" href="/zh/docs/agent/quickstart" cta="打开 Agent 指南" arrow="true">
  构建并筛选公司或人员名单，再用从多个来源收集到的字段丰富每条记录。
</Card>