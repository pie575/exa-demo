...
描述你关注的能力、生态系统和约束条件。这样即可根据库的实际功能来检索候选项，而无需依赖确切的项目名称。
...
写明产品名称和具体操作。这样 search 就能优先返回 API 文档和实现指南，而不是泛泛的讨论内容。
...
当兼容性很重要时，请注明发布渠道或版本号，以减少与旧版本相关的结果。
...
明确指出产物类型和具体任务，而不要搜索“AI 工具”这类宽泛的表述。
...
如果结构化数据需要跨多个来源调研才能得到，可以使用 [Exa Agent 任务运行](/zh/docs/agent/quickstart)。只需描述你需要的库、技术 criteria 和输出字段，Agent 就会返回经 schema 校验的结果，并附上引用来源。
