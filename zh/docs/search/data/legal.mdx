---
title: "法律与公共记录"
sidebarTitle: "法律与公共记录"
icon: "scale"
description: "使用 Exa Search 查找法院判决意见、专利、制裁名单、政府合同及其他公共记录。"
---
...
使用 Exa Search 检索一手法律资料和政府记录，以及针对它们撰写的评述内容。
...
### 获取公开统计数据 {#pull-public-statistics}
...
如果所需的结构化数据需要跨多个 source 展开研究，可使用 [Exa Agent 任务运行](/zh/docs/agent/quickstart)。只需描述你需要的司法辖区、记录 类型、criteria 和输出 fields，agent 即会返回经过 schema 校验的结果，并附带引用来源。

<Card title="启动 Agent 任务" icon="bot" href="/zh/docs/agent/quickstart" cta="打开 Agent 指南" arrow="true">
  跨记录 类型筛查某个实体，或借助 primary sources 与相关报道追踪某项监管变化。
</Card>