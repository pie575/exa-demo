...
icon: "scale"
description: "使用 Exa Search 查找法院判决意见、专利、制裁名单、政府合同等公共记录。"
---
...
使用 Exa Search 查找一手法律文献和政府记录，以及针对这些内容撰写的评注。

<div id="included">
  ## 包含内容
</div>

* 美国法院判决意见，含全文、法院、案卷号及引证元数据
...
若所需的结构化数据需要跨多个来源展开研究，可使用 [Exa Agent 任务运行](/zh/docs/agent/quickstart)。只需描述你需要的司法辖区、记录类型、criteria 和输出字段，Agent 即会返回经 schema 校验并附带引用来源的结果。

<Card title="启动 Agent 任务" icon="bot" href="/zh/docs/agent/quickstart" cta="打开 Agent 指南" arrow="true">
  跨多种记录类型筛查某个实体，或通过一手来源与相关报道追踪某项监管变更。
</Card>