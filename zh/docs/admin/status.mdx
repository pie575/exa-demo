---
title: "状态页"
description: "Exa 服务的实时可用性、进行中的故障事件以及可用性历史记录。"
keywords: ["状态", "可用性", "故障事件", "服务中断", "性能降级", "维护"]
---
...
Exa 在 [status.exa.ai](https://status.exa.ai) 发布服务可用性信息：Search API、Websets 和 Exa MCP 的当前状态、正在进行中的故障事件及其实时进展更新，以及 90 天的可用性历史记录。

<ExaStatus />

## 订阅更新 {#subscribe-to-updates}

在状态页上订阅后，故障事件创建、更新或解决时你都会收到通知。通知可通过电子邮件、Slack、Google Chat 或 webhook 发送。

## 获取帮助 {#get-help}

如果所有系统均显示运行正常，但请求仍持续失败，原因很可能出在请求本身，请先查看[错误码](/zh/docs/admin/error-codes)。
