...
title: "开发者快速开始"
sidebarTitle: "快速开始"
description: "获取 API 密钥，然后在你的代码或 agent 中使用 Exa。"
boost: 3
---
...
<div className="docs-quickstart-section">
  ...
  Exa 有两种方式接入你的应用：在自己的代码中调用 API，或连接你已经在用的 agent。

  <Columns cols={2}>
    <Card title="调用 API" icon="code" href="#3-install-an-sdk" cta="安装 SDK">
      在你自己的代码中使用 Search、Contents 和 Exa Agent。在下方安装 SDK
      并发起第一个请求。
    </Card>
  ...
  想基于 API 开发？可以从这些入口开始：

  | 从这里开始                                   | 适用场景                   |
  | --------------------------------------- | ---------------------- |
  | [Search](/zh/docs/search/quickstart)       | 查找相关网页，并在 2 秒内返回整合后的内容 |
  | [Deep Search](/zh/docs/search/deep-search) | 更高质量的搜索，由 LLM 迭代找出更优结果 |
  | [Agent](/zh/docs/agent/quickstart)         | 长时间运行的异步研究、列表构建、增强或报告  |
  | [Contents](/zh/docs/contents/quickstart)   | 在已有 URL 的情况下提取页面内容     |
</div>
...
<div className="docs-quickstart-section">
  ## 4. 发起你的第一个请求 {#4-make-your-first-request}
