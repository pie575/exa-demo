---
title: "Similarweb"
description: "获取网站流量估算、全球排名和竞争对手发现。"
icon: "/docs/images/sidebar-icons/similarweb.svg"
---

[Similarweb](https://www.similarweb.com) 是数字市场情报的领先来源，它对数百万网站和应用的流量与互动情况进行建模，涵盖预估访问量、流量来源、受众画像以及任一域名所处的竞争格局。

通过 [Exa Connect](/zh/docs/agent/connect/overview) 在 [Exa Agent](/zh/docs/agent/quickstart) 运行中附加 `similarweb`，Agent 便会在执行 Exa 网页搜索的同时查询 Similarweb。

<div id="use-it-for">
  ## 适用场景
</div>

* 将某家公司的网站流量和用户参与度与同行进行对标。
...
Similarweb 按数据额度计费，单价为 `$0.30 / credit`，每次调用按 Similarweb 报告的额度数扣费。额度随返回的数据量增加——大致每个数据点 (指标 × 行 × 月份) 消耗 1 个额度——因此单次调用的价格取决于其参数：
...
| 流量来源    | 10                                   |
| 主要引荐来源  | 每返回一个引荐来源 3 (1–5)                    |
| 主要国家/地区 | 每返回一个国家/地区 3 (1–5)                   |
| 热门页面    | 每返回一个页面 2 (1–7)                      |
| 热门关键词   | 1–10 (每 100 个关键词数据点约 1；50 个关键词约 ~7)  |
| 关键词概览   | 1–2                                  |
| 受众画像    | 8                                    |
| 受众重叠    | 每个域名组合 2 (2–3 个域名：6–14)              |
| 技术栈     | 10                                   |
| 分类热门站点  | 每返回一个站点 1 (1–10)                     |

未返回任何数据的调用 (未知或低流量域名、无搜索量的关键词) 不计费。`numResults` 和 `months` 决定你需要为多少数据点付费，因此请在满足任务需要的前提下尽量取小值。
...
* [Fiber.ai](/zh/docs/agent/connect/fiber)：将发现的竞争对手转化为经过丰富化的公司记录。
* [Affiliate.com](/zh/docs/agent/connect/affiliatecom)：在推荐某商家的产品前，先评估其影响力覆盖范围。
...
<Columns cols={2}>
  ...
  <Card title="获取 API key" icon="key" href="https://dashboard.exa.ai/api-keys" cta="创建 key" arrow="true">
    在控制台中创建一个 key，即可原样运行本页示例。新账户还会赠送免费额度。
  </Card>
</Columns>