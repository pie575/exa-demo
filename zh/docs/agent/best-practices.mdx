...
title: "Agent 最佳实践"
sidebarTitle: "最佳实践"
description: "为生产环境的 Exa Agent 集成调优 query 质量、结构化输出、effort 与成本。"
keywords: ["agent api", "列表构建", "enrichment", "structured output", "effort", "grounding"]
---

完成 [Exa Agent 快速开始](/zh/docs/agent/quickstart)后，可参考本指南提升 query 质量、结构化输出，并控制运行时长与成本。如需完整的请求示例，请从 [Agent 示例](/zh/docs/agent/examples)入手。

<div id="core-principles">
  ## 核心原则
</div>

把 `query` 当作任务说明书来写：明确 Agent 要找什么、工作范围有多大、需要哪些证据，以及怎样才算一个完整的结果。
...
若不提供 `outputSchema`，Agent 会在 `output.text` 中返回文字叙述，并在 `output.grounding` 中返回引用来源。只有在某个字段确有用途时才添加它：

| 字段                      | 使用场景                                                     |
| ----------------------- | -------------------------------------------------------- |
| `outputSchema`          | 下游代码需要结构化字段                                              |
| `input.data`            | 你已有待丰富的数据行                                               |
| `input.exclusion`       | 不希望返回某些已知记录                                              |
| `dataSources`           | 某个字段应来自 [Exa Connect](/zh/docs/agent/connect/overview) 合作伙伴 |
| `previousRunId`         | 本次请求是对某次已完成运行的延续                                         |
| `effort`                | 需要显式设置成本或研究深度                                            |
| `budget.maxCostDollars` | `auto` 或 `max` 运行需要硬性成本上限                                |

数据行、排除项和响应结构请放在各自专用的字段中，不要写进 `query` 里。

<div id="writing-list-building-and-enrichment-queries">
  ## 编写列表构建与 enrichment query
</div>

列表构建时，需明确实体、目标数量、入选 criteria、排除项和证据标准。enrichment 时，将已有记录放入 `input.data`，并只描述需要 Agent 补充调研的内容。

如果入选与否需要主观判断，请要求给出理由。只有当某条 criterion 存在多种合理解读时，才提供示例。
...
发现类请求示例参见 [Find all GTM members](/zh/docs/agent/examples#find-all-code)，对应的按行 enrichment 模式参见 [Enrich input rows](/zh/docs/agent/examples#enrich-input-rows-code)。

<div id="handle-asynchronous-runs">
  ## 处理异步运行
</div>

Agent 运行需要搜索、阅读和推理，耗时可能从数秒到数分钟不等。请围绕运行的生命周期来设计，而不是让应用请求一直挂起。
...
<Steps>
  ...
  <Step title="等待终态">
    使用 SDK 提供的轮询辅助方法、轮询 `GET /agent/runs/{id}`，或消费 SSE 流。只要运行处于 `queued` 或 `running`，就继续等待。
  </Step>
...
对于批量场景，请先用有代表性的任务做基准测试，再估算 concurrency 或将 Agent 放到同步的 UI 路径上。运行时长会随项目数量、schema 复杂度、来源可用性和 effort 而变化。

对于启用零数据保留的团队，请消费实时流或在保留窗口内轮询，`previousRunId` 和 Connect `dataSources` 均不可用。参见[零数据保留](/zh/docs/admin/security/zero-data-retention)。

<div id="write-custom-json-schemas-for-structured-output">
  ## 编写自定义 JSON schema 以获取结构化输出
</div>

当下游代码需要机器可读的字段、归一化的值、表格行或 enrichment 记录时，使用 `outputSchema`。如果一段文字性回答就够用，可以省略它并直接读取 `output.text`；结构化输出会增加格式化方面的工作量，还可能拉高延迟。

把研究指令写在 `query` 里，把响应结构写在 `outputSchema` 里。属性名和描述要清晰，类型尽量选最窄且够用的，并用 `maxItems` 限制数组长度。
...
schema 只校验结构，不校验事实。当证据不足以支撑某个字段时，Agent 可能返回 `null`，即使你提交的 schema 将该字段标记为必填或非空。`stopReason: schema_satisfied` 表示 Agent 认为在允许这些空值的前提下，预期结构已经完整，但并不保证能通过针对所提交 schema 的严格校验。

不要在自己的 schema 中重复 Exa 内置的引用来源或置信度信息。只有当每个 item 都需要说明自身符合条件的理由时，才添加 rationale 字段，并将 `output.grounding` 与结构化结果一并持久化。上线前请对照来源核实重要论断，并用有代表性的输入测试 schema 变更。

浏览[结构化 Agent 示例](/zh/docs/agent/examples)，对比列表构建、KYB、招聘信息、排除项和续跑任务等场景下的 schema 写法。
...
| 快速研究与信息综合               | [Deep Search](/zh/docs/search/deep-search) |
| 异步列表构建、多跳研究或 Enrichment | [Agent](/zh/docs/agent/quickstart)         |

如果任务需要多步检索、逐个实体核验，或对已有记录做 Enrichment，请使用 Agent。如果只需快速获取网页，后续推理由你的应用自行完成，则使用 Search。
...
| 如果你需要             | 使用                                                           | 避免                           |
| ----------------- | ------------------------------------------------------------ | ---------------------------- |
| 规模未知的调研清单         | `auto` 搭配有界的 `outputSchema`                                  | 固定的低档 effort 搭配无界数组          |
| 对已有记录做 Enrichment | `input.data` 并指定要补充的字段                                       | 把整张表格粘贴进 `query`             |
| 基于上一次结果集继续追问      | `previousRunId`                                              | 重新发送完整的上一次输出                 |
| 不希望再次出现的记录        | `input.exclusion` 并在下游去重                                     | 把排除项当作严格的身份保证                |
| 高级提供方数据           | [Exa Connect](/zh/docs/agent/connect/overview) 搭配 `dataSources` | 让 Agent 从公开网络推测仅提供方才有的字段     |
| 可预测的单次请求成本        | 固定的 `effort`                                                 | 在没有预算约束的情况下使用 `auto` 或 `max` |
| 完整性优先于延迟与成本       | `xhigh` 或 `max`                                              | 在收紧 query 之前就提高 effort       |
