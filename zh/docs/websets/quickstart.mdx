...
title: "Websets"
sidebarTitle: "快速开始"
description: "从网络构建经过验证、丰富的数据集。"
boost: 4
---

## 什么是 Websets？ {#what-are-websets}

Webset 以一条自然语言 query 和目标项目数量为起点。你可以添加每条 result 都必须满足的 criteria，并指定为每个已接受项目填充的增强 fields。结果会通过控制台、API 或 webhook 异步返回。
...
<Info>
  ...
  本指南适用于维护或扩展现有的 Websets integration。
...
2. **搜索并验证：** Websets 查找候选结果，并逐一对照你的 criteria 进行核验。只有符合条件的结果才会成为项目。
3. **执行增强：** 对于每个经过验证的项目，Websets 会搜索你所需的补充数据，例如 CEO 姓名、融资金额或联系方式。
4. **获取结果：** 轮询状态、通过 webhook 接收更新，或在项目陆续产生时在控制台中查看。

## 核心能力 {#key-capabilities}

| 功能              | 作用                                 |
| --------------- | ---------------------------------- |
| **Criteria 验证** | 每条结果都会依据你定义的规则进行校验，确保你只得到真正相关的匹配结果 |
| **增强**          | 为每条结果提取特定数据点 (文本、数字、日期、布尔值)        |
| **Monitors**    | 设定周期性 search，自动保持 webset 更新        |
| **Webhooks**    | 在项目被添加或丰富时实时接收 HTTP 回调             |
| **导入**          | 导入你自己的 URL 并对其运行增强                 |

## 人类快速开始 {#human-quickstart}
