...
当你收到来自 Exa 的 webhook 时，应验证其确实来自我们，以确保数据的完整性和真实性。Exa 会使用你的 webhook 端点专属的密钥，对所有 webhook 负载进行签名。
...
2. 将时间戳、一个句点和原始请求体拼接起来，构造出待签名负载
...
* **始终验证签名** - 未验证签名前，切勿处理 webhook 数据。这可防止攻击者向你的端点发送伪造的 webhook。

* **使用恒定时间比较** - 比较签名时，请使用 Python 中的 `hmac.compare_digest()` 或 Node.js 中的 `crypto.timingSafeEqual()` 等函数，以防止时序攻击。

* **检查时间戳时效性** - 可考虑拒绝时间戳过旧的 webhook (例如超过 5 分钟) ，以防止重放攻击。

* **安全存储密钥** - 将 webhook 密钥存储在环境变量或安全的密钥管理系统中，切勿硬编码在应用程序里。**重要提示**：webhook 密钥仅在你[创建 webhook](/zh/docs/websets/api/webhooks/create-a-webhook) 时返回一次，请务必妥善保存，之后将无法再次获取。

* **使用 HTTPS** - 始终为 webhook 使用 HTTPS 端点，以确保数据在传输过程中加密。

* **注册最终 URL** - webhook 投递不会跟随 HTTP 重定向 (3xx 响应) 。如果你的端点发生重定向，该次投递会被视为失败。请始终注册直接处理负载的 URL。
...
1. **检查原始负载**：确保使用的是原始请求体，而不是解析后的 JSON 对象
2. **核对密钥**：确保使用的是创建 webhook 时获得的正确 webhook 密钥
3. **检查 header 解析**：确保从 header 中正确提取了时间戳和签名
4. **编码问题**：确保整个验证过程中始终使用 UTF-8 编码

<div id="testing-signatures-locally">
  ### 在本地测试签名
</div>

你可以使用 webhook 密钥 和示例负载来测试你的签名验证逻辑：
