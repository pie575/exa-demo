...
title: "SDK 快速开始"
sidebarTitle: "快速开始"
description: "安装并使用 Exa 的 Python 和 JavaScript SDK"
---

Exa 官方 SDK。搜索网页、获取页面内容，并获得带引用来源的答案。
...
<Note>
  两种客户端都会从 `EXA_API_KEY` 环境变量中读取你的密钥。若要显式指定，
  可以内联传入：`Exa(api_key="your-api-key")` 或 `new Exa("your-api-key")`。
</Note>
...
| ------ | --------------------------------------------- |
| 起点     | 使用 `search`                                   |
| 搜索类型   | 保持 `auto`，除非延迟或综合方面的需求需要其他类型                  |
| 页面内容   | 先从 `highlights: true` 入手                      |
| 已知 URL | 使用 `get_contents` / `getContents`             |
| 新鲜度    | 仅当过时内容不可用时才设置 `max_age_hours` / `maxAgeHours` |

<Warning>
  这两种请求类型接受相同的 content options，但传入位置不同：

  | 方法                             | content option 传入位置                                                   |
  | ------------------------------ | --------------------------------------------------------------------- |
...
<Tip>
  ...
  调整摘录长度；只有当你的应用需要固定上限时，才设置 `max_characters` / `maxCharacters`。
</Tip>
...
<Note>
  ...
  可通过 `system_prompt` / `systemPrompt` 指定来源偏好或需要强调的内容。
  grounding 会自动在 `output.grounding` 中返回，因此无需在 schema 中重复定义引用来源或 confidence。
</Note>

如果输出需要跨多次 search 进行研究，建议使用 deep 模式：轻量级研究可用 `deep-lite`，需要多步搜索和更强综合能力时可用 `deep`。完整的请求选项请参见 [Search 指南](/zh/docs/search/quickstart)。

## Contents {#contents}

从你已知的 URL 中提取 highlights、full text 或摘要。建议先从 highlights 入手，再添加 query，让其聚焦于你所需的信息。
...
当你需要更完整的上下文或文档结构时，请使用 full text。有关输出结构、新鲜度控制和子页面抓取，请参阅 [Contents 指南](/zh/docs/contents/quickstart)。
...
<Columns cols={3}>
  <Card title="Search 指南" icon="search" href="/zh/docs/search/quickstart" cta="打开指南" arrow="true">
    返回 Search 主指南，了解请求写法、过滤条件和更深入的搜索模式。
  </Card>

  <Card title="Search 参考文档" icon="square-terminal" href="/zh/docs/reference/search" cta="打开参考文档" arrow="true">
