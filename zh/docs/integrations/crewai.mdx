---
title: "CrewAI"
description: "了解如何为你的 CrewAI agent 添加 Exa retrieval 能力。"
...
在本指南中，我们将创建一个由两个 agent 组成的 crew，基于 Exa 的搜索结果生成一份新闻通讯。我们将介绍如何：

1. 创建由 Exa 驱动的自定义 CrewAI 工具
2. 设置 agent，并为其分配使用该 Exa 搜索工具的具体角色
3. 将这些 agent 组织成一个用于撰写新闻通讯的 crew

<Note>
  CrewAI 也内置了 [`ExaSearchTool`](https://docs.crewai.com/en/tools/search-research/exasearchtool)，无需编写自定义 wrapper 即可直接使用。如果你希望完全掌控结果的格式化方式，下面的自定义工具会很有用；两种方式都可行。
</Note>
...
<Steps>
  ...
  <Step title="在 crewAI 中定义自定义 Exa 工具">
    我们使用 crewAI 的 [@tool 装饰器](https://docs.crewai.com/concepts/tools#utilizing-the-tool-decorator)创建一个[自定义工具](https://docs.crewai.com/concepts/tools)。在该工具内部，我们可以初始化来自 [Exa Python SDK](https://github.com/exa-labs/exa-py) 的 Exa 类，发起请求，并返回解析后的结果。
    ...
    <Note> 请确保你的 API 密钥已正确初始化。在本示例中，OpenAI 和 Exa 密钥对应的环境变量名分别为 `OPENAI_API_KEY` 和 `EXA_API_KEY`。 </Note>
  ...
  <Step title="设置 crewAI agent">
    ...
    接着，我们创建[两个 agent](https://docs.crewai.com/concepts/Agents/)，并把它们组成一个 [crew](https://docs.crewai.com/concepts/Crews/)：

    * 一个负责用 Exa 做研究 (使用上面定义的自定义工具)
    * 另一个负责撰写新闻通讯作为输出 (借助 LLM)

    ```Python Python theme={null}
    # 创建一个启用记忆和详细输出模式的资深研究员 agent
    researcher = Agent(
      role='Researcher',
  ...
  <Step title="为 agent 定义任务">
    接下来，我们将为每个 agent 定义[任务](https://docs.crewai.com/concepts/Tasks/)，并利用上面配置好的所有组件创建完整的 crew。
  ...
  <Step title="启动团队">
    ...
    crew 会根据 Exa search 工具返回的内容撰写这份简报。
  </Step>
</Steps>