---
title: "CrewAI"
description: "了解如何为你的 CrewAI agent 添加 Exa 检索能力。"
...
在本指南中，我们将创建一个由两个 agent 组成的 crew，根据 Exa 的搜索结果生成一份新闻简报。我们将介绍如何：

1. 创建一个由 Exa 驱动的自定义 CrewAI 工具
2. 设置 agent，并为其分配使用该 Exa 搜索工具的具体角色
3. 将这些 agent 组成一个 crew，共同撰写新闻简报
...
<Steps>
  ...
  <Step title="在 crewAI 中定义自定义 Exa 工具">
    ...
    <Note> 请确保已正确初始化你的 API key。在本示例中，OpenAI 和 Exa 的 key 所对应的环境变量名分别为 `OPENAI_API_KEY` 和 `EXA_API_KEY`。 </Note>
  ...
  <Step title="设置 crewAI agent">
    ...
    接下来，我们创建[两个 agent](https://docs.crewai.com/concepts/Agents/)，并将它们[编入同一个 crew](https://docs.crewai.com/concepts/Crews/)：

    * 一个使用 Exa 进行调研 (即上面定义的自定义工具)
    * 另一个负责撰写新闻简报作为输出 (使用 LLM)
  ...
  <Step title="为 agent 定义任务">
    接下来，我们将为每个 agent 定义[任务](https://docs.crewai.com/concepts/Tasks/)，并利用上面搭建好的所有组件来创建完整的 crew。
  ...
  <Step title="启动团队">
    ...
    该 crew 会基于 Exa search 工具返回的内容撰写这份通讯。
  </Step>
</Steps>