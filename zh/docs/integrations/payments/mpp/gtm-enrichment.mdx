---
title: "Tempo MPP GTM 增强指南"
sidebarTitle: "使用 MPP 进行 GTM 增强"
description: "构建 GTM 增强工作流，通过 Tempo MPP 按每次 Exa search 与页面内容请求付费，无需 API 密钥。"
icon: "/docs/images/sidebar-icons/tempo.svg"
---

参照本指南，在 Exa 的 `/search` 与 `/contents` 端点之上构建 GTM 增强 agent 或
pipeline，并通过 Machine Payments Protocol (MPP) 按请求付费。MPP 支持多种支付方式，
本文示例使用 [Tempo](https://tempo.xyz) 上的稳定币。无需按月订阅、无需
API 密钥，也没有按席位计价：只需向 wallet 充入 USDC.e，在丰富潜在客户或公司数据的
同时按量付费。

<Info>
  目前 MPP 仅支持 Exa 的 `/search` 与 `/contents` 端点。
...
一个轻量级的增强 pipeline：给定一组公司名称或目标描述，它会：

1. 使用 Exa `/search`，配合 `type: "deep"` 与 `outputSchema`，找到公司官方页面并提取关键元数据。
2. 对返回的结果使用 `contents.highlights`，提取融资、总部、员工人数和产品相关的来源片段。
3. 为每条输入生成一条 CSV 或 JSON 格式的增强记录。

该模式适用于线索列表增强、客户调研和外呼个性化场景。由于它由一个个独立的 `/search` + `/contents` 调用组成，每一步都可以通过 MPP 付费。

## 前置条件 {#prerequisites}

* 一个兼容 Tempo 的 wallet，且已在 Tempo mainnet 上充值 **USDC.e**。
* 一种在运行时安全加载 wallet 私钥的方式 (详见下文；切勿提交私钥或将其暴露在源代码中) 。
* 已安装 `mppx` (TypeScript) 或 `pympp` (Python) 。

<Info>
  如果希望使用无需原始私钥的命令行方案，可使用 [Tempo Wallet CLI](/zh/docs/integrations/payments/mpp/quickstart#pay-from-the-command-line)。`tempo wallet login` 会创建或连接一个 wallet，新注册用户还可能获得免费的 MPP 积分。
</Info>
...
<CodeGroup>
  ```bash TypeScript theme={null}
  # 在你的 shell 或 CI secrets 存储中设置；切勿提交该值
...
<CodeGroup>
  ...
  // 在 production 中，请从 secrets manager 加载该值，切勿将原始值提交到代码仓库。
...
请求成功时，响应中除 Exa 结果外还会包含一个 `Payment-Receipt` header，其中是 on-chain 交易哈希。
...
### 增强人物档案 {#enrich-a-person-profile}

本方案使用 `type: "deep"`、`contents.highlights` 和 `outputSchema` 来调研某个人物并返回结构化的档案。
...
<Note>
  这里用 `type: "deep"` 获得更深入的推理，并用 `outputSchema` 来规定响应的结构。深度搜索的价格为每次请求 $0.012，`contents.highlights` 每条结果额外收费 $0.001。
</Note>

### 结构化输出 {#structured-output}

如果你需要 JSON 字段而非纯文本，可以在 search 请求中使用 `outputSchema`。Exa 会返回一个与你的 schema 结构一致的 `output` 对象。
...
<Note>
  `outputSchema` 搭配 `deep-lite` 或 `deep` 搜索类型使用效果最佳。它会在 Exa 侧增加一次
  LLM 调用，因此按 `deep-lite`/`deep` 计价。
</Note>

## 定价与限制 {#pricing-and-limits}

MPP 采用与 API 密钥计费相同的按请求定价。MPP search 请求
最多返回 10 条结果。
...
完整参考请参见[使用 MPP (Tempo) 支付](/zh/docs/integrations/payments/mpp/quickstart)，
其中包含速率限制、网络详情和支付 header。

## production 提示 {#production-tips}

* **仅向 wallet 充值 USDC.e。** Exa 会代付 Tempo 网络手续费，因此 wallet
  无需额外准备 gas token。
* **处理 `402` 响应。** MPP SDK 会自动重试，但自定义客户端在收到 `402` 时应根据
  `WWW-Authenticate: Payment` 挑战进行重试。
* **缓存 `/contents` 结果。** 页面内容按 URL 计费，请按 URL 缓存，避免为同一个公司页面重复付费。
* **注意 10 条结果的上限。** MPP search 会将 `numResults` 限制为 10。
* **切勿提交私钥。** 请从 secrets manager 加载 `WALLET_PRIVATE_KEY`，不要放进源代码管理。

## FAQ {#faq}

<AccordionGroup>
  <Accordion title="我可以在 Exa Agent API 上使用 MPP 吗？">
    不可以。在 Exa 代码库中，MPP 仅接入了 `/search` 和 `/contents`。
    `/agent/runs` 和 `/answer` 需要 Exa API 密钥，并采用标准的 API 密钥
    计费方式。
  </Accordion>

  <Accordion title="我可以在同一个请求中同时使用 MPP 和 Exa API 密钥吗？">
    不可以。如果请求中包含 `x-api-key` 或 `Authorization: Bearer`，则 API
    密钥流程优先，MPP 将被跳过。
  </Accordion>

  <Accordion title="如果 MPP 结算失败会怎样？">
    Exa 会返回 `402`，并附带一个新的 `WWW-Authenticate: Payment` 挑战，同时不返回
    任何结果。你的客户端可以用新的支付重试。在结算成功之前，不会返回任何结果。
  </Accordion>

  <Accordion title="每个环境都需要单独的 Tempo wallet 吗？">
    你可以复用同一个 wallet，但我们建议 development 和 production 分别使用
    独立的 wallet。单个 wallet 的 QPS 限制为 10 请求/秒，涵盖该 wallet 发出的
    所有请求。
  </Accordion>
</AccordionGroup>
