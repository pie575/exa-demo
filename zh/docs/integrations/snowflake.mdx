---
title: "Snowflake"
description: "在 Snowflake 存储过程和 Cortex Agent 中使用 Exa 的 search 与内容检索能力。"
icon: "/docs/images/sidebar-icons/snowflake.svg"
---

直接在 Snowflake 中调用 Exa 的 Search API，无需离开 SQL 即可用实时的网络信息丰富你的数据。

该集成利用 Snowflake 的 [External Access](https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview) 功能，从存储过程中安全调用 Exa API，支持两类主要工作流：

1. **Cortex Agent 工具** — 让 Snowflake Cortex Agent 具备搜索网络、获取页面内容的能力
...
1. 一个 Exa API key
2. `ACCOUNTADMIN` 角色 (创建 External Access Integrations 时必需)
3. 一个支持 External Access 的 Snowflake 账户 (付费账户)

<Card title="获取你的 Exa API key" icon="key" horizontal href="https://dashboard.exa.ai/api-keys">
  在控制台中创建 key。新账户可获赠免费额度。
</Card>
...
* 一个 **secret**，用于存储你的 API key (已加密，在 SQL 中不可见)
* 一个 **External Access 集成**，将该规则与 secret 绑定在一起
...
使用 Exa 的 Search API 搜索网络。返回的结果包含标题、URL 和分数，并可选择性地包含正文文本、highlights 或摘要。
...
| `include_domains`      | `STRING`  | `NULL`   | 仅限定的域名，以逗号分隔                                                               |
...
| `get_contents`         | `BOOLEAN` | `TRUE`   | 获取正文文本、highlights 和摘要                                                      |
...
Exa 可以用实时网络数据为 Snowflake 表进行 enrichment —— CEO/创始人信息、最新新闻等 —— 全程无需离开平台。
...
在为大型表格做数据增强时，逐行顺序调用 Exa 会很慢。你可以在单个存储过程中使用 Python 的 `concurrent.futures` 并行发起请求，同时发送多个 Exa API 调用。
...
| ------------- | -------------------------------------------------- |
| `5`           | 保守 —— 远低于速率限制                                      |
| `10`          | 适用于大多数 Enrichment 任务的默认值                           |
| `20–50`       | 速率[限制](/zh/docs/admin/billing#rate-limits)等级较高时的大型数据集 |

<div id="snowflake-task-scheduling">
  ### Snowflake 任务调度
</div>

对于周期性的 enrichment 作业，可将该存储过程封装为 [Snowflake Task](https://docs.snowflake.com/en/user-guide/tasks-intro)：
...
你还可以将 Exa 存储过程注册为 [Snowflake Cortex Agent](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-agents) 的工具，让它能够以对话的方式搜索网络。
...
| **External Access 集成** | 将网络规则与 secret 绑定在一起               |
...
* [Exa 控制台](https://dashboard.exa.ai) — 获取你的 API key
