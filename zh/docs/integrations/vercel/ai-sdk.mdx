---
title: "Vercel AI SDK"
description: "使用 @exalabs/ai-sdk 包为 AI SDK 应用添加 Exa 网页搜索。"
icon: "/docs/images/sidebar-icons/vercel.svg"
---

使用 `@exalabs/ai-sdk` 包，为基于 Vercel AI SDK 构建的应用添加 Exa 网页搜索。你只需提供 Exa API 密钥，`webSearch()` 工具即可为你的模型处理搜索请求。
...
* `maxAgeHours`：默认的缓存回退值；如需更严格的新鲜度，请设置该选项
...
| `userLocation`                            | 两字母国家代码，用于地理位置感知搜索。                                                                |
...
| `contents.maxAgeHours`                                 | 仅当缓存内容在指定时效内时才使用缓存，否则使用 livecrawl。              |
...
<Columns cols={2}>
  <Card title="使用 Vercel AI Gateway" icon="cloud" href="/zh/docs/integrations/vercel/ai-gateway" cta="打开指南" arrow="true">
    通过 Vercel 的 AI Gateway，无需 Exa API 密钥即可使用 Exa 网页搜索。
  </Card>
