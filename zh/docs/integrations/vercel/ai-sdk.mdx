---
title: "Vercel AI SDK"
description: "使用 @exalabs/ai-sdk 软件包为 AI SDK 应用添加 Exa 网页搜索。"
icon: "/docs/images/sidebar-icons/vercel.svg"
---

使用 `@exalabs/ai-sdk` 软件包，即可为基于 Vercel AI SDK 构建的应用添加 Exa 网页搜索。你只需提供 Exa API key，`webSearch()` 工具便会为你的模型处理搜索请求。
...
<Info>
  运行示例前，请将你的 key 设置到环境变量 `EXA_API_KEY` 中。该软件包会自动读取此环境变量。
</Info>
...
* `maxAgeHours`：默认的缓存回退值；如需更高的内容新鲜度，请显式设置该选项
...
| `contents.maxAgeHours`                                 | 仅当缓存内容在指定时效范围内时才使用缓存，否则执行 livecrawl。            |
| `contents.livecrawlTimeout`                            | 设置 livecrawl 超时时间。                              |
...
<Columns cols={2}>
  ...
  <Card title="探索 AI SDK 软件包" icon="git-branch" href="https://github.com/exa-labs/ai-sdk" cta="查看源码" arrow="true">
    在 GitHub 上查看源代码和软件包的详细信息。
  </Card>
</Columns>

你也可以在 [npm](https://www.npmjs.com/package/@exalabs/ai-sdk) 上找到该软件包，并阅读 [Vercel AI SDK 网页搜索指南](https://ai-sdk.dev/cookbook/node/web-search-agent#exa)。