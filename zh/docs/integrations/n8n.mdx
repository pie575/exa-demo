...
title: "n8n"
sidebarTitle: "n8n"
description: "在 n8n 工作流中使用 Exa 的 search 与 contents。"
icon: "/docs/images/sidebar-icons/n8n.svg"
---

官方 [Exa n8n 节点](https://github.com/exa-labs/n8n-integration)为可视化工作流引入了网页 search、内容提取、有据可循的答案以及 Exa Agent 运行能力。你可以把它当作普通的工作流步骤使用，也可以作为工具连接到 n8n AI Agent。
...
<Steps>
  <Step title="添加社区节点">
    ...
    该节点要求 n8n 1.60 及以上版本，以及 Node.js 20.15 及以上版本。
  </Step>
  ...
  <Step title="添加 Exa 凭据">
    在 n8n 中添加 **Exa API** 凭据并粘贴你的 key。在每个需要使用该账户的 Exa 节点上选择此凭据。
  </Step>
</Steps>
...
5. 选择响应格式：
   * **Results**：返回经过排序的页面
   * **Text**：返回综合生成的答案
   * **Structured**：返回符合你所定义 schema 的 JSON
6. 运行该节点，并将其输出传递给工作流的下一个步骤。

搜索还可以返回每条结果的正文、highlights、摘要、链接和图片。域名过滤、发布日期、类别、`maxAgeHours` 和子页面抓取等功能可在该节点的可选字段中配置。
...
| **Answer**   | 生成有据可循的答案并带引用，可选择输出结构化结果。                                                             |
| **Agent**    | 创建、查看、列出、流式传输、轮询和取消多步骤 Agent 运行。                                                       |
...
Search 和 Answer 很适合用作 grounding 工具。当任务需要多步研究、列表构建、结构化 enrichment 或高级 [Exa Connect](/zh/docs/agent/connect/overview) 数据时，请使用 Agent 资源。
...
对于长时间运行或异步的工作流，请关闭 **Wait for Completion**，保存返回的运行 `id`，稍后再通过 **Get Run** 获取结果。n8n 步骤结束后，该运行仍会在 Exa 上继续执行。
...
<AccordionGroup>
  <Accordion title="节点选择器中找不到 Exa 节点">
    请让实例所有者安装已验证的社区包 `n8n-nodes-exa-official`。社区节点能否使用，取决于 n8n 实例的托管方式。
  </Accordion>

  <Accordion title="Exa 凭据被拒绝">
    请确认所选凭据中的 key 来自 [Exa 控制台](https://dashboard.exa.ai/api-keys) 且处于启用状态，并且该 key 仍有可用额度。
  </Accordion>
