...
<Info> LangChain 团队针对极为相似的配置录制了 YouTube 教程，可在[此处](https://www.youtube.com/watch?v=dA1cHGACXCo)查看。 </Info>

<Info> LangChain 的完整参考文档请见[此处](https://python.langchain.com/docs/integrations/providers/exa%5Fsearch/)。 </Info>
...
<Steps>
  ...
  <Step title="使用 Exa Search 为 LangChain 工具提供支持">
    使用 `ExaSearchRetriever` 设置一个 Retriever 工具。该检索器会连接 Exa Search，通过语义搜索查找相关文档。首先导入所需的库，并实例化 ExaSearchRetriever。
    ...
    # 定义使用 Exa Search 的检索器，获取 3 条结果，并解析每条结果中的 highlights
    retriever = ExaSearchRetriever(api_key=os.getenv("EXA_API_KEY"), k=3, highlights=True)
    ```
  </Step>

  <Step title="创建提示词模板（可选）">
    我们使用 LangChain 的 [PromptTemplate](https://python.langchain.com/v0.1/docs/modules/model%5Fio/prompts/quick%5Fstart/#prompttemplate) 定义一个带占位符的模板，用于从 Exa retriever 返回的结果中解析出 URL 和 highlights。
  ...
  <Step title="整合 Exa 结果与内容，用于检索">
    将 Exa 检索器、解析器和一个简短的 lambda 函数串联起来，完成检索链——这一步至关重要，它能把结果拼成单个字符串，供下一步的 LLM 作为上下文使用。
  ...
  <Step title="配置其余工具链，包括用于生成内容的 OpenAI">
    在这一步中，我们定义系统提示词，其中的 Query 和 Context 模板输入分别来自用户和 Exa Search。首先，同样先从 LangChain 的库中导入相关库和组件
    ...
    接着我们定义一个生成提示词——该提示词模板会结合来自 Exa 的上下文来执行 RAG。
    ...
    我们将生成所用的 [LLM 设置为 OpenAI](https://python.langchain.com/v0.1/docs/integrations/chat/openai/)，然后用 [RunnableParallel](https://python.langchain.com/v0.1/docs/expression%5Flanguage/primitives/parallel/) 并行连接将各部分组合起来。随后，包含 query 和上下文的生成提示会传给 LLM，并经过[解析以获得更好的输出呈现](https://api.python.langchain.com/en/latest/output%5Fparsers/langchain%5Fcore.output%5Fparsers.string.StrOutputParser.html)。
    ...
    # 连接整条链，其中并行接入来自用户的 query 和来自步骤 2 中 Exa 检索链的 context。
  ...
  <Step title="（可选）流式传输链的输出">
    ...
    以流式方式输出。[了解更多](https://python.langchain.com/v0.1/docs/expression%5Flanguage/streaming/) `.stream` 方法的相关内容，包括分块处理和输出解析。
  </Step>
</Steps>