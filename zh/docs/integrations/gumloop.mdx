...
description: "在 Gumloop flow 中使用 Exa search 和页面内容。"
icon: "/docs/images/sidebar-icons/gumloop.svg"
---

[Gumloop](https://www.gumloop.com/) 内置了 Exa 作为 MCP integration。将其添加到 agent 或 Agent Node 后，即可在工作流中搜索网页、提取页面、查找相关 sources，并生成带引用来源的答案。
...
<Steps>
  ...
  <Step title="选择 tools">
    打开已连接的 Exa integration，仅启用 agent 需要的 tools。这样工具选择更清晰，也能避免 agent 调用无关的操作。
  </Step>
...
| **Find Similar**         | 发现与某个来源 URL 相关的页面。           |
...
在确定性的 Gumloop flow 中添加 **Agent Node**，并将 Exa 接入为它的 tools 之一。该 node 可以自行判断是执行 search、抓取完整页面，还是先串联多次 Exa 调用，再把输出传给下一个工作流步骤。

它非常适合以下场景：

* 用最新的网络证据丰富 CRM 或电子表格中的行数据
* 监控新闻，并将带来源的 summary 发送到 Slack 或邮箱
* 在把 records 转入销售工作流之前先对公司做研究
* 对比产品并将结果写入文档

### 可复用的自定义 MCP node {#reusable-custom-mcp-node}

若只需一个可重复执行的操作，可创建专用 node：

1. 打开 node 库并搜索 Exa。
2. 选择 **Create a node with AI**。
3. 描述单个操作，例如 `Search for funding announcements from the past seven days`。
4. 测试生成的 node，确认其输入和输出无误后保存。
...
<AccordionGroup>
  ...
  <Accordion title="读取已知页面">
    ```text theme={null}
    获取该 URL 的完整页面内容，并以 JSON 格式提取定价档位。
    ```
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="agent 选择了错误的操作">
    在请求中明确说明应当执行 search、读取已知 URL、查找相似页面，还是基于 sources 作答。同时禁用该工作流中 agent 用不到的 Exa tools。
  </Accordion>

  <Accordion title="工作流需要一次结果可预测的调用">
    用输入和任务均固定的自定义 Exa MCP node 替换通用型 agent 步骤。
  </Accordion>
</AccordionGroup>
...
<Columns cols={3}>
  <Card title="Gumloop Exa integration" icon="book-open" href="https://docs.gumloop.com/nodes/mcp/exa" cta="阅读指南" arrow="true">
    了解 Gumloop 当前提供的 tools 及 Agent Node 工作流。
  </Card>
