...
Webset 以一条自然语言 query 和目标项目数量为起点。你可以添加每条结果都必须满足的 criteria，并指定需要为每个通过的项目填充的 enrichment 字段。结果会通过控制台、API 或 webhook 异步返回。
...
<Info>
  ...
  Websets API 需要付费的 Websets 套餐；Search API 积分与 Websets 积分相互独立。
</Info>
...
1. **定义 search：** 提供自然语言 query、结果数量，以及可选的验证 criteria 和 enrichments。
2. **搜索并验证：** Websets 查找候选结果，并逐一对照你的 criteria 进行检查。只有符合条件的结果才会成为项目。
3. **运行 enrichments：** 对于每个已验证的项目，Websets 会搜索你所请求的附加数据,例如 CEO 姓名、融资金额或联系方式。
...
| 功能              | 作用                                  |
| --------------- | ----------------------------------- |
| **Criteria 校验** | 每条结果都会依照你定义的规则进行核验，确保只返回相关的匹配项      |
| **Enrichment**  | 为每条结果提取特定数据点 (文本、数字、日期、布尔值)         |
| **Monitor**     | 定时执行周期性 search，自动保持 webset 更新       |
| **Webhook**     | 在项目被添加或完成 enrichment 时，实时接收 HTTP 回调 |
...
<Note>
  产品可用性请参见 [零数据保留](/zh/docs/admin/security/zero-data-retention)。
</Note>
...
* [**控制台 指南**](./dashboard/get-started) - 在 控制台 中使用 Websets 的分步指南
