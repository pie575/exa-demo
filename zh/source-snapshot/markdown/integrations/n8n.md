...
<Steps>
  <Step title="添加社区 node">
    ...
    该 node 要求 n8n 1.60 及以上版本，以及 Node.js 20.15 及以上版本。
  </Step>
...
| 资源           | 操作                                                                                    |
| ------------ | ------------------------------------------------------------------------------------- |
| **Search**   | 使用 `auto`、`instant`、`fast`、`deep-lite`、`deep` 或 `deep-reasoning` 搜索网络，并可选择启用综合与结构化输出。 |
...
<AccordionGroup>
  <Accordion title="node picker 中没有 Exa node">
    请联系 instance 所有者安装已验证的社区包 `n8n-nodes-exa-official`。社区 node 能否使用，取决于 n8n instance 的托管方式。
  </Accordion>

  <Accordion title="Exa 凭据被拒绝">
    请确认所选凭据中包含来自 [Exa 控制台](https://dashboard.exa.ai/api-keys) 的有效密钥，且该密钥仍有可用积分。
  </Accordion>

  <Accordion title="agent 工作流超时">
    关闭 **Wait for Completion**，保存返回的运行 `id`，然后在后续步骤中通过 **Get Run** 获取结果。
  </Accordion>
</AccordionGroup>
