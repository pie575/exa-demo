...
ElevenLabs 语音 agent 可以将 Exa 作为 **webhook 工具**，在对话过程中实时搜索网页。当 agent 判断需要获取最新信息时，ElevenLabs 会直接向 Exa 的 `/search` 端点发起 HTTP POST 请求——你无需自行搭建任何服务器或中间件。

将 Exa 接入 ElevenLabs 有两种方式：

| 方式                     | 配置方法                  | 灵活性                   |
| ---------------------- | --------------------- | --------------------- |
| **Webhook 工具** (推荐)    | 通过 API 或控制台配置         | 完全掌控搜索参数、内容选项和 header |
| **内置 Exa 集成** (alpha)  | 在 ElevenLabs 控制台中一键接入 | 更简单，但配置能力有限           |

本指南介绍 webhook 工具方式，它让你完全掌控 Exa 的调用方式。你也可以通过 [ElevenLabs 控制台](https://elevenlabs.io/app/conversational-ai)配置该集成。
...
3. ElevenLabs 带上你配置的 header 和请求体，向 `https://api.exa.ai/search` 发起 POST 请求
4. 由 LLM 决定的参数 (搜索 `query`) 与你设置的常量值 (`type`、`numResults`、`contents`) 合并
5. Exa 的结果回传给 LLM，由其以对话方式作出回应

无需服务器、回调 URL 或监听器。ElevenLabs 本身就是直接调用 Exa 的 HTTP 客户端。工具调用的超时时间为 20 秒。
...
<Steps>
  <Step title="创建 webhook 工具">
    ...
    * `numResults: 5` —— 每次搜索返回 5 条结果
    * `contents.highlights: true` —— 返回节省 token 的 highlights 片段 (最适合语音场景的延迟要求)

    保存返回的 `id` —— 后续将工具绑定到 agent 时需要用到。

    <Note>
      如果你已经有 agent，可以跳过第 2 步，在 ElevenLabs 控制台的 **Agent &gt; Tools** 中，或通过 [Update Agent API](https://elevenlabs.io/docs/api-reference/agents/update) 将该工具添加到现有 agent。工具只有接入 agent 后才会生效。
    </Note>
  </Step>

  <Step title="创建带该工具的 agent">
    创建一个对话式 agent，并通过 ID 接入该 webhook 工具。
...
该脚本一次运行即可同时创建 webhook 工具和 agent：
...
webhook 工具的请求体 schema 与 [Exa 的 Search API](/zh/docs/reference/search) 直接对应。以下是几种常见配置：

<div id="search-type">
  ### 搜索类型
</div>

通过 `type` 常量控制速度与质量的取舍：
...
语音 agent 建议先使用 `instant`。若希望由 Exa 针对每个 query 自动选择当前最合适的搜索模式，请使用 `auto`。
...
* **`highlights`** — 节省 token 的摘录片段。若你只想获取相关片段，又不想撑爆 LLM 上下文，请使用此选项。传入 `true` 即可采用质量最高的默认配置。
* **`text`** — 完整页面的 markdown。当 agent 需要完整页面内容时使用。可通过 `maxCharacters` 限制长度。
* **`summary`** — 由 LLM 为每个页面生成的 summary。延迟较高，但能提供经过综合提炼的内容。

对于语音 agent，建议默认使用 `highlights: true` —— 它在相关性与响应速度之间取得了平衡。
...
* **`constant_value`** — 每次请求都发送的固定值，LLM 无法看到或修改。支持字符串、数字和布尔值。
* **`description`** — LLM 在运行时根据该描述确定取值。适用于 `query` 等动态参数。
* **嵌套对象** — 使用 `type: "object"` 配合 `properties` 构建 `contents.highlights` 这类嵌套结构。

每个参数在仪表板中都有一个模式开关 —— **Fixed** 或 **LLM**：

<Frame>
  <img src="https://mintcdn.com/exa-52/Una64IRjof2yadw_/images/integrations/elevenlabs/parameters.png?fit=max&auto=format&n=Una64IRjof2yadw_&q=85&s=618ca64cac86308c571a8268f48342a5" alt="ElevenLabs webhook 工具参数配置，显示 Fixed 与 LLM 模式开关" width="1692" height="898" data-path="images/integrations/elevenlabs/parameters.png" />
</Frame>

设置为 **Fixed** 的参数 (在 API 中以 `constant_value` 标记) 会在每次请求中原样发送；设置为 **LLM** 的参数 (以 `description` 标记) 则由模型在运行时决定取值。尽可能多地将参数设为 Fixed —— 每多一个由 LLM 决定的参数，就会多一步工具调用，从而拉长响应延迟。
...
ElevenLabs 还在 agent 控制台的 **Tools &gt; Integrations** 中提供了内置的 Exa 集成。它的配置更简单，但相比 webhook 工具方式，自定义搜索参数更为受限。
