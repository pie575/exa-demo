...
| 代码                          | 含义                                                  | 处理方式                                        |
| --------------------------- | --------------------------------------------------- | ------------------------------------------- |
| `400` Bad Request           | 请求体、query 参数、headers 或选项组合无效。                       | 根据返回的消息修正请求。                                |
| `401` Unauthorized          | 缺少 API 密钥或密钥无效。                                     | 检查认证 header 和 API 密钥。                       |
| `402` Payment Required      | 积分已用尽或超出支出预算。                                       | [充值积分](https://dashboard.exa.ai)或联系团队管理员。   |
| `403` Forbidden             | 该 API 密钥无权访问所请求的 feature，或请求被策略拦截。                  | 查看返回的消息以及你所在 plan 可使用的 feature 权限。          |
| `404` Not Found             | 路由或所请求的 resource 不存在。                               | 检查端点和 resource ID。                          |
| `409` Conflict              | 请求与现有状态冲突，例如已存在使用相同 `externalId` 的 Webset。          | 获取已有 resource，或改用其他 identifier。             |
| `422` Unprocessable Entity  | Websets 预览 query 无法拆解为有效的实体和 criteria。              | 重新表述预览 query。                               |
| `429` Too Many Requests     | 你的 API 密钥、团队或网络超出了速率或并发限制。                      | 降低请求速率；若返回了 `Retry-After`，请等待相应秒数，否则使用指数退避。 |
| `500` Internal Server Error | 发生了意外的服务器错误。                                        | 稍后重试。若问题持续，请联系支持团队。                         |
| `503` Service Unavailable   | Exa 暂时超载 (`SERVICE_OVERLOADED`) 或不可用。该请求未被处理，也不会计费。 | 使用指数退避重试。这与你的请求速率无关，因此降低速率无济于事，重试才有效。       |
| `504` Gateway Timeout       | 请求超出了处理时限。                                          | 重试该请求或缩小其 scope。                            |
...
| `API_KEY_BUDGET_EXCEEDED` | `402`    | 该 API 密钥已超出其消费预算 — 请联系你的团队管理员。                                 |
| `TEAM_BUDGET_EXCEEDED`    | `402`    | 团队已超出当前计费周期的消费预算。                                              |
...
| `NUM_RESULTS_EXCEEDED`    | `400`    | 请求的结果数量超出了你所在 plan 的限制。                      |
...
`httpStatusCode` 描述的是目标页面，而不是 `/contents` 的响应。

| Tag                       | 说明                                       | 处理方式                                     |
| ------------------------- | ---------------------------------------- | ---------------------------------------- |
| `CRAWL_NOT_FOUND`         | 未找到目标页面。                                 | 确认 URL 正确且可访问。                           |
| `CRAWL_HTTP_{status}`     | 目标返回了 HTTP 错误，例如 `CRAWL_HTTP_403`。       | 按其中嵌入的目标状态码处理。                           |
| `CRAWL_TIMEOUT`           | 抓取目标页面时超时。                               | 重试该请求或稍后再试。                              |
| `CRAWL_LIVECRAWL_TIMEOUT` | 实时 retrieval 超过了你设置的 `livecrawlTimeout`。 | 增大 `livecrawlTimeout` 或调整 `maxAgeHours`。 |
| `SOURCE_NOT_AVAILABLE`    | 该来源禁止访问，或当前不可用。                          | 检查该来源是否需要认证或存在访问限制。                      |
| `UNSUPPORTED_URL`         | 该 URL 协议不支持内容抓取。                         | 请使用标准的 HTTP 或 HTTPS URL。                 |
| `CRAWL_UNKNOWN_ERROR`     | 抓取因其他原因失败。                               | 重试该请求；若持续失败请联系支持团队。                      |
...
* 查阅该端点的 [API 参考](/zh/docs/reference/search)，确认请求要求。
