...
> 使用 Exa Search 查找最新报道、行业资讯和热点新动态。
...
使用 Exa Search 检索主流媒体、行业刊物和垂直小众媒体的报道。新文章发布后数分钟内即可被搜索到。若对发布时间窗口有硬性要求，可将自然语言 query 与日期筛选条件结合使用。
...
指明主题、来源类型和发布时间窗口，让结果聚焦于该事件的当前阶段。
...
如果你想获取从业者的分析，而不是泛泛的新闻报道，请在查询中指明来源类型。
...
在 query 中同时写明形式和主题，这样 search 就能覆盖全网的单集页面和文字记录。
...
请同时描述你要调查的负面信号和实体类别，不要把 query 简化成“公司名 + 新闻”。
...
如果结构化数据需要跨多个来源进行研究，可以使用 [Exa Agent 任务运行](/zh/docs/agent/quickstart)。只需描述所需的报道、字段和时间范围，Agent 便会返回经过 schema 校验的结果并附带引用来源。

<Card title="启动 Agent 任务" icon="bot" href="/zh/docs/agent/quickstart" cta="打开 Agent 指南" arrow="true">
  生成有来源支撑的新闻简报、对比各方报道，或从持续发展的事件中提取标准化事实。
</Card>