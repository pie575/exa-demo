...
description: "Offrez à OpenClaw la recherche web en temps réel et les page contents avec Exa."
...
<Note>
  Privilégiez `EXA_API_KEY` ou une SecretRef OpenClaw plutôt que de committer une API key dans un fichier de configuration.
</Note>
...
| `type`                       | Mode de recherche Exa, dont `auto`, `neural`, `fast`, `instant`, `deep` et `deep-reasoning`.              |
| `freshness`                  | Restreindre les résultats au dernier jour, à la dernière semaine, au dernier mois ou à la dernière année. |
| `date_after` / `date_before` | Restreindre les résultats avec des bornes au format `YYYY-MM-DD`.                                         |
| `contents`                   | Renvoyer le texte intégral, les highlights ou les résumés avec chaque résultat.                           |
...
Par défaut, OpenClaw met en cache les résultats de recherche web pendant 15 minutes. Modifiez `tools.web.search.cacheTtlMinutes` ou définissez cette valeur sur `0` si chaque requête doit renvoyer des données à jour.
...
<AccordionGroup>
  ...
  <Accordion title="OpenClaw signale une clé Exa manquante">
    Vérifiez que `EXA_API_KEY` est accessible au processus du gateway, et pas seulement à votre shell interactif. Pour une installation en gateway, placez-la dans `~/.openclaw/.env` et redémarrez le gateway.
  </Accordion>
...
<Columns cols={3}>
  <Card title="Fournisseur Exa pour OpenClaw" icon="book-open" href="https://docs.openclaw.ai/tools/exa-search" cta="Lire le guide" arrow="true">
  ...
  <Card title="Fraîcheur du contenu" icon="clock" href="/fr/docs/contents/quickstart#content-freshness" cta="Lire le guide" arrow="true">
    Maîtrisez les page contents indexés et récupérés en direct.
  </Card>
</Columns>