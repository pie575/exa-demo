...
description: "Offrez à OpenClaw la recherche web en temps réel et le page content avec Exa."
icon: "/docs/images/sidebar-icons/openclaw.svg"
---

[OpenClaw](https://openclaw.ai/) prend en charge Exa comme provider `web_search` natif. Une fois Exa sélectionné, chaque agent OpenClaw peut utiliser les search modes d&#39;Exa, les filtres de date et l&#39;extraction de contenu via le tool web intégré.
...
<Steps>
  ...
  <Step title="Enregistrer la clé">
    Pour une installation via gateway, ajoutez la clé à `~/.openclaw/.env` :
...
| `freshness`                  | Limiter les résultats au dernier jour, à la dernière semaine, au dernier mois ou à la dernière année. |
| `date_after` / `date_before` | Limiter les résultats avec des bornes au format `YYYY-MM-DD`.                                         |
| `contents`                   | Renvoyer le texte intégral, les highlights ou les summaries avec chaque résultat.                     |
...
<AccordionGroup>
  <Accordion title="OpenClaw n'affiche pas Exa comme provider">
  ...
  <Accordion title="OpenClaw signale une clé Exa manquante">
    Vérifiez que `EXA_API_KEY` est bien accessible au processus du gateway, et pas seulement à votre shell interactif. Dans le cas d&#39;une installation en gateway, placez-la dans `~/.openclaw/.env` puis redémarrez le gateway.
  </Accordion>

  <Accordion title="Les résultats de search semblent obsolètes">
    OpenClaw met les résultats en cache indépendamment d&#39;Exa. Réduisez `tools.web.search.cacheTtlMinutes` ou fixez-le à `0`, puis utilisez les options de content freshness d&#39;Exa lors de la récupération des page contents.
  </Accordion>
</AccordionGroup>
