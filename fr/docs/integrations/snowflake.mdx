---
title: "Snowflake"
description: "Utilisez la recherche et la récupération de contenu Exa depuis les procédures stockées Snowflake et les Cortex Agents."
...
1. **Tools pour Cortex Agent** — donnez à un Cortex Agent de Snowflake la capacité de rechercher sur le web et de récupérer le contenu des pages
2. **Enrichment de données** — enrichissez vos tables par batch avec des informations sur les PDG/fondateurs, les dernières actualités et d&#39;autres données web
...
* Un **secret** stockant votre API key (chiffrée, non visible en SQL)
...
Effectue une recherche sur le web via la Search API d&#39;Exa. Renvoie des résultats comprenant des titres, des URL, des scores et, en option, le texte, les highlights ou les summaries.
...
| `get_contents`         | `BOOLEAN` | `TRUE`   | Récupérer le texte, les highlights et les summaries                                       |

<div id="exa_get_contents">
  ### EXA_GET_CONTENTS
</div>

Récupère le contenu des pages (texte, highlights, summaries) pour des URL spécifiques.
...
<div id="data-enrichment">
  ## Enrichment des données
</div>

Exa peut enrichir vos tables Snowflake avec des données web en temps réel — informations sur le PDG ou les fondateurs, dernières actualités, et bien plus — sans jamais quitter la plateforme.
...
Lors de l&#39;enrichissement de tables volumineuses, appeler Exa séquentiellement ligne par ligne est lent. Vous pouvez paralléliser les requêtes grâce au module `concurrent.futures` de Python au sein d&#39;une même procédure stockée, en envoyant plusieurs appels à l&#39;API Exa à la fois.
...
<div id="choosing-parallelism">
  ### Choisir le niveau de parallélisme
</div>
...
| `20–50`       | Grands jeux de données avec un palier de [limite de débit](/fr/docs/admin/billing#rate-limits) élevé |

<div id="snowflake-task-scheduling">
  ### Planification de tâches Snowflake
</div>

Pour les jobs d&#39;enrichment récurrents, encapsulez la procédure dans une [tâche Snowflake](https://docs.snowflake.com/en/user-guide/tasks-intro) :
...
Cette tâche exécute l&#39;Enrichment parallèle tous les jours à 8 h (heure du Pacifique). Ajustez la planification cron et `max_workers` en fonction de votre volume et de vos limites de débit.

<div id="cortex-agent-integration">
  ## Intégration Cortex Agent
</div>

Vous pouvez également enregistrer les procédures stockées Exa comme tools pour un [Snowflake Cortex Agent](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-agents), ce qui lui permet d&#39;effectuer des recherches sur le web de manière conversationnelle.
...
| **Network Rule**                | Trafic sortant autorisé uniquement vers `api.exa.ai:443`               |
| **Secret**                      | API key stockée chiffrée, invisible dans les requêtes SQL              |
