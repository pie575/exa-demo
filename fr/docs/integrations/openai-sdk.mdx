...
description: "Utilisez les endpoints d'Exa comme remplacement direct d'OpenAI — compatibles avec les API chat completions et responses."
...
| `/responses`        | API Responses        | `exa-agent`         | API Agent (recherche asynchrone, enrichment, list-building) |

<Info>
  `/chat/completions` est routé vers [`/answer`](/fr/docs/reference/answer), et `/responses` vers l&#39;[API Agent](/fr/docs/agent/quickstart). Voir [Agent via API Responses](#agent-via-responses-api) ci-dessous.
</Info>
...
2. Remplacez la clé API par votre clé API Exa
...
L&#39;endpoint [`/responses`](https://api.exa.ai/responses) d&#39;Exa expose l&#39;[API Agent](/fr/docs/agent/quickstart) via l&#39;interface OpenAI Responses : les SDK OpenAI fonctionnent donc avec lui sans aucune modification. Définissez `model: "exa-agent"` et choisissez un mode d&#39;exécution :
...
| Streaming  | `stream: true`                          | La requête diffuse les événements OpenAI Responses (SSE) au fil du run, et se termine par `response.completed`.         |
...
<Warning>
  Les runs avec un `reasoning.effort` à `high`, `xhigh` ou `max` sont trop longs pour une requête synchrone et renvoient `400`. Utilisez `stream: true` ou `background: true` pour ces runs. `/responses` ne comporte pas de field `budget` ; max applique son plafond par run par défaut.
</Warning>
...
La requête reste bloquée jusqu&#39;à la fin du run et renvoie l&#39;objet `response` final.
...
Définissez `stream: true` pour recevoir les événements du flux Responses via SSE. Chaque événement porte un `sequence_number` monotone et le flux se termine par `response.completed` ; il n&#39;y a pas de sentinelle `[DONE]`. Le flux peut contenir des lignes de commentaire `: keep-alive`, que les clients SSE ignorent.
...
<CodeGroup>
  ...
  # Interroger avec l'identifiant de response renvoyé
...
Exa fournit un wrapper Python qui enrichit automatiquement toute complétion de chat OpenAI de capacités RAG. En une seule ligne de code, vous pouvez transformer n&#39;importe quelle complétion de chat OpenAI en un système RAG propulsé par Exa, qui gère automatiquement la recherche, le découpage en segments et le prompting.
...
<CodeGroup>
  ...
  # S'utilise exactement comme le client OpenAI classique
...
Le client encapsulé fonctionne exactement comme le client OpenAI natif, à ceci près qu&#39;il enrichit automatiquement vos complétions de résultats de recherche pertinents lorsque c&#39;est nécessaire.
