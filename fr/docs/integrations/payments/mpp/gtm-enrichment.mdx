...
description: "Créez un workflow d'enrichment GTM facturé à la requête Exa search et contents avec Tempo MPP — sans API key."
...
via le Machine Payments Protocol (MPP). MPP prend en charge plusieurs moyens de
paiement ; les exemples présentés ici utilisent des stablecoins sur
[Tempo](https://tempo.xyz). Pas d&#39;abonnement mensuel, pas d&#39;API key ni de
tarification par siège : approvisionnez un wallet en USDC.e et payez au fur et
à mesure que vous enrichissez vos leads ou vos entreprises.
...
<Info>
  ...
  par le flux de facturation standard par API key.
</Info>
...
Un pipeline d&#39;enrichment léger qui, à partir d&#39;une liste de noms d&#39;entreprises ou
de descriptions cibles :

1. Utilise `/search` d&#39;Exa avec `type: "deep"` et `outputSchema` pour trouver la
   page officielle de l&#39;entreprise et en extraire les metadata essentielles.
2. Utilise `contents.highlights` sur le résultat renvoyé pour récupérer des extraits
   sources sur les levées de fonds, le siège social, les effectifs et le produit.
3. Produit un enregistrement d&#39;enrichment au format CSV ou JSON pour chaque entrée.

Ce modèle convient à l&#39;enrichment de listes de prospects, à la recherche sur les
comptes et à la personnalisation des campagnes sortantes. Comme il repose sur
des appels `/search` + `/contents` distincts, chaque étape peut être payée avec MPP.
...
Si vous souhaitez obtenir des champs JSON plutôt que du texte brut, utilisez `outputSchema` dans la
requête de search. Exa renvoie un objet `output` conforme à votre schema.
...
<Note>
  ...
  call LLM côté Exa : il est donc facturé comme `deep-lite`/`deep`.
</Note>
...
MPP applique la même tarification par requête que la facturation par API key. Les requêtes de search via MPP
sont limitées à 10 résultats.
...
notamment les rate limits, les détails du réseau et les headers de paiement.

<div id="production-tips">
  ## Conseils pour la production
</div>

* **Approvisionnez le wallet uniquement en USDC.e.** Exa prend en charge les frais de réseau Tempo : le wallet n&#39;a donc pas besoin d&#39;un jeton de gas distinct.
...
* **Ne commitez jamais de private keys.** Chargez `WALLET_PRIVATE_KEY` depuis un secrets manager, et non depuis votre gestionnaire de versions.
...
<AccordionGroup>
  ...
  <Accordion title="Puis-je combiner MPP et une API key Exa dans la même requête ?">
    ...
    par API key est prioritaire et MPP est contourné.
  </Accordion>

  <Accordion title="Que se passe-t-il si le règlement MPP échoue ?">
    ...
    résultat n&#39;est renvoyé tant que le règlement n&#39;a pas abouti.
  </Accordion>
  ...
  <Accordion title="Ai-je besoin d'un wallet Tempo distinct par environnement ?">
    ...
    distincts pour le développement et la production. Le QPS par wallet est de
...
* [Payer avec MPP (Tempo)](/fr/docs/integrations/payments/mpp/quickstart) : référence MPP complète
* [Guide de l&#39;API Exa Search](/fr/docs/search/quickstart) : référence des paramètres de search
* [Guide de l&#39;API Exa Contents](/fr/docs/contents/quickstart) : référence des paramètres de contents
* [Documentation MPP de Tempo](https://mpp.dev/protocol) : détails du protocole et du SDK