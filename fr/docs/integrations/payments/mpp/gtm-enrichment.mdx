---
title: "Cookbook d'enrichment GTM avec Tempo MPP"
sidebarTitle: "Enrichment GTM avec MPP"
description: "Créez un workflow d'enrichment GTM facturé à la requête Exa search et contenu avec Tempo MPP, sans API key."
icon: "/docs/images/sidebar-icons/tempo.svg"
---

Utilisez ce cookbook pour créer un agent ou un pipeline d&#39;enrichment GTM
reposant sur les endpoints `/search` et `/contents` d&#39;Exa, facturés à la requête
via le Machine Payments Protocol (MPP). MPP prend en charge plusieurs méthodes
de paiement ; les exemples ci-dessous utilisent des stablecoins sur
[Tempo](https://tempo.xyz). Pas d&#39;abonnement mensuel, pas d&#39;API key et pas de
tarification par siège : approvisionnez un wallet en USDC.e et payez au fur et à
mesure que vous enrichissez vos leads ou vos entreprises.

<Info>
  MPP n&#39;est actuellement pris en charge que sur les endpoints `/search` et `/contents` d&#39;Exa.
...
3. Produit un record d&#39;enrichment en CSV ou JSON pour chaque entrée.

Ce modèle convient à l&#39;enrichment de listes de prospects, à la recherche sur les comptes et à la
...
### Effectuer une requête contents payante {#make-a-paid-contents-request}
...
### Enrichir le profil d&#39;une personne {#enrich-a-person-profile}

Cette recette utilise `type: "deep"`, `contents.highlights` et `outputSchema` pour
faire des recherches sur une personne et renvoyer un profil structuré.
...
Si vous souhaitez des fields JSON plutôt que du texte brut, utilisez `outputSchema` dans la
requête de search. Exa renvoie un objet `output` conforme à votre schéma.
...
<Note>
  ...
  appel LLM côté Exa, il est donc facturé comme `deep-lite`/`deep`.
</Note>

## Tarifs et limites {#pricing-and-limits}

MPP utilise la même tarification par requête que la facturation par API key. Les requêtes de recherche MPP
...
notamment les limites de débit, les détails du réseau et les headers de paiement.

## Conseils pour la production {#production-tips}

* **Alimentez le wallet uniquement en USDC.e.** Exa prend en charge les frais de réseau Tempo : le wallet n&#39;a donc pas besoin d&#39;un jeton de gas distinct.
* **Gérez les réponses `402`.** Le SDK MPP réessaie automatiquement, mais un client personnalisé doit réessayer en cas de `402` à l&#39;aide du challenge `WWW-Authenticate: Payment`.
* **Mettez en cache les résultats de `/contents`.** Le contenu est facturé par URL. Utilisez l&#39;URL comme clé de cache pour éviter de payer deux fois la même page d&#39;entreprise.
* **Attention au plafond de 10 résultats.** La search MPP limite `numResults` à 10.
* **Ne committez jamais de private keys.** Chargez `WALLET_PRIVATE_KEY` depuis un gestionnaire de secrets, et non depuis votre gestionnaire de versions.

## FAQ {#faq}

<AccordionGroup>
  <Accordion title="Puis-je utiliser MPP avec l'API Exa Agent ?">
    Non. Dans la base de code Exa, MPP est raccordé uniquement à `/search` et `/contents`.
  ...
  <Accordion title="Que se passe-t-il si le settlement MPP échoue ?">
    ...
    aucun résultat. Votre client peut réessayer avec un nouveau paiement. Aucun résultat n&#39;est renvoyé
    tant que le settlement n&#39;a pas abouti.
  </Accordion>

  <Accordion title="Ai-je besoin d'un wallet Tempo distinct par environnement ?">
    Vous pouvez réutiliser le même wallet, mais nous recommandons des wallets distincts pour
    le développement et la production. Le QPS par wallet est de 10 requêtes/seconde, toutes
    requests confondues provenant de ce wallet.
  </Accordion>
</AccordionGroup>
