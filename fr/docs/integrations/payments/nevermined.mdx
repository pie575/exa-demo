...
description: "Paiements autonomes d'agents pour Exa via la délégation de carte x402 de Nevermined. Un achat de 7 USD provisionne ou recharge une API key Exa avec 7 USD de crédits."
icon: "/docs/images/sidebar-icons/nevermined.svg"
---

Les agents paient Exa par carte bancaire grâce au schéma [x402 card-delegation](https://nevermined.ai/docs/specs/x402-card-delegation) de [Nevermined](https://nevermined.ai). Chaque **achat de $7** renvoie une API key Exa assortie de **$7 de crédits Exa**.

<Info>
  Utilisez ce plan ID Nevermined :<br />`27800462147494506865542649899724877617306579171265399959488097895839186996870`<br />Ce plan fonctionne dans l&#39;environnement live de Nevermined (API keys préfixées par live). L&#39;achat porte sur des crédits d&#39;API, et non sur une seule search request.
</Info>
...
* **Réponse (nouveau payeur) :** `{ status: "ok", apiKey: "…", expiresAt: null }` — une nouvelle API key Exa avec 7 $ de crédit.
* **Réponse (payeur déjà connu) :** `{ status: "ok", apiKey: "…", expiresAt: null }` — la même API key Exa, avec 7 $ de crédit supplémentaires.
...
2. **L&#39;agent trouve sa délégation.** Le SDK Nevermined permet à l&#39;agent de découvrir les délégations sur lesquelles sa clé peut dépenser et d&#39;en choisir une dont le budget restant est suffisant (au moins 7 $). S&#39;il n&#39;en existe aucune, le propriétaire en crée une dans le tableau de bord, ou un agent entièrement autonome peut en créer une via le SDK, dans les limites de la carte.
...
Les requêtes Search incluent le texte et les highlights pour un maximum de 10 résultats. Les résultats au-delà de ces 10 premiers ainsi que les résumés par IA sont facturés séparément.<br />
Pour tous les détails sur la tarification, consultez [la tarification Exa](https://exa.ai/pricing).
...
Générez un nouveau jeton x402 avec le même plan ID et la même délégation, puis envoyez-le de nouveau en POST sur le même endpoint `/purchase-key`. Exa ajoute alors 7 $ de crédits supplémentaires à la même API key.
...
* [Tarification d’Exa](https://exa.ai/pricing)
