...
[x402](https://x402.org) est un standard de paiement ouvert basé sur le code de statut HTTP `402 Payment Required`. Il permet aux clients de payer l&#39;accès à une API à la requête avec des stablecoins USDC sur Base ou Solana, sans compte, API key ni abonnement.

Exa prend en charge x402 sur deux endpoints : **`/search`** et **`/contents`**. Lorsque vous envoyez une requête sans API key ni header de paiement, Exa répond par un `402` et un header `PAYMENT-REQUIRED` contenant les détails de tarification ainsi que les réseaux de paiement pris en charge. Votre client signe un paiement USDC, relance la requête avec un header `PAYMENT-SIGNATURE`, puis reçoit les résultats dès que le settlement est confirmé on-chain.

C&#39;est la solution idéale pour les **agents IA** qui doivent payer de façon autonome une recherche web sans credentials provisionnés à l&#39;avance.

<Info>
  x402 et l&#39;accès par API key sont indépendants. Si votre requête inclut un header `x-api-key` ou `Authorization: Bearer`, c&#39;est le flux normal de facturation par API key qui s&#39;applique et x402 est entièrement contourné.
</Info>
...
<Frame>
  <img src="/docs/images/integrations/payments/x402/payment-flow.png" alt="Diagramme de séquence du flux de paiement x402 : le client envoie une requête au serveur, reçoit un 402 avec le header PAYMENT-REQUIRED, crée un payload de paiement, réessaie avec PAYMENT-SIGNATURE, le serveur vérifie via le facilitator, exécute le traitement, effectue le règlement on-chain, puis renvoie un 200 avec les résultats et PAYMENT-RESPONSE" width="4224" height="2720" data-path="images/integrations/payments/x402/payment-flow.png" />
</Frame>
...
Le montant (`amount`) est exprimé en unités atomiques USDC (6 décimales), donc `"7000"` = 0,007 $.
Le client peut payer avec n&#39;importe quelle entrée `accepts` annoncée qu&#39;il prend en charge. Les entrées Solana comportent des fields fournis par le facilitator, tels que `extra.feePayer` ; utilisez l&#39;entrée exacte issue du header `PAYMENT-REQUIRED` pour construire le paiement.
...
Si le settlement échoue, vous obtenez un `402` accompagné de `PAYMENT-RESPONSE` (détails de l&#39;erreur) et de `PAYMENT-REQUIRED` (pour pouvoir réessayer).

## Tarification {#pricing}

x402 utilise la même tarification groupée que la facturation par API key. Les prix sont calculés à l&#39;avance en fonction des paramètres de votre requête (et non des résultats réellement renvoyés).
...
<Tip>
  Vous ne voulez pas gérer de private keys ? Les [Coinbase Agentic Wallets](https://docs.cdp.coinbase.com/agent-kit/core-concepts/wallet-management) offrent une gestion des clés isolée dans un TEE pour les agents IA. Votre agent ne voit jamais la private key. Le wallet étant compatible viem, il fonctionne directement avec `@x402/fetch`.
</Tip>
...
<CodeGroup>
  ...
  // accept Solana telles que `solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp` :
...
<Info>
  cURL impose de signer le paiement manuellement. En production, utilisez le SDK JavaScript ou Python, qui gère automatiquement tout le flux 402 &gt; signature &gt; nouvelle tentative.
</Info>

### Mode discovery (aucun wallet nécessaire) {#discovery-mode-no-wallet-needed}
...
Les deux utilisent l&#39;USDC à 6 décimales (`1000000` = 1,00 $) et procèdent au règlement on-chain via un facilitateur x402.

## Limites de débit {#rate-limits}

x402 applique ses propres limites de débit, distinctes de celles des API keys :
...
Au-delà de 5 discovery requests `402` non authentifiées provenant de la même IP en 60 secondes, les requêtes suivantes renvoient `429 Too Many Requests`. Une requête payée réussie décrémente le compteur.
...
| `429`  | `X402_TOO_MANY_UNPAID`     | Trop de discovery requests non payées depuis cette IP                           |
...
<AccordionGroup>
  <Accordion title="Puis-je utiliser x402 et une API key en même temps ?">
    Si votre requête inclut un header `x-api-key` ou un token `Authorization: Bearer`, le flux par API key est prioritaire et x402 est ignoré. Les deux ne se cumulent pas : c&#39;est l&#39;un ou l&#39;autre pour chaque requête.
  </Accordion>
  ...
  <Accordion title="Pourquoi numResults est-il plafonné à 10 ?">
    Les requêtes x402 imposent un maximum de 10 résultats par search. Si vous en avez besoin de davantage, utilisez le flux par API key avec un plan payant.
  </Accordion>
...
* [Guide de l&#39;Exa Search API](/fr/docs/search/quickstart) : référence complète des paramètres de recherche
