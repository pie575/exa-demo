...
<Warning>
  **Plusieurs comptes Google :** l&#39;add-on doit s&#39;exécuter sous le premier compte Google (celui par défaut) de votre profil de navigateur. Si vous êtes connecté à plusieurs comptes, vous risquez de ne pas pouvoir enregistrer ni charger votre API key. Pour y remédier, ouvrez Sheets dans une fenêtre de navigation privée avec un seul compte, ou déconnectez-vous des comptes supplémentaires afin que le compte souhaité devienne celui par défaut. [En savoir plus](https://developers.google.com/apps-script/guides/projects#fix_issues_with_multiple_google_accounts).
</Warning>
...
* **Exa Agent** pour les tableaux complets et les tâches portant sur plusieurs cellules
...
* prolonger un tableau en y ajoutant de nouvelles lignes
...
Par défaut, le tableau commence à la cellule sélectionnée. Vous pouvez choisir une autre cellule de départ dans **More options**.
...
Avant d&#39;utiliser **Fill cells**, sélectionnez des cellules vides dans un tableau dont les en-têtes sont clairement définis.
...
### Poursuivre les lignes {#continue-rows}

Vous pouvez également sélectionner des lignes vides sous un tableau.

Si votre tableau se termine au rang 55 et que vous sélectionnez les deux lignes vides suivantes, Exa peut le prolonger avec les rangs 56 et 57.

Exa s&#39;appuie sur les lignes existantes comme exemples, conserve les mêmes colonnes et évite de répéter les items déjà présents dans le tableau.

## `=EXA(...)` {#exa}

Utilisez `=EXA(...)` lorsque vous souhaitez obtenir une réponse unique dans une seule cellule. La fonction effectue une recherche sur le web, lit les premiers résultats et renvoie une réponse concise.
...
Réponses IA avancées avec un contrôle total sur le format de sortie. Utilisez cette fonction lorsque vous avez besoin de system prompts, d&#39;une sortie JSON structurée, de citations ou d&#39;un type de recherche spécifique.
...
| `systemPrompt`     | Non    | `""`     | Instructions système pour contrôler le format de l&#39;output (par ex. `"only return a number"`).              |
...
| `outputSchemaJson`   | Non    | `""`     | Chaîne JSON pour `outputSchema` (par ex. `"{""type"":""text"",""description"":""summarize""}"`). Si défini, renvoie un texte de sortie synthétisé au lieu des URL. |
...
Trouve des URL similaires à une URL de référence, avec des filtres facultatifs sur le domaine et le texte.
...
Utilisez **Batch** lorsque vous souhaitez traiter plusieurs cellules de formule Exa en une seule fois.

Batch permet de :

* actualiser les cellules sélectionnées contenant des formules Exa
* convertir les formules Exa sélectionnées en valeurs classiques
...
* Les requêtes à l&#39;API Exa sont décomptées de votre quota d&#39;utilisation. Utilisez **Batch → Convert to values** pour figer les résultats et empêcher les formules de se recalculer.
* L&#39;add-on réessaie automatiquement jusqu&#39;à 3 fois avec un backoff exponentiel lorsque la limite de débit est atteinte (HTTP 429).
