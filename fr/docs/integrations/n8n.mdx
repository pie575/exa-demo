...
Le [node Exa pour n8n](https://github.com/exa-labs/n8n-integration) officiel ajoute la web search, l&#39;extraction de contenu, les grounded answers et les Exa Agent runs à vos workflows visuels. Utilisez-le comme une étape de workflow classique ou connectez-le à un AI Agent n8n en tant que tool.
...
La recherche peut également renvoyer le texte, les highlights, les summaries, les liens et les images de chaque résultat. Les filtres de domaine, les dates de publication, les catégories, `maxAgeHours` et l&#39;exploration des sous-pages sont disponibles dans les fields facultatifs du node.
...
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------- |
| **Search**   | Recherchez sur le web avec `auto`, `instant`, `fast`, `deep-lite`, `deep` ou `deep-reasoning`, avec synthèse et structured output en option. |
| **Contents** | Récupérez le texte nettoyé, les highlights, les summaries, les liens et les images pour une liste d&#39;URL.                                 |
| **Answer**   | Générez une grounded answer assortie de citations, avec structured output en option.                                                         |
| **Agent**    | Créez, inspectez, listez, diffusez en flux, interrogez et annulez des Agent runs multi-étapes.                                               |
...
Search et Answer font d&#39;excellents outils de grounding. Utilisez la ressource Agent lorsque la tâche nécessite une recherche en plusieurs étapes, du list building, une Enrichment structurée ou des données premium [Exa Connect](/fr/docs/agent/connect/overview).
...
* **Stream** pour maintenir ouverte une connexion server-sent events jusqu&#39;à la fin du run
* **Poll** pour vérifier le run à intervalles réguliers

Pour les workflows longs ou asynchrones, désactivez **Wait for Completion**, conservez l&#39;`id` du run renvoyé, puis utilisez **Get Run** plus tard. Le run se poursuit sur Exa une fois l&#39;étape n8n terminée.
...
<AccordionGroup>
  <Accordion title="Le node Exa n'apparaît pas dans le sélecteur de nodes">
    Demandez au propriétaire de l&#39;instance d&#39;installer le package communautaire vérifié `n8n-nodes-exa-official`. La disponibilité des nodes communautaires dépend du mode d&#39;hébergement de votre instance n8n.
  </Accordion>

  <Accordion title="Le credential Exa est refusé">
    Vérifiez que le credential sélectionné contient une key active issue du [dashboard Exa](https://dashboard.exa.ai/api-keys) et que cette key dispose de credits.
  </Accordion>
...
<Columns cols={3}>
  <Card title="Exa node officiel" icon="github" href="https://github.com/exa-labs/n8n-integration" cta="Voir le repository" arrow="true">
