...
description: "Utilisez le tool use de Claude pour ajouter Exa web search et la récupération du contenu des pages à votre application."
...
<Steps>
  ...
  <Step title="Configurer vos API keys">
    Définissez les variables d&#39;environnement `EXA_API_KEY` et `ANTHROPIC_API_KEY`. Rendez-vous sur la [console Anthropic](https://console.anthropic.com/settings/keys) et le [dashboard Exa](https://dashboard.exa.ai/api-keys) pour générer vos API keys.
  ...
  <Step title="Ajouter les tools Exa à votre boucle de tools">
    ...
    L&#39;exemple ne montre qu&#39;un seul tour, par souci de concision. Un véritable agent conserve `tools` dans chaque requête et répète l&#39;étape du handler jusqu&#39;à ce que le modèle réponde sans bloc `tool_use` — c&#39;est ainsi qu&#39;un résultat de search débouche sur la lecture d&#39;une page en follow-up.
...
Le modèle choisit la `query` de search et les `urls` à lire ; tout le reste est fixé au moment où vous créez le tool, il ne peut donc pas modifier ce qui est exploré ou extrait.

`name` (valant par défaut `"web_search"` et `"get_contents"`) et `description` remplacent quant à eux la définition du tool telle que la voit le modèle. Anthropic exige que les noms de tools soient uniques : un nom personnalisé permet donc au tool Exa de coexister avec le server tool intégré `web_search_20250305` d&#39;Anthropic, qui réserve le nom `web_search` :
...
`handle_tool_use` répond à chaque bloc `tool_use` du message : un bloc désignant un tool qu&#39;il ne parvient pas à résoudre reçoit un résultat `Error: unknown tool "<name>"` au lieu d&#39;être ignoré, si bien que la requête de follow-up n&#39;omet jamais un résultat de tool requis. Si vous exécutez vos propres tools aux côtés de ceux d&#39;Exa, remplacez ces résultats d&#39;erreur par les vôtres avant la requête suivante.

<div id="writing-the-loop-by-hand">
  ## Écrire la boucle à la main
</div>

Si vous préférez gérer vous-même le tool schema et l&#39;exécution, définissez le tool et traitez les blocs `tool_use` manuellement. `exa.tools.web_search()` et `exa.tools.get_contents()` vous fournissent les mêmes spécifications de tools indépendantes du provider (avec une méthode `run`) pour vos boucles maison, ou vous pouvez tout écrire de zéro :
