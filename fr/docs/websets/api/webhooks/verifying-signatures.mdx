...
description: "Découvrez comment vérifier de façon sécurisée les signatures des webhooks pour vous assurer que les requêtes proviennent bien d'Exa"
timestamp: false
---
...
2. Construisez le payload signé en concaténant l&#39;horodatage, un point et le corps brut de la requête
3. Calculez la signature attendue avec HMAC SHA256 et votre secret du webhook
4. Comparez la signature ainsi calculée aux signatures fournies
...
<CodeGroup>
  ...
  // Important : utilisez un parseur de corps brut pour la vérification du webhook
      ...
      */
      public static boolean verifyWebhookSignature(String payload, String signatureHeader, String webhookSecret) {
          try {
              ...
              // Facultatif : vérifie si l'horodatage est récent (moins de 5 minutes)
              ...
              // Compare avec les signatures fournies à l'aide d'une comparaison à temps constant
      ...
      */
      ...
      public static void main(String[] args) {
          ...
          try {
              ...
              // Test avec un horodatage manquant
...
* **Contrôlez la fraîcheur de l&#39;horodatage** - Envisagez de rejeter les webhooks dont l&#39;horodatage est trop ancien (par exemple, de plus de 5 minutes) afin de prévenir les attaques par replay.

* **Stockez les secrets de façon sécurisée** - Conservez vos secrets de webhook dans des variables d&#39;environnement ou dans un système sécurisé de gestion des secrets. Ne les codez jamais en dur dans votre application. **Important** : le secret du webhook n&#39;est renvoyé qu&#39;au moment où vous [créez un webhook](/fr/docs/websets/api/webhooks/create-a-webhook) - veillez à l&#39;enregistrer en lieu sûr, car il ne pourra pas être récupéré par la suite.

* **Utilisez HTTPS** - Utilisez toujours des endpoints HTTPS pour vos webhooks afin que les données soient chiffrées en transit.

* **Enregistrez l&#39;URL finale** - Les deliveries de webhooks ne suivent pas les redirections HTTP (réponses 3xx). Si votre endpoint redirige, la delivery sera considérée comme un échec. Enregistrez toujours l&#39;URL qui traite directement le payload.
...
3. **Vérifiez l&#39;analyse du header** : assurez-vous d&#39;extraire correctement l&#39;horodatage et les signatures depuis le header
