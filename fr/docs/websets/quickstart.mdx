...
<div id="what-are-websets">
  ## Que sont les Websets ?
</div>

Un Webset part d&#39;une requête en langage naturel et d&#39;un nombre d&#39;items cible. Ajoutez les critères que chaque résultat doit satisfaire ainsi que les champs d&#39;enrichment à renseigner pour chaque item accepté. Les résultats arrivent de façon asynchrone via le dashboard, l&#39;API ou les webhooks.

Vous pouvez également créer des websets visuellement depuis le [Dashboard](/fr/docs/websets/dashboard/get-started), sans écrire une seule ligne de code.

<Info>
  Vous démarrez un nouveau workflow de list-building ou d&#39;enrichment ? Utilisez [Exa Agent](/fr/docs/agent/quickstart).
  Ce guide sert à maintenir ou étendre une integration Websets existante.
  L&#39;API Websets nécessite un plan Websets payant ; les credits de la Search API et les credits Websets sont distincts.
</Info>
...
1. **Définissez une search :** fournissez une requête en langage naturel, un nombre de résultats, ainsi que des critères de vérification et des enrichments facultatifs.
2. **Recherchez et vérifiez :** Websets identifie des candidats et confronte chacun d&#39;eux à vos critères. Seuls les résultats correspondants deviennent des items.
3. **Exécutez les enrichments :** pour chaque item vérifié, Websets recherche les données supplémentaires que vous avez demandées, comme le nom d&#39;un PDG, un montant de financement ou des coordonnées.
4. **Recevez les résultats :** interrogez le status, utilisez des webhooks pour suivre les mises à jour ou consultez le dashboard à mesure que les items arrivent.
...
| **Monitors**                  | Planifier des searches récurrentes pour tenir votre webset à jour automatiquement                                       |
| **Webhooks**                  | Recevoir des callbacks HTTP en temps réel à mesure que des items sont ajoutés ou enrichis                               |
