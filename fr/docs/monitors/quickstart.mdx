...
description: "Exécutez des recherches récurrentes et recevez les nouveaux résultats via un webhook."
boost: 4
---

Les monitors exécutent des recherches Exa selon un schedule récurrent et envoient les résultats vers un endpoint webhook.

Utilisez les Monitors pour suivre l&#39;actualité, les annonces de vos concurrents, les levées de fonds, les évolutions réglementaires, les publications de recherche ou tout autre sujet qui évolue au fil du temps.

<div id="how-monitors-work">
  ## Fonctionnement des Monitors
</div>

À chaque run, Exa exécute la search configurée, applique un filtre temporel, écarte les résultats ou éléments que le monitor a déjà renvoyés, puis envoie le nouvel output à votre webhook.

Chaque monitor conserve son propre historique de runs : formulez donc la query autour du signal continu que vous souhaitez suivre, plutôt que d&#39;y ajouter vous-même une plage de dates glissante.
...
Conservez le `webhookSecret` dès la création du monitor : il n&#39;est renvoyé qu&#39;une seule fois et il est indispensable pour
...
| JSON structuré  | Ajoutez un objet `outputSchema` | Un JSON conforme au schema dans `output.content` |
...
les highlights, le texte intégral ou des résumés avec chaque résultat, et `includeDomains` ou `excludeDomains` pour
...
Les status possibles d&#39;un run sont :
...
schedule est calé sur l&#39;heure de création du monitor — un monitor quotidien créé à 14 h 30 s&#39;exécute
chaque jour aux alentours de 14 h 30 — mais chaque run peut subir un retard allant jusqu&#39;à 30 minutes : ne comptez donc pas sur une
heure de livraison précise.

Omettez `trigger` pour créer un monitor déclenché uniquement de façon manuelle. Mettre en pause un monitor planifié interrompt également les runs
automatiques, tout en conservant la possibilité de déclenchements manuels.

<Note>
  Les monitor runs ne se chevauchent pas. Si le prochain run planifié démarre alors que le précédent est encore
  en cours, Exa annule le run précédent.
</Note>
...
envoie aussi les événements de cycle de vie du monitor et les événements de création de run.
...
le bon espace de travail, le bon canal ou le bon traitement interne.
...
Chaque livraison inclut un header `Exa-Signature` au format `t=<timestamp>,v1=<signature>`.
...
<Columns cols={2}>
  ...
  <Card title="Guide de search" icon="search" href="/fr/docs/search/quickstart" cta="Ouvrir le guide" arrow="true">
    Configurez les requêtes, les filtres, les highlights, le texte intégral et la freshness.
  </Card>

  <Card title="Bonnes pratiques de search" icon="sparkles" href="/fr/docs/search/best-practices" cta="Lire le guide" arrow="true">
    Améliorez la qualité de la retrieval tout en conservant un output ciblé.
  </Card>
</Columns>