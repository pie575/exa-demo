...
description: "Exécutez des recherches récurrentes et recevez les nouveaux résultats découverts via webhook."
boost: 4
---

Les monitors exécutent des Exa searches selon un schedule récurrent et transmettent les résultats à un endpoint webhook.

Utilisez les Monitors pour suivre l&#39;actualité, les annonces de vos concurrents, les levées de fonds, les évolutions réglementaires, les publications de recherche ou tout autre sujet qui évolue dans le temps.

<div id="how-monitors-work">
  ## Fonctionnement des Monitors
</div>

À chaque run, Exa exécute la search configurée, applique un filtre temporel, écarte les résultats ou
informations que le monitor a déjà renvoyés, puis envoie le nouvel output vers votre webhook.

Chaque monitor conserve son propre historique de runs : formulez donc la requête autour du signal
continu que vous souhaitez suivre plutôt que d&#39;y ajouter vous-même une plage de dates glissante.
...
Conservez le `webhookSecret` lors de la création du monitor. Il n&#39;est renvoyé qu&#39;une seule fois et il est indispensable pour
...
Ajoutez `outputSchema` lorsque le code en aval nécessite des fields
cohérents :
...
| `completed` | Le run est terminé ; récupérez-le par son ID pour lire son output complet |
...
schedule est calé sur l&#39;heure de création du monitor — un monitor quotidien créé à 14h30 s&#39;exécute
chaque jour aux alentours de 14h30 — mais chaque run peut être retardé de 30 minutes au maximum : ne vous fiez donc pas à
une heure de restitution précise.
...
<Note>
  Les runs d&#39;un monitor ne se chevauchent pas. Si le run planifié suivant démarre alors que le précédent est toujours
...
Chaque delivery inclut un header `Exa-Signature` de la forme `t=<timestamp>,v1=<signature>`.
...
<Columns cols={2}>
  <Card title="Créer un monitor" icon="bell" href="/fr/docs/reference/monitors/create-a-monitor" cta="Ouvrir la référence" arrow="true">
    Découvrez tous les fields de Search, schedule, output, métadonnées et webhook.
  </Card>

  <Card title="Runs de monitor" icon="clock" href="/fr/docs/reference/monitors/runs/get-a-run" cta="Ouvrir la référence" arrow="true">
    Inspectez le statut, l&#39;output, le grounding et la cause d&#39;échec d&#39;un run.
  </Card>
