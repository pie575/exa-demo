...
Exa fonctionne à l&#39;usage. Aucun abonnement ni montant minimum : vous créditez votre compte et êtes facturé à la requête, aux tarifs ci-dessous.

<Check>
  **Commencez gratuitement.** Les nouveaux comptes reçoivent 20 $ de crédits offerts (environ 2 800 recherches), et l&#39;offre gratuite ajoute 10 $ de crédits chaque mois. Obtenez une API key et lancez-vous.

  **Vous montez en charge ?** Pour de gros volumes, des index personnalisés, des limites de débit plus élevées, des SLA ou le Zero Data Retention, [contactez-nous](https://exa.ai/contact/sales) au sujet d&#39;un [plan Enterprise](#enterprise) avec remises sur volume.
</Check>
...
Chaque endpoint a un prix de base par requête qui inclut jusqu&#39;à 10 résultats. Les résultats supplémentaires et les résumés de page générés par Exa sont facturés en sus.

| Endpoint    | Prix de base<br />(jusqu&#39;à 10 résultats) | Chaque résultat au-delà de 10 | Résumés de page par IA |
| ----------- | -------------------------------------------- | ----------------------------- | ---------------------- |
| `/search`   | 7 $ / 1k requests                            | 1 $ / 1k résultats            | 1 $ / 1k pages         |
| `/answer`   | 5 $ / 1k requests                            | —                             | —                      |
| `/monitors` | 15 $ / 1k requests                           | 1 $ / 1k résultats            | 1 $ / 1k pages         |
...
Les runs mesurés facturent l&#39;utilisation réelle, dans la limite de leur plafond par run. Le plafond par défaut est de 5 $ pour `auto` et de 20 $ pour le mode bêta `max` :
...
[tarification de Connect](/fr/docs/agent/connect/overview#pricing) pour
connaître les tarifs de tous les providers.

## Deep Search {#deep-search}

Se définit avec `type` sur [`/search`](/fr/docs/search/deep-search). Les résultats supplémentaires et les résumés de page par IA coûtent le même prix qu&#39;une recherche standard.
...
<Columns cols={3}>
  <Card title="Recherche puissante" icon="gauge">
    Jusqu&#39;à 1 000 résultats par recherche, requests au-delà de 25 résultats, limites de débit (QPS) personnalisées, modération sur mesure et index personnalisés.
  </Card>

  <Card title="Support Enterprise" icon="headphones">
    SLA et MSA, onboarding et support individuels, et [Zero Data Retention](/fr/docs/admin/security/zero-data-retention).
  </Card>

  <Card title="Tarification personnalisée" icon="tag">
    Remises sur volume et facturation sur facture postpayée.
  </Card>
</Columns>

<Card title="Parlons-en" icon="mail" horizontal href="https://exa.ai/contact/sales">
...
<AccordionGroup>
  ...
  <Accordion title="Agent Compute Unit (ACU)">
    L&#39;unité de calcul de modèle consommée par un run d&#39;Agent, indiquée dans `usage.agentComputeUnits`. Les runs plus longs, des `input.data` plus volumineux et davantage d&#39;étapes de raisonnement consomment plus d&#39;ACU.
  </Accordion>
