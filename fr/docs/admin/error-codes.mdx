...
description: "Référence des codes d'erreur courants utilisés par l'API Exa"
---

Les API Exa signalent les échecs par des codes de statut HTTP standard et un corps JSON.
...
| `422` Unprocessable Entity  | Une query de prévisualisation Websets n&#39;a pas pu être décomposée en une entité et des criteria valides.                                 | Reformulez la query de prévisualisation.                                                                                                                 |
| `429` Too Many Requests     | Votre API key, votre team ou votre réseau a dépassé une limite de débit ou de concurrency.                                                  | Réduisez votre taux de requêtes ; attendez le nombre de secondes indiqué par `Retry-After` lorsqu&#39;il est présent, sinon utilisez un backoff exponentiel. |
...
| `503` Service Unavailable   | Exa est temporairement en surcapacité (`SERVICE_OVERLOADED`) ou indisponible. La requête n&#39;a pas été traitée et n&#39;est pas facturée. | Réessayez avec un backoff exponentiel. Ce cas est indépendant de votre taux de requêtes : le réduire ne sert à rien — seul le fait de réessayer est utile.   |
| `504` Gateway Timeout       | La requête a dépassé son délai de traitement.                                                                                               | Réessayez la requête ou réduisez son scope.                                                                                                              |

<Note>
  Les échecs au niveau d&#39;une URL provenant de `/contents` sont signalés dans le field `statuses` d&#39;une réponse `200` réussie, et non comme des erreurs au niveau de la requête. Voir [Content fetch status tags](#content-fetch-status-tags).
</Note>
...
L&#39;ensemble des tags est extensible et les noms de tags sont explicites. Effectuez d&#39;abord un branchement sur le code de statut HTTP, et traitez les tags non reconnus comme des informations complémentaires plutôt que comme des erreurs d&#39;analyse.

<div id="common-error-tags">
  ## Tag d&#39;erreur courantes
</div>
...
| `RATE_LIMIT_EXCEEDED`     | `429`     | Votre API key, votre team ou votre réseau a dépassé son propre rate limit — réduisez votre taux de requêtes.               |
| `SERVICE_OVERLOADED`      | `503`     | Exa dépasse temporairement sa capacité et a écarté la requête avant de la traiter — réessayez avec un backoff exponentiel. |
...
| `X402_WALLET_RATE_LIMITED` | `429`     | Le wallet x402 a dépassé sa rate limit.                    |
...
Lorsque `/contents` reçoit plusieurs URL, l&#39;une d&#39;elles peut échouer alors que les autres aboutissent. Les échecs au niveau d&#39;une URL sont renvoyés dans le champ `statuses` et n&#39;entraînent pas l&#39;échec de la requête :
...
| `CRAWL_HTTP_{status}`     | La cible a renvoyé une erreur HTTP, par exemple `CRAWL_HTTP_403`.                 | Traitez le statut cible intégré au tag.                             |
...
Ces status tags sont propres à `/contents` ; `/search` ne renvoie pas de field `statuses`.

<div id="getting-help">
  ## Obtenir de l&#39;aide
</div>

* Consultez [Exa Status](/fr/docs/admin/status) si des erreurs `500`, `503` ou `504` persistent.
* Consultez les [limites de débit](/fr/docs/admin/billing#rate-limits) pour connaître les limites en vigueur.
* Consultez la [référence de l&#39;API](/fr/docs/reference/search) de l&#39;endpoint pour connaître les exigences relatives à la requête.
* Écrivez à [hello@exa.ai](mailto:hello@exa.ai) en indiquant le statut de la réponse, le corps de l&#39;erreur et le `requestId`.