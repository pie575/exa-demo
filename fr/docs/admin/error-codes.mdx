...
| `403` Forbidden             | L&#39;API key n&#39;a pas accès à la fonctionnalité demandée, ou la requête a été bloquée par une règle.                                    | Consultez le message renvoyé et les fonctionnalités accessibles selon votre plan.                                                                          |
...
| `503` Service Unavailable   | Exa est temporairement en surcapacité (`SERVICE_OVERLOADED`) ou indisponible. La requête n&#39;a pas été traitée et n&#39;est pas facturée. | Réessayez avec un backoff exponentiel. Ce cas est indépendant de votre cadence de requêtes : la réduire ne sert à rien — seul un nouvel essai fonctionne.  |
...
L&#39;ensemble des tags est extensible et les noms de tag sont explicites. Branchez d&#39;abord sur le code de statut HTTP et traitez les tags non reconnus comme une information complémentaire plutôt que comme une erreur d&#39;analyse.
...
| `SERVICE_OVERLOADED`      | `503`     | Exa est temporairement saturé et a écarté la requête avant de la traiter — réessayez avec un backoff exponentiel.     |
...
| `INVALID_JSON_SCHEMA`     | `400`     | Le schéma d&#39;output fourni est invalide.                                                                    |
...
| `X402_INTERNAL_ERROR`      | `500`     | Exa n&#39;a pas pu créer les exigences de paiement x402. |

<div id="content-fetch-status-tags">
  ## Tags de statut de récupération du contenu
</div>

Lorsque `/contents` reçoit plusieurs URL, une URL peut échouer tandis que les autres aboutissent. Les échecs au niveau d&#39;une URL sont renvoyés dans le field `statuses` et ne font pas échouer la requête :
...
`httpStatusCode` décrit la page cible, pas la réponse de `/contents`.
...
Ces tags de statut sont propres à `/contents` ; `/search` ne renvoie pas de field `statuses`.
...
* Consultez la [référence d&#39;API](/fr/docs/reference/search) de l&#39;endpoint pour connaître les prérequis de la requête.
