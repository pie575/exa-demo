...
[Pydantic AI](https://pydantic.dev/docs/ai/) est un framework d&#39;agents Python développé par l&#39;équipe à l&#39;origine de Pydantic. Son [harness](https://pydantic.dev/docs/ai/harness/exa-search/) embarque une intégration Exa officielle sous la forme de deux capacités composables :

* **`ExaSearch`** : des outils de recherche web adossés à l&#39;Exa Search API : `web_search` (les meilleurs résultats accompagnés de leurs extraits les plus pertinents, ainsi qu&#39;un résumé textuel synthétisé en option), `get_page` (retrieval de la page complète pour une URL donnée) et `deep_search` en option (une réponse synthétisée et sourcée en un seul appel).
* **`ExaAgent`** : délègue la recherche de longue durée à l&#39;[API Exa Agent](/fr/docs/agent/quickstart) sous forme de deferred tool calls.

Une capacité regroupe les outils, les budgets d&#39;output par outil et de brèves consignes de recherche dans le system prompt : vous n&#39;avez donc pas à relier vous-même une API de recherche à un récupérateur de pages, ni à demander à l&#39;agent de mener ses recherches de façon méthodique.

<Info> Consultez la référence complète de Pydantic [ici](https://pydantic.dev/docs/ai/harness/exa-search/). </Info>

<Card title="Lire l'article de Pydantic sur la création d'un agent de recherche avec Exa" icon="https://mintcdn.com/exa-52/Una64IRjof2yadw_/images/integrations/pydantic-ai/logo.svg?fit=max&auto=format&n=Una64IRjof2yadw_&q=85&s=aee1bf45859bf6a3debf4177d0aefb3f" horizontal href="https://pydantic.dev/articles/harness-exa" width="120" height="120" data-path="images/integrations/pydantic-ai/logo.svg">
...
<Steps>
  <Step title="Prérequis et installation">
    Installez le harness avec l&#39;extra Exa et définissez votre variable d&#39;environnement `EXA_API_KEY`.
  ...
  <Step title="Ajouter ExaSearch à un agent">
    ...
    | Outil        | Utilité                                                                                                                                 |
    | ------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
    | `web_search` | Recherche sur le web et renvoie les `num_results` premières pages, chacune avec son titre, son URL et ses extraits les plus pertinents. |
    ...
    `web_search` renvoie de courts extraits (highlights Exa) plutôt que le texte intégral de la page, ce qui permet de passer en revue plusieurs sources à moindre coût ; l&#39;agent lit ensuite la page de son choix avec `get_page`. Une URL ou une question qui ne renvoie aucun contenu, une limite de débit ou une défaillance transitoire est remontée au modèle sous forme de `ModelRetry`, ce qui permet au run de se rétablir ; les échecs d&#39;authentification (401/403) sont propagés comme des erreurs de configuration.
  </Step>

  <Step title="Activer la deep search (facultatif)">
    `deep_search` exécute la [deep search](/fr/docs/search/quickstart) multi-étapes d&#39;Exa (`type='deep'`) : Exa décline la question en plusieurs requêtes, lance les recherches et renvoie une réponse fondée sur des preuves avec citations en un seul appel d&#39;outil. Comme elle demande plus de temps et de profondeur de recherche que `web_search`, elle est désactivée par défaut. Activez-la explicitement :
    ...
    Lorsqu&#39;elle est activée, les instructions de la capacité indiquent au modèle de traiter `deep_search` comme une escalade par rapport à `web_search`, et non comme un remplacement.
  </Step>
</Steps>
...
ExaSearch(
    ...
    max_text_chars=10_000,     # limite de texte get_page, en caractères (1 à 10 000)
...
Par défaut (`execution='inline'`), la capacité résout elle-même ses appels différés au sein de l&#39;agent run en interrogeant le run Exa jusqu&#39;à son achèvement, si bien que l&#39;outil se comporte comme un outil classique (quoique lent). Avec `execution='external'`, les appels remontent sous forme d&#39;output `DeferredToolRequests`, que l&#39;application hôte se charge de résoudre hors bande.
...
ExaAgent(
    ...
    execution='inline',   # 'inline' effectue le polling jusqu'à la fin ; 'external' remonte DeferredToolRequests
    ...
    poll_interval=1000,   # ms entre chaque poll en résolution inline
    timeout_ms=3_600_000, # ms d'attente d'un run en résolution inline
    guidance=None,        # None = instructions par défaut, '' = aucune, str = personnalisées
...
Passez `custom_capability_types` pour que le chargeur de spécifications sache comment instancier les capacités. Les instances chargées depuis une spécification construisent toujours le client par défaut à partir de `EXA_API_KEY`.
...
* [**Configuration MCP**](/fr/docs/get-started/exa-mcp) - Le MCP server hébergé d&#39;Exa
