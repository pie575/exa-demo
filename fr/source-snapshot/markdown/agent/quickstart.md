...
Exa Agent est un endpoint asynchrone, facturé à l&#39;utilisation, conçu pour les tâches à forte charge de calcul comme la constitution de listes, l&#39;enrichment et la deep research. Il gère les raisonnements complexes et peut renvoyer de nombreux fields de sortie structurée.

Voyez-le comme un agent de context : vous décrivez les données que vous souhaitez et la forme sous laquelle elles doivent vous parvenir, et Exa Agent orchestre les appels d&#39;outils nécessaires pour y arriver. Un seul run peut lancer de nombreuses recherches sous différents angles, lire et condenser les pages correspondantes, découper la constitution de listes en sous-tâches exécutées en parallèle, vérifier chaque candidat au regard de vos critères, enrichir des contacts et interroger les data partners [Exa Connect](/fr/docs/agent/connect/overview) que vous attachez. Vous récupérez le context assemblé sous la forme d&#39;un unique résultat structuré et fondé sur des preuves, sans avoir à orchestrer vous-même chaque appel `/search` et `/contents`.

Chaque run peut renvoyer une réponse en langage naturel, du JSON validé par schéma, un grounding au niveau des fields, des métadonnées et une ventilation des coûts. Vous pouvez récupérer ultérieurement les runs terminés, lister les runs passés, rejouer les événements ou reprendre à partir d&#39;un run précédent.
...
* Exécuter des tâches multi-étapes du type « trouver des entreprises, puis trouver leurs décideurs »
...
<Note>
  ...
  Pointez le SDK OpenAI vers `https://api.exa.ai`, utilisez `model: "exa-agent"`, puis choisissez
  une exécution synchrone, en streaming ou en background. Voir [Compatibilité du SDK
...
<Note>
  ...
  échec d&#39;inspection (`cannot_verify`) d&#39;une véritable preuve négative
...
| `agent_run.started`   | `{ id, status: "running" }`           | Marquez le run comme en cours de traitement.                                                                                |
...
Les événements associés à une même étape de recherche comportent un `callId`, qui correspond à `item.call_id` dans les événements de progression des outils. Utilisez-le pour regrouper les traces de search, les sources et la progression des outils. Certaines descriptions de traces de search sont générées de façon asynchrone et peuvent arriver après la source ou l&#39;événement d&#39;outil qu&#39;elles décrivent : ne les corrélez donc pas uniquement selon leur ordre d&#39;arrivée.
...
L&#39;endpoint de replay envoie les événements stockés au moment de la requête, puis se ferme ; il ne continue pas à suivre un run en cours. Les runs ZDR ne conservent pas les événements et ne peuvent pas être rejoués.
...
Pour demander des informations de contact, décrivez les fields de contact souhaités dans `outputSchema`. Utilisez les structures JSON Schema standard, par exemple `{ "type": "string", "format": "email" }` pour les adresses e-mail, `{ "type": "string", "format": "phone" }` pour les numéros de téléphone et `{ "type": "string", "format": "uri" }` pour les URL. Limitez autant que possible la taille des listes avec `maxItems`, afin que le coût maximal de l&#39;enrichissement de contact reste prévisible.
...
<Note>
  L&#39;enrichissement de contact est distinct des composants de tarification ci-dessus : l&#39;enrichissement de contact par e-mail coûte `$0.02 / email`, et l&#39;enrichissement de contact par numéro de téléphone coûte `$0.07 / phone number`.
</Note>

`usage.agentComputeUnits` mesure le calcul du modèle sur l&#39;ensemble du run. Les requêtes complexes, en particulier celles dont le field `input.data` est volumineux, nécessitent davantage d&#39;étapes de raisonnement et d&#39;appels d&#39;outil, et consomment donc plus d&#39;ACU.
...
| --------- | ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------ |
| `minimal` | Consultations au coût le plus bas, tâches factuelles très ciblées, réponses courtes | Un ou deux fields, schéma peu profond                                           | Le moins cher, le moins exhaustif    |
| `low`     | Consultations simples, tâches factuelles ciblées, réponses courtes                  | Quelques fields, schéma peu profond                                             | Rapide, recherche légère             |
| `medium`  | Point de départ par défaut pour la plupart des tâches de recherche standard         | Nombre de fields modéré, objets imbriqués simples                               | Équilibre qualité/durée              |
| `high`    | Recherche plus difficile, davantage de citations, exhaustivité plus stricte         | Schémas plus vastes ou fields plus nuancés                                      | Plus lent, plus approfondi           |
| `xhigh`   | Tâches à forte valeur où l&#39;exhaustivité prime sur le coût et la latence         | Schémas complexes, nombreux fields, vérification difficile                      | L&#39;effort fixe le plus lent       |
| `auto`    | Tâches à portée variable, constitution de listes, difficulté inconnue               | Flexible ; utile quand le nombre d&#39;entités ou le travail requis est inconnu | Variable                             |
| `max`     | Recherche à effort maximal (bêta)                                                   | Schémas complexes, nombreux fields, vérification difficile                      | La plus longue durée d&#39;exécution |

Commencez par `medium` pour de la recherche standard sur une seule entité. Passez à `low` ou `minimal` lorsque le coût et la latence comptent davantage que l&#39;exhaustivité. Montez à `high` ou `xhigh` lorsque le schéma de sortie est plus vaste, que les fields nécessitent une vérification ou que la tâche demande un raisonnement plus poussé. Utilisez `auto` lorsque vous ne connaissez pas la portée à l&#39;avance, par exemple pour la constitution de listes ou des workflows susceptibles de renvoyer de nombreuses entités.
...
* Créez des runs en streaming (`Accept: text/event-stream`) pour consommer l&#39;output en direct, ou interrogez les runs asynchrones pendant la fenêtre de rétention.
