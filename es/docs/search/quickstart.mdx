...
description: "Busca en la web en lenguaje natural y obtén contenido de páginas limpio y relevante en una sola solicitud."
keywords: ["search api", "web search", "semantic search", "retrieval", "rag", "highlights"]
boost: 6
---

Exa Search recibe una query en lenguaje natural y devuelve resultados web ordenados por relevancia con contenido de página limpio.
...
Search devuelve hasta 10 resultados de forma predeterminada. Define `numResults` para solicitar hasta 100 resultados; puede que se devuelvan menos si hay menos páginas relevantes disponibles. Search no admite paginación.
...
Los resultados se ordenan por relevancia. Cada uno incluye metadatos como título, URL y fecha de publicación, además de lo que hayas solicitado en `contents`.

<div id="writing-queries">
  ## Cómo escribir queries
</div>

El campo `query` es el único obligatorio al usar la Search API.

Escribe las queries en lenguaje natural. Incluye el tema y, cuando resulte útil, el tipo de fuente y el periodo de tiempo que te interesa.

Las queries pueden ser amplias y exploratorias. `"Latest news on EU battery policy"` le da a Exa suficiente contexto sobre tu intención para encontrar páginas relevantes; `"news"` no. Cuando el tipo de fuente sea importante, indícalo en la query:
...
Los modos deep ejecutan un proceso de investigación en lugar de una única pasada de recuperación. Consulta [Deep Search](/es/docs/search/deep-search) para ver cómo funciona ese proceso y cómo usar sus controles adicionales.

<Tip>
  En lugar de `deep-reasoning`, usa [Exa Agent](/es/docs/agent/quickstart) para investigaciones de larga duración, creación
  de listas y enrichment de varios saltos. Agent dispone de más cómputo por ejecución y devuelve resultados
...
Cada resultado incluye metadatos como el título, la URL y la fecha de publicación. Usa `contents` para añadir highlights, el texto completo o un summary de la página.
...
<Tabs>
  <Tab title="Highlights">
    ...
    Empieza con un simple `highlights: true`. Exa usa la query para seleccionar una cantidad adecuada de
...
<Warning>
  ...
  * **`/search`** anida `highlights`, `text` y `summary` dentro del objeto `contents`:
    `"contents": { "highlights": true }`
  * **`/contents`** no tiene el envoltorio `contents`. Su cuerpo son las propias opciones de contenido, así que esos
...
Agrega `outputSchema` cuando quieras que Exa sintetice los resultados de la búsqueda. Funciona con todos los tipos de búsqueda y añade un objeto `output` a la respuesta.

Las páginas clasificadas permanecen en `results`. El valor generado se devuelve en `output.content`, con las fuentes a nivel de campo y la confianza en `output.grounding`.
...
<Note>
  Mantén los esquemas de objetos pequeños: admiten hasta 2 niveles de anidamiento y 10 propiedades. No agregues
...
Los filtros son restricciones estrictas: añade uno cuando un resultado que quede fuera te resultaría inservible, y expresa las preferencias de fuentes más flexibles en el texto de la query. La [referencia de la API](/es/docs/reference/search) incluye el conjunto completo.
...
`excludeDomains` elimina los resultados de dominios o rutas específicos. Admite los mismos prefijos de ruta y comodines de subdominio que `includeDomains`. Úsalo cuando esas fuentes vuelvan inservible un resultado, no para expresar una preferencia.
...
La mayoría de las búsquedas deberían omitir este campo. Configúralo cuando el contenido desactualizado de una página resulte inservible, como en precios, disponibilidad o páginas que cambian con frecuencia.
...
<Columns cols={2}>
  ...
  <Card title="Referencia de la API" icon="square-terminal" href="/es/docs/reference/search" cta="Abrir la referencia" arrow="true">
    Todos los parámetros de solicitud y campos de respuesta, con un entorno de pruebas interactivo.
  </Card>

  <Card title="Contents" icon="file-text" href="/es/docs/contents/quickstart" cta="Abrir la guía" arrow="true">
    Ya tienes las URLs y quieres texto limpio, highlights o resúmenes.
  </Card>
