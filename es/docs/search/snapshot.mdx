...
Exa detecta las URL candidatas y luego conserva solo las páginas que tengan una versión almacenada en `snapshotAsOf` o antes de esa fecha.
...
Añade `snapshotAsOf` en el nivel superior de una solicitud a `/contents`.
...
Exa devuelve la versión almacenada más reciente en esa fecha y hora o anterior.
...
<Tip>
  Los ID que no tengan ninguna versión válida se omiten de `results` y se reportan en `statuses` con
...
<Note>
  En Search, la fecha de corte limita el contenido, no el ranking. Exa sigue usando señales de recuperación
  actuales para descubrir URLs candidatas. Usa los resultados como evidencia limitada por `snapshotAsOf`, y no como una
...
<AccordionGroup>
  <Accordion title="Acceso, límite de tasa y ventana histórica">
    El plan de pago por uso incluye 10 QPS y acceso a una ventana móvil de 5 meses del índice. Se rechaza cualquier `snapshotAsOf`
    anterior a esa ventana. Después de 100 solicitudes, [habla con el equipo de ventas](https://exa.ai/contact/sales)
    para continuar.
  </Accordion>
  ...
  <Accordion title="Las solicitudes históricas usan contenido almacenado">
    ...
    alguno de ellos junto con `snapshotAsOf` se rechazan con `INVALID_REQUEST`.
  </Accordion>
