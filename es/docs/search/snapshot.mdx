...
description: "Fija Search y Contents a una versión almacenada de una página en la fecha y hora que elijas."
keywords: ["exa snapshot", "snapshotAsOf", "contenido histórico", "búsqueda versionada"]
---
...
<Info>
  Exa Snapshot está disponible en pago por uso a 10 QPS, con una ventana de índice móvil de 5 meses.
  Después de 100 solicitudes, [habla con ventas](https://exa.ai/contact/sales) para continuar.
</Info>
...
Exa detecta las URL candidatas y luego conserva únicamente las páginas que tengan una versión almacenada en `snapshotAsOf` o antes de esa fecha.
...
Exa devuelve la versión almacenada más reciente en esa fecha y hora o anterior a ella.
...
<Tip>
  Los ID sin ninguna versión elegible se omiten de `results` y se informan en `statuses` con
...
| -------------- | --------- | ----------------------------------------------------------------------------------------------------- |
| `snapshotAsOf` | Solicitud | Fecha y hora límite. Exa devuelve la versión almacenada más reciente en ese instante o anterior a él. |

Para ambos endpoints:

* El contenido de la página devuelto proviene de esa versión almacenada.
...
<Note>
  En Search, el límite acota el contenido, no el ranking. Exa sigue usando señales de recuperación
  actuales para descubrir URL candidatas. Usa los resultados como evidencia acotada por `snapshotAsOf`,
  no como una reconstrucción exacta de lo que una búsqueda habría posicionado en ese momento.
</Note>
...
<AccordionGroup>
  <Accordion title="Acceso, límite de tasa y ventana histórica">
    El pago por uso incluye 10 QPS y acceso al índice de los últimos 5 meses en ventana móvil. Un `snapshotAsOf` anterior
    a esa ventana se rechaza. Después de 100 solicitudes, [habla con el equipo de ventas](https://exa.ai/contact/sales)
    para continuar.
  </Accordion>
