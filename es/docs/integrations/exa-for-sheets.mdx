...
<Warning>
  **Varias cuentas de Google:** el add-on debe ejecutarse con la primera cuenta de Google (la predeterminada) del perfil de tu navegador. Si tienes la sesión iniciada en varias cuentas, es posible que no puedas guardar ni cargar tu API key. Para solucionarlo, abre Sheets en una ventana de incógnito con una sola cuenta, o cierra la sesión en las cuentas adicionales para que la cuenta que quieres usar sea la predeterminada. [Más información](https://developers.google.com/apps-script/guides/projects#fix_issues_with_multiple_google_accounts).
</Warning>
...
* **Exa Agent** para tablas completas y tareas en varias celdas
...
<Steps>
  ...
  <Step title="Abre una Google Sheet">
...
* generar una tabla completa a partir de un prompt
* completar celdas vacías en una tabla existente
...
Selecciona las celdas vacías de Apple y haz clic en **Fill selected cells**. Exa usa el nombre de la empresa y las filas cercanas como contexto.
...
Si tu tabla termina en la posición 55 y seleccionas las dos filas en blanco siguientes, Exa puede continuarla con las posiciones 56 y 57.

Exa toma las filas existentes como ejemplos, mantiene las mismas columnas y evita repetir elementos que ya están en la tabla.

<div id="exa">
  ## `=EXA(...)`
</div>

Usa `=EXA(...)` cuando quieras una única respuesta en una sola celda. Busca en la web, lee los primeros resultados y devuelve una respuesta concisa.
...
| `context` | No          | Referencia de celda o texto que se va a enriquecer (por ejemplo, un nombre de empresa en `A2`). |
...
Respuestas avanzadas con IA y control total sobre el formato del output. Úsala cuando necesites prompts de sistema, salida JSON estructurada, citas o un tipo de búsqueda específico.
...
| ------------------ | ----------- | ----------------- | ------------------------------------------------------------------------------------------------------------- |
| `prompt`           | Sí          | —                    | La pregunta o prompt principal.                                                                               |
...
Busca en la web y devuelve una lista vertical de URL. Admite filtrado por dominio, filtrado por categoría, highlights de contenido y output sintetizado mediante `outputSchema`.
...
| `includeDomainsStr`  | No          | `""`              | Dominios que se deben incluir, separados por comas (p. ej., `"linkedin.com,crunchbase.com"`).                                                                           |
| `excludeDomainsStr`  | No          | `""`              | Dominios que se deben excluir, separados por comas.                                                                                                                     |
...
| `outputSchemaJson`   | No          | `""`              | Cadena JSON para `outputSchema` (p. ej., `"{""type"":""text"",""description"":""summarize""}"`). Si se define, devuelve el texto de output sintetizado en lugar de URL. |
...
* Las solicitudes a la API de Exa se descuentan de tu cuota de uso. Usa **Batch → Convertir a valores** para congelar los resultados e impedir que las fórmulas se recalculen.
