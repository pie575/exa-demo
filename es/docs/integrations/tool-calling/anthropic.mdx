---
title: "Llamadas a herramientas con Anthropic"
description: "Aprovecha el uso de herramientas de Claude para añadir Exa web search y contenido de páginas a tu aplicación."
...
El [uso de herramientas](https://docs.anthropic.com/en/docs/build-with-claude/tool-use) de Claude permite que los modelos llamen a funciones que defines en tu código. Los SDK de Exa incluyen herramientas listas para usar de búsqueda web y lectura de páginas para Anthropic, así que no necesitas escribir a mano el esquema de la herramienta, analizar los bloques `tool_use` ni dar formato a los resultados de Exa.
...
<Steps>
  ...
  <Step title="Añade las herramientas de Exa a tu bucle de herramientas">
    ...
    `web_search` busca en la web páginas que el modelo no ha visto; `get_contents` lee páginas cuya URL ya conoce, ya provenga de una búsqueda anterior o del usuario. Registra una o ambas.
...
Los argumentos con nombre son opciones habituales de Exa que se pasan cuando se ejecuta la herramienta: las opciones de búsqueda a `exa.search()` y las de contenido a `exa.get_contents()`:
...
## Combinar tus propias herramientas {#mixing-in-your-own-tools}

`handle_tool_use` responde a todos los bloques `tool_use` del mensaje: si un bloque nombra una herramienta que no puede resolver, devuelve un resultado `Error: unknown tool "<name>"` en lugar de descartarlo, de modo que la solicitud de follow-up nunca omite un resultado de herramienta obligatorio. Si ejecutas tus propias herramientas junto a las de Exa, sustituye esos resultados de error por los tuyos antes de la siguiente solicitud.
