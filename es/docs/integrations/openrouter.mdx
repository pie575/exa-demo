...
description: "Fundamenta cualquier modelo de OpenRouter con Exa web search mediante la herramienta de servidor openrouter:web_search."
icon: "/docs/images/sidebar-icons/openrouter.svg"
keywords: ["OpenRouter", "web search", "server tool", "grounding", "url_citation"]
---

Exa es el motor de búsqueda detrás de la búsqueda web de [OpenRouter](https://openrouter.ai). OpenRouter te da una sola API para cientos de modelos, y Exa le da a esos modelos acceso web en tiempo real: cualquier modelo sin búsqueda nativa se fundamenta con Exa de forma predeterminada, y cualquier modelo puede dirigirse a Exa de manera explícita. No hace falta una API key de Exa. OpenRouter ejecuta las búsquedas del lado del servidor y las cobra a tus créditos de OpenRouter.

## Usar la herramienta de servidor de búsqueda web {#use-the-web-search-server-tool}

Agrega `openrouter:web_search` a tu arreglo `tools` y el modelo decidirá cuándo buscar, qué buscar y si debe volver a buscar dentro de la misma solicitud. Las [herramientas de servidor](https://openrouter.ai/docs/guides/features/server-tools/web-search) están en beta en OpenRouter y sustituyen al plugin `web` deprecado y a las variantes de modelo `:online`; consulta la [guía de migración](https://openrouter.ai/docs/guides/features/server-tools/web-search#migrating-from-the-web-search-plugin) de OpenRouter si usas alguno de ellos.
...
Con el valor por defecto `engine: "auto"`, OpenRouter usa la búsqueda nativa del proveedor cuando el modelo dispone de una, y Exa en todos los demás casos. Define `engine: "exa"` para mantener un mismo comportamiento de búsqueda en todos los modelos:
...
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mode`                                | Sacrificar latencia a cambio de profundidad: `instant`, `fast`, `auto` (predeterminado), `deep-lite`, `deep` o `deep-reasoning`. Los modos se corresponden con los [tipos de búsqueda](/es/docs/search/quickstart) de Exa. |
...
OpenRouter solicita [highlights de Exa](/es/docs/search/highlights) para cada resultado en lugar del texto completo de la página: extractos de tamaño adaptativo, normalmente de 2.000 a 4.000 caracteres por resultado, salvo que configures `max_characters` o `search_context_size`. El modelo lee los extractos y quienes llaman a la API los reciben en anotaciones `url_citation` estandarizadas dentro del mensaje de respuesta. Dentro de un mismo resultado, los marcadores `[...]` separan extractos tomados de distintas partes de la página.
...
<Columns cols={2}>
  <Card title="Documentación de herramientas de servidor" icon="wrench" href="https://openrouter.ai/docs/guides/features/server-tools/web-search" cta="Abrir documentación" arrow="true">
  ...
  <Card title="Caso de cliente" icon="book-open" href="https://exa.ai/customers/openrouter" cta="Leer el caso" arrow="true">
    Cómo OpenRouter dota de búsqueda web a cientos de modelos con Exa.
  </Card>
</Columns>