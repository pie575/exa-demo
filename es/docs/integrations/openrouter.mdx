...
description: "Fundamenta cualquier modelo de OpenRouter con la búsqueda web de Exa mediante la herramienta de servidor openrouter:web_search."
icon: "/docs/images/sidebar-icons/openrouter.svg"
keywords: ["OpenRouter", "búsqueda web", "herramienta de servidor", "grounding", "url_citation"]
---

Exa es el motor que impulsa la búsqueda web de [OpenRouter](https://openrouter.ai). OpenRouter te ofrece una sola API para cientos de modelos, y Exa les da a esos modelos acceso a la web en tiempo real: cualquier modelo sin búsqueda nativa se fundamenta a través de Exa de forma predeterminada, y cualquier modelo puede dirigirse a Exa de forma explícita. No hace falta ninguna API key de Exa. OpenRouter ejecuta las búsquedas del lado del servidor y las cobra a tus créditos de OpenRouter.

<div id="use-the-web-search-server-tool">
  ## Usar la herramienta de servidor de búsqueda web
</div>

Agrega `openrouter:web_search` al arreglo `tools` y el modelo decidirá cuándo buscar, qué buscar y si debe volver a buscar dentro de la misma solicitud. Las [herramientas de servidor](https://openrouter.ai/docs/guides/features/server-tools/web-search) están en beta en OpenRouter y reemplazan al complemento `web` y a las variantes de modelo `:online`, ambos obsoletos; si usas alguno de ellos, consulta la [guía de migración](https://openrouter.ai/docs/guides/features/server-tools/web-search#migrating-from-the-web-search-plugin) de OpenRouter.
...
OpenRouter solicita [highlights de Exa](/es/docs/search/highlights) para cada resultado en lugar del texto completo de la página: extractos de tamaño adaptativo, normalmente de 2.000 a 4.000 caracteres por resultado, salvo que definas `max_characters` o `search_context_size`. El modelo lee los extractos, y las aplicaciones que llaman a la API los reciben en anotaciones `url_citation` estandarizadas dentro del mensaje de respuesta. Dentro de un mismo resultado, los marcadores `[...]` separan extractos tomados de distintas partes de la página.

<div id="pricing">
  ## Precios
</div>

Las búsquedas de Exa se cobran a tus créditos de OpenRouter, además del costo de los tokens que el modelo consume al leer los resultados. Los modos `instant`, `fast` y `auto` cuestan $0.007 por búsqueda, `deep-lite` y `deep` cuestan $0.012, y `deep-reasoning` cuesta $0.015. Cada búsqueda incluye hasta 10 resultados, y cada resultado adicional cuesta $0.001. Consulta la [documentación de búsqueda web de OpenRouter](https://openrouter.ai/docs/guides/features/server-tools/web-search) para ver las tarifas vigentes.
...
<Columns cols={2}>
  <Card title="Documentación de herramientas de servidor" icon="wrench" href="https://openrouter.ai/docs/guides/features/server-tools/web-search" cta="Abrir documentación" arrow="true">
    Referencia de configuración completa de `openrouter:web_search`.
  </Card>
