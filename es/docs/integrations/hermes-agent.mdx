...
Añade tu key al archivo de entorno de Hermes:
...
La configuración específica de cada capacidad tiene prioridad sobre `web.backend`. Esto te permite usar Exa solo para búsqueda o solo para extracción cuando combinas proveedores.
...
Hermes trunca las páginas extensas que extrae según el presupuesto de caracteres configurado y guarda el texto completo en disco. Cambia el valor predeterminado con `web.extract_char_limit` o deja que el agente solicite un `char_limit` mayor para una llamada concreta.

<Note>
  Hermes puede usar Exa mediante su grupo de proveedores gratuitos sin credenciales, es decir, sin una API key. Ese grupo tiene límites de tasa y puede rotar entre proveedores. Configura `EXA_API_KEY` y selecciona la opción de Exa respaldada por API key cuando necesites que las solicitudes usen siempre tu cuenta de Exa.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Hermes indica que falta EXA_API_KEY">
    Agrega la key a `~/.hermes/.env` y luego reinicia Hermes para que recargue el entorno.
  </Accordion>
