...
* **`ExaSearch`**: herramientas de investigación web respaldadas por la Search API de Exa: `web_search` (los mejores resultados con sus extractos más relevantes, además de un resumen de texto sintetizado opcional), `get_page` (recuperación de la página completa de una URL concreta) y `deep_search` opcional (una respuesta sintetizada y con citas en una sola llamada).
* **`ExaAgent`**: delega investigaciones de larga duración a la [API de Exa Agent](/es/docs/agent/quickstart) mediante llamadas a herramientas diferidas.

Una capacidad agrupa las herramientas, los presupuestos de salida por herramienta y breves pautas de investigación en el prompt del sistema, de modo que no tengas que conectar tú mismo una API de búsqueda con un recuperador de páginas ni pedirle al agente que investigue de forma metódica.
...
<Steps>
  <Step title="Requisitos previos e instalación">
    Instala el harness con el extra de Exa y define la variable de entorno `EXA_API_KEY`.
  ...
  <Step title="Añadir ExaSearch a un agente">
    ...
    | ------------ | ------------------------------------------------------------------------------------------------------------------------------- |
    | `web_search` | Busca en la web y devuelve las `num_results` páginas principales, cada una con título, URL y sus fragmentos más relevantes.     |
    ...
    `web_search` devuelve fragmentos breves (highlights de Exa) en lugar del texto completo de la página, de modo que explorar varias fuentes resulta barato; después el agente lee la página elegida con `get_page`. Una URL o pregunta que no devuelve contenido, un límite de tasa o un fallo transitorio se le presentan al modelo como un `ModelRetry`, de modo que la ejecución pueda recuperarse; los fallos de autenticación (401/403) se propagan como errores de configuración.
  </Step>

  <Step title="Habilitar la búsqueda profunda (opcional)">
    `deep_search` ejecuta la [búsqueda profunda](/es/docs/search/quickstart) de varios pasos de Exa (`type='deep'`): Exa amplía la pregunta en varias consultas, busca y devuelve una respuesta respaldada por citas en una sola llamada a la herramienta. Requiere más tiempo y mayor profundidad de búsqueda que `web_search`, por lo que está desactivada de forma predeterminada. Actívala explícitamente:
    ...
    Cuando está habilitada, las instrucciones de la capacidad indican al modelo que trate `deep_search` como una escalada a partir de `web_search`, no como un sustituto.
  </Step>
</Steps>
...
Define `text_summary` para que cada llamada a `web_search` solicite además un resumen sintetizado en texto plano de los resultados. Pasa `True` para obtener un resumen sin restricciones, o una cadena de texto que describa el formato deseado:
...
El cliente predeterminado es `exa_py.AsyncExa`, configurado a partir de `EXA_API_KEY`. Pasa cualquier objeto que cumpla el protocolo `ExaClient` para configurar explícitamente la autenticación o la URL base, o para usar un objeto simulado en las pruebas:
...
<div id="exa-agent-runs">
  ## Ejecuciones del Exa Agent
</div>

La [API de Exa Agent](/es/docs/agent/quickstart) ejecuta tareas de investigación abiertas de forma asíncrona. La capacidad `ExaAgent` adapta ese ciclo de vida a las [llamadas diferidas a herramientas](https://pydantic.dev/docs/ai/deferred-tools/) de Pydantic AI: su herramienta `exa_agent` crea la ejecución y la difiere, llevando el ID de ejecución de Exa en los metadatos de la llamada diferida.
...
De forma predeterminada (`execution='inline'`), la capacidad resuelve sus propias llamadas diferidas dentro de la ejecución del agente sondeando la ejecución de Exa hasta que finaliza, por lo que la herramienta se comporta como una herramienta normal (aunque lenta). Con `execution='external'`, las llamadas se propagan como salida de `DeferredToolRequests` para que la aplicación anfitriona las resuelva por su cuenta.

Todos los campos de `ExaAgent` con su valor predeterminado:

```Python Python theme={null}
from pydantic_ai_harness.exa import ExaAgent

ExaAgent(
    effort=None,          # 'low' | 'medium' | 'high' | 'xhigh' | 'auto' -- None = valor predeterminado de la API
    execution='inline',   # 'inline' sondea hasta que termina; 'external' propaga DeferredToolRequests
    ...
    guidance=None,        # None = instrucciones predeterminadas, '' = ninguna, str = personalizadas
...
Ambas capacidades son compatibles con la [agent spec](https://pydantic.dev/docs/ai/agents/#agent-spec) de Pydantic AI, por lo que puedes declararlas en un archivo de configuración en lugar de en Python:
...
* [**Agent API**](/es/docs/agent/quickstart) - Ejecuciones de investigación asíncronas y abiertas
