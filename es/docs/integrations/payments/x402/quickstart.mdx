...
sidebarTitle: "Pagos con x402"
description: "Usa las APIs de Search y Contents de Exa sin una API key. Paga por solicitud con USDC en Base o Solana mediante el protocolo x402."
...
Los demás endpoints **no** están disponibles mediante x402.

## Cómo funciona {#how-it-works}

<Frame>
  <img src="/docs/images/integrations/payments/x402/payment-flow.png" alt="Diagrama de secuencia del flujo de pago x402: el cliente envía una solicitud al server, recibe un 402 con el encabezado PAYMENT-REQUIRED, crea el payload de pago, reintenta con PAYMENT-SIGNATURE, el server verifica mediante el facilitador, ejecuta el trabajo, liquida on-chain y devuelve un 200 con los resultados y PAYMENT-RESPONSE" width="4224" height="2720" data-path="images/integrations/payments/x402/payment-flow.png" />
</Frame>
...
* Un encabezado `PAYMENT-RESPONSE` con el comprobante de la liquidación (codificado en base64), incluido el hash de la transacción on-chain
...
x402 utiliza el mismo precio por paquetes que la facturación con API key. Los precios se calculan por adelantado en función de los parámetros de tu solicitud (no de los resultados realmente devueltos).
...
<Warning>
  Las solicitudes x402 están limitadas a un **máximo de 10 resultados**. Si solicitas más de 10, `numResults` se ajusta silenciosamente a 10 y el precio se calcula con base en 10 resultados.
</Warning>
...
<Tip>
  ¿No quieres gestionar keys privadas? [Coinbase Agentic Wallets](https://docs.cdp.coinbase.com/agent-kit/core-concepts/wallet-management) ofrece gestión de keys aislada en TEE para agentes de IA. Tu agente nunca ve la key privada. La wallet es compatible con viem, así que funciona directamente con `@x402/fetch`.
</Tip>
...
### Modo de descubrimiento (sin wallet) {#discovery-mode-no-wallet-needed}
...
Exa publica todas las redes admitidas actualmente en el arreglo `accepts`. Elige la entrada que corresponda a tu wallet y al esquema de cliente x402 registrado.
...
Tras 5 solicitudes de descubrimiento `402` sin autenticar desde la misma IP en 60 segundos, las solicitudes siguientes devuelven `429 Too Many Requests`. Realizar una solicitud pagada con éxito decrementa el contador.

Las QPS por wallet se aplican al conjunto de solicitudes pagadas que provienen de la misma dirección de wallet.
...
| ------ | -------------------------- | ----------------------------------------------------------------------------- |
| `402`  | `X402_PAYMENT_REQUIRED`    | No se proporcionó pago. Incluye el precio en el encabezado `PAYMENT-REQUIRED` |
| `402`  | `X402_VERIFICATION_FAILED` | La firma de pago no superó la verificación del facilitador                    |
| `400`  | `X402_INVALID_SIGNATURE`   | Firma de pago mal formada o no interpretable                                  |
...
| `500`  | `X402_INTERNAL_ERROR`      | Error del lado del servidor al generar los requisitos de pago                 |
...
<AccordionGroup>
  ...
  <Accordion title="¿Por qué numResults está limitado a 10?">
    Las solicitudes x402 imponen un máximo de 10 resultados por búsqueda. Si necesitas más, usa el flujo de API key con un plan de pago.
  </Accordion>
...
* [Guía de la Exa Contents API](/es/docs/contents/quickstart): referencia completa de los parámetros de contenido