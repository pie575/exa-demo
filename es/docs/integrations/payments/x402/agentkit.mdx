...
description: "Permite que los agentes de IA respaldados por personas verificadas accedan a Exa gratis con World AgentKit: sin necesidad de USDC."
icon: "/docs/images/sidebar-icons/world.svg"
---

## ¿Qué es AgentKit? {#what-is-agentkit}

[World AgentKit](https://docs.world.org/agents/agent-kit) es un conjunto de herramientas que permite a los agentes de IA demostrar que están respaldados por una persona real y verificada mediante [World ID](https://world.org). Al integrarse con [x402](/es/docs/integrations/payments/x402/quickstart), habilita una vía de **prueba gratuita**: los agentes registrados en el [AgentBook](https://docs.world.org/agents/agent-kit/integrate) de World pueden acceder a los endpoints `/search` y `/contents` de Exa sin pagar USDC.

Esto funciona junto con el flujo de pago x402 estándar. Cada persona verificada dispone de **100 solicitudes gratuitas al mes** repartidas entre todos los agentes que respalda. Una vez agotadas, el agente vuelve a la vía de pago habitual en USDC. Los contadores se reinician al inicio de cada mes natural (UTC).
...
La CLI inicia un flujo de verificación en World App y luego envía una transacción de registro en World Chain. Una vez completado el proceso, cualquier server que use AgentKit puede consultar tu wallet y confirmar que está respaldada por una persona real.

### 2. Envía una solicitud (obtén el desafío) {#2-send-a-request-get-the-challenge}
...
### 3. Firma el desafío y vuelve a enviar la solicitud {#3-sign-the-challenge-and-resubmit}
...
Esta skill gestiona automáticamente todo el flujo cuando el agente recibe una respuesta `402` con una extensión de AgentKit.

## Detalles de la prueba gratuita {#free-trial-details}

* Cada persona verificada dispone de **100 solicitudes gratuitas al mes** entre todos los agentes que respalda
...
* Una vez agotados los usos de prueba gratuita del mes, el agente pasa al [flujo de pago x402](/es/docs/integrations/payments/x402/quickstart) estándar
...
<AccordionGroup>
  <Accordion title="¿Puedo usar x402 pago y AgentKit a la vez?">
    Sí. La respuesta `PAYMENT-REQUIRED` incluye tanto el precio del pago como el desafío de AgentKit. Tu cliente puede optar por cualquiera de las dos vías. Si se agotan los usos de prueba gratuita, el agente puede recurrir al pago con USDC.
  </Accordion>

  <Accordion title="¿Qué ocurre si mi agente no está registrado en AgentBook?">
    La verificación de AgentKit falla de forma silenciosa y la solicitud se trata como un `402` estándar: tu agente puede pagar igualmente con USDC mediante el flujo x402 habitual.
  </Accordion>

  <Accordion title="¿Dos agentes respaldados por la misma persona obtienen cuotas de prueba gratuita distintas?">
  ...
  <Accordion title="¿Qué tipos de wallet son compatibles?">
    Tanto las EOA (cuentas de propiedad externa) que usan firmas EIP-191 como las smart contract wallets (por ejemplo, Coinbase Smart Wallet, Safe) que usan ERC-1271. Consulta la [referencia del SDK de World AgentKit](https://docs.world.org/agents/agent-kit/sdk-reference) para más detalles.
  </Accordion>
</AccordionGroup>

## Recursos {#resources}

* [Guía de pagos con x402](/es/docs/integrations/payments/x402/quickstart): flujo de pago estándar con USDC
