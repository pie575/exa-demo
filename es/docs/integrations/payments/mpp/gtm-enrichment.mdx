...
description: "Crea un flujo de trabajo de enrichment GTM que paga por cada solicitud a Exa search y contenido con Tempo MPP, sin necesidad de API key."
...
Payments Protocol (MPP). MPP admite varios métodos de pago; los ejemplos de
esta guía usan stablecoins en [Tempo](https://tempo.xyz). Sin suscripción mensual, sin
...
Un pipeline de enrichment ligero que, dada una lista de nombres de empresas o
...
2. Usa `contents.highlights` sobre el resultado devuelto para obtener fragmentos
   de las fuentes sobre financiación, sede, empleados y producto.
3. Genera un registro de enrichment en CSV o JSON por cada entrada.

Este patrón sirve para el enrichment de listas de leads, la investigación de
cuentas y la personalización de campañas salientes. Como se compone de llamadas
discretas a `/search` + `/contents`, cada paso se puede pagar con MPP.
...
* Una forma segura de cargar la key privada de la wallet en tiempo de ejecución (ver más abajo; nunca subas
  la key a un repositorio ni la expongas en el código fuente).
* `mppx` (TypeScript) o `pympp` (Python) instalado.

<Info>
  Para una configuración desde la línea de comandos que no requiere una key privada en texto plano, usa la [CLI de Tempo Wallet](/es/docs/integrations/payments/mpp/quickstart#pay-from-the-command-line). `tempo wallet login` crea o conecta una wallet y puede incluir MPP Credits gratuitos para nuevos registros.
</Info>

## Configuración de MPP {#mpp-setup}

### Instalar el cliente {#install-the-client}
...
Nunca escribas una key privada directamente en el código. Los siguientes ejemplos leen `WALLET_PRIVATE_KEY` del entorno de ejecución solo para desarrollo local. En producción, cárgala desde un gestor de secretos como 1Password, AWS Secrets Manager o HashiCorp Vault.
...
<CodeGroup>
  ...
  // En producción, cárgalo desde un secrets manager — nunca subas el valor en texto plano.
...
Una respuesta correcta devuelve los resultados de Exa junto con un encabezado `Payment-Receipt` que contiene
el hash de la transacción on-chain.

### Realizar una solicitud de pago a contenido {#make-a-paid-contents-request}
...
### Enriquecer el perfil de una persona {#enrich-a-person-profile}
...
<Note>
  Aquí se usa `type: "deep"` para obtener un razonamiento más profundo y
  `outputSchema` para definir la forma de la respuesta. La deep search cuesta
  $0.012 por solicitud, y `contents.highlights` añade $0.001 por resultado.
</Note>

### Salida estructurada {#structured-output}

Si quieres campos JSON en lugar de texto plano, usa `outputSchema` en la
solicitud de búsqueda. Exa devuelve un objeto `output` con la forma de tu esquema.
...
MPP usa los mismos precios por solicitud que la facturación con API key. Las solicitudes de búsqueda con MPP
...
* **Financia la wallet solo con USDC.e.** Exa cubre la tarifa de red de Tempo, por lo que
...
* **Guarda en caché los resultados de `/contents`.** El contenido se cobra por URL. Usa la URL como clave de caché para
  evitar pagar dos veces por la misma página de empresa.
* **Ten en cuenta el límite de 10 resultados.** La búsqueda por MPP limita `numResults` a 10.
...
<AccordionGroup>
  ...
  <Accordion title="¿Necesito una wallet de Tempo distinta por entorno?">
    Puedes reutilizar la misma wallet, pero recomendamos usar wallets separadas para
    desarrollo y producción. El QPS por wallet es de 10 solicitudes/segundo entre todas
