---
title: "Snowflake"
description: "Usa Exa search y la recuperación de contenido desde procedimientos almacenados de Snowflake y Cortex Agents."
...
Esta integración usa la función [External Access](https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview) de Snowflake para llamar de forma segura a la API de Exa desde procedimientos almacenados, lo que habilita dos flujos de trabajo principales:

1. **Herramientas de Cortex Agent**: dale a un Cortex Agent de Snowflake la capacidad de buscar en la web y obtener el contenido de las páginas
2. **enrichment de datos**: enriquece tablas por lotes con información de CEO/fundadores, noticias recientes y otros datos de la web
...
<Card title="Obtén tu API key de Exa" icon="key" horizontal href="https://dashboard.exa.ai/api-keys">
  Crea una key en el panel de control. Las cuentas nuevas incluyen créditos gratuitos.
</Card>
...
* Una **regla de red** que solo permite tráfico de salida hacia `api.exa.ai:443`
...
Busca en la web mediante la Search API de Exa. Devuelve resultados con títulos, URLs, puntuaciones y, opcionalmente, texto, highlights o resúmenes.
...
| `include_domains`      | `STRING`  | `NULL`         | Dominios separados por comas a los que limitar la búsqueda                              |
| `exclude_domains`      | `STRING`  | `NULL`         | Dominios separados por comas que se excluirán                                           |
| `start_published_date` | `STRING`  | `NULL`         | Fecha límite inferior en formato ISO 8601                                               |
| `end_published_date`   | `STRING`  | `NULL`         | Fecha límite superior en formato ISO 8601                                               |
...
Recupera el contenido (texto, highlights, resúmenes) de URLs específicas.
...
CALL EXA_INTEGRATION.TOOLS.EXA_SEARCH(
    'últimos avances en energías renovables',
    5,        -- num_results
    'auto',   -- search_type
    'news',   -- category
    NULL, NULL,
...
<div id="get-contents-for-specific-urls">
  ### Obtener contenido de URLs específicas
</div>
...
Exa puede enriquecer tablas de Snowflake con datos web en tiempo real (información de CEO y fundadores, noticias recientes y mucho más) sin salir de la plataforma.
...
<div id="enrich-with-ceofounder-info">
  ### Enriquecer con información del CEO o fundador
</div>
...
También puedes registrar los procedimientos almacenados de Exa como herramientas de un [Cortex Agent de Snowflake](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-agents), lo que le permite buscar en la web de forma conversacional.
...
| Capa                            | Detalle                                                       |
| ------------------------------- | ------------------------------------------------------------- |
| **Roles**                       | `exa_agent_role` para acceso acotado a procedimientos y datos |
...
| **External Access Integration** | Vincula la regla de red con el secreto                        |
...
* [Documentación de Snowflake External Access](https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview)
