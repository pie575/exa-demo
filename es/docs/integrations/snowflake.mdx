...
Esta integración utiliza la funcionalidad [External Access](https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview) de Snowflake para llamar de forma segura a la API de Exa desde procedimientos almacenados, lo que habilita dos flujos de trabajo principales:
...
2. **enrichment de datos**: enriquece tablas con información de CEO/fundadores, noticias recientes y otros datos web en batch
...
Busca en la web mediante la Search API de Exa. Devuelve resultados con títulos, URL, puntuaciones y, opcionalmente, texto/highlights/resúmenes.
...
| `exclude_domains`      | `STRING`  | `NULL`            | Dominios separados por comas que se deben excluir                                       |
...
Exa puede enriquecer tablas de Snowflake con datos web en tiempo real — información del CEO o fundador, noticias recientes y más — todo sin salir de la plataforma.
...
También puedes registrar los procedimientos almacenados de Exa como herramientas de un [Snowflake Cortex Agent](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-agents), lo que le permite buscar en la web de forma conversacional.
