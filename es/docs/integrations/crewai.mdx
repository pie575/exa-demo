---
title: "CrewAI"
description: "Aprende a añadir capacidades de recuperación de Exa a tus agentes de CrewAI."
...
[CrewAI](https://crewai.com/) es un framework para orquestar agentes de IA que colaboran entre sí para completar tareas complejas.
...
2. Configurar agentes y asignarles roles específicos que usen esa herramienta de búsqueda basada en Exa
3. Organizar los agentes en un crew que escribirá un boletín

<Note>
  CrewAI también incluye una herramienta integrada, [`ExaSearchTool`](https://docs.crewai.com/en/tools/search-research/exasearchtool), que puedes usar tal cual sin escribir un wrapper personalizado. La herramienta personalizada que se muestra a continuación resulta útil si quieres control total sobre el formato de los resultados; ambos enfoques funcionan.
</Note>
...
<Steps>
  ...
  <Step title="Configurar un agente de crewAI">
    Importa los módulos de crewAI correspondientes. Luego, define `exa_tools` para que haga referencia al método de búsqueda personalizado que definimos anteriormente.
    ...
    Luego configuramos[ dos agentes](https://docs.crewai.com/concepts/Agents/) y los reunimos en un [crew](https://docs.crewai.com/concepts/Crews/):

    * Uno para investigar con Exa (usando la herramienta personalizada definida arriba)
    * Otro para redactar un boletín como output (usando un LLM)
  ...
  <Step title="Definir tareas para los agentes">
    A continuación, definiremos [tareas](https://docs.crewai.com/concepts/Tasks/) para cada agente y crearemos el crew completo con todos los componentes que configuramos anteriormente.
  ...
  <Step title="Puesta en marcha del equipo">
    ...
    El crew redacta el boletín a partir del contenido devuelto por la herramienta Exa Search.
  </Step>
</Steps>