...
description: "Integra Exa desde tu terminal usando la CLI de Stripe Projects."
...
Tu `.env` ahora contiene una `EXA_API_KEY`. Los [SDKs de Exa](/es/docs/sdks/quickstart) y el [Quickstart](/es/docs/search/quickstart) leen esta variable automáticamente, así que tu código funciona sin necesidad de cambios.

<Info>
  La key se aprovisiona en una cuenta de Exa de tu propiedad. Gestiona el uso, las keys y la facturación cuando quieras desde el [Exa Dashboard](https://dashboard.exa.ai).
</Info>
...
`stripe projects init` escribe un [skill de agente](https://projects.dev) de Stripe Projects en tu proyecto, de modo que puedas dejar que tu agente (Claude Code, Cursor, Codex y otros) ejecute el flujo por ti:
...
* [Quickstart](/es/docs/search/quickstart): realiza tu primera búsqueda con Exa usando nuestros SDKs.
