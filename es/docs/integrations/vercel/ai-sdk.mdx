---
title: "AI SDK de Vercel"
description: "Añade la búsqueda web de Exa a tus aplicaciones con AI SDK mediante el paquete @exalabs/ai-sdk."
icon: "/docs/images/sidebar-icons/vercel.svg"
---

Usa el paquete `@exalabs/ai-sdk` para añadir la búsqueda web de Exa a aplicaciones creadas con el AI SDK de Vercel. Solo tienes que proporcionar una Exa API key y la herramienta `webSearch()` se encarga de las solicitudes de búsqueda de tu modelo.
...
* `maxAgeHours`: el respaldo de caché predeterminado; ajusta esta opción cuando necesites mayor frescura en los resultados

<div id="configure-search">
  ## Configurar la search
</div>

Usa las siguientes opciones para ajustar la search y la extracción de contenido:
...
| `numResults`                              | Cantidad de resultados que se devolverán.                                                                 |
| `includeDomains` / `excludeDomains`       | Incluir o excluir dominios concretos.                                                                     |
| `startPublishedDate` / `endPublishedDate` | Filtrar los resultados por fecha de publicación en formato ISO 8601.                                      |
...
| `contents.maxAgeHours`                                 | Usa el contenido en caché solo si su antigüedad no supera el límite indicado; de lo contrario, usa livecrawl. |
...
<Columns cols={2}>
  <Card title="Usar Vercel AI Gateway" icon="cloud" href="/es/docs/integrations/vercel/ai-gateway" cta="Abrir guía" arrow="true">
    Usa la búsqueda web de Exa sin una API key de Exa a través del AI Gateway de Vercel.
  </Card>
