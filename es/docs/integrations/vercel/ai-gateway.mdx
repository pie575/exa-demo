...
Cuando despliegues tu aplicación en Vercel, puedes usar en su lugar el `VERCEL_OIDC_TOKEN`, disponible automáticamente. Consulta la [documentación sobre authentication y BYOK](https://vercel.com/docs/ai-gateway/authentication-and-byok) de Vercel.
...
| Opción                                                 | Descripción                                                              |
| ------------------------------------------------------ | ------------------------------------------------------------------------ |
| `type`                                                 | Modo de búsqueda: `auto` (por defecto), `fast` o `instant`.              |
| `numResults`                                           | Número de resultados a devolver, de 1 a 100. El valor por defecto es 10. |
...
| `contents.maxAgeHours`                                 | Definir la antigüedad máxima del contenido en caché.                     |
| `contents.livecrawlTimeout`                            | Definir el tiempo de espera del livecrawl.                               |
...
Los agentes creados con [eve](https://eve.dev) incluyen la herramienta `web_search` integrada, y los modelos de AI Gateway la ejecutan en Exa de forma predeterminada, sin necesidad de configuración ni de una API key de Exa. Para fijar el proveedor de forma explícita, expórtalo desde `agent/tools/web_search.ts`:
...
<Tip>
  Exa web search es **gratuito hasta el 31 de agosto** en AI Gateway y eve, así que hoy mismo puedes empezar a desarrollar con él sin costo.
</Tip>
