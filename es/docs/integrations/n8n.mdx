...
title: "n8n"
sidebarTitle: "n8n"
description: "Usa Exa Search y contenido dentro de flujos de trabajo de n8n."
...
<Steps>
  <Step title="Agregar el nodo comunitario">
    Busca **Exa** en el selector de nodos de n8n. Si no está disponible en tu instancia, el propietario de la instancia puede instalar `n8n-nodes-exa-official` siguiendo la [guía de instalación de nodos comunitarios](https://docs.n8n.io/integrations/community-nodes/installation/) de n8n.
  ...
  <Step title="Agregar credenciales de Exa">
    Agrega una credencial **Exa API** en n8n y pega tu key. Selecciona esa credencial en cada nodo de Exa que deba usar la cuenta.
  </Step>
</Steps>
...
| **Agent**    | Crea, inspecciona, lista, transmite, sondea y cancela runs del agente de varios pasos.                                                  |
...
## Esperar a un run del agente {#wait-for-an-agent-run}

Al crear un run del agente, **Wait for Completion** admite:

* **Stream** para mantener abierta una conexión de server-sent events hasta que el run termine
* **Sondear** para consultar el run a intervalos regulares

Para flujos de trabajo asíncronos o de larga duración, desactiva **Wait for Completion**, guarda el `id` del run devuelto y usa **Get Run** más adelante. El run continúa en Exa una vez finalizado el paso de n8n.
...
<AccordionGroup>
  ...
  <Accordion title="Se rechaza la credencial de Exa">
    Comprueba que la credencial seleccionada contenga una key activa del [Exa Dashboard](https://dashboard.exa.ai/api-keys) y que esa key tenga créditos disponibles.
  </Accordion>
...
<Columns cols={3}>
  <Card title="Nodo de Exa oficial" icon="github" href="https://github.com/exa-labs/n8n-integration" cta="Ver repositorio" arrow="true">
