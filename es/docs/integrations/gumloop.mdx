...
description: "Usa Exa search y contenido dentro de los flujos de Gumloop."
...
<Steps>
  <Step title="Abre el agente">
    Abre la configuración de tu agente y selecciona **Add tools** → **Connect an app with MCP**.
  </Step>
  ...
  <Step title="Elige las herramientas">
    Abre la integración de Exa ya conectada y habilita solo las herramientas que el agente necesite. Así la selección de herramientas queda más clara y se evita que el agente invoque acciones que no vienen al caso.
  </Step>
  ...
  <Step title="Prueba la conexión">
    ...
    Revisa la ejecución para confirmar que el agente llamó a Exa y devolvió las fuentes citadas.
  </Step>
</Steps>
...
Para un agente conversacional, habilita primero Search, Get Contents y Answer. Añade el resto de herramientas solo cuando el flujo de trabajo lo requiera.
...
Añade un **Agent Node** a un flujo determinista de Gumloop y conéctale Exa como una de sus herramientas. El nodo puede decidir si realizar una search, recuperar páginas completas o encadenar varias llamadas a Exa antes de pasar su resultado al siguiente paso del flujo de trabajo.

Esto resulta útil para:

* enriquecer filas de un CRM o de hojas de cálculo con información web actualizada
* monitorizar noticias y enviar un summary con fuentes a Slack o por correo electrónico
...
3. Describe una sola acción, por ejemplo `Search for funding announcements from the past seven days`.
4. Prueba el nodo generado, verifica sus entradas y salidas y guárdalo.
...
<AccordionGroup>
  ...
  <Accordion title="Enriquecer una empresa">
    ...
    el anuncio de financiación más reciente y dos fuentes de noticias recientes.
    ```
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Exa no está disponible para el agente">
    Vuelve a abrir las herramientas MCP del agente, confirma que Exa esté conectado y habilita la herramienta necesaria. Una integración conectada puede tener herramientas individuales deshabilitadas.
  </Accordion>

  <Accordion title="El agente elige la acción equivocada">
    Indica de forma explícita en la solicitud si debe buscar, leer URLs conocidas, encontrar páginas similares o responder a partir de fuentes. Deshabilita las herramientas de Exa que el agente no necesite para ese flujo de trabajo.
  </Accordion>
...
<Columns cols={3}>
  <Card title="Integración de Exa con Gumloop" icon="book-open" href="https://docs.gumloop.com/nodes/mcp/exa" cta="Leer guía" arrow="true">
    Consulta las herramientas actuales de Gumloop y el flujo de trabajo del Agent Node.
  </Card>
