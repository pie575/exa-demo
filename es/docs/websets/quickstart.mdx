...
Un Webset comienza con una consulta en lenguaje natural y un número objetivo de items. Agrega criteria que todo result debe cumplir y campos de enrichment que se completarán para cada item aceptado. Los resultados llegan de forma asíncrona a través del panel, la API o los webhooks.

También puedes crear websets de forma visual en el [panel](/es/docs/websets/dashboard/get-started), sin necesidad
de código.

<Info>
  ¿Vas a iniciar un nuevo flujo de trabajo de creación de listas o enrichment? Usa [Exa Agent](/es/docs/agent/quickstart).
...
1. **Define una búsqueda:** Proporciona una consulta en lenguaje natural, un número de resultados y, de forma opcional, criteria de verificación y enrichments.
...
| ---------------------------- | ------------------------------------------------------------------------------------------------------- |
| **Verificación de criteria** | Cada result se contrasta con las reglas que definas, de modo que solo obtienes coincidencias relevantes |
...
| **Webhooks**                 | Recibe llamadas HTTP en tiempo real a medida que se añaden o enriquecen items                           |
