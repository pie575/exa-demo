...
description: "Aprende a verificar de forma segura las firmas de los webhooks para confirmar que las solicitudes provienen de Exa"
timestamp: false
---

Cuando recibas un webhook de Exa, conviene que verifiques que proviene de nosotros para garantizar la integridad y autenticidad de los datos. Exa firma todos los payloads de los webhooks con una key secreta única para cada endpoint de webhook.
...
<CodeGroup>
  ...
  def verify_webhook_signature(payload, signature_header, webhook_secret):
      ...
      """
      try:
          # Analizar el encabezado de firma
  ...
  # Ejemplo de uso en un endpoint de webhook con Flask
  ...
  function verifyWebhookSignature(payload, signatureHeader, webhookSecret) {
      ...
      try {
          ...
          // Comparar con las firmas recibidas mediante una comparación de tiempo constante
      ...
      */
      ...
      public static void main(String[] args) {
          ...
          try {
              ...
              // Prueba con encabezado mal formado
...
* **Comprueba la vigencia de la marca de tiempo** - Plantéate rechazar los webhooks con marcas de tiempo demasiado antiguas (por ejemplo, de más de 5 minutos) para prevenir ataques de repetición.

* **Almacena los secretos de forma segura** - Guarda los secretos de tus webhooks en variables de entorno o en un sistema seguro de gestión de secretos. Nunca los escribas directamente en el código de tu aplicación. **Importante**: el secreto del webhook solo se devuelve al [crear un webhook](/es/docs/websets/api/webhooks/create-a-webhook); asegúrate de guardarlo en un lugar seguro, ya que no se puede recuperar después.

* **Usa HTTPS** - Utiliza siempre endpoints HTTPS en tus webhooks para garantizar que los datos viajen cifrados.

* **Registra la URL definitiva** - Las entregas de webhooks no siguen redirecciones HTTP (respuestas 3xx). Si tu endpoint redirige, la entrega se considerará fallida. Registra siempre la URL que procesa directamente el payload.
...
1. **Revisa el payload sin procesar**: asegúrate de usar el cuerpo de la solicitud en bruto, no un objeto JSON ya analizado
2. **Verifica el secreto**: asegúrate de usar el secreto correcto que se generó al crear el webhook
3. **Revisa el análisis del encabezado**: asegúrate de extraer correctamente la marca de tiempo y las firmas del encabezado
4. **Problemas de codificación**: mantén una codificación UTF-8 consistente durante todo el proceso de verificación
