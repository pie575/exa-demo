...
Usa este skill para enseñarle a tu agente a llamar a Exa Contents con cURL o HTTP puro, siguiendo las mejores prácticas.
...
Configura la agent skill exa-contents de Exa en esta máquina.

Objetivo:
- Instalar la skill exa-contents para que mi agente de programación pueda usarla y llamar a Exa Contents directamente con cURL o HTTP puro.
...
Qué hacer:
1. Instala la skill PRIMERO, antes de configurar cualquier key. Prefiere una instalación local al proyecto si trabajas dentro de un repositorio; en caso contrario, usa el directorio global correspondiente de la lista anterior. Crea el directorio de skills elegido y descarga la skill:
   ...
   Luego verifica que exista <skills-dir>/exa-contents/SKILL.md.
2. Comprueba si ya hay una Exa API key disponible DESDE TU PROPIO ENTORNO DE EJECUCIÓN DE COMANDOS: usa la misma herramienta o shell con la que ejecutarás la skill, y no me pidas que la muestre con echo. La skill resuelve la key primero desde EXA_API_KEY y después desde el archivo ~/.config/exa/key, así que comprueba ambos sin imprimir nunca un valor:
   printf '%s\n' "${EXA_API_KEY:+env-set}"; [ -s ~/.config/exa/key ] && printf 'file-set\n'
   Lo más probable es que tu shell no sea interactiva y que NO cargue automáticamente perfiles interactivos como ~/.zshrc o ~/.bashrc, así que una key que yo haya definido ahí puede parecer presente para mí y estar vacía para ti. Si no aparece ninguna, puede que la key siga estando en un perfil interactivo que tu shell omite: averigua en qué archivo está SIN imprimir su valor con `grep -l EXA_API_KEY ~/.zshrc ~/.zshenv ~/.bashrc ~/.profile ~/.config/fish/config.fish 2>/dev/null` (solo lista nombres; NUNCA ejecutes un `grep`/`cat`/`echo` directo sobre un perfil, ya que una línea `export EXA_API_KEY=...` filtraría el secreto a nuestro chat). Después aplica `source` a ese archivo dentro de tu comando y repite la comprobación anterior; si aparece, antepón ese mismo `source ...;` a todos los comandos posteriores que necesiten la key.
...
4. Haz una prueba rápida de la key desde tu propia shell: resuélvela desde la variable de entorno o el archivo e imprime solo el código de estado:
   ...
   Mantén el endpoint, los encabezados y el cuerpo exactamente como están escritos (no adivines el esquema). Debe devolver 200, no 401/429. Si en el paso 2 necesitaste el prefijo `source ...;` para ver una key de entorno, antepónlo también aquí.
