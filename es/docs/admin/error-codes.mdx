...
description: "Referencia de los códigos de error comunes que usa la API de Exa"
---

Las APIs de Exa señalan los errores con códigos de estado HTTP estándar y un cuerpo de error en JSON.
...
| `403` Forbidden             | La API key no tiene acceso a la funcionalidad solicitada, o una política bloqueó la solicitud.                        | Revisa el mensaje devuelto y el acceso a funcionalidades de tu plan.                                                              |
...
| `429` Too Many Requests     | Tu API key, equipo o red superó un límite de tasa o de concurrencia.                                                  | Reduce tu tasa de solicitudes; espera los segundos de `Retry-After` cuando esté presente y, si no, usa exponential backoff.       |
...
| `503` Service Unavailable   | Exa está temporalmente sobrecargado (`SERVICE_OVERLOADED`) o no disponible. La solicitud no se procesó ni se factura. | Reintenta con exponential backoff. Esto es independiente de tu tasa de solicitudes, por lo que reducirla no ayuda; reintentar sí. |
...
El conjunto de tags es abierto y los nombres de los tags se explican por sí solos. Ramifica primero según el código de estado HTTP y trata los tags no reconocidos como información adicional, no como errores de análisis.
...
| `FEATURE_DISABLED`        | `403`       | El endpoint, tipo de búsqueda u opción solicitada no está habilitado para tu plan.                                                 |
...
| Tag                        | Código HTTP | Descripción                                     |
| -------------------------- | ----------- | ----------------------------------------------- |
| `X402_PAYMENT_REQUIRED`    | `402`       | Se requiere un pago.                            |
...
| Tag                       | Descripción                                                           | Cómo gestionarlo                                                       |
| ------------------------- | --------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `CRAWL_NOT_FOUND`         | No se encontró la página de destino.                                  | Verifica que la URL sea correcta y accesible.                          |
| `CRAWL_HTTP_{status}`     | El destino devolvió un error HTTP, como `CRAWL_HTTP_403`.             | Gestiona el estado del destino que se incluye en el tag.               |
...
| `SOURCE_NOT_AVAILABLE`    | El acceso al origen está prohibido o el origen no está disponible.    | Comprueba si el origen requiere autenticación o está restringido.      |
...
* Consulta los [límites de tasa](/es/docs/admin/billing#rate-limits) para ver los límites actuales.
* Revisa la [referencia de la API](/es/docs/reference/search) del endpoint para conocer los requisitos de la solicitud.
