...
title: "Batch API"
sidebarTitle: "Batch"
description: "Ejecuta solicitudes de la API de Exa de forma asíncrona en batches."
boost: 4
---
...
La Batch API te permite enviar muchas solicitudes de la API de Exa a la vez y recuperar sus resultados más tarde en un archivo JSONL. En lugar de enviar miles de solicitudes individuales y gestionar por tu cuenta los límites de tasa y los reintentos, envías un solo batch, sondeas su estado y descargas todos los resultados en un único archivo.
...
Cada elemento necesita un `customId` único dentro del batch. Ese mismo `customId` se devuelve en el archivo de resultados para que puedas asociar las filas del output con tus datos de entrada.
...
| `cancelling`  | Se solicitó la cancelación y el trabajo en curso se está terminando de procesar |
| `cancelled`   | El batch se canceló                                                             |
| `expired`     | Los resultados ya no están disponibles                                          |

Cuando el batch se completa, `resultsUrl` contiene una URL de descarga del archivo JSONL de resultados y `expiresAt` se fija al final de la ventana de retención de resultados.

<Warning>
  `resultsUrl` es una URL prefirmada de corta duración. Vuelve a consultar el batch para obtener una URL nueva cada vez que necesites descargar los resultados de nuevo.
</Warning>
...
La respuesta se pagina mediante cursor: `data` contiene hasta `limit` batches y, cuando `hasMore` es `true`, debes pasar `nextCursor` como parámetro de consulta `cursor` para obtener la página siguiente.

Pasa `status=completed` para listar solo los batches completados:
