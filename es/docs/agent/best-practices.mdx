...
description: "Ajusta la calidad de las consultas, la salida estructurada, el effort y el costo en integraciones de Exa Agent en producción."
keywords: ["agent api", "creación de listas", "enrichment", "salida estructurada", "effort", "grounding"]
---
...
| `input.exclusion`       | Hay registros conocidos que no deben devolverse                                     |
...
| `effort`                | El costo o la profundidad de la investigación requiere un valor explícito           |
...
Para la creación de listas, define la entidad, el número objetivo, los criteria de calificación, las exclusiones y el nivel mínimo de evidencia. Para el enrichment, coloca los registros existentes en `input.data` y describe únicamente la investigación que Agent debe añadir.

Solicita una justificación cuando la calificación requiera criterio propio. Incluye ejemplos solo cuando un criterio admita varias interpretaciones plausibles.
...
Los runs de Agent pueden tardar de segundos a minutos mientras buscan, leen y razonan. Diseña tu aplicación en torno al ciclo de vida en lugar de mantener abierta una solicitud.
...
<Steps>
  ...
  <Step title="Esperar a un estado terminal">
    Usa un helper de sondeo del SDK, sondea `GET /agent/runs/{id}` o consume el stream SSE. Continúa mientras el run esté `queued` o `running`.
  </Step>
...
Persistir el ID del run permite que tu aplicación se recupere tras un reinicio, se reconecte a un stream e inspeccione los fallos. Reduce la latencia acotando el scope, limitando la cantidad de resultados, manteniendo el esquema enfocado y eligiendo `minimal` o `low` cuando la velocidad importa más que la exhaustividad.

Para los batches, mide tareas representativas antes de estimar la concurrencia o de poner Agent en una ruta síncrona de la interfaz. El tiempo de ejecución varía según la cantidad de items, la complejidad del esquema, la disponibilidad de las sources y el effort.

Para los equipos con Zero Data Retention, consume el stream en vivo o sondea dentro de la ventana de retención. `previousRunId` y los `dataSources` de Connect no están disponibles. Consulta [Zero Data Retention](/es/docs/admin/security/zero-data-retention).

## Escribe esquemas JSON personalizados para salida estructurada {#write-custom-json-schemas-for-structured-output}
...
No dupliques en tu esquema las citas ni la confianza que Exa ya incluye. Añade un campo de justificación solo cuando cada item deba explicar por qué califica, y conserva `output.grounding` junto con el resultado estructurado. Contrasta las afirmaciones importantes con sus fuentes y prueba los cambios de esquema con entradas representativas antes de publicar.

Explora los [ejemplos estructurados de Agent](/es/docs/agent/examples) para comparar esquemas de creación de listas, KYB, ofertas de empleo, exclusiones y runs continuados.
...
| --------------------------------------------------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| Una lista investigada de tamaño desconocido         | `auto` y un `outputSchema` acotado                            | Un effort bajo fijo y un arreglo sin límite                                            |
...
| Datos de proveedores premium                        | [Exa Connect](/es/docs/agent/connect/overview) con `dataSources` | Pedirle a Agent que infiera campos exclusivos del proveedor a partir de la web abierta |
...
<Columns cols={2}>
  <Card title="Quickstart de Agent" icon="bot" href="/es/docs/agent/quickstart" cta="Abrir guía" arrow="true">
    Crea un run, recibe eventos en streaming, define el effort y lee la salida estructurada.
  </Card>
