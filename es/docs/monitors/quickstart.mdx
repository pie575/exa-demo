...
description: "Ejecuta búsquedas recurrentes y recibe los nuevos resultados mediante un webhook."
boost: 4
---

Los Monitors ejecutan búsquedas de Exa de forma recurrente y envían los resultados a un endpoint de webhook.

Usa Monitors para hacer seguimiento de noticias, anuncios de la competencia, rondas de financiación, cambios normativos, publicaciones
de investigación o cualquier otro tema que cambie con el tiempo.
...
| Forma del output  | Cómo usarlo                    | Valor devuelto                                       |
| ----------------- | ------------------------------ | ---------------------------------------------------- |
...
Lanza un run de inmediato en lugar de esperar a la siguiente ejecución programada y, después, lista sus runs:
...
El intervalo mínimo es de una hora. Usa una única duración como `1h`, `6h`, `1d` o `7d`. El
schedule se ancla a la hora de creación del monitor: un monitor diario creado a las 2:30 PM se ejecuta
cada día alrededor de las 2:30 PM, pero cada run puede retrasarse hasta 30 minutos, así que no cuentes con una
hora de entrega exacta.

Omite `trigger` para crear un monitor solo manual. Pausar un monitor programado también detiene los
runs automáticos, pero mantiene disponibles los disparos manuales.

<Note>
  Los runs de un monitor no se solapan. Si el siguiente run programado comienza mientras el anterior
  sigue en ejecución, Exa cancela el run anterior.
</Note>
...
envía también los eventos del ciclo de vida del monitor y los de creación de runs.

El payload del run completado incluye el estado y el output del run. Los `metadata` opcionales del monitor se
replican en las entregas del webhook, lo que te permite enrutar cada actualización al cliente,
espacio de trabajo, canal o trabajo interno correcto.

<Accordion title="Payload del webhook de run completado">
  El output y los timestamps aparecen abreviados a continuación.
...
<Columns cols={2}>
  ...
  <Card title="Monitor runs" icon="clock" href="/es/docs/reference/monitors/runs/get-a-run" cta="Abrir referencia" arrow="true">
