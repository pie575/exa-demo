...
> Añade la búsqueda web de Exa a tus aplicaciones con el AI SDK mediante el paquete @exalabs/ai-sdk.

Usa el paquete `@exalabs/ai-sdk` para añadir la búsqueda web de Exa a las aplicaciones creadas con el AI SDK by Vercel. Solo tienes que proporcionar una Exa API key: la herramienta `webSearch()` se encarga de las solicitudes de búsqueda de tu modelo.
...
<Info>
  Define tu key en la variable `EXA_API_KEY` antes de ejecutar el ejemplo. El paquete lee esta variable de entorno automáticamente.
</Info>
...
* `maxAgeHours`: el valor de reserva de caché predeterminado; configura esta opción cuando necesites mayor frescura en los datos
...
| `numResults`                              | Número de resultados a devolver.                                                                          |
...
| `contents.summary`                                     | Devuelve un summary generado por IA. Admite una `query`.                                                          |
| `contents.maxAgeHours`                                 | Usa el contenido en caché solo si su antigüedad no supera el límite especificado; de lo contrario, usa livecrawl. |
| `contents.livecrawlTimeout`                            | Establece el tiempo de espera del livecrawl.                                                                      |
...
<Columns cols={2}>
  <Card title="Usar Vercel AI Gateway" icon="cloud" href="/es/docs/integrations/vercel/ai-gateway" cta="Abrir guía" arrow="true">
    Usa la búsqueda web de Exa sin una Exa API key mediante el AI Gateway de Vercel.
  </Card>
