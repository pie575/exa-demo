...
Esta integración utiliza la función [External Access](https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview) de Snowflake para llamar de forma segura a la API de Exa desde procedimientos almacenados, lo que habilita dos flujos de trabajo principales:

1. **Herramientas de Cortex Agent**: dale a un Cortex Agent de Snowflake la capacidad de buscar en la web y recuperar el contenido de las páginas
2. **enrichment de datos**: enriquece tablas por lotes con información de directores ejecutivos y fundadores, últimas noticias y otros datos de la web
...
<div id="network-rule-secret-and-external-access">
  ### Regla de red, secreto y acceso externo
</div>
...
* Un **secreto** que almacena tu API key (cifrada, no visible en SQL)
...
| ---------------------- | --------- | -------------------- | --------------------------------------------------------------------------------------- |
| `query`                | `STRING`  | obligatorio          | La query de búsqueda                                                                    |
...
| `start_published_date` | `STRING`  | `NULL`               | Límite inferior de fecha en formato ISO 8601                                            |
| `end_published_date`   | `STRING`  | `NULL`               | Límite superior de fecha en formato ISO 8601                                            |
...
Recupera el contenido de las páginas (texto, highlights, resúmenes) de URLs específicas.
...
<div id="get-contents-for-specific-urls">
  ### Obtener contents de URLs específicas
</div>
...
Exa puede enriquecer tablas de Snowflake con datos web en tiempo real —información de CEO/fundadores, últimas noticias y mucho más—, todo sin salir de la plataforma.

<div id="create-a-prospect-table">
  ### Crear una tabla de prospectos
</div>
...
<div id="enrich-with-ceofounder-info">
  ### Enriquecer con información del CEO/fundador
</div>
...
<div id="enrich-with-latest-news">
  ### Enriquecer con las últimas noticias
</div>
...
<div id="parallelizing-exa-requests">
  ## Paralelizar solicitudes a Exa
</div>

Al enriquecer tablas grandes, llamar a Exa de forma secuencial fila por fila resulta lento. Puedes paralelizar las solicitudes con `concurrent.futures` de Python dentro de un único procedimiento almacenado, enviando varias llamadas a la API de Exa a la vez.

<div id="concurrent-enrichment-procedure">
  ### Procedimiento de enrichment concurrente
</div>
...
<div id="choosing-parallelism">
  ### Elegir el nivel de paralelismo
</div>

| `max_workers` | Caso de uso                                                                                       |
| ------------- | ------------------------------------------------------------------------------------------------- |
| `5`           | Conservador: se mantiene holgadamente dentro de los límites de tasa                               |
...
<div id="snowflake-task-scheduling">
  ### Programación de tareas en Snowflake
</div>

Para trabajos de enrichment recurrentes, encapsula el procedimiento en una [Task de Snowflake](https://docs.snowflake.com/en/user-guide/tasks-intro):
...
Esto ejecuta el enrichment en paralelo todos los días a las 8 AM PT. Ajusta la programación cron y `max_workers` según tu volumen y tus limits.

<div id="cortex-agent-integration">
  ## Integración con Cortex Agent
</div>

También puedes registrar los procedimientos almacenados de Exa como herramientas de un [Snowflake Cortex Agent](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-agents), lo que le permitirá buscar en la web de forma conversacional.
...
* &quot;¿Cuáles son las principales empresas de IA que hay que seguir?&quot;

<div id="security">
  ## Seguridad
</div>

La integración utiliza las funciones de seguridad integradas de Snowflake:

| Capa                            | Detalle                                                           |
| ------------------------------- | ----------------------------------------------------------------- |
| **Roles**                       | `exa_agent_role` para acceso restringido a procedimientos y datos |
| **Regla de red**                | Salida restringida únicamente a `api.exa.ai:443`                  |
| **Secreto**                     | API key almacenada cifrada, no visible en consultas SQL           |
| **External Access Integration** | Vincula la regla de red y el secreto                              |

Otorga acceso a más usuarios:
...
* [Panel de Exa](https://dashboard.exa.ai) — obtén tu API key
* [Documentación de acceso externo de Snowflake](https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview)
