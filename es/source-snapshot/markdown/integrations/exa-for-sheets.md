...
Usa Exa dentro de Google Sheets para investigar en la web, generar tablas y completar los datos que falten.
...
<Steps>
  <Step title="Instala el add-on">
    Ve al [add-on de Exa AI](https://workspace.google.com/marketplace/app/exa_ai/465545439521) en Google Workspace Marketplace y haz clic en **Install**.
  </Step>

  <Step title="Abre una hoja de Google">
  ...
  <Step title="Empieza a usar Exa">
    Abre **Exa Agent** y empieza a usar Exa en tu hoja.
  </Step>
</Steps>
...
```text theme={null}
Encuentra las 40 mejores empresas de IA y devuelve el nombre de la empresa, la URL del sitio web, el CEO, la fecha de fundación, la sede y una breve descripción.
```
...
Exa analiza la tabla que rodea tu selección y completa los espacios en blanco.

Antes de usar **Rellenar celdas**, selecciona celdas vacías de una tabla que ya tenga encabezados claros.
...
Exa toma las filas existentes como ejemplo, mantiene las mismas columnas y evita repetir elementos que ya figuran en la tabla.

<div id="exa">
  ## `=EXA(...)`
</div>

Usa `=EXA(...)` cuando quieras obtener una única respuesta en una sola celda. Busca en la web, lee los mejores resultados y devuelve una respuesta concisa.
...
| --------- | ----------- | --------------------------------------------------------------------------------------------------- |
| `prompt`  | Sí          | La información que quieres obtener (por ejemplo, `"Return only the CEO name"`).                     |
| `context` | No          | Referencia de celda o texto que se va a enriquecer (por ejemplo, el nombre de una empresa en `A2`). |
...
Usa `=EXA(...)` para respuestas sencillas de una sola celda. Usa **Exa Agent** cuando quieras crear o rellenar una tabla entera.

<div id="exa_answer">
  ## `=EXA_ANSWER(...)`
</div>

Respuestas avanzadas con IA y control total sobre el formato de salida. Úsala cuando necesites prompts de sistema, salida estructurada en JSON, citas o un tipo de búsqueda específico.
...
| `prefix`           | No          | `""`                 | Texto que se añade antes del prompt.                                                                       |
| `suffix`           | No          | `""`                 | Texto que se añade después del prompt.                                                                     |
...
| `returnRawJson`    | No          | `FALSE`              | Si es `TRUE` y se ha definido `outputSchema`, devuelve el JSON completo en lugar de extraer el valor.      |
...
Busca en la web y devuelve una lista vertical de URLs. Admite filtrado por dominio, filtrado por categoría, highlights de contenido y salida sintetizada mediante `outputSchema`.
...
| `prefix`             | No          | `""`                 | Texto que se añade antes de la query.                                                                                                                                     |
| `suffix`             | No          | `""`                 | Texto que se añade después de la query.                                                                                                                                   |
| `includeDomainsStr`  | No          | `""`                 | Dominios separados por comas que se incluirán (p. ej., `"linkedin.com,crunchbase.com"`).                                                                                  |
| `excludeDomainsStr`  | No          | `""`                 | Dominios separados por comas que se excluirán.                                                                                                                            |
...
| `outputSchemaJson`   | No          | `""`                 | Cadena JSON para `outputSchema` (p. ej., `"{""type"":""text"",""description"":""summarize""}"`). Si se especifica, devuelve texto de salida sintetizado en lugar de URLs. |
...
Encuentra URLs similares a una URL de referencia, con filtros opcionales de dominio y texto.
...
| Ampliar una tabla con nuevas filas                            | Exa Agent → Fill cells     |
| Obtener un único valor en una celda                           | `=EXA(...)`                |
| Obtener una respuesta con system prompt o salida estructurada | `=EXA_ANSWER(...)`         |
...
| Guardar los resultados de las fórmulas como texto plano       | Batch → Convert to values  |

<div id="notes">
  ## Notas
</div>

* Las solicitudes a la API de Exa se descuentan de tu cuota de uso. Usa **Batch → Convert to values** para fijar los resultados e impedir que las fórmulas se recalculen.
* El add-on reintenta automáticamente hasta 3 veces con retroceso exponencial cuando se alcanza el límite de solicitudes (HTTP 429).
