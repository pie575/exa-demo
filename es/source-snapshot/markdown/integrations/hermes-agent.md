...
<Steps>
  ...
  <Step title="Prueba el acceso a la web">
    ...
    ```text theme={null}
    Busca en la web las últimas novedades de producto de Exa y luego lee el resultado más relevante.
    ```
...
Hermes trunca las páginas extraídas que son largas según su presupuesto de caracteres configurado y guarda el texto completo en disco. Cambia el valor predeterminado con `web.extract_char_limit` o deja que el agente solicite un `char_limit` mayor para una llamada concreta.
...
<AccordionGroup>
  ...
  <Accordion title="La búsqueda funciona, pero la extracción usa otro proveedor">
    Hermes puede configurar la búsqueda y la extracción de forma independiente. Asigna el valor `exa` tanto a `web.search_backend` como a `web.extract_backend`.
  </Accordion>
</AccordionGroup>
