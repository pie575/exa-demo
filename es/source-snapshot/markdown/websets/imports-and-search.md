...
Cuando ya tienes una lista de URLs (empresas, personas, productos, etc.), puedes **importarlas** a un Webset. Según cómo configures el Webset, los items importados pueden enriquecerse, evaluarse según criteria o combinarse con resultados de descubrimiento web.
...
| Persol Group | `https://www.persol-group.co.jp` | Empresa de dotación de personal, enfocada en Japón, presencia mínima en EE. UU. |

Elegimos estas empresas porque 4 de las 5 cumplen claramente los criteria típicos de consultoría de TI (oficina en EE. UU., servicios de TI). **Persol Group** es la excepción: es una empresa japonesa de dotación de personal con presencia mínima en EE. UU., por lo que no debería superar los criteria centrados en EE. UU.

Los criteria que usaremos en los ejemplos siguientes:
...
# Paso 3: Crea un Webset que use este import (solo enrichments, sin búsqueda/criteria)
...
| Supplier     | ¿En el Webset? | Origen   | Evaluations | Enrichments | ¿Por qué?                                     |
| ------------ | -------------- | -------- | ----------- | ----------- | --------------------------------------------- |
| Accenture    | **Sí**         | `import` | 0           | 2           | Importado, sin criteria con los que evaluarlo |
| Infosys      | **Sí**         | `import` | 0           | 2           | Importado, sin criteria con los que evaluarlo |
| Wipro        | **Sí**         | `import` | 0           | 2           | Importado, sin criteria con los que evaluarlo |
| EPAM Systems | **Sí**         | `import` | 0           | 2           | Importado, sin criteria con los que evaluarlo |
| Persol Group | **Sí**         | `import` | 0           | 2           | Importado, sin criteria con los que evaluarlo |

Cada item tiene `source: "import"` y `evaluations: []`. Los 5 se conservan y se enriquecen sin importar si cumplirían algún criterio, porque en esta config no hay criteria.
...
| Nuestros 5 Suppliers  | ¿En el Webset? | ¿Por qué?                                                                  |
| --------------------- | -------------- | -------------------------------------------------------------------------- |
...
Dio la casualidad de que la búsqueda web encontró a Accenture entre sus 35 resultados, pero no descubrió a los otros 4 suppliers. Esto es lo esperado: los websets de solo búsqueda únicamente devuelven lo que encuentra el rastreo web, no una lista predeterminada. Ejemplos de otras empresas descubiertas: Artech, TurnKey Staffing, DataArt, Insight Global, entre otras.
...
<CodeGroup>
  ...
  ```bash cURL theme={null}
  # Paso 1: Crea un import CSV y súbelo (igual que en la Config 1, pasos 1-2)
  # ... (consulta la Config 1 para ver el flujo completo del import)
  # Obtendrás un <IMPORT_ID>

  # Paso 2: Crea un Webset con un search con scope -- evalúa cada URL importada según los criteria
  # El import se programa automáticamente para su procesamiento al crear el Webset.
  curl -s -X POST "https://api.exa.ai/websets/v0/websets" \
    -H "Authorization: Bearer $EXA_API_KEY" \
    -H "Content-Type: application/json" \
...
| Supplier     | ¿En el Webset?       | Origen   | ¿Tiene evaluations? | ¿Por qué?                                                                                 |
| ------------ | -------------------- | -------- | ------------------- | ----------------------------------------------------------------------------------------- |
| Accenture    | **Sí**               | `search` | Sí (2)              | Cumplió: tiene oficina en EE. UU., ofrece consultoría de TI                               |
| Infosys      | **Sí**               | `search` | Sí (2)              | Cumplió: tiene oficina en EE. UU., ofrece servicios de TI                                 |
| Wipro        | **Sí**               | `search` | Sí (2)              | Cumplió: tiene oficina en EE. UU., ofrece servicios de TI                                 |
| EPAM Systems | **Sí**               | `search` | Sí (2)              | Cumplió: cotiza en EE. UU., ofrece servicios de ingeniería de software                    |
| Persol Group | **No -- descartada** | --       | --                  | No cumplió &quot;tiene una oficina en Estados Unidos&quot;: se centra sobre todo en Japón |

Importamos 5 suppliers, pero solo 4 aparecen en los resultados. **Persol Group se evaluó y no cumplió**, así que queda fuera. Cada item visible tiene `source: "search"` con las `evaluations` completas, que muestran el razonamiento de cada criterio.

<Warning>
  Los items que no cumplen los criteria **se descartan de los resultados**. Si necesitas conservar todos los items y solo ver cuáles cumplen y cuáles no, usa la Config 1 (solo import, sin filtrado) como un webset aparte, junto con la Config 3.
</Warning>
...
<CodeGroup>
  ...
  ```bash cURL theme={null}
  # Paso 1: Crea un import de CSV y súbelo (igual que en Config 1, pasos 1-2)
  # ... (consulta Config 1 para ver el flujo completo de import)
  # Obtendrás un <IMPORT_ID>
  ...
  # Paso 3: Espera a que termine el search con scope y luego añade una búsqueda web para descubrir nuevas coincidencias
...
| Supplier                               | ¿En el Webset?       | Origen   | ¿Tiene evaluaciones? | ¿Por qué?                                                                      |
| -------------------------------------- | -------------------- | -------- | -------------------- | ------------------------------------------------------------------------------ |
| Accenture                              | **Sí**               | `search` | Sí (2)               | Aprobó el search con scope: tiene oficina en EE. UU., ofrece consultoría de TI |
| Infosys                                | **Sí**               | `search` | Sí (2)               | Aprobó el search con scope: tiene oficina en EE. UU., ofrece servicios de TI   |
| Wipro                                  | **Sí**               | `search` | Sí (2)               | Aprobó el search con scope: tiene oficina en EE. UU., ofrece servicios de TI   |
| EPAM Systems                           | **Sí**               | `search` | Sí (2)               | Aprobó el search con scope: cotiza en EE. UU., ofrece ingeniería de software   |
| Persol Group                           | **No -- descartada** | --       | --                   | No aprobó el search con scope: sin oficina en EE. UU.                          |
| *(25 empresas descubiertas en la web)* | **Sí**               | `search` | Sí (2 cada una)      | Encontradas mediante búsqueda web, cumplieron ambos criteria                   |

El search con scope evalúa tu lista importada frente a los criteria (y descarta a Persol Group), mientras que la búsqueda web añadida descubre 25 empresas adicionales. El resultado es un único webset con tus imports puntuados y los nuevos hallazgos de la web.

<Note>
  La búsqueda web usa `"behavior": "append"`, por lo que añade resultados a los existentes en lugar de reemplazarlos. Si la búsqueda web descubre una empresa que ya estaba en los resultados del search con scope (por ejemplo, Accenture), el duplicado se gestiona automáticamente.
</Note>
...
| **2. Search Only**                           | Descubre nuevas coincidencias en la web         | N/D (sin imports)                         | Sí, solo se devuelven los items que pasan                 |
...
* **«Puntúa mi lista y descarta las que no coincidan»** -- Config 3
* **«Puntúa mi lista Y encuentra nuevas empresas que coincidan»** -- Config 4