...
La búsqueda devuelve hasta 10 resultados de forma predeterminada. Configura `numResults` para solicitar hasta 100 resultados; pueden devolverse menos si hay menos páginas relevantes disponibles. La búsqueda no admite paginación.
...
Consulta [Qué incluye el índice de Exa](/es/docs/search/data/overview) para conocer qué contiene el índice de Exa y cómo buscar esos tipos de contenido.
...
| `deep-reasoning` | La exhaustividad y la profundidad del razonamiento importan más que la latencia |
...
<Tip>
  En lugar de `deep-reasoning`, usa [Exa Agent](/es/docs/agent/quickstart) para investigación de larga duración, creación de
  listas y enrichment de varios saltos. Agent cuenta con más cómputo por run y devuelve resultados fundamentados y
  estructurados.
</Tip>
...
<Tabs>
  <Tab title="Highlights">
    ...
    evidencia que necesitan sin llenar la ventana de contexto con partes irrelevantes de cada página.

    Esta es la forma de salida recomendada para la mayoría de las tareas.
...
Elige una sola vista de contenido por solicitud. Solicitar highlights y texto a la vez devuelve y factura dos vistas de la misma página. `summary` es una tercera opción, pero añade una llamada a un modelo de lenguaje por cada resultado.

<Warning>
  `/search` y `/contents` aceptan las mismas opciones de contenido en lugares distintos:
...
<Tabs>
  <Tab title="Texto libre">
    Usa `type: "text"` para prosa generada. Añade una `description` para especificar su formato o extensión.
...
Usa `systemPrompt` para instrucciones como las preferencias de fuentes o el énfasis; usa `outputSchema` para definir la estructura de la respuesta. En Python se usan `system_prompt` y `output_schema`.
...
La mayoría de las búsquedas deberían omitir este campo. Úsalo cuando el contenido desactualizado de una página resulte inservible, como en el caso de precios, disponibilidad o páginas que cambian con frecuencia.
...
<Columns cols={2}>
  ...
  <Card title="Referencia de la API" icon="square-terminal" href="/es/docs/reference/search" cta="Abrir referencia" arrow="true">
    Todos los parámetros de la solicitud y campos de la respuesta, con un playground interactivo.
  </Card>
