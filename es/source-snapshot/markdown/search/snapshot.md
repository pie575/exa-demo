...
> Fija Search y contenido a una versión almacenada de una página en la fecha y hora que elijas.
...
Úsalo para hacer backtest de agentes, ejecutar evaluaciones reproducibles y comparar versiones anteriores de documentación, páginas de precios, políticas e informes regulatorios.
...
## Búsqueda en una fecha y hora {#search-at-a-datetime}
...
Exa detecta las URL candidatas y luego conserva únicamente las páginas que tengan una versión almacenada en `snapshotAsOf` o antes.
...
## Fijar el contenido a una fecha y hora {#pin-contents-to-a-datetime}
...
Exa devuelve la versión almacenada más reciente correspondiente a esa fecha y hora o anterior a ella.
...
<Tip>
  Los ID que no tengan ninguna versión elegible se omiten de `results` y se informan en `statuses` con
  `"status": "error"` y `"tag": "CONTENT_NOT_CACHED"`.
</Tip>

## Cómo funcionan las instantáneas {#how-snapshots-work}

| Campo          | Dónde     | Significado                                                                                             |
| -------------- | --------- | ------------------------------------------------------------------------------------------------------- |
| `snapshotAsOf` | Solicitud | Fecha y hora de corte. Exa devuelve la versión almacenada más reciente en ese instante o anterior a él. |
...
* Se omiten las páginas que no tengan una versión elegible dentro de la ventana de 5 meses.

<Note>
  En Search, el corte limita el contenido, no el ranking. Exa sigue usando señales de recuperación
  actuales para descubrir URL candidatas. Usa los resultados como evidencia acotada por `snapshotAsOf`,
  no como una reconstrucción exacta de lo que una búsqueda habría clasificado en ese momento.
</Note>

## Límites y compatibilidad {#limits-and-compatibility}

<AccordionGroup>
  <Accordion title="Acceso, límite de tasa y ventana histórica">
    El pago por uso incluye 10 QPS y acceso al índice de los últimos 5 meses en ventana móvil. Se rechaza cualquier
    `snapshotAsOf` anterior a esa ventana. Después de 100 solicitudes, [habla con ventas](https://exa.ai/contact/sales)
    para continuar.
  </Accordion>
  ...
  <Accordion title="Las solicitudes históricas usan contenido almacenado">
    ...
    Omite por completo `livecrawl`, `livecrawlTimeout`, `maxAgeHours` y `subpages`; las solicitudes que
    incluyan alguno de ellos junto con `snapshotAsOf` se rechazan con `INVALID_REQUEST`.
  </Accordion>

  <Accordion title="Solicitudes de Search admitidas">
...
## Usos comunes {#common-uses}

Usa Exa Snapshot cuando la tarea dependa de lo que Exa tenía almacenado en una fecha y hora concretas:

* Hacer un backtest de un agente sin exponerlo a actualizaciones posteriores de las páginas.
* Ejecutar una evaluación sobre un límite de contenido reproducible.
* Comparar versiones anteriores de documentación, precios, políticas o informes regulatorios.