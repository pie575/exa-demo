...
> Set up Enterprise Managed Auth (EMA) so Claude connects to Exa MCP through your identity provider, including Okta Cross App Access (XAA).

By default, every member connects the [Exa connector](/docs/get-started/exa-mcp) in Claude by signing in to Exa once through OAuth. With **Enterprise Managed Auth (EMA)** they get it silently through Okta instead: no Exa login screen, no consent prompt, no API keys handed around.
...
<Steps>
  <Step title="Provision your members in Exa">
    Every member who will use the connector must already exist in Exa and belong to a team in your Exa organization, with the same email address Okta asserts, on a domain verified on your organization. EMA never creates accounts. Use directory sync, or [invite them to the team](/docs/admin/team-management).
  </Step>
