...
<Info>
  `/chat/completions` routes to [`/answer`](/docs/reference/answer), and `/responses` routes to the [Agent API](/docs/agent/quickstart). See [Agent via Responses API](#agent-via-responses-api) below.
</Info>
...
Exa's [`/responses`](https://api.exa.ai/responses) endpoint exposes the [Agent API](/docs/agent/quickstart) through the OpenAI Responses interface, so the OpenAI SDKs work against it unchanged. Set `model: "exa-agent"` and choose an execution mode:
