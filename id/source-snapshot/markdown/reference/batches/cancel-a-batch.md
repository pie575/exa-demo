...
Batch akan berhenti menjadwalkan permintaan baru dan menuntaskan pekerjaan yang masih berjalan sebelum berakhir pada status `cancelled`. Membatalkan batch yang sudah berstatus `cancelling` atau `cancelled` akan mengembalikannya tanpa perubahan; batch yang berstatus `completed` atau `expired` tidak dapat dibatalkan.
