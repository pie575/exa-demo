...
> Gunakan Exa search dan pengambilan konten dari stored procedure Snowflake dan Cortex Agents.

Gunakan Search API Exa langsung dari Snowflake untuk memperkaya data Anda dengan intelijen web real-time — tanpa perlu keluar dari SQL.

Integrasi ini memanfaatkan fitur [External Access](https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview) milik Snowflake untuk memanggil Exa API secara aman dari stored procedure, sehingga memungkinkan dua workflow utama:

1. **Tool Cortex Agent** — memberi Snowflake Cortex Agent kemampuan untuk menelusuri web dan mengambil page contents
2. **Data enrichment** — memperkaya tabel dengan informasi CEO/pendiri, berita terbaru, dan data web lainnya secara batch
...
1. Exa API key
2. Peran `ACCOUNTADMIN` (diperlukan untuk membuat External Access Integration)
...
* Sebuah **network rule** yang hanya mengizinkan egress ke `api.exa.ai:443`
* Sebuah **secret** untuk menyimpan API key Anda (terenkripsi, tidak terlihat di SQL)
* Sebuah **external access integration** yang menyatukan network rule dan secret tersebut
...
Melakukan pencarian di web menggunakan Search API dari Exa. Mengembalikan hasil berupa judul, URL, skor, dan secara opsional text/highlights/summaries.
...
| `include_domains`      | `STRING`  | `NULL`   | Domain yang dipisahkan koma untuk membatasi hasil                                            |
...
| `get_contents`         | `BOOLEAN` | `TRUE`   | Mengambil teks, highlights, dan summaries                                                    |

<div id="exa_get_contents">
  ### EXA_GET_CONTENTS
</div>

Mengambil page contents (teks, highlights, summaries) untuk URL tertentu.
...
<div id="get-contents-for-specific-urls">
  ### Mengambil contents untuk URL tertentu
</div>
...
Exa dapat meng-enrich tabel Snowflake dengan data web terkini — informasi CEO/pendiri, berita terbaru, dan lainnya — tanpa perlu keluar dari platform.

<div id="create-a-prospect-table">
  ### Membuat tabel prospect
</div>
...
<div id="enrich-with-ceofounder-info">
  ### Enrich dengan informasi CEO/pendiri
</div>
...
Saat memperkaya tabel berukuran besar, memanggil Exa satu per satu untuk setiap baris akan berjalan lambat. Anda dapat memparalelkan permintaan menggunakan `concurrent.futures` dari Python di dalam satu stored procedure, sehingga beberapa panggilan API Exa dikirim sekaligus.

<div id="concurrent-enrichment-procedure">
  ### Prosedur enrichment secara konkuren
</div>
...
Untuk job enrichment yang berjalan berkala, bungkus prosedur tersebut dalam sebuah [Snowflake Task](https://docs.snowflake.com/en/user-guide/tasks-intro):
...
Ini menjalankan Enrichment paralel setiap hari pukul 08.00 PT. Sesuaikan jadwal cron dan `max_workers` dengan volume dan rate limit Anda.

<div id="cortex-agent-integration">
  ## Integrasi Cortex Agent
</div>

Anda juga dapat mendaftarkan stored procedure Exa sebagai tool untuk [Snowflake Cortex Agent](https://docs.snowflake.com/en/user-guide/snowflake-cortex/cortex-agents), sehingga agent tersebut dapat menelusuri web melalui percakapan.
...
Setelah dibuat, buka Snowflake Intelligence dan coba tanyakan hal berikut kepada agent Anda:
...
Integrasi ini menggunakan fitur keamanan bawaan Snowflake:

| Lapisan                         | Detail                                                                   |
| ------------------------------- | ------------------------------------------------------------------------ |
| **Peran**                       | `exa_agent_role` untuk akses terbatas ke prosedur dan data               |
| **Network Rule**                | Egress dibatasi hanya ke `api.exa.ai:443`                                |
| **Secret**                      | API key disimpan dalam bentuk terenkripsi, tidak terlihat pada kueri SQL |
| **External Access Integration** | Menggabungkan network rule + secret menjadi satu                         |
