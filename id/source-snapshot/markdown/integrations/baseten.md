...
Tersedia tiga tool Exa. Berikan model search sekaligus fetch jika model perlu menemukan sources lalu membaca halaman yang dipilihnya.
...
| `baseten__exa__web_search_advanced_exa` | Search dengan filter domain, crawling subhalaman, dan ringkasan opsional per hasil           |
| `baseten__exa__web_fetch_exa`           | [Full page contents](/id/docs/contents/quickstart) untuk URL yang sudah dimiliki model          |

Selector tidak memerlukan field tambahan; model mengisi sendiri argumen tool dari schema Exa. Gunakan system prompt untuk mengarahkan kebijakan search, misalnya kapan harus melakukan search, apakah perlu mengambil primary sources, dan bagaimana cara mengutipnya. Gunakan `baseten.tool_settings` untuk membatasi loop:
...
<Columns cols={2}>
  <Card title="Dokumentasi web search Baseten" icon="wrench" href="https://docs.baseten.co/inference/model-apis/web-search" cta="Buka dokumentasi" arrow="true">
    Contoh Messages, Responses, dan Chat Completions yang dapat dijalankan dengan server-side tool.
  </Card>
  ...
  <Card title="Referensi server-side tool" icon="book-open" href="https://docs.baseten.co/reference/inference-api/server-side-tool-execution" cta="Buka referensi" arrow="true">
    Katalog tool, pengaturan loop, format `tool_choice`, dan bentuk response.
  </Card>
</Columns>