...
Gunakan Exa MCP untuk menyempurnakan web search bawaan di ChatGPT, Claude, dan tool yang kompatibel dengan MCP dengan kemampuan pencarian Exa, termasuk web search, pencarian kode, [Exa Agent](/id/docs/agent/quickstart), dan [Exa Connect](/id/docs/agent/connect/overview).

Exa menyediakan server terkelola yang dapat digunakan di MCP client mana pun:
...
<div className="docs-tabs">
  <Tabs>
    <Tab title="ChatGPT & Codex" icon="https://mintcdn.com/exa-52/Una64IRjof2yadw_/images/mcp-clients/chatgpt.svg?fit=max&auto=format&n=Una64IRjof2yadw_&q=85&s=877edee72e2a7a4f7b9c7c936c6d4316" width="24" height="24" data-path="images/mcp-clients/chatgpt.svg">
      ...
      <Steps>
        ...
        <Step title="Mulai sesi baru">
          Skill dimuat pada chat dan sesi CLI yang dimulai setelah instalasi, jadi buka sesi baru dan ajukan permintaan yang membutuhkan akses web.
        </Step>
      </Steps>
      ...
      Lihat [Exa di Codex dan ChatGPT](/id/docs/integrations/chatgpt-codex) untuk panduan lengkap penyiapan dan workflow.
    </Tab>
    ...
    <Tab title="Claude" icon="https://mintcdn.com/exa-52/Una64IRjof2yadw_/images/mcp-clients/claude.svg?fit=max&auto=format&n=Una64IRjof2yadw_&q=85&s=443a9b17d5b63c875f924a4aecc01e56" width="24" height="24" data-path="images/mcp-clients/claude.svg">
      ...
      <Steps>
        ...
        <Step title="Gunakan Exa">
          Mulai sesi Claude Code baru dan ajukan permintaan yang membutuhkan akses web.
        </Step>
      </Steps>
      ...
      <Steps>
        ...
        <Step title="Gunakan Exa">
          Mulai obrolan baru dan ajukan permintaan yang membutuhkan informasi terkini dari web.
        </Step>
      </Steps>
      ...
      Sebagai alternatif, admin Claude Team dan Enterprise dapat menyediakan konektor untuk semua anggota melalui identity provider mereka: lihat [Enterprise Managed Auth](/id/docs/admin/mcp-enterprise-managed-auth).
    </Tab>
    ...
    <Tab title="VS Code" icon="https://mintcdn.com/exa-52/Una64IRjof2yadw_/images/mcp-clients/vscode.svg?fit=max&auto=format&n=Una64IRjof2yadw_&q=85&s=9828a7b963d47467df217a38c716fea2" width="24" height="24" data-path="images/mcp-clients/vscode.svg">
      Gunakan [pemasangan sekali klik](https://vscode.dev/redirect/mcp/install?name=exa\&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fmcp.exa.ai%2Fmcp%22%7D), atau tambahkan ke `.vscode/mcp.json` di proyek Anda:
    ...
    <Tab title="Client lain" icon="https://mintcdn.com/exa-52/Una64IRjof2yadw_/images/mcp-clients/other-clients.svg?fit=max&auto=format&n=Una64IRjof2yadw_&q=85&s=187e423022b8fc3ed950a967a10ff700" width="24" height="24" data-path="images/mcp-clients/other-clients.svg">
      Sebagian besar client menggunakan format `mcpServers` standar:
      ...
      Lokasi penyimpanan config dan nama key URL berbeda-beda di tiap client:
      ...
      Jika client Anda tidak mendukung remote MCP server, gunakan bridge `mcp-remote`:
...
| --------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| Tanpa key | Penggunaan gratis dengan batas laju tanpa masuk atau API key      | Hubungkan ke `https://mcp.exa.ai/mcp`                                                                              |
...
| ------------------------- | ---------------------------------- | ------------------------------------------------------------------------------------ |
| `web_search_exa`          | Aktif secara default               | Melakukan web search dan mengembalikan konten relevan yang siap pakai                |
| `web_fetch_exa`           | Aktif secara default               | Membaca konten bersih dari satu atau beberapa URL yang diketahui                     |
| `web_search_advanced_exa` | Tersedia jika diaktifkan           | Mengonfigurasi web search dengan filter dan kontrol lanjutan                         |
| `agent_run`               | Tersedia dengan OAuth atau API key | Menjalankan research multi-langkah, list building, enrichment, dan terstruktur output |

Gunakan parameter URL `tools` untuk menentukan tool apa saja yang dilihat client Anda. Misalnya, untuk mengaktifkan semua tool:
...
Agent run bersifat berbasis penggunaan, sehingga `agent_run` memerlukan OAuth atau API key. URL ini memulai OAuth dan menambahkan Agent bersama tool default:

```text theme={null}
https://mcp.exa.ai/mcp?login&tools=web_search_exa,web_fetch_exa,agent_run
```

Jika Anda menggunakan API key, hilangkan `login` dan tambahkan key seperti dijelaskan di [Authentication](#authentication).

<Steps>
  <Step title="Jelaskan apa yang Anda butuhkan">
    Mintalah Research dengan bahasa sehari-hari. Asisten Anda meneruskan permintaan tersebut ke `agent_run` dengan sebuah `query`, lalu Exa Agent menentukan apa yang perlu dicari, membaca sources, dan mencocokkan temuannya dengan permintaan Anda.

    Minta Agent Anda menyediakan `outputSchema` hanya jika aplikasi Anda membutuhkan temuan dalam format JSON yang konsisten. Anda bisa membagikannya ke asisten Anda melalui system prompt, atau meminta asisten membuatkannya untuk Anda.
  </Step>

  <Step title="Dapatkan hasilnya">
    Ketika Research selesai, tool call menyerahkan paket Research lengkap ke asisten Anda:
    ...
    * Penggunaan dan biaya
    ...
    Asisten Anda menyusun balasannya dari paket ini, jadi beri tahu apa yang Anda inginkan terhadap output tersebut. Anda bisa memintanya meringkas temuan, membandingkannya, menyimpannya ke sebuah file, atau hal lain sesuai kebutuhan.
  </Step>

  <Step title="Lanjutkan jika masih butuh waktu">
    Research yang berlangsung lebih lama dari satu panggilan MCP tidak akan gagal: tool melaporkan `status: "running"` beserta sebuah `id` selama run masih berjalan di Exa. Asisten Anda memanggil `agent_run` lagi dengan `id` tersebut sebagai `runId` untuk melanjutkan run yang sama.
  </Step>
</Steps>
...
<Accordion title="Kontrol opsional" icon="sliders-horizontal">
  | Field             | Gunakan untuk                                                                |
  | ----------------- | ---------------------------------------------------------------------------- |
  | `systemPrompt`    | Memberi Agent instruksi tambahan untuk meneliti atau menilai hasil           |
  ...
  | `previousRunId`   | Membangun permintaan baru di atas Research yang sudah selesai                |
  | `effort`          | Memilih seberapa dalam Research yang harus dilakukan Agent                   |
</Accordion>

<Tip>
  Gunakan `runId` untuk terus menunggu pekerjaan yang sedang berjalan. Gunakan `previousRunId` untuk mengajukan follow-up baru berdasarkan pekerjaan yang sudah selesai.
</Tip>
...
Advanced Search tidak memerlukan autentikasi, meskipun koneksi yang terautentikasi menggunakan plan dan rate limit Anda sendiri. Aktifkan bersama tool default dengan:
...
<AccordionGroup>
  <Accordion title="Kesalahan rate limit (429)">
    Koneksi ini menggunakan rate limit gratis Exa. Masuk dengan OAuth atau tambahkan API key Anda sendiri, lalu sambungkan kembali agar permintaan menggunakan plan dan batas Team Anda.
  ...
  <Accordion title="Agent tidak muncul atau meminta autentikasi">
    `agent_run` tidak diaktifkan secara default dan tidak dapat menggunakan rate limit gratis. Tambahkan tool tersebut ke parameter URL `tools`, lalu sambungkan dengan `?login` atau konfigurasikan API key. Lihat [Exa Agent](#exa-agent) untuk URL lengkapnya.
  </Accordion>

  <Accordion title="Halaman masuk OAuth tidak terbuka">
    Pastikan client Anda mendukung MCP OAuth dan sambungkan ke `https://mcp.exa.ai/mcp?login`. Mulai ulang client setelah mengubah URL. Jika client tidak dapat menyelesaikan MCP OAuth, gunakan API key sebagai gantinya.
  </Accordion>

  <Accordion title="Tool tidak muncul">
    Parameter `tools` yang eksplisit akan menggantikan daftar tool default. Pastikan setiap tool yang Anda inginkan tercantum di URL, lalu mulai ulang MCP client Anda agar daftar tool diambil ulang.
  </Accordion>
  ...
  <Accordion title="File config tidak ditemukan">
    Lokasi config yang umum:
...
<Columns cols={2}>
  ...
  <Card title="Agent skills" icon="wrench" href="/id/docs/get-started/agent-skills/overview" cta="Jelajahi skill" arrow="true">
    Skill portabel yang berpasangan dengan Exa MCP.
  </Card>
