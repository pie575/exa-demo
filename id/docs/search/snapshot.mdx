...
Exa Snapshot menyimpan stored version dari halaman-halaman yang telah di-crawl Exa. Kirim `snapshotAsOf` untuk mengunci permintaan ke suatu datetime.

Gunakan ini untuk melakukan backtest pada agent, menjalankan evaluasi yang dapat direproduksi, serta membandingkan versi terdahulu dari dokumentasi, halaman harga, kebijakan, dan filings.
...
## Menyematkan contents ke suatu datetime {#pin-contents-to-a-datetime}
...
* Halaman yang tidak memiliki eligible version dalam rentang 5 bulan akan dilewati.

<Note>
  Pada Search, batas waktu ini membatasi konten, bukan peringkat. Exa tetap memakai sinyal retrieval terkini untuk
  menemukan URL kandidat. Perlakukan hasilnya sebagai evidence yang dibatasi oleh `snapshotAsOf`, bukan sebagai
  rekonstruksi persis dari peringkat yang akan dihasilkan sebuah search pada waktu itu.
</Note>

## Batas dan kompatibilitas {#limits-and-compatibility}

<AccordionGroup>
  <Accordion title="Akses, rate limit, dan rentang waktu ke belakang">
  ...
  <Accordion title="Permintaan historis menggunakan konten tersimpan">
    Jangan gabungkan `snapshotAsOf` dengan opsi yang dapat menjangkau web secara langsung atau meluas ke halaman lain.
...
* Menjalankan evaluasi terhadap batas konten yang dapat diulang.
* Membandingkan versi dokumentasi, harga, kebijakan, atau filings yang lebih lama.