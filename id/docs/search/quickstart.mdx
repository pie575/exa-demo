...
description: "Cari di web dengan bahasa alami dan dapatkan konten halaman yang bersih dan relevan dalam satu permintaan."
keywords: ["search api", "web search", "semantic search", "retrieval", "rag", "highlights"]
boost: 6
---

Exa Search menerima query dalam bahasa alami dan mengembalikan hasil web yang sudah diperingkat beserta konten halaman yang bersih.

## Buat permintaan pertama Anda {#make-your-first-request}

Mulailah dengan `query` dalam bahasa alami dan `contents: { highlights: true }`, yang mengembalikan kutipan dengan panjang menyesuaikan relevance tiap hasil. Field lain mengatur cara Exa melakukan search dan apa saja yang disertakan pada setiap hasil; bagian selanjutnya di halaman ini membahas field yang benar-benar akan Anda pakai.
...
Secara default, search mengembalikan maksimal 10 hasil. Atur `numResults` untuk meminta hingga 100 hasil; hasil yang dikembalikan bisa lebih sedikit bila halaman relevan yang tersedia memang lebih sedikit. Search tidak mendukung penomoran halaman.
...
Hasil diurutkan berdasarkan relevance. Setiap hasil menyertakan metadata seperti judul, URL, dan tanggal publikasi, ditambah apa pun yang Anda minta lewat `contents`.
...
Query boleh bersifat luas dan eksploratif. `"Latest news on EU battery policy"` memberi Exa maksud yang cukup jelas untuk menemukan halaman yang relevan, sedangkan `"news"` tidak. Jika jenis sumber penting, sebutkan dalam query:
...
`type` menentukan search mode, yang masing-masing disetel untuk keseimbangan kecepatan, kedalaman pencarian, dan synthesis yang berbeda. `auto` adalah default dan cocok untuk sebagian besar searches.

| Type             | Gunakan saat                                                            |
| ---------------- | ----------------------------------------------------------------------- |
| `auto`           | Anda ingin keseimbangan default terbaik antara kualitas dan kecepatan   |
...
| `deep`           | Tugas membutuhkan pencarian multi-langkah dan synthesis yang lebih kuat |
...
<Tip>
  ...
  daftar, dan enrichment multi-hop. Agent memiliki komputasi lebih besar per run dan mengembalikan hasil
  terstruktur yang grounded.
</Tip>

## Bentuk output {#output-shapes}
...
<Tabs>
  <Tab title="Kutipan">
    Kutipan mengembalikan bagian teks yang paling relevan dengan query Anda. Kutipan memberi model dan agent
    evidence yang dibutuhkan tanpa memenuhi jendela konteks dengan bagian halaman yang tidak relevan.

    Ini adalah bentuk output yang disarankan untuk sebagian besar tugas.

    Mulailah cukup dengan `highlights: true`. Exa akan menggunakan query untuk memilih jumlah konten
    ...
    Lihat [Highlights](/id/docs/search/highlights) untuk Dynamic Highlights dan instruksi tentang kapan sebaiknya mengaktifkannya.
  </Tab>

  <Tab title="Teks penuh">
    Teks penuh mengembalikan isi halaman yang sudah bersih. Gunakan ini bila tugas Anda bergantung pada konteks yang lebih luas,
...
Pilih satu tampilan konten per permintaan. Meminta kutipan dan teks sekaligus akan mengembalikan sekaligus menagih dua tampilan dari halaman yang sama. `summary` adalah opsi ketiga, tetapi opsi ini menambah satu pemanggilan model bahasa untuk setiap hasil.

<Warning>
  `/search` dan `/contents` menerima opsi konten yang sama, tetapi di tempat yang berbeda:

  * **`/search`** menempatkan `highlights`, `text`, dan `summary` di dalam objek `contents`:
    `"contents": { "highlights": true }`
  * **`/contents`** tidak memiliki wrapper `contents`. Body-nya adalah opsi konten itu sendiri, sehingga
    field yang sama berada di tingkat teratas bersama `urls`: `"urls": [...], "highlights": true`
</Warning>
...
Halaman yang telah diberi peringkat tetap berada di `results`. Nilai yang dihasilkan dikembalikan dalam `output.content`, lengkap dengan sources dan confidence per field di `output.grounding`.
...
Gunakan `systemPrompt` untuk instruksi seperti source preferences atau penekanan; gunakan `outputSchema` untuk menentukan bentuk response. Di Python, gunakan `system_prompt` dan `output_schema`.

<Note>
  Jaga agar schema objek tetap sederhana: schema hanya mendukung hingga 2 tingkat nesting dan 10 properti. Jangan tambahkan
...
Filter adalah batasan ketat: gunakan filter jika hasil di luar batasan tersebut memang tidak berguna bagi Anda, dan biarkan preferensi sumber yang lebih longgar tetap berada di dalam teks query. [API reference](/id/docs/reference/search) memuat daftar lengkapnya.
...
Cantumkan path di dalam filter, jangan mengulanginya sebagai operator `site:` di dalam query.

### Kecualikan domain atau path {#exclude-domains-or-paths}

`excludeDomains` menghapus hasil dari domain atau path tertentu. Parameter ini mendukung prefiks path dan wildcard subdomain yang sama seperti `includeDomains`. Gunakan parameter ini ketika sources tersebut membuat hasil menjadi tidak berguna, bukan sekadar untuk menyatakan preferensi.
...
`contents.maxAgeHours` mengatur seberapa baru konten yang diekstraksi dari setiap hasil harus dibuat. Field ini tidak memfilter hasil berdasarkan tanggal publikasi.

| Nilai                  | Perilaku                                                                            |
| ---------------------- | ----------------------------------------------------------------------------------- |
| Dikosongkan            | Gunakan konten cache jika tersedia dan ambil halaman bila diperlukan                |
| Bilangan bulat positif | Gunakan konten cache jika lebih baru dari jumlah jam ini; jika tidak, ambil halaman |
| `0`                    | Selalu ambil konten terbaru                                                         |
| `-1`                   | Hanya gunakan konten cache                                                          |

Sebagian besar pencarian sebaiknya mengosongkan field ini. Setel field ini ketika konten halaman yang usang tidak dapat digunakan, misalnya untuk harga, ketersediaan, atau halaman yang sering berubah.
...
<Columns cols={2}>
  <Card title="Praktik terbaik" icon="sparkles" href="/id/docs/search/best-practices" cta="Baca panduan" arrow="true">
    Anggaran token, kebaruan konten, output terstruktur, dan system prompt.
  </Card>
  ...
  <Card title="Contents" icon="file-text" href="/id/docs/contents/quickstart" cta="Buka panduan" arrow="true">
    Anda sudah punya URL-nya dan ingin teks bersih, kutipan, atau ringkasan.
  </Card>

  <Card title="Exa Agent" icon="bot" href="/id/docs/agent/quickstart" cta="Buka panduan" arrow="true">
    Anda perlu research jangka panjang, list building, atau enrichment.
  </Card>
</Columns>