---
title: "Snowflake"
description: "Gunakan Exa search dan pengambilan konten dari stored procedure Snowflake dan Cortex Agents."
icon: "/docs/images/sidebar-icons/snowflake.svg"
---

Gunakan Search API Exa langsung dari Snowflake untuk memperkaya data Anda dengan intelijen web terkini — tanpa perlu keluar dari SQL.

Integrasi ini memanfaatkan fitur [External Access](https://docs.snowflake.com/en/developer-guide/external-network-access/external-network-access-overview) Snowflake untuk memanggil Exa API secara aman dari stored procedure, sehingga memungkinkan dua workflow utama:
...
* Sebuah **secret** yang menyimpan API key Anda (terenkripsi, tidak terlihat di SQL)
* Sebuah **external access integration** yang menyatukan rule dan secret tersebut

## Stored procedure {#stored-procedures}

### EXA_SEARCH {#exa_search}

Melakukan pencarian di web menggunakan Search API dari Exa. Mengembalikan hasil berisi judul, URL, nilai, serta secara opsional teks/kutipan/ringkasan.
...
| `include_domains`      | `STRING`  | `NULL`   | Domain yang disertakan, dipisahkan koma                                                      |
| `exclude_domains`      | `STRING`  | `NULL`   | Domain yang dikecualikan, dipisahkan koma                                                    |
| `start_published_date` | `STRING`  | `NULL`   | Batas bawah tanggal ISO 8601                                                                 |
| `end_published_date`   | `STRING`  | `NULL`   | Batas atas tanggal ISO 8601                                                                  |
| `include_text`         | `STRING`  | `NULL`   | Frasa yang harus muncul pada hasil                                                           |
...
Saat meng-enrich tabel berukuran besar, memanggil Exa secara berurutan untuk setiap baris akan lambat. Anda dapat memparalelkan permintaan menggunakan `concurrent.futures` dari Python di dalam satu stored procedure, sehingga beberapa panggilan API Exa dikirim sekaligus.
...
Untuk job enrichment yang berulang, bungkus prosedur tersebut dalam sebuah [Snowflake Task](https://docs.snowflake.com/en/user-guide/tasks-intro):
...
Setelah dibuat, buka Snowflake Intelligence dan coba ajukan pertanyaan berikut ke agent Anda:
...
| **Secret**                      | API key disimpan dalam bentuk terenkripsi, tidak terlihat di kueri SQL |
