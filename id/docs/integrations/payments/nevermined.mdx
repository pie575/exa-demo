...
description: "Pembayaran agent otonom untuk Exa melalui card delegation x402 Nevermined. Pembelian senilai 7 USD menyediakan atau mengisi ulang Exa API key dengan credits senilai 7 USD."
icon: "/docs/images/sidebar-icons/nevermined.svg"
---

Agent membayar Exa dengan kartu kredit melalui skema [x402 card-delegation](https://nevermined.ai/docs/specs/x402-card-delegation) dari [Nevermined](https://nevermined.ai). Setiap **pembelian $7** menghasilkan satu Exa API key dengan **credits Exa senilai $7**.

<Info>
  Gunakan plan ID Nevermined berikut:<br />`27800462147494506865542649899724877617306579171265399959488097895839186996870`<br />Plan ini berjalan di lingkungan live Nevermined (API key berawalan live). Pembelian ini untuk credits API, bukan untuk satu permintaan search.
</Info>

Untuk payer Nevermined yang baru pertama kali, `POST /team-management/nevermined/purchase-key` akan menyediakan Exa API key baru dan menambahkan credits senilai $7. Jika key habis, cetak x402 token baru dengan delegation yang sama lalu panggil endpoint yang sama kembali. Exa akan mengembalikan API key yang sama dengan tambahan credits senilai $7.
...
* **Biaya:** $7 per pembelian, dibebankan ke kartu di balik delegation yang dirujuk oleh x402 token.
...
4. **Agent melakukan POST token ke endpoint di atas** melalui header `payment-signature` dan menerima Exa API key dari response.
...
Credits dipakai sesuai harga standar Exa API. Dengan tarif saat ini, $7 credits kira-kira mencakup:
...
Exa mengembalikan **`HTTP 402`** pada endpoint API biasa setelah credits pada API key habis:
