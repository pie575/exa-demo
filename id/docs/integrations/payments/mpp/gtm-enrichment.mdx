---
title: "Cookbook Enrichment GTM dengan Tempo MPP"
sidebarTitle: "GTM enrichment with MPP"
...
API key, dan tanpa Harga berbasis kursi: isi dana wallet dengan USDC.e dan bayar seiring
Anda meng-enrich lead atau perusahaan.
...
1. Menggunakan `/search` Exa dengan `type: "deep"` dan `outputSchema` untuk menemukan
...
3. Menghasilkan satu catatan enrichment dalam bentuk CSV atau JSON per input.

Pola ini cocok untuk enrichment daftar prospek, riset akun, dan
personalisasi outbound. Karena tersusun dari pemanggilan `/search` + `/contents` yang terpisah,
...
* Cara yang aman untuk memuat private key wallet saat runtime (lihat di bawah; jangan pernah
  melakukan commit key tersebut atau mengeksposnya dalam kode sumber).
* `mppx` (TypeScript) atau `pympp` (Python) sudah terinstal.

<Info>
  Untuk penyiapan lewat baris perintah yang tidak memerlukan private key mentah, gunakan [Tempo Wallet CLI](/id/docs/integrations/payments/mpp/quickstart#pay-from-the-command-line). `tempo wallet login` akan membuat atau menghubungkan wallet dan bisa disertai MPP Credits gratis untuk pendaftaran baru.
</Info>

## Pengaturan MPP {#mpp-setup}
...
Jangan pernah menuliskan private key langsung di dalam kode. Contoh di bawah ini membaca `WALLET_PRIVATE_KEY` dari
environment runtime Anda, hanya untuk pengembangan lokal. Di production, muat nilai tersebut dari
...
Dari sebuah daftar nama perusahaan, cari halaman masing-masing perusahaan lalu ekstrak
detail terstruktur.
...
<Note>
  ...
  membentuk response. Deep search dikenakan biaya $0.012 per permintaan, dan
...
permintaan search. Exa mengembalikan objek `output` yang bentuknya mengikuti schema Anda.
...
<Note>
  `outputSchema` bekerja paling baik dengan search type `deep-lite` atau `deep`. Field ini menambahkan satu
...
* **Tangani response `402`.** MPP SDK mencoba ulang secara otomatis, tetapi
  client kustom sebaiknya mencoba ulang saat menerima `402` menggunakan challenge `WWW-Authenticate: Payment`.
...
* **Perhatikan batas 10 hasil.** MPP search membatasi `numResults` maksimal 10.
...
<AccordionGroup>
  ...
  <Accordion title="Bisakah saya menggabungkan MPP dan Exa API key dalam satu permintaan yang sama?">
    Tidak. Jika sebuah permintaan menyertakan `x-api-key` atau `Authorization: Bearer`,
    alur API key yang diutamakan dan MPP dilewati.
  </Accordion>
  ...
  <Accordion title="Apa yang terjadi jika settlement MPP gagal?">
    ...
    hasil apa pun. Client Anda dapat mencoba lagi dengan payment baru. Tidak ada hasil yang dikembalikan
    sampai settlement berhasil.
  </Accordion>

  <Accordion title="Apakah saya perlu wallet Tempo terpisah untuk setiap environment?">
    Anda bisa memakai wallet yang sama, tetapi kami menyarankan wallet terpisah untuk
