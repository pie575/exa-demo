...
Exa mendukung x402 pada dua endpoint: **`/search`** dan **`/contents`**. Ketika Anda mengirim permintaan tanpa API key atau header payment, Exa merespons dengan `402` beserta header `PAYMENT-REQUIRED` yang memuat detail harga dan jaringan payment yang didukung. Klien Anda menandatangani payment USDC, mengirim ulang permintaan dengan header `PAYMENT-SIGNATURE`, lalu menerima hasilnya setelah settlement terkonfirmasi on-chain.

Pendekatan ini sangat cocok untuk **AI agents** yang perlu membayar web search secara mandiri tanpa credential yang sudah provisioned sebelumnya.

<Info>
  x402 dan akses API key bersifat independen. Jika permintaan Anda menyertakan header `x-api-key` atau `Authorization: Bearer`, alur API key billing yang biasa akan digunakan dan x402 dilewati sepenuhnya.
</Info>
...
<Frame>
  <img src="/docs/images/integrations/payments/x402/payment-flow.png" alt="Diagram sekuens x402 payment flow: Klien mengirim permintaan ke server, menerima 402 dengan header PAYMENT-REQUIRED, membuat payload payment, mencoba ulang dengan PAYMENT-SIGNATURE, server memverifikasi melalui facilitator, menjalankan pekerjaan, menyelesaikan transaksi on-chain, lalu mengembalikan 200 dengan hasil dan PAYMENT-RESPONSE" width="4224" height="2720" data-path="images/integrations/payments/x402/payment-flow.png" />
</Frame>
...
`amount` dinyatakan dalam satuan atomik USDC (6 desimal), jadi `"7000"` = $0,007.
Klien dapat membayar menggunakan entri `accepts` mana pun yang diiklankan dan didukungnya. Entri Solana menyertakan field yang disediakan facilitator seperti `extra.feePayer`; gunakan entri yang persis sama dari header `PAYMENT-REQUIRED` saat menyusun payment.

<div id="step-2-pay-and-retry">
  ### Langkah 2: Bayar dan coba lagi
</div>

Tanda tangani payment dengan wallet Anda, lalu kirim ulang permintaan dengan header `PAYMENT-SIGNATURE` yang berisi payload payment Anda dalam format base64. SDK klien x402 menangani hal ini secara otomatis.

<div id="step-3-settlement">
  ### Langkah 3: Settlement
</div>

Exa memverifikasi signature payment Anda dengan facilitator, lalu memulai settlement on-chain **secara paralel** dengan pemrosesan permintaan Anda. Respons ditahan sampai settlement terkonfirmasi. Jika berhasil, Anda akan menerima:

* HTTP `200` berisi hasil Anda
* Header `PAYMENT-RESPONSE` yang memuat settlement receipt (dienkode base64), termasuk hash transaksi on-chain

Jika settlement gagal, Anda akan mendapatkan `402` disertai `PAYMENT-RESPONSE` (detail error) dan `PAYMENT-REQUIRED` (agar Anda dapat mencoba lagi).

<div id="pricing">
  ## Harga
</div>

x402 menggunakan harga paket yang sama seperti billing API key. Harga dihitung di muka berdasarkan parameter permintaan Anda (bukan berdasarkan hasil aktual yang dikembalikan).
...
<Note>
  cURL tidak memerlukan instalasi apa pun, tetapi Anda harus menangani challenge 402 dan penandatanganan payment secara manual. Untuk penggunaan produksi, pendekatan SDK lebih disarankan.
</Note>
...
<Info>
  cURL mengharuskan penandatanganan payment secara manual. Untuk produksi, gunakan SDK JavaScript atau Python yang menangani seluruh alur 402 &gt; tanda tangan &gt; coba lagi secara otomatis.
</Info>
...
Exa mencantumkan semua jaringan yang saat ini didukung dalam array `accepts`. Pilih entri yang sesuai dengan wallet Anda dan skema klien x402 yang terdaftar.
...
x402 memiliki rate limiting tersendiri yang terpisah dari limit API key:

| Limit                                  | Ambang batas     | Jendela  |
| -------------------------------------- | ---------------- | -------- |
| Discovery request tanpa bayar (per IP) | 5 request        | 60 detik |
| Request berbayar (per wallet)          | 10 request/detik | 1 detik  |

Setelah 5 discovery request `402` tanpa autentikasi dari IP yang sama dalam 60 detik, request selanjutnya akan mengembalikan `429 Too Many Requests`. Request berbayar yang berhasil akan mengurangi penghitung tersebut.

QPS per wallet diberlakukan untuk seluruh request berbayar dari alamat wallet yang sama.
...
| ------------------ | -------------------------------------------- | ----------------------------------------------------------------------- |
| `PAYMENT-REQUIRED` | Respons `402`                                | Objek `PaymentRequired` dalam Base64 berisi harga dan instruksi payment |
| `PAYMENT-RESPONSE` | `200` atau `402` (setelah percobaan payment) | Hasil settlement dalam Base64 berisi hash transaksi atau error          |
...
| `429`  | `X402_TOO_MANY_UNPAID`     | Terlalu banyak permintaan discovery tanpa payment dari IP ini                       |
| `429`  | `X402_WALLET_RATE_LIMITED` | Wallet melampaui 10 permintaan/detik                                                   |
| `500`  | `X402_INTERNAL_ERROR`      | Kesalahan sisi server saat membuat persyaratan payment                              |
...
<AccordionGroup>
  <Accordion title="Bisakah saya menggunakan x402 dan API key secara bersamaan?">
    Jika permintaan Anda menyertakan header `x-api-key` atau token `Authorization: Bearer`, alur API key yang diprioritaskan dan x402 dilewati. Keduanya tidak bisa digabungkan—hanya salah satu yang berlaku di setiap permintaan.
  </Accordion>

  <Accordion title="Apa yang terjadi jika settlement gagal setelah permintaan saya diproses?">
    Respons Anda akan diblokir. Anda menerima `402` beserta `PAYMENT-RESPONSE` (berisi error) dan `PAYMENT-REQUIRED` (agar klien Anda dapat mencoba ulang). Tidak ada hasil yang dikembalikan sampai settlement berhasil.
  </Accordion>

  <Accordion title="Mengapa numResults dibatasi hingga 10?">
    Permintaan x402 menerapkan batas maksimum 10 hasil per pencarian. Jika Anda membutuhkan lebih banyak, gunakan alur API key dengan paket berbayar.
  </Accordion>

  <Accordion title="Wallet apa saja yang didukung?">
    Wallet apa pun yang kompatibel dengan EVM dan dapat menandatangani typed data EIP-712 di Base, atau wallet Solana yang didukung oleh klien x402 SVM untuk `solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp`. SDK x402 mendukung `viem`, `ethers`, signer Coinbase Wallet, dan signer Solana SVM. Untuk AI agents berbasis EVM, [Coinbase Agentic Wallets](https://docs.cdp.coinbase.com/agent-kit/core-concepts/wallet-management) menyediakan manajemen key yang terisolasi dengan TEE sehingga agent Anda tidak pernah menangani private keys mentah secara langsung.
  </Accordion>
</AccordionGroup>
