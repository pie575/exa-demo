...
<Warning>
  **Beberapa akun Google:** add-on harus dijalankan dengan akun Google pertama (default) di profil browser Anda. Jika Anda login ke beberapa akun sekaligus, Anda mungkin tidak dapat menyimpan atau memuat API key Anda. Untuk mengatasinya, buka Sheets di jendela incognito dengan hanya satu akun, atau keluar dari akun lainnya agar akun yang Anda inginkan menjadi akun default. [Pelajari selengkapnya](https://developers.google.com/apps-script/guides/projects#fix_issues_with_multiple_google_accounts).
</Warning>
...
* **Exa Agent** untuk tabel penuh dan tugas multi-sel
...
Exa Agent memungkinkan Anda menggunakan Exa di berbagai sel dalam Google Sheets.

Gunakan Exa Agent saat Anda ingin:
...
Exa menelusuri web dan menuliskan tabelnya ke dalam sheet Anda.

Secara bawaan, tabel dimulai dari sel yang sedang dipilih. Anda dapat memilih sel awal lain di **More options**.
...
1. Pilih sel-sel kosong di lembar kerja Anda.
2. Buka **Exa Agent**.
3. Pilih **Fill cells**.
4. Klik **Fill selected cells**.

Exa akan melihat tabel di sekitar bagian yang Anda pilih dan mengisi sel-sel yang kosong.

Pilih sel kosong pada tabel yang sudah memiliki header yang jelas sebelum menggunakan **Fill cells**.

Contoh:

| Perusahaan | Situs Web                                | CEO           | Kantor Pusat  |
| ---------- | ---------------------------------------- | ------------- | ------------- |
| Apple      | [https://apple.com](https://apple.com)   |               |               |
| Google     | [https://google.com](https://google.com) | Sundar Pichai | Mountain View |

Pilih sel kosong pada baris Apple, lalu klik **Fill selected cells**. Exa memakai nama perusahaan dan baris-baris di dekatnya sebagai konteks.
...
Exa menggunakan baris yang sudah ada sebagai contoh, mempertahankan kolom yang sama, dan menghindari pengulangan item yang sudah ada di tabel.

<div id="exa">
  ## `=EXA(...)`
</div>

Gunakan `=EXA(...)` bila Anda ingin satu jawaban dalam satu sel. Formula ini melakukan search di web, membaca hasil teratas, lalu mengembalikan jawaban yang ringkas.
...
Argumen kedua adalah konteksnya. Anda dapat menarik rumus ke bawah pada satu kolom untuk menjalankannya di banyak baris.
...
| `returnRawJson`    | Tidak | `FALSE`  | Jika `TRUE` dan `outputSchema` diisi, mengembalikan JSON lengkap alih-alih hanya mengekstrak nilainya.  |
...
| `category`           | Tidak | `""`     | Filter berdasarkan jenis: `"company"`, `"publication"`, `"news"`, `"personal site"`, `"financial report"`, `"people"`.                                             |
...
Mencari URL yang mirip dengan URL acuan, dengan filter domain dan teks yang bersifat opsional.
...
* memperbarui sel terpilih yang menggunakan formula Exa
...
* Permintaan ke Exa API dihitung dalam kuota usage Anda. Gunakan **Batch → Convert to values** untuk membekukan hasil agar formula tidak menghitung ulang.
* Add-on akan otomatis mencoba ulang hingga 3 kali dengan exponential backoff saat terkena rate limit (HTTP 429).
* Mulailah dengan batch kecil (10–20 baris) sebelum meningkatkannya hingga ratusan baris.
