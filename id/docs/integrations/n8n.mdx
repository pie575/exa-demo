...
[Node Exa resmi untuk n8n](https://github.com/exa-labs/n8n-integration) menambahkan web search, extraction konten, grounded answer, dan Exa Agent run ke dalam workflow visual. Gunakan sebagai langkah workflow biasa atau hubungkan ke AI Agent n8n sebagai tool.
...
<Steps>
  <Step title="Tambahkan community node">
    ...
    Node ini memerlukan n8n 1.60 atau lebih baru dan Node.js 20.15 atau lebih baru.
  </Step>
...
5. Pilih format respons:
   * **Results** untuk halaman yang sudah diperingkat
   * **Text** untuk jawaban hasil sintesis
   * **Structured** untuk JSON yang sesuai dengan schema Anda
6. Jalankan node tersebut, lalu teruskan output-nya ke langkah workflow berikutnya.

Search juga dapat mengembalikan teks, highlights, summaries, tautan, dan gambar dari setiap hasil. Filter domain, tanggal publikasi, kategori, `maxAgeHours`, dan crawling subhalaman tersedia pada kolom opsional node.
...
| Sumber daya  | Operasi                                                                                                                                            |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Search**   | Mencari di web menggunakan `auto`, `instant`, `fast`, `deep-lite`, `deep`, atau `deep-reasoning`, dengan synthesis dan structured output opsional. |
| **Contents** | Mengambil teks yang sudah dibersihkan, highlights, summaries, tautan, dan gambar untuk daftar URL.                                                 |
| **Answer**   | Menghasilkan grounded answer dengan citations dan structured output opsional.                                                                      |
| **Agent**    | Membuat, memeriksa, mendaftar, melakukan streaming, poll, dan membatalkan Agent run multi-langkah.                                                 |
...
Search dan Answer berfungsi baik sebagai alat grounding. Gunakan resource Agent saat tugas memerlukan riset multi-langkah, list building, enrichment terstruktur, atau data premium [Exa Connect](/id/docs/agent/connect/overview).
...
* **Stream** untuk menjaga satu koneksi server-sent events tetap terbuka sampai run selesai
* **Poll** untuk memeriksa run secara berkala
...
<AccordionGroup>
  <Accordion title="Node Exa tidak muncul di pemilih node">
    Minta pemilik instance untuk memasang package komunitas terverifikasi `n8n-nodes-exa-official`. Ketersediaan node komunitas bisa bergantung pada cara instance n8n Anda dihosting.
  </Accordion>
...
<Columns cols={3}>
  <Card title="Node Exa resmi" icon="github" href="https://github.com/exa-labs/n8n-integration" cta="Lihat repository" arrow="true">
    Tinjau operasi, kompatibilitas, dan kode sumber terkini.
  </Card>
