...
title: "n8n"
sidebarTitle: "n8n"
description: "Gunakan Exa search dan contents di dalam workflow n8n."
icon: "/docs/images/sidebar-icons/n8n.svg"
---

[Node Exa resmi untuk n8n](https://github.com/exa-labs/n8n-integration) menambahkan web search, ekstraksi konten, jawaban grounded, dan Exa Agent run ke dalam workflow visual. Gunakan node ini sebagai langkah workflow biasa atau hubungkan ke n8n AI Agent sebagai tool.
...
## Menjalankan pencarian {#run-a-search}
...
Search juga dapat mengembalikan teks, kutipan, ringkasan, tautan, dan gambar dari setiap hasil. Filter domain, tanggal publikasi, kategori, `maxAgeHours`, dan crawling subhalaman tersedia pada field opsional di node tersebut.
...
| **Contents** | Mengambil teks bersih, kutipan, ringkasan, tautan, dan gambar untuk sekumpulan URL.                                                                |
| **Answer**   | Menghasilkan jawaban yang grounded beserta sitasi dan output terstruktur opsional.                                                                  |
| **Agent**    | Membuat, memeriksa, menampilkan daftar, melakukan stream, poll, dan membatalkan Agent run multi-langkah.                                           |
...
Search dan Answer bekerja dengan baik sebagai tool grounding. Gunakan sumber daya Agent jika tugas tersebut memerlukan research multi-langkah, list building, enrichment terstruktur, atau data premium [Exa Connect](/id/docs/agent/connect/overview).
...
Untuk workflow yang berjalan lama atau asinkron, matikan **Wait for Completion**, simpan `id` run yang dikembalikan, lalu gunakan **Get Run** nanti. Run akan tetap berjalan di Exa setelah langkah n8n selesai.

## Pemecahan Masalah {#troubleshooting}

<AccordionGroup>
  <Accordion title="Node Exa tidak muncul di node picker">
    Minta pemilik instance untuk menginstal package komunitas terverifikasi `n8n-nodes-exa-official`. Ketersediaan node komunitas dapat bergantung pada cara instance n8n Anda dihosting.
  </Accordion>

  <Accordion title="Credential Exa ditolak">
    Pastikan credential yang dipilih berisi key aktif dari [Exa dashboard](https://dashboard.exa.ai/api-keys) dan key tersebut masih memiliki sisa credits.
  </Accordion>
...
<Columns cols={3}>
  <Card title="Node Exa resmi" icon="github" href="https://github.com/exa-labs/n8n-integration" cta="Lihat repositori" arrow="true">
    Tinjau operasi, kompatibilitas, dan source yang tersedia saat ini.
  </Card>
  ...
  <Card title="Praktik terbaik search" icon="search" href="/id/docs/search/best-practices" cta="Baca panduan" arrow="true">
    Tulis query yang lebih baik dan pilih search mode yang tepat.
  </Card>
</Columns>