...
[Tool use](https://docs.anthropic.com/en/docs/build-with-claude/tool-use) Claude memungkinkan model memanggil fungsi yang Anda definisikan di dalam kode Anda. SDK Exa sudah menyertakan tool web search dan pembacaan halaman yang siap pakai untuk Anthropic, jadi Anda tidak perlu menulis sendiri schema tool, mengurai blok `tool_use`, atau memformat hasil Exa secara manual.
...
<Steps>
  ...
  <Step title="Tambahkan tool Exa ke tool loop Anda">
    ...
    `web_search` melakukan search di web untuk halaman yang belum pernah dilihat model; `get_contents` membaca halaman yang URL-nya sudah dimiliki model, baik dari search sebelumnya maupun dari user. Daftarkan salah satu atau keduanya.
    ...
    Contoh ini hanya satu putaran agar ringkas. Agent sungguhan selalu menyertakan `tools` pada setiap permintaan dan mengulang langkah handler sampai model membalas tanpa blok `tool_use` — begitulah hasil search berlanjut menjadi pembacaan halaman.

    Memanggil factory tanpa argumen akan memakai nilai default yang direkomendasikan Exa: `type="auto"` dengan `contents={"highlights": True}` untuk search. Highlights mengembalikan excerpt yang relevan dengan query — bukan membatasi teks halaman hingga 10.000 karakter. Factory contents mengembalikan teks halaman; limit 10.000 karakter pada SDK hanya berlaku untuk `text`, dan hanya jika Anda tidak menyertakan `max_characters`.
  </Step>
</Steps>
...
Sementara itu, `name` (dengan nilai bawaan `"web_search"` dan `"get_contents"`) dan `description` menimpa definisi tool yang dilihat model. Anthropic mewajibkan nama tool unik, jadi dengan nama kustom, tool Exa bisa berjalan berdampingan dengan server tool bawaan Anthropic `web_search_20250305`, yang sudah memakai nama `web_search`:
...
`handle_tool_use` merespons setiap blok `tool_use` dalam pesan: blok yang menyebut tool yang tidak dikenali akan mendapat hasil `Error: unknown tool "<name>"` alih-alih diabaikan, sehingga permintaan lanjutan tidak pernah melewatkan hasil tool yang diperlukan. Jika Anda menjalankan tool sendiri berdampingan dengan tool Exa, ganti hasil error tersebut dengan hasil milik Anda sebelum permintaan berikutnya.

<div id="writing-the-loop-by-hand">
  ## Menulis loop secara manual
</div>

Jika Anda lebih suka mengendalikan sendiri schema dan eksekusi tool-nya, definisikan tool tersebut dan proses blok `tool_use` secara manual. `exa.tools.web_search()` dan `exa.tools.get_contents()` memberi Anda spesifikasi tool yang netral terhadap provider (lengkap dengan metode `run`) untuk loop buatan sendiri, atau Anda bisa menulis semuanya dari nol:
