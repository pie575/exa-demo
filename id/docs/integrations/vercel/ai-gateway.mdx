...
Saat Anda men-deploy aplikasi di Vercel, Anda dapat menggunakan `VERCEL_OIDC_TOKEN` yang tersedia secara otomatis. Lihat [dokumentasi authentication dan BYOK](https://vercel.com/docs/ai-gateway/authentication-and-byok) milik Vercel.
...
Gunakan `streamText` untuk memproses teks yang dihasilkan dan event tool search begitu diterima:
...
| `userLocation`                                         | Kode negara ISO dua huruf untuk search berbasis lokasi.                    |
...
| `contents.livecrawlTimeout`                            | Menetapkan timeout livecrawl.                                              |
...
Lihat [referensi Exa web search](https://vercel.com/docs/ai-gateway/models-and-providers/web-search) dari Vercel untuk daftar lengkap parameter dan perilakunya.

<div id="vercel-eve-agents">
  ## Vercel eve agents
</div>

Agent yang dibangun dengan [eve](https://eve.dev) otomatis mendapatkan tool bawaan `web_search`, dan model AI Gateway menjalankannya di Exa secara default, tanpa perlu konfigurasi atau Exa API key. Untuk menetapkan provider secara eksplisit, export dari `agent/tools/web_search.ts`:
...
Setelah itu, Vercel menagih permintaan melalui AI Gateway sesuai tarif yang tercantum di [dokumentasi web search](https://vercel.com/docs/ai-gateway/models-and-providers/web-search) Vercel.

<Note>
  Saat ini integrasi ini mendukung mode search standar dan kontrol extraction konten dari Exa. Mode deep synthesis dan summaries yang dihasilkan belum tersedia.
</Note>
