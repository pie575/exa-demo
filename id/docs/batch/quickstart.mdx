...
title: "Batch API"
sidebarTitle: "Batch"
description: "Jalankan permintaan Exa API secara asinkron dalam batch."
boost: 4
---
...
Batch API memungkinkan Anda mengirim banyak permintaan Exa API sekaligus dan mengambil hasilnya nanti dalam bentuk file JSONL. Alih-alih mengirim ribuan permintaan satu per satu sambil mengelola sendiri rate limit dan percobaan ulang, Anda cukup mengirim satu batch, poll status-nya, lalu mengunduh seluruh hasil dalam satu file.

Gunakan fitur ini untuk enrichment offline, backfill, atau pekerjaan lain yang tidak memerlukan response langsung. Schema lengkap untuk permintaan dan response tersedia di [API reference](/id/docs/reference/batches/create-a-batch).
...
Setiap item memerlukan `customId` yang unik dalam satu batch. `customId` yang sama akan dikembalikan dalam file hasil sehingga Anda dapat memetakan baris output kembali ke data input Anda.
...
Ketika batch selesai, `resultsUrl` berisi URL unduhan untuk file hasil JSONL, dan `expiresAt` disetel ke akhir masa penyimpanan hasil.
...
## Menampilkan daftar batch {#list-batches}
...
Response menggunakan paginasi berbasis cursor: `data` berisi hingga `limit` batch, dan jika `hasMore` bernilai `true`, kirimkan `nextCursor` sebagai query parameter `cursor` untuk mengambil halaman berikutnya.
...
`completed` adalah satu-satunya nilai yang didukung; nilai lain akan mengembalikan error. Listing yang berstatus `completed` diurutkan berdasarkan waktu kedaluwarsa dan menggunakan cursor tersendiri, jadi tetap kirimkan `status=completed` pada setiap halaman — cursor untuk `completed` dan cursor tanpa filter tidak dapat saling menggantikan.
...
Setiap baris JSONL berisi `customId` asli serta salah satu dari `response` atau `error`:
