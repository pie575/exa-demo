...
Gunakan untuk enrichment offline, pengisian data lampau, atau pekerjaan lain yang tidak membutuhkan respons seketika. Schema lengkap permintaan dan respons tersedia di [referensi API](/id/docs/reference/batches/create-a-batch).
...
| `cancelling`  | Pembatalan telah diminta dan pekerjaan yang sedang berjalan sedang dituntaskan |
| `cancelled`   | Batch telah dibatalkan                                                         |
| `expired`     | Hasil sudah tidak tersedia lagi                                                |

Saat batch selesai, `resultsUrl` berisi URL unduhan untuk berkas hasil JSONL, dan `expiresAt` disetel ke akhir masa penyimpanan hasil.
...
Respons menggunakan paginasi berbasis cursor: `data` memuat batch sebanyak-banyaknya `limit`, dan ketika `hasMore` bernilai `true`, kirimkan `nextCursor` sebagai parameter query `cursor` untuk mengambil halaman berikutnya.

Kirimkan `status=completed` untuk hanya menampilkan batch yang sudah selesai:
...
`completed` adalah satu-satunya nilai yang didukung; nilai lain akan mengembalikan error. Daftar batch yang sudah selesai diurutkan berdasarkan waktu kedaluwarsa dan menggunakan cursor tersendiri, jadi selalu kirimkan `status=completed` di setiap halaman — cursor untuk hasil completed dan hasil tanpa filter tidak bisa saling dipertukarkan.
...
Setiap baris JSONL berisi `customId` asli dan salah satu dari `response` atau `error`:
