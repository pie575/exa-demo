---
title: "Memverifikasi signature"
description: "Pelajari cara memverifikasi signature webhook secara aman untuk memastikan permintaan berasal dari Exa"
timestamp: false
---

Saat menerima webhook dari Exa, Anda sebaiknya memverifikasi bahwa webhook tersebut memang berasal dari kami untuk memastikan integritas dan keaslian data. Exa menandatangani semua payload webhook dengan secret key yang unik untuk setiap endpoint webhook Anda.
...
<CodeGroup>
  ...
  public class WebhookTest {
      ...
      */
      ...
      /**
      * Perbandingan string timing-safe untuk mencegah serangan timing.
      */
      ...
      public static void main(String[] args) {
          ...
          try {
              ...
              if (isValid) {
                  ...
                  // Di sini Anda akan mem-parse JSON dan menangani event webhook
...
Mengikuti praktik berikut akan membantu memastikan implementasi webhook Anda aman dan tangguh:
...
* **Gunakan Perbandingan Timing-Safe** - Saat membandingkan signature, gunakan fungsi seperti `hmac.compare_digest()` di Python atau `crypto.timingSafeEqual()` di Node.js untuk mencegah serangan berbasis waktu (timing attack).

* **Periksa Kebaruan Timestamp** - Pertimbangkan untuk menolak webhook dengan timestamp yang terlalu lama (misalnya, lebih dari 5 menit) guna mencegah serangan replay.

* **Simpan Secret dengan Aman** - Simpan webhook secret Anda di variabel lingkungan atau sistem pengelolaan secret yang aman. Jangan pernah menuliskannya langsung di dalam kode aplikasi Anda. **Penting**: Webhook secret hanya dikembalikan saat Anda [membuat webhook](/id/docs/websets/api/webhooks/create-a-webhook) - pastikan Anda menyimpannya dengan aman karena nilainya tidak dapat diambil kembali di kemudian hari.

* **Gunakan HTTPS** - Selalu gunakan endpoint HTTPS untuk webhook Anda agar data terenkripsi saat pengiriman.

* **Daftarkan URL Final** - Pengiriman webhook tidak mengikuti pengalihan HTTP (response 3xx). Jika endpoint Anda melakukan pengalihan, pengiriman akan dianggap gagal. Selalu daftarkan URL yang langsung menangani payload.
...
### Error Signature Tidak Valid {#invalid-signature-errors}
...
2. **Periksa secret**: Pastikan Anda menggunakan webhook secret yang benar, yaitu secret yang diberikan saat webhook dibuat
3. **Periksa parsing header**: Pastikan Anda mengekstrak timestamp dan signature dari header dengan benar
4. **Masalah encoding**: Pastikan encoding UTF-8 konsisten selama proses verifikasi

### Menguji Signature Secara Lokal {#testing-signatures-locally}

Anda dapat menguji logika verifikasi signature Anda menggunakan webhook secret dan contoh payload:
