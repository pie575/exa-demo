...
description: "Pelajari cara memverifikasi signature webhook secara aman untuk memastikan permintaan benar-benar berasal dari Exa"
timestamp: false
---

Saat menerima webhook dari Exa, sebaiknya Anda memverifikasi bahwa webhook tersebut benar-benar berasal dari kami untuk menjaga integritas dan keaslian data. Exa menandatangani semua payload webhook dengan secret key yang unik untuk setiap endpoint webhook Anda.
...
Format signature-nya adalah sebagai berikut:
...
<CodeGroup>
  ...
  def verify_webhook_signature(payload, signature_header, webhook_secret):
      ...
      try:
          ...
          # Bandingkan dengan signature yang dikirim
  ...
  function verifyWebhookSignature(payload, signatureHeader, webhookSecret) {
      ...
      try {
          ...
          // Bandingkan dengan signature yang diterima memakai perbandingan timing-safe
  ...
  public class WebhookTest {
      ...
      *
      * @param payload Body request mentah dalam bentuk string
      ...
      public static void main(String[] args) {
          ...
          try {
              ...
              if (isValid) {
                  ...
                  // Di sini Anda dapat mem-parse JSON dan menangani event webhook
...
Mengikuti praktik berikut akan membantu memastikan implementation webhook Anda aman dan andal:
...
* **Periksa Freshness Timestamp** - Pertimbangkan untuk menolak webhook dengan timestamp yang terlalu lama (misalnya, lebih dari 5 menit) untuk mencegah serangan replay.
...
* **Gunakan HTTPS** - Selalu gunakan endpoint HTTPS untuk webhook Anda agar data terenkripsi selama transmisi.
