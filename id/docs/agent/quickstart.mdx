...
description: "Jalankan workflow deep research, list building, dan enrichment yang mengembalikan terstruktur output."
boost: 6
---

Exa Agent adalah endpoint asinkron berbasis penggunaan untuk tugas berkomputasi tinggi seperti list building, enrichment, dan deep research. Endpoint ini menangani penalaran kompleks dan dapat mengembalikan banyak field terstruktur output.

Anggap saja sebagai agent konteks: Anda menjelaskan data yang Anda inginkan beserta bentuk hasil yang diharapkan, lalu Exa Agent mengatur tool call yang dibutuhkan untuk mencapainya. Satu run dapat menjalankan banyak search sekaligus dari berbagai sudut, membaca dan meringkas halaman di baliknya, memecah list building menjadi subtugas yang berjalan paralel, memverifikasi setiap kandidat terhadap kriteria Anda, meng-enrich kontak, serta melakukan query ke data partner [Exa Connect](/id/docs/agent/connect/overview) mana pun yang Anda attach. Konteks yang sudah dirangkai dikembalikan sebagai satu hasil terstruktur dan grounded, alih-alih Anda harus mengatur sendiri setiap panggilan `/search` dan `/contents`.

Setiap run dapat mengembalikan jawaban dalam bahasa alami, JSON yang tervalidasi schema, grounding pada level field, metadata, dan rincian cost. Anda dapat mengambil run yang telah selesai di kemudian hari, menampilkan daftar run sebelumnya, melakukan replay event, atau melanjutkan dari run sebelumnya.

<Tip>
  Lebih suka MCP? Exa Agent dan [Exa Connect](/id/docs/agent/connect/overview) tersedia di [Exa MCP](/id/docs/get-started/exa-mcp#exa-agent). Aktifkan `tools=agent_run` untuk menjalankan Research multi-langkah, list building, enrichment, dan terstruktur output dari Claude, Cursor, dan MCP client lainnya.
</Tip>

## Kapan menggunakan Exa Agent {#when-to-use-exa-agent}

Gunakan Exa Agent ketika sebuah workflow membutuhkan lebih dari satu panggilan search atau extraction, atau ketika Anda harus menulis loop sendiri yang terdiri dari searches, pembacaan halaman, dan langkah verifikasi untuk menyusun data:

* Membangun daftar dari kriteria terbuka, lalu enrich setiap hasil
* Melakukan Research terhadap entitas di banyak fields lengkap dengan sitasi
* Menjalankan tugas multi-hop seperti &quot;temukan perusahaan, lalu temukan para pengambil keputusannya&quot;
...
* Melanjutkan run sebelumnya dengan follow-up request seperti &quot;temukan 10 hasil lagi&quot;

Exa Agent dirancang dengan latency lebih tinggi dan bersifat async. Untuk satu search dengan latency rendah di mana Anda sendiri yang mengatur calls-nya, mulailah dengan [Search API](/id/docs/search/quickstart).

## Quickstart {#quickstart}

Contoh ini memulai run yang menyusun daftar orang terstruktur sesuai kriteria Anda. Hasilnya dikembalikan dalam bentuk JSON di `output.structured`.
...
Tambahkan `Accept: text/event-stream` saat membuat run untuk menerima server-sent events saat run masuk antrean, dimulai, dan selesai. Lihat [Stream events](#stream-events) untuk detail selengkapnya.
...
## Memverifikasi dan meng-enrich entitas tertentu {#verify-and-enrich-a-specific-entity}

Selain list building, gunakan Exa Agent untuk menelaah satu entitas yang sudah diketahui, memverifikasi sebuah klaim terhadap sources yang otoritatif, dan mengembalikan enrichment yang terstruktur. Contoh ini memeriksa apakah situs resmi sebuah perusahaan memiliki halaman pricing yang dapat diakses publik, lalu meng-enrich hasilnya dengan detail harga jika tersedia. Schema hanya mewajibkan `domain` dan `verdict`; selebihnya bersifat enrichment opsional.
...
<Note>
  ...
  field yang mungkin tidak dapat diverifikasi sebagai nullable dan jangan sertakan dalam `required`,
  agar agent bisa mengembalikan `null` alih-alih mengarang nilai. Enum `verdict`
  membedakan pemeriksaan yang gagal (`cannot_verify`) dari evidence negatif
  yang sesungguhnya (`absent`): situs yang tidak dapat dijangkau bukanlah evidence bahwa
  halaman tersebut tidak ada.
</Note>

## Stream events {#stream-events}

Streaming membuat permintaan create tetap terbuka dan mengirimkan Server-Sent Events (SSE) hingga run selesai. Lihat [Event format](#event-format) untuk jenis event dan payload-nya.

Atur `stream=True` di Python, `stream: true` di JavaScript, atau kirim `Accept: text/event-stream` melalui HTTP:
...
Stream juga dapat berisi baris komentar seperti `: keep-alive`. Client SSE otomatis mengabaikan komentar; parser kustom sebaiknya melakukan hal yang sama.
...
| `agent_run.started`   | `{ id, status: "running" }`           | Tandai run sebagai sedang berjalan.                                                                                |
...
| `agent_run.failed`    | `{ id, status: "failed", error }`     | Tampilkan `error.code` dan `error.message`; tidak ada output final yang tersedia.                                  |
| `agent_run.cancelled` | `{ id, status: "cancelled", ... }`    | Hentikan pembacaan stream dan perlakukan run sebagai dibatalkan.                                                   |

Event yang terkait dengan langkah research yang sama menyertakan `callId`, yang sesuai dengan `item.call_id` pada event progres tool. Gunakan nilai tersebut untuk mengelompokkan jejak search, sources, dan progres tool. Sebagian deskripsi jejak search dibuat secara asinkron dan bisa tiba setelah event source atau tool yang dijelaskannya, jadi jangan mengaitkannya hanya berdasarkan urutan kedatangan.

Perlakukan `agent_run.source.added` sebagai pratinjau langsung, bukan daftar sitasi yang lengkap. Nilai `output.grounding` pada run terminal adalah output grounding yang otoritatif.

### Replay event tersimpan {#replay-stored-events}

Untuk run non-ZDR, [`GET /agent/runs/{id}/events`](/id/docs/reference/agent-api/list-run-events) mengembalikan event tersimpan sebagai JSON berhalaman. Kirim `Accept: text/event-stream` untuk melakukan replay event tersimpan sebagai SSE, dan `Last-Event-ID` untuk melewati event yang sudah diproses oleh client Anda:
...
Untuk meminta informasi kontak, jelaskan field kontak yang diinginkan di `outputSchema`. Gunakan bentuk JSON Schema standar seperti `{ "type": "string", "format": "email" }` untuk alamat email, `{ "type": "string", "format": "phone" }` untuk nomor telepon, dan `{ "type": "string", "format": "uri" }` untuk URL. Bila memungkinkan, batasi ukuran daftar dengan `maxItems` agar biaya maksimum enrichment kontak dapat diprediksi.
...
Gunakan `input.data` jika Anda sudah memiliki sekumpulan data yang ingin di-enrich. Anda dapat menambahkan lebih banyak field ke setiap entitas data, memunculkan entitas lain berdasarkan data yang Anda masukkan, atau keduanya.
...
Gunakan `input.exclusion` untuk mencegah entri tertentu muncul dalam run. Pada contoh di bawah, kita ingin mencari 10 hewan paling menggemaskan, tetapi kambing dan panda dikecualikan dari run karena kita sudah tahu betapa menggemaskannya mereka.
...
Gunakan `previousRunId` untuk mengajukan follow-up terhadap response sebelumnya. Setiap follow-up memulai run baru dengan ID tersendiri. `previousRunId` membawa konteks ke run baru; nilainya tidak dipakai ulang sebagai ID run baru tersebut.
...
Biaya dihitung berdasarkan penggunaan dan ditagih per komponen:
...
<Note>
  Contact enrichment terpisah dari komponen harga inti di atas: contact enrichment email adalah `$0.02 / email`, dan contact enrichment nomor telepon adalah `$0.07 / phone number`.
</Note>

`usage.agentComputeUnits` mengukur komputasi model selama keseluruhan run. Query yang kompleks, terutama yang memiliki field `input.data` berukuran besar, memerlukan lebih banyak langkah penalaran dan tool call sehingga mengonsumsi lebih banyak ACU.
...
<Info>
  ...
  diverifikasi. Fitur ini berada dalam beta publik: permintaan dengan
  `effort: "max"` harus menyertakan `Exa-Beta: agent-max-effort-2026-07-27`.
  Header ini menerima daftar token beta yang dipisahkan koma.
</Info>

`budget.maxCostDollars` adalah batas atas opsional per run untuk `auto` dan `max`. Nilai yang diterima adalah `$1`–`$100`; maksimum yang tersedia saat ini adalah `$100`, meskipun server dapat menetapkan maksimum yang lebih rendah. Batas default-nya adalah `$5` untuk `auto` dan `$20` untuk `max`. Ini merupakan batas atas, bukan harga tetap: run yang selesai lebih cepat akan lebih murah. Budget tidak berlaku untuk effort tetap.

### Memilih mode effort {#choosing-an-effort-mode}

Mode effort tetap cocok ketika Anda menginginkan harga per permintaan yang dapat diprediksi untuk research standar. Gunakan `auto` untuk pekerjaan dengan scope yang bervariasi seperti list building, di mana jumlah entitas bisa berbeda-beda di tiap permintaan.

| Effort    | Paling cocok untuk                                                                        | Kompleksitas schema yang disarankan                                                   | Ekspektasi runtime                |
| --------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------- | --------------------------------- |
| `minimal` | Pencarian berbiaya terendah, tugas faktual yang sangat sempit, jawaban singkat            | Satu atau dua field, schema dangkal                                                   | Termurah, paling tidak menyeluruh |
| `low`     | Pencarian sederhana, tugas faktual sempit, jawaban singkat                                | Beberapa field, schema dangkal                                                        | Cepat, research ringan            |
...
| `high`    | Research yang lebih sulit, lebih banyak sitasi, kelengkapan lebih ketat                   | Schema lebih besar atau field yang lebih bernuansa                                    | Lebih lambat, lebih menyeluruh    |
| `xhigh`   | Tugas bernilai tinggi yang mengutamakan kelengkapan dibanding cost/latency                | Schema kompleks, banyak field, verifikasi sulit                                       | Effort tetap paling lambat        |
| `auto`    | Pekerjaan dengan scope bervariasi, list building, tingkat kesulitan tugas tidak diketahui | Fleksibel; berguna saat jumlah entitas atau pekerjaan yang dibutuhkan tidak diketahui | Bervariasi                        |
| `max`     | Research dengan effort tertinggi (beta)                                                   | Schema kompleks, banyak field, verifikasi sulit                                       | Berjalan paling lama              |

Mulailah dengan `medium` untuk research entitas tunggal yang standar. Turunkan ke `low` atau `minimal` saat cost dan latency lebih penting daripada kelengkapan. Naikkan ke `high` atau `xhigh` saat schema output lebih besar, field perlu diverifikasi, atau tugas menuntut penalaran yang lebih mendalam. Gunakan `auto` saat Anda belum mengetahui scope-nya sejak awal, misalnya pada list building atau workflow yang berpotensi mengembalikan banyak entitas.

Runtime bervariasi tergantung tingkat kesulitan query, kompleksitas schema, dan ketersediaan source eksternal. Perlakukan mode effort sebagai tradeoff kualitas/cost/runtime, bukan sebagai jaminan latency yang ketat.
...
* Data run tersedia selama run berjalan dan hingga 10 menit setelah run mencapai status terminal. Setelah itu, run tidak dapat diambil lagi.
