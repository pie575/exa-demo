...
description: "Jalankan deep research, list-building, dan workflow enrichment yang mengembalikan structured output."
boost: 6
---
...
Anggap saja sebagai agen konteks: Anda mendeskripsikan data yang Anda inginkan beserta bentuk hasil yang diharapkan, lalu Exa Agent mengorkestrasi pemanggilan tool yang dibutuhkan untuk mewujudkannya. Satu run dapat menyebar menjadi banyak searches dari berbagai sudut pandang, membaca dan meringkas halaman di baliknya, memecah list building menjadi subtugas yang berjalan paralel, memverifikasi setiap kandidat terhadap criteria Anda, memperkaya kontak, dan melakukan query ke data partners [Exa Connect](/id/docs/agent/connect/overview) mana pun yang Anda attach. Konteks yang sudah dirangkai itu Anda terima sebagai satu hasil terstruktur dan grounded, alih-alih harus mengorkestrasi sendiri setiap panggilan `/search` dan `/contents`.

Setiap run dapat mengembalikan jawaban dalam bahasa alami, JSON yang tervalidasi schema, grounding di tingkat field, metadata, dan rincian cost. Anda dapat mengambil run yang telah selesai di kemudian hari, menampilkan daftar runs sebelumnya, melakukan replay event, atau melanjutkan dari run sebelumnya.
...
Gunakan Exa Agent ketika sebuah workflow membutuhkan lebih dari satu panggilan search atau extraction, atau ketika Anda harus menulis sendiri perulangan berisi searches, pembacaan halaman, dan langkah verification untuk menyusun data:

* Membangun daftar dari criteria yang terbuka, lalu enrich setiap hasilnya
* Meneliti entitas pada banyak bidang lengkap dengan citations
* Menjalankan tugas multi-langkah seperti &quot;temukan perusahaan, lalu temukan para pengambil keputusannya&quot;
...
Exa Agent memang dirancang untuk berjalan secara async dengan latensi yang lebih tinggi. Untuk satu search berlatensi rendah yang panggilannya Anda atur sendiri, mulailah dengan [Search API](/id/docs/search/quickstart).

<div id="quickstart">
  ## Quickstart
</div>

Contoh ini memulai run yang menyusun daftar terstruktur berisi orang-orang yang memenuhi criteria Anda. Hasilnya dikembalikan dalam bentuk JSON di `output.structured`.
...
Tambahkan `Accept: text/event-stream` saat membuat run untuk menerima server-sent events ketika run masuk antrean, dimulai, dan selesai. Lihat [Stream events](#stream-events) untuk detail selengkapnya.
...
* `output.structured`: JSON tervalidasi apabila Anda menyediakan `outputSchema`
* `output.grounding`: citations untuk teks atau field terstruktur, jika dihasilkan
...
<Note>
  ...
  eksekusi sinkron, streaming, atau background. Lihat [kompatibilitas SDK
...
Selain list building, gunakan Exa Agent untuk menelaah satu entitas yang sudah diketahui, memverifikasi klaim terhadap sumber tepercaya, dan mengembalikan enrichment terstruktur. Contoh ini memeriksa apakah situs resmi sebuah perusahaan memiliki halaman harga yang dapat diakses publik, lalu meng-enrich hasilnya dengan detail harga jika tersedia. Schema hanya mewajibkan `domain` dan `verdict`; sisanya merupakan enrichment opsional.
...
<Note>
  Schema untuk workflow verification sebaiknya memperhitungkan ketidakpastian.
  Buat field yang mungkin tidak dapat diverifikasi menjadi nullable dan jangan
  masukkan ke `required`, agar agent bisa mengembalikan `null` alih-alih
  mengarang nilai. Enum `verdict` membedakan pemeriksaan yang gagal
  (`cannot_verify`) dari evidence negatif yang sesungguhnya (`absent`): situs
  yang tidak dapat dijangkau bukan berarti evidence bahwa halaman tersebut
  tidak ada.
</Note>
...
Streaming membuat permintaan create tetap terbuka dan mengirimkan Server-Sent Events (SSE) sampai run selesai. Lihat [Format event](#event-format) untuk mengetahui tipe event dan payload-nya.
...
Event yang terkait dengan langkah riset yang sama menyertakan `callId`. Nilai ini sesuai dengan `item.call_id` pada event progres tool. Gunakan nilai tersebut untuk mengelompokkan jejak search, sumber, dan progres tool. Beberapa deskripsi jejak search dihasilkan secara asinkron dan dapat tiba setelah event sumber atau tool yang dijelaskannya, jadi jangan mengorelasikannya hanya berdasarkan urutan kedatangan.

Perlakukan `agent_run.source.added` sebagai pratinjau langsung, bukan sebagai daftar citations yang lengkap. Nilai `output.grounding` dari run terminal adalah output grounding yang otoritatif.

<div id="replay-stored-events">
  ### Replay event tersimpan
</div>

Untuk run non-ZDR, [`GET /agent/runs/{id}/events`](/id/docs/reference/agent-api/list-run-events) mengembalikan event tersimpan sebagai JSON terpaginasi. Kirim `Accept: text/event-stream` untuk melakukan replay event tersimpan sebagai SSE, dan `Last-Event-ID` untuk melewati event yang sudah diproses klien Anda:
...
Gunakan `input.data` jika Anda sudah memiliki kumpulan data yang ingin di-enrich. Anda dapat menambahkan lebih banyak field pada setiap entitas data, memunculkan entitas tambahan berdasarkan data yang Anda masukkan, atau keduanya.
...
Lihat [Exa Connect](/id/docs/agent/connect/overview) untuk daftar lengkap data partner, lengkap dengan contoh untuk masing-masing.

<div id="continue-from-a-previous-run">
  ## Melanjutkan dari run sebelumnya
</div>

Gunakan `previousRunId` untuk mengirim follow-ups atas respons sebelumnya. Setiap follow-up akan memulai run baru dengan ID tersendiri. `previousRunId` membawa konteks ke run baru; ID tersebut tidak dipakai ulang sebagai ID run baru.
...
Tampilkan daftar run terbaru dan periksa status masing-masing:
...
Biaya dihitung berdasarkan usage dan ditentukan per komponen:
...
`usage.agentComputeUnits` mengukur komputasi model selama satu run penuh. Query yang kompleks, terutama yang memiliki field `input.data` berukuran besar, membutuhkan lebih banyak langkah penalaran dan pemanggilan tool sehingga mengonsumsi lebih banyak ACU.

Lihat [Agent limits](/id/docs/admin/billing#agent-limits) untuk informasi concurrency dan rate limit.

<div id="effort">
  ### Effort
</div>

Gunakan `effort` untuk menentukan tingkat biaya dan penalaran pada setiap run. Nilai yang didukung adalah `minimal`, `low`, `medium`, `high`, `xhigh`, `auto`, dan `max`; nilai defaultnya `auto`. Effort tetap memiliki harga per permintaan yang dapat diprediksi, sedangkan `auto` dan `max` (beta) dihitung berdasarkan usage:
...
| `auto`    | Berbasis usage; hingga batas default `$5`            |
| `max`     | **Beta**, berbasis usage; hingga batas default `$20` |

<Info>
  Agent Max adalah tingkat effort tertinggi untuk pekerjaan yang mengutamakan kelengkapan
  dan ketelitian di atas latensi atau cost, termasuk list building berskala besar,
  riset deep dari banyak sumber, dan criteria yang sulit diverifikasi. Fitur ini masih
  dalam beta publik: permintaan dengan `effort: "max"` harus menyertakan
  `Exa-Beta: agent-max-effort-2026-07-27`. Header ini menerima daftar token beta yang
  dipisahkan koma.
</Info>

`budget.maxCostDollars` adalah batas atas opsional per run untuk `auto` dan `max`. Nilainya menerima `$1`–`$100`; maksimum yang tersedia adalah `$100`, meskipun server dapat menetapkan maksimum yang lebih rendah. Batas defaultnya adalah `$5` untuk `auto` dan `$20` untuk `max`. Ini adalah batas atas, bukan harga tetap: run yang selesai lebih cepat akan lebih murah. Budget tidak berlaku untuk effort tetap.

<div id="choosing-an-effort-mode">
  ### Memilih mode effort
</div>

Mode effort tetap cocok digunakan ketika Anda menginginkan biaya per permintaan yang dapat diprediksi untuk riset standar. Gunakan `auto` untuk pekerjaan dengan scope yang bervariasi seperti list building, di mana jumlah entitas bisa berbeda dari satu permintaan ke permintaan lain.
...
| `high`    | Riset yang lebih sulit, lebih banyak citations, kelengkapan yang lebih ketat                   | Schema lebih besar atau field yang lebih bernuansa                                      | Lebih lambat, lebih menyeluruh    |
| `xhigh`   | Tugas bernilai tinggi saat kelengkapan lebih penting daripada biaya/latensi                    | Schema kompleks, banyak field, verification yang sulit                                  | Effort tetap paling lambat        |
| `auto`    | Pekerjaan dengan scope bervariasi, list building, tingkat kesulitan tugas yang tidak diketahui | Fleksibel; berguna ketika jumlah entitas atau pekerjaan yang dibutuhkan tidak diketahui | Bervariasi                        |
| `max`     | Riset dengan effort tertinggi (beta)                                                           | Schema kompleks, banyak field, verification yang sulit                                  | Paling lama berjalan              |

Mulailah dengan `medium` untuk riset entitas tunggal yang standar. Turunkan ke `low` atau `minimal` ketika biaya dan latensi lebih penting daripada kelengkapan. Naikkan ke `high` atau `xhigh` ketika schema output lebih besar, field perlu verification, atau tugas membutuhkan penalaran yang lebih mendalam. Gunakan `auto` ketika Anda belum mengetahui scope-nya sejak awal, misalnya pada list building atau workflow yang berpotensi mengembalikan banyak entitas.

Runtime bervariasi tergantung tingkat kesulitan query, kompleksitas schema, dan ketersediaan sumber eksternal. Perlakukan mode effort sebagai kompromi antara kualitas, biaya, dan runtime, bukan sebagai jaminan latensi yang pasti.
...
Contoh SDK di atas membutuhkan versi `exa-py` atau `exa-js` yang sudah mendukung Agent Max.
...
* Buat runs dengan streaming (`Accept: text/event-stream`) untuk menerima output secara langsung, atau lakukan poll pada runs asinkron selama rentang waktu retensi.
