...
description: "Jalankan pencarian berulang dan terima hasil yang baru ditemukan melalui webhook."
boost: 4
---
...
research, atau topik apa pun yang berubah seiring waktu.
...
lacak, bukan dengan menambahkan rentang tanggal yang bergeser sendiri.
...
Sources untuk fields hasil sintesis dikembalikan secara otomatis di `output.grounding`.

Tambahkan `outputSchema` jika kode downstream memerlukan
...
kutipan, teks penuh, atau ringkasan pada setiap hasil, serta `includeDomains` atau `excludeDomains` untuk
membatasi sources.
...
Interval minimum adalah satu jam. Gunakan satu durasi saja seperti `1h`, `6h`, `1d`, atau `7d`. Jadwal
...
run otomatis, tetapi pemicu manual tetap berfungsi.
...
Berlangganan `monitor.run.completed` jika Anda hanya memerlukan run yang sudah selesai. Jika `events` tidak disertakan, Exa
...
dikirim ulang dalam deliveries webhook, sehingga Anda bisa meneruskan pembaruan ke pelanggan,
workspace, kanal, atau job internal yang tepat.
...
Susun `<timestamp>.<raw-request-body>`, hitung digest HMAC-SHA256-nya dengan
`webhookSecret` sekali pakai, lalu bandingkan hasilnya dengan `v1` menggunakan perbandingan waktu konstan.
...
<Columns cols={2}>
  <Card title="Membuat monitor" icon="bell" href="/id/docs/reference/monitors/create-a-monitor" cta="Buka referensi" arrow="true">
    Lihat semua field search, schedule, output, metadata, dan webhook.
  </Card>
  ...
  <Card title="Panduan search" icon="search" href="/id/docs/search/quickstart" cta="Buka panduan" arrow="true">
    Atur query, filter, kutipan, teks penuh, dan kebaruan.
  </Card>
